...
class PyViz
{
  public:
    ...
    enum PacketCaptureMode
    {
        ...
        PACKET_CAPTURE_FILTER_HEADERS_OR,  // packet capture if any of the indicated
                                           // headers is present
        PACKET_CAPTURE_FILTER_HEADERS_AND, // packet capture if all of the
    ...
    // Yes, I know, this is just a utility function, not really related to the
    // class in any way.
    /**
     ...
     * \param [in,out] lineX1 Line, minimum X coord (any on input, clipped to the
     * bounding box on output) \param [in,out] lineY1 Line, minimum Y coord (any
     * on input, clipped to the bounding box on output) \param [in,out] lineX2
     * Line, maximum X coord (any on input, clipped to the bounding box on output)
     * \param [in,out] lineY2 Line, maximum Y coord (any on input, clipped to the
     * bounding box on output)
     */
    // -#- @lineX1(direction=inout); @lineY1(direction=inout);
    // @lineX2(direction=inout);
    ...
    static void LineClipping(double boundsX1,
                             ...
                             double& lineX2,
                             double& lineY2);
    // Don't break the above line or pybindgen will not be able to pick up the
    // above annotation :(
  ...
  private:
    ...
    std::set<uint32_t> m_nodesOfInterest;         ///< list of node IDs whose
                                                  ///< transmissions will be monitored
