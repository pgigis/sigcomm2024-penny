...
class WifiDefaultAckManager : public WifiAckManager
{
  public:
     ...
     * numbers of the given MPDU and of all the QoS data frames included in the
     * given TX parameters.
     *
     * \param mpdu the given MPDU
     * \param txParams the given TX parameters
     * \return the maximum distance between the starting sequence number of the
     * Block Ack agreement which the given MPDU belongs to and each of the
     * sequence numbers of the given MPDU and of all the QoS data frames included
     * in the given TX parameters
     */
  ...
  protected:
    /**
     * Determine whether the (A-)MPDU containing the given MPDU and the MPDUs (if
     * any) included in the given TX parameters requires an immediate response
     * (Normal Ack, Block Ack or Block Ack Request followed by Block Ack).
     *
     * \param mpdu the given MPDU.
     * \param txParams the given TX parameters.
    ...
    /**
     * \param mpdu the given MPDU
     * \return whether there exist MPDUs with lower sequence number than the given
     * MPDU that are inflight on the same link as the given MPDU
     */
    bool ExistInflightOnSameLink(Ptr<const WifiMpdu> mpdu) const;

  private:
    /**
     * Compute the information about the acknowledgment of the current multi-user
     * frame (as described by the given TX parameters) if the given MPDU is added.
     * If the computed information is the same as the current one, a null pointer
     * is returned. Otherwise, the computed information is returned. This method
     * can only be called if the selected acknowledgment method for DL multi-user
     * frames consists of a sequence of BlockAckReq and BlockAck frames.
     *
     * \param mpdu the given MPDU
     * \param txParams the TX parameters describing the current multi-user frame
     * \return the new acknowledgment method or a null pointer if the
     * acknowledgment method is unchanged
     */
     ...
     * is returned. Otherwise, the computed information is returned. This method
     * can only be called if the selected acknowledgment method for DL multi-user
     * frames consists of a MU-BAR Trigger Frame sent as single-user frame.
     *
     * \param mpdu the given MPDU
     * \param txParams the TX parameters describing the current multi-user frame
     * \return the new acknowledgment method or a null pointer if the
     ...
     * is returned. Otherwise, the computed information is returned. This method
     * can only be called if the selected acknowledgment method for DL multi-user
     * frames consists of MU-BAR Trigger Frames aggregated to the PSDUs of the MU
     * PPDU.
     *
     * \param mpdu the given MPDU
     * \param txParams the TX parameters describing the current multi-user frame
     ...
     * \param txParams the current TX parameters (just the TXVECTOR needs to be
     * set) \return the acknowledgment method for the TB PPDUs solicited by the
     * given Trigger Frame
     */
    ...
    uint8_t m_maxMcsForBlockAckInTbPpdu;      //!< Max MCS used to send a BlockAck in
                                              //!< a TB PPDU
};
