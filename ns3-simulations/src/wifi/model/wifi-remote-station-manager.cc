    ...
    static TypeId tid =
        TypeId("ns3::WifiRemoteStationManager")
            ...
            .AddAttribute("ErpProtectionMode",
                          "Protection mode used when non-ERP STAs are connected to an ERP "
                          "AP: "
            ...
            .AddAttribute("HtProtectionMode",
                          "Protection mode used when non-HT STAs are connected to a HT AP: "
                          "Rts-Cts "
                          "or Cts-To-Self",
                          EnumValue(WifiRemoteStationManager::CTS_TO_SELF),
            ...
            .AddTraceSource(
                "MacTxFinalRtsFailed",
                "The transmission of a RTS has exceeded the maximum "
                "number of attempts",
...
WifiRemoteStationManager::NeedRts(const WifiMacHeader& header, uint32_t size)
{
    ...
    {
        NS_LOG_DEBUG("WifiRemoteStationManager::NeedRTS returning true to protect "
                     "non-ERP stations");
        return true;
    }
...
WifiRemoteStationManager::NeedCtsToSelf(WifiTxVector txVector)
{
    ...
    if (m_erpProtectionMode == CTS_TO_SELF &&
        ...
        m_useNonErpProtection)
    {
        NS_LOG_DEBUG("WifiRemoteStationManager::NeedCtsToSelf returning true to "
                     "protect non-ERP stations");
        return true;
    }
