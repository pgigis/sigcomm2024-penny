...
class VhtConfiguration : public Object
{
  public:
     ...
     *
     * \param enable true if 20 MHz, 40 MHz, 80 MHz and 160 MHz operation is to be
     * supported, false if 20 MHz, 40 MHz and 80 MHz operation is to be supported
     */
    ...
    using SecondaryCcaSensitivityThresholds =
        std::tuple<double,
                   double,
                   double>; //!< Tuple identifying CCA sensitivity
                            //!< thresholds for secondary channels

    /**
     * Sets the CCA sensitivity thresholds for PPDUs that do not occupy the
     * primary channel. The thresholds are defined as a tuple {threshold for 20MHz
     * PPDUs, threshold for 40MHz PPDUs, threshold for 80MHz PPDUs}.
     *
    ...
    /**
     * \return the CCA sensitivity thresholds for PPDUs that do not occupy the
     * primary channel
     */
    ...
    /**
     * \return the CCA sensitivity thresholds for PPDUs that do not occupy the
     * primary channel, indexed by signal bandwidth (MHz)
     */
  ...
  private:
    ...
    std::map<uint16_t, double>
        m_secondaryCcaSensitivityThresholds; ///< CCA sensitivity thresholds for
                                             ///< signals that do not occupy the
                                             ///< primary channel, indexed by
