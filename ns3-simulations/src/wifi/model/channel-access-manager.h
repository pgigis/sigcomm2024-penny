...
class ChannelAccessManager : public Object
{
  public:
    ...
    /**
     * Set up (or reactivate) listener for PHY events on the given PHY. The new
     * (or reactivated) listener becomes the active listener and the previous
     * active listener attached to another PHY, if any, is deactivated.
     *
    ...
    /**
     * Deactivate current registered listener for PHY events on the given PHY. All
     * notifications but channel switch notifications coming from an inactive
     ...
     *
     * \param linkId the ID of the link this Channel Access Manager is associated
     * with
     */
     ...
     *
     * The ChannelAccessManager does not take ownership of this pointer so, the
     * callee must make sure that the Txop pointer will stay valid as long as the
     * ChannelAccessManager is valid. Note that the order in which Txop objects
     * are added to a ChannelAccessManager matters: the first Txop added has the
     * highest priority, the second Txop added, has the second highest priority,
     * etc.
     */
     ...
     * granted channel access during this period and the backoff timer will be
     * frozen. After this period, the EDCA will start normal operations again by
     * resuming the backoff timer.
     */
     ...
     * \return the width of the largest primary channel that has been idle for the
     *         given time interval before the given time, if any primary channel
     * has been idle, or zero, otherwise
     */
    uint16_t GetLargestIdlePrimaryChannel(Time interval, Time end);

    /**
     * \param indices a set of indices (starting at 0) specifying the 20 MHz
     * channels to test \return true if per-20 MHz CCA indicates busy for at least
     * one of the specified 20 MHz channels, false otherwise
     */
     ...
     * reported. \param per20MhzDurations vector that indicates for how long each
     * 20 MHz subchannel (corresponding to the index of the element in the vector)
     * is busy and where a zero duration indicates that the subchannel is idle.
     * The vector is non-empty if  the PHY supports 802.11ax or later and if the
    ...
    /**
     * Notify that another EMLSR link is being used, hence medium access should be
     * disabled.
     */
    void NotifyStartUsingOtherEmlsrLink();
    /**
     * Notify that another EMLSR link is no longer being used, hence medium access
     * can be resumed.
     */
    ...
    /**
     * Notify that the given PHY is about to switch to the given operating
     * channel, which is used by the given link. This notification is sent by the
     * EMLSR Manager when a PHY object switches operating channel to operate on
     * another link.
     *
  ...
  private:
     ...
     * so, the last idle start time and end time for each channel type are
     * updated. Otherwise, no change is made by this method. This method is
     * normally called when we are notified of the start of a transmission,
     * reception, CCA Busy or switching to correctly maintain the information
     * about the last idle period.
     */
    ...
    std::vector<Time> m_lastPer20MHzBusyEnd; /**< the last busy end time per 20
                                                MHz channel (HE stations and
    ...
    Time m_lastUsingOtherEmlsrLinkEnd; //!< the last time we were blocked because
                                       //!< using another EMLSR link
    ...
    /// Store information about the PHY objects that are going to operate on
    /// another EMLSR link
