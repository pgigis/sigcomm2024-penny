...
DsssPhy::GetHeaderMode(const WifiTxVector& txVector) const
{
    ...
    {
        // Section 16.2.3 "PPDU field definitions" and Section 16.2.2.2 "Long PPDU
        // format"; IEEE Std 802.11-2016
...
DsssPhy::GetPreambleDuration(const WifiTxVector& txVector) const
{
    ...
    {
        // Section 16.2.2.3 "Short PPDU format" Figure 16-2 "Short PPDU format";
        // IEEE Std 802.11-2016
        return MicroSeconds(72);
    }
    else
    {
        // Section 16.2.2.2 "Long PPDU format" Figure 16-1 "Long PPDU format"; IEEE
        // Std 802.11-2016
...
DsssPhy::GetHeaderDuration(const WifiTxVector& txVector) const
{
    if (txVector.GetPreambleType() == WIFI_PREAMBLE_SHORT &&
        (txVector.GetMode().GetDataRate(22) > 1000000))
    {
        // Section 16.2.2.3 "Short PPDU format" and Figure 16-2 "Short PPDU format";
        // IEEE Std 802.11-2016
        return MicroSeconds(24);
    }
    else
    {
        // Section 16.2.2.2 "Long PPDU format" and Figure 16-1 "Short PPDU format";
        // IEEE Std 802.11-2016
...
DsssPhy::CreateDsssMode(std::string uniqueName, WifiModulationClass modClass)
{
    ...
    NS_ASSERT_MSG(it != m_dsssModulationLookupTable.end(),
                  ...
                  "the lookup table!");
    NS_ASSERT_MSG(modClass == WIFI_MOD_CLASS_DSSS || modClass == WIFI_MOD_CLASS_HR_DSSS,
