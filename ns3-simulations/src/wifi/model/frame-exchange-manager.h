...
class FrameExchangeManager : public Object
{
  public:
     ...
     * \param allowedWidth the allowed width in MHz for the frame exchange
     * sequence \return true if a frame exchange sequence was started, false
     * otherwise
     */
     ...
     *
     * \param linkId the ID of the link this Frame Exchange Manager is associated
     * with
     */
     ...
     * acknowledgment method. The acknowledgment time is stored in the
     * acknowledgment object itself.
     *
    ...
    /**
     * \return the set of stations that have successfully received an RTS in this
     * TXOP.
     */
  ...
  protected:
     ...
     * first fragment; otherwise, return the given MPDU. Note that, if
     * fragmentation is applied, the given MPDU is dequeued from the MAC queue and
     * the first fragment is enqueued in its place.
     *
     * \param mpdu the given MPDU
     * \return the first fragment if fragmentation is needed, the given MPDU
     * otherwise
     */
    ...
    /**
     * Update the NAV, if needed, based on the Duration/ID of the given
     * <i>psdu</i>.
     *
     * \param psdu the received PSDU
     * \param txVector TxVector of the received PSDU
    ...
    /**
     * This method handles the reception of an MPDU (possibly included in an
     * A-MPDU)
     *
     ...
     * \param txVector the TXVECTOR used to transmit the MPDU that was
     * acknowledged \param ackTxVector the TXVECTOR used to transmit the Normal
     * Ack frame \param rxInfo the info on the received signal (\see RxSignalInfo)
    ...
    /**
     * Make the sequence numbers of MPDUs included in the given PSDU available
     * again if the MPDUs have never been transmitted.
     *
    ...
    /**
     * Get the size in bytes of the given MPDU, which is to be transmitted with
     * the given TXVECTOR. The purpose of this method is that it can be overridden
     * to compute the size of an S-MPDU.
     *
     * \param mpdu the given MPDU
    ...
    uint16_t m_allowedWidth;                //!< the allowed width in MHz for the current
                                            //!< transmission
     ...
     * Tasks performed by this method include setting the Power Management flag in
     * the MAC header.
     *
     * \param psdu the given PSDU
     */
     ...
     * \param fragmentedPacket the packet that originated the frame to transmit,
     * in case the latter is a fragment \return the computed Duration/ID value
     */
     ...
     * frame \return the computed Duration/ID value for the RTS frame
     */
     ...
     * \param txDuration the TX duration of the data frame
     * \param response the time taken by the response (acknowledgment) to the data
     * frame \return the computed Duration/ID value for the CTS-to-self frame
     */
     ...
     * \param dataTxVector the TXVECTOR used to transmit the frame soliciting the
     * Normal Ack \param dataSnr the SNR of the frame soliciting the Normal Ack in
     * linear scale
     */
     ...
     * \param txVector the TXVECTOR used to transmit the frame soliciting the
     * Normal Ack
     */
