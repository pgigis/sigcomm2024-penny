...
class BlockAckManager : public Object
{
  ...
  public:
     ...
     *
     * \return a const reference to the block ack agreement with the given
     * recipient, if it exists
     *
     * Check if we are the originator of an existing block ack agreement with the
     * given recipient.
     */
     ...
     *
     * \return a const reference to the block ack agreement with the given
     * originator, if it exists
     *
     * Check if we are the recipient of an existing block ack agreement with the
     * given originator.
     */
     ...
     *
     * Creates a new originator block ack agreement in pending state. When a ADDBA
     * response with a successful status code is received, the relative agreement
     * becomes established.
     */
     ...
     *
     * Stores <i>mpdu</i> for a possible future retransmission. Retransmission
     * occurs if the packet, in a BlockAck frame, is indicated by recipient as not
     * received.
     */
    void StorePacket(Ptr<WifiMpdu> mpdu);
    /**
     * Invoked upon receipt of an Ack frame on the given link after the
     * transmission of a QoS data frame sent under an established block ack
     * agreement. Remove the acknowledged frame from the outstanding packets and
     * update the starting sequence number of the transmit window, if needed.
     *
     ...
     * transmission of a QoS data frame sent under an established block ack
     * agreement. Remove the acknowledged frame from the outstanding packets and
     * insert it in the retransmission queue.
     *
     * \param linkId the ID of the given link
     ...
     * \param index the index of the Per AID TID Info subfield, in case of
     * Multi-STA Block Ack, or 0, otherwise \return a pair of values indicating
     * the number of successfully received MPDUs and the number of failed MPDUs
     *
     * Invoked upon receipt of a BlockAck frame on the given link. Typically, this
     * function is called by ns3::QosTxop object. Performs a check on which MPDUs,
     * previously sent with Ack Policy set to Block Ack, were correctly received
     * by the recipient. An acknowledged MPDU is removed from the buffer,
     * retransmitted otherwise. Note that <i>tids</i> is only used if
     * <i>blockAck</i> is a Multi-STA Block Ack using All-ack context.
     */
     ...
     * \param tid Traffic ID.
     *
     * Invoked upon missed reception of a block ack frame on the given link.
     * Typically, this function is called by ns3::QosTxop object. Performs a check
     * on which MPDUs, previously sent with ack policy set to Block Ack, should be
     * placed in the retransmission queue.
     */
     ...
     *
     * Returns the number of packets buffered for a specified agreement. This
     * methods doesn't return the number of buffered MPDUs but the number of
     * buffered MSDUs.
     */
     ...
     *
     * Puts corresponding originator agreement in established state and updates
     * number of packets and starting sequence field. Invoked typically after a
     * block ack refresh.
     */
     ...
     * \param tid Traffic ID of transmitted packet.
     *
     * Marks an originator agreement as rejected. This happens if <i>recipient</i>
     * station reject block ack setup by an ADDBA Response frame with a failure
     * status code. For now we assume that every QoS station accepts a block ack
     * setup.
     */
     ...
     * \param tid Traffic ID of transmitted packet.
     *
     * Marks an originator agreement after not receiving response to ADDBA
     * request. During this state any packets in queue will be transmitted using
     * normal MPDU. This also unblocks recipient address.
     */
     ...
     * \param tid Traffic ID of transmitted packet.
     *
     * Set Originator BA agreement to a transitory state to reset it after not
     * receiving response to ADDBA request.
     */
     ...
     *
     * Upon receipt of a BlockAck frame, if total number of packets (packets in
     * WifiMacQueue and buffered packets) is greater of <i>nPackets</i>, they are
     * transmitted using block ack mechanism.
     */
     ...
     * retransmit queue and from the queue of the block ack agreement, and (ii)
     * the scheduling of a BlockAckRequest.
     */
     ...
     * frames to transmit. If a BAR for the same recipient and TID is already
     * present in the queue, it is replaced by the new one. If the given BAR is
     * retransmitted, it is placed at the head of the queue, otherwise at the
     * tail.
     */
    ...
    /**
     * \return the list of BA agreements (identified by the recipient and TID
     * pair) for which a BAR shall only be sent if there are queued data frames
     * belonging to those agreements
     */
    const std::list<AgreementKey>& GetSendBarIfDataQueuedList() const;
    /**
     * Add the given (recipient, TID) pair to the list of BA agreements for which
     * a BAR shall only be sent if there are queued data frames belonging to those
     * agreements
     *
     * \param recipient the recipient
     * \param tid the TID
    ...
    /**
     * Remove the given (recipient, TID) pair from the list of BA agreements for
     * which a BAR shall only be sent if there are queued data frames belonging to
     * those agreements
     *
     * \param recipient the recipient
     * \param tid the TID
  ...
  private:
     ...
     * \param mpduIt an iterator pointing to the MPDU in the queue of in flight
     * MPDUs \param status the status of the in flight MPDU \param it iterator
     * pointing to the Block Ack agreement \param now the current time \return an
     * iterator pointing to the next MPDU in the queue of in flight MPDUs
     */
    ...
    /**
     * This data structure contains, for each originator block ack agreement
     * (recipient, TID), a set of packets for which an ack by block ack is
     * requested. Every packet or fragment indicated as correctly received in
     * BlockAck frame is erased from this data structure. Pushed back in
     * retransmission queue otherwise.
     */
