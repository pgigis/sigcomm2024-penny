 ...
 * This model assumes that a preamble is successfully detected if SNR is at or
 * above a given threshold (set to 4 dB by default). However, if RSSI is below a
 * minimum RSSI (set to -82 dBm by default), the PHY preamble is not detected.
 */
class ThresholdPreambleDetectionModel : public PreambleDetectionModel
{
  ...
  private:
    double m_threshold; ///< SNR threshold in dB used to decide whether a
                        ///< preamble is successfully received
    double m_rssiMin;   ///< Minimum RSSI in dBm that shall be received to start
                        ///< the decision
};
