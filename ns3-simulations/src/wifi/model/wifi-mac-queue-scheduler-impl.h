...
class WifiMacQueueSchedulerImpl : public WifiMacQueueScheduler
{
  public:
    ...
    /** \copydoc
     * ns3::WifiMacQueueScheduler::GetNext(AcIndex,std::optional<uint8_t>) */
    std::optional<WifiContainerQueueId> GetNext(AcIndex ac, std::optional<uint8_t> linkId) final;
    /**
     *  \copydoc
     * ns3::WifiMacQueueScheduler::GetNext(AcIndex,std::optional<uint8_t>, const
  ...
  protected:
    ...
    /**
     * Set the priority for the given container queue belonging to the given
     * Access Category.
     *
     ...
     *
     * \note We cannot store iterators to QueueInfoMap because if rehashing occurs
     * due to an insertion, all iterators are invalidated. References are not
     * invalidated instead. Therefore, we store reference wrappers (which can be
     * reassigned).
     */
     ...
     * \return a const reference to the sorted list of container queues for the
     * given Access Category
     */
  ...
  private:
    /**
     * If no information for the container queue used to store the given MPDU of
     * the given Access Category is present in the queue info map, add the
     * information for such a container queue and initialize the list of the IDs
     * of the links over which packets contained in that container queue can be
     * sent.
     *
     * \param ac the given Access Category
     * \param mpdu the given MPDU
     * \return an iterator to the information associated with the container queue
     * used to store the given MPDU of the given Access Category
     */
    typename QueueInfoMap::iterator InitQueueInfo(AcIndex ac, Ptr<const WifiMpdu> mpdu);

    /**
     * Get the next queue to serve. The search starts from the given one. The
     * returned queue is guaranteed to contain at least an MPDU whose lifetime has
     * not expired. Queues containing MPDUs that cannot be sent over the given
     * link are ignored.
     *
     * \param ac the Access Category that we want to serve
     * \param linkId the ID of the link on which MPDUs contained in the returned
     * queue must be allowed to be sent \param sortedQueuesIt iterator pointing to
     * the queue we start the search from \return the ID of the selected container
     * queue (if any)
     */
    ...
    /**
     * Notify the scheduler that the given MPDU has been enqueued by the given
     * Access Category. The container queue in which the MPDU has been enqueued
     * must be assigned a priority value.
     *
     ...
     * given Access Category. The container queues which became empty after
     * dequeuing the MPDUs are removed from the sorted list of queues.
     *
     ...
     * given Access Category. The container queues which became empty after
     * removing the MPDUs are removed from the sorted list of queues.
     *
    ...
    /**
     * Block or unblock the given set of links for the container queues of the
     * given types and Access Category that hold frames having the given Receiver
     * Address (RA), Transmitter Address (TA) and TID (if needed) for the given
     * reason.
     *
...
WifiMacQueueSchedulerImpl<Priority, Compare>::InitQueueInfo(AcIndex ac, Ptr<const WifiMpdu> mpdu)
{
        ...
        mpdu->GetHeader().GetAddr2() == GetMac()->GetAddress())
    {
        ...
        // between sender and receiver (unless the receiver is the broadcast
        // address)
        NS_ASSERT_MSG(GetMac()->CanForwardPacketsTo(rxAddr), "Cannot forward frame to " << rxAddr);
        // we have to include all the links in case of broadcast frame (we are an
        // AP) and the links that have been setup with the receiver in case of
        // unicast frame
    ...
    else
    {
        ...
        // set the link map to contain one entry corresponding to the computed link
        // ID; unless the link map already contained such an entry (in which case
                                                          ...
                                                          const Priority& priority)
{
    ...
    {
        // an element for queueId is present in the set of sorted queues. If the
        // priority has not changed, do nothing. Otherwise, unlink the node
        // containing such element, change the priority and insert it back
    ...
    typename SortedQueues::iterator sortedQueuesIt)
{
    ...
    while (sortedQueuesIt != m_perAcInfo[ac].sortedQueues.end())
    {
        ...
        {
            // Packets in this queue can be sent over the link we got channel access
            // on. Now remove packets with expired lifetime from this queue. In case
            // the queue becomes empty, the queue is removed from the sorted list and
            // sortedQueuesIt is invalidated; thus, store an iterator to the previous
            // queue in the sorted list (if any) to resume the search afterwards.
...
WifiMacQueueSchedulerImpl<Priority, Compare>::NotifyEnqueue(AcIndex ac, Ptr<WifiMpdu> mpdu)
{
    ...
    // add information for the queue storing the MPDU to the queue info map, if
    // not present yet
    auto queueInfoIt = InitQueueInfo(ac, mpdu);
