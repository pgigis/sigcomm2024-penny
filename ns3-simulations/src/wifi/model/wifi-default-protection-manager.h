...
class WifiDefaultProtectionManager : public WifiProtectionManager
{
  ...
  private:
     ...
     * current DL MU PPDU (represented by the given TX parameters). If the
     * computed protection method is the same as the current one, a null pointer
     * is returned. Otherwise, the computed protection method is returned. The TX
     * width of the PPDU containing the MU-RTS is the same as the DL MU PPDU being
     * protected. Each non-AP station is solicited to transmit a CTS occupying a
     * bandwidth equal to the minimum between the TX width of the DL MU PPDU and
     * the maximum channel width supported by the non-AP station.
     *
     * \param mpdu the given MPDU
     * \param txParams the TX parameters describing the current DL MU PPDU
     * \return the new protection method or a null pointer if the protection
     * method is unchanged
     */
    ...
    /**
     * Calculate the protection method for the UL MU transmission solicited by the
     * given Trigger Frame. The TX width of the PPDU containing the MU-RTS is the
     * same as the TB PPDUs being solicited by the given Trigger Frame. Each
     * non-AP station is solicited to transmit a CTS occupying a bandwidth equal
     * to the minimum between the TX width of the PPDU containing the MU-RTS and
     * the maximum channel width supported by the non-AP station.
     *
     * \param mpdu the given Trigger Frame
     * \param txParams the current TX parameters (just the TXVECTOR needs to be
     * set) \return the protection method for the UL MU transmission solicited by
