 ...
 * \ingroup wifi
 *
 * This class is used to handle the timer that a station starts when
 * transmitting a frame that solicits a response. The timeout can be rescheduled
 * (multiple times) when the RXSTART.indication is received from the PHY.
 */
class WifiTxTimer
{
  public:
     ...
     * this function is called. Note that nothing is done if the timer is not
     * running.
     *
     * \param delay the time to the expiration of the timer
     */
    ...
    /**
     * Set the callback to invoke when the TX timer following the transmission of
     * an MPDU expires.
     *
     * \param callback the callback to invoke when the TX timer following the
     * transmission of an MPDU expires
     */
     ...
     *
     * \param callback the callback to invoke when the TX timer following the
     * transmission of a PSDU expires
     */
     ...
     *
     * \param callback the callback to invoke when the TX timer following the
     * transmission of a PSDU map expires
     */
  ...
  private:
    ...
    std::set<Mac48Address> m_staExpectResponseFrom; //!< the set of stations we expect to receive a
                                                    //!< response from
