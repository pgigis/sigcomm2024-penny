...
class MacRxMiddle : public SimpleRefCount<MacRxMiddle>
{
  ...
  private:
     ...
     * The method creates a new OriginatorRxStatus if one is not already
     * presented.
     *
     ...
     * If the packet is the last fragment, the method tries to re-construct the
     * full packet and return the packet if success.
     *
     * \param packet the packet
     * \param hdr the MAC header
     * \param originator the packet originator status
     *
     * \return a packet if the packet is successfully reassembled (or not a
     * fragment), 0 if we failed to reassemble the packet (e.g. missing
