...
class TableBasedErrorRateModel : public ErrorRateModel
{
  ...
  private:
    ...
    /**
     * Fetch the frame success rate for a given Wi-Fi mode, TXVECTOR, SNR and
     * frame size. \param mode the Wi-Fi mode \param txVector the TXVECTOR \param
     * snr the SNR (linear scale) \param nbits the number of bits \return the
     * frame success rate for a given Wi-Fi mode, TXVECTOR, SNR and frame size
     */
    double FetchFsr(WifiMode mode, const WifiTxVector& txVector, double snr, uint64_t nbits) const;

    Ptr<ErrorRateModel> m_fallbackErrorModel; //!< Error rate model to fallback to if no value is
                                              //!< found in the table

    uint64_t m_threshold; //!< Threshold in bytes over which the table for large
                          //!< size frames is used
};
