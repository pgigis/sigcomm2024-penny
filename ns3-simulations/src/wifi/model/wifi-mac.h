...
class WifiMac : public Object
{
  public:
     ...
     * Get the ID of the link (if any) on which the given PHY is operating.
     *
     * \param phyId the index of the given PHY in the vector of PHYs held by
     * WifiNetDevice \return the ID of the link (if any) on which the given PHY is
     * operating
     */
     ...
     * \return the MLD address of the remote device having the given (MLD or link)
     * address, if the remote device is an MLD.
     */
     ...
     * Specifically:
     * - If the given remote address is the address of a STA affiliated with a
     * remote MLD and operating on a setup link, the address of the local STA
     * operating on such a link is returned.
     * - If the given remote address is the MLD address of a remote MLD (with
     * which some link has been setup), the MLD address of this device is
     * returned.
     * - If this is a single link device, the unique MAC address of this device is
     * returned.
     * - Otherwise, return the MAC address of the affiliated STA (which must
     * exists) that can be used to communicate with the remote device.
     *
    ...
    /**
     * Block the transmission on the given links of all unicast frames addressed
     * to the station with the given address for the given reason. The given MAC
     * address must be the MLD address in case the addressed device is multi-link.
     *
    ...
    /**
     * Unblock the transmission on the given links of all unicast frames addressed
     * to the station with the given address for the given reason. The given MAC
     * address must be the MLD address in case the addressed device is multi-link.
     *
    ...
    /**
     * \param linkId the ID (starting at 0) of the link of the
     * RemoteStationManager object to retrieve \return the remote station manager
     ...
     *
     * This method completes the configuration process for a requested PHY
     * standard by creating the Frame Exchange Manager and the Channel Access
     * Manager and configuring the PHY dependent parameters. This method can only
     * be called after a configured PHY has been set.
     */
     ...
     *
     * Checks if an originator block ack agreement is established with station
     ...
     *
     * Checks if a recipient block ack agreement is established with station
     * addressed by <i>originator</i> for TID <i>tid</i>.
     */
     ...
     *
     * This function returns the type of Block Ack Requests sent by the
     * originator.
     */
    BlockAckReqType GetBarTypeAsRecipient(Mac48Address originator, uint8_t tid) const;

    /**
     * Get the TID-to-Link Mapping negotiated with the given MLD (if any) for the
     * given direction. An empty mapping indicates the default mapping.
     *
    ...
    /**
     * Check whether the given TID is mapped on the given link in the given
     * direction for the given MLD.
     *
     * \param mldAddr the MLD address of the given MLD
     * \param dir the given direction (DL or UL)
     * \param tid the given TID
     * \param linkId the ID of the given link
     * \return whether the given TID is mapped on the given link in the given
     * direction for the given MLD
     */
  ...
  protected:
    ...
    /**
     * Apply the TID-to-Link Mapping negotiated with the given MLD for the given
     * direction by properly configuring the queue scheduler.
     *
    ...
    /**
     * Swap the links based on the information included in the given map. This
     * method is normally called by a non-AP MLD upon completing ML setup to have
     * its link IDs match AP MLD's link IDs.
     *
     * \param links a set of pairs (from, to) each mapping a current link ID to
     * the link ID it has to become (i.e., link 'from' becomes link 'to')
     */
     ...
     * devices. For previous amendments, only one link can be created. Therefore,
     * "link" has not to be confused with the general concept of link for a
     * NetDevice (used by the m_linkUp and m_linkDown callbacks).
     */
    struct LinkEntity
    {
    ...
    /**
     * Update the TID-to-Link Mappings for the given MLD in the given direction
     * based on the given negotiated mappings. An empty mapping indicates the
     * default mapping.
     *
     * \param mldAddr the MLD address of the given MLD
     * \param dir the given direction (DL or UL)
  ...
  private:
     ...
     * \param isDsss vector of flags to indicate whether PHY is DSSS or HR/DSSS
     * for every link \param ac the access category for the DCF
     *
    ...
    /**
     * Configure PHY dependent parameters such as CWmin and CWmax on the given
     * link.
     *
     * \param linkId the ID of the given link
     */
    ...
    /**
     * This method is intended to be called when a link changes ID in order to
     * update the link ID stored by the Frame Exchange Manager and the Channel
     * Access Manager operating on that link.
     *
    ...
    /**
     * This method is called if this device is an MLD to determine the MAC address
     * of the affiliated STA used to communicate with the single link device
     * having the given MAC address. This method is overridden because its
     * implementation depends on the type of station.
     *
     * \param remoteAddr the MAC address of the remote single link device
     * \return the MAC address of the affiliated STA used to communicate with the
     * remote device
     */
    ...
    bool m_ctsToSelfSupported;     ///< flag indicating whether CTS-To-Self is
                                   ///< supported
    ...
    /// @brief DL TID-to-Link Mapping negotiated with an MLD (identified by its
    /// MLD address)
    std::unordered_map<Mac48Address, WifiTidLinkMapping, WifiAddressHash> m_dlTidLinkMappings;
    /// @brief UL TID-to-Link Mapping negotiated with an MLD (identified by its
    /// MLD address)
     ...
     * before the timeout \param missingStations the MAC addresses of the stations
     * that did not respond \param nTotalStations the total number of stations
     * that had to respond
     */
