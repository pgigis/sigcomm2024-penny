                                        ...
                                        const WifiTxParameters& txParams) const
{
    ...
    // An immediate response (Ack or Block Ack) is needed if any of the following
    // holds:
...
WifiDefaultAckManager::TryAddMpdu(Ptr<const WifiMpdu> mpdu, const WifiTxParameters& txParams)
{
    ...
    // if the current acknowledgment method (if any) is already BLOCK_ACK, it will
    // not change by adding an MPDU
    if (txParams.m_acknowledgment &&
    ...
    // we get here if mpdu is a QoS data frame related to an established Block Ack
    // agreement or mpdu is a BlockAckReq frame
                                                 ...
                                                 const WifiTxParameters& txParams)
{
    ...
    if (psduInfo)
    {
        ...
        // acknowledgment points to the m_acknowledgment member of txParams, which
        // is passed as const reference because it must not be modified. Therefore,
        // we make a copy of the object pointed to by acknowledgment and make
        // changes to the copy
                                   ...
                                   !acknowledgment->stationsReplyingWithBlockAck.empty())))
    {
        ...
        if (acknowledgment)
        {
            // txParams.m_acknowledgment points to an existing WifiDlMuBarBaSequence
    ...
    // Add the receiver as the station that will immediately reply with a Normal
    // Ack
    if (acknowledgment)
    {
        // txParams.m_acknowledgment points to an existing WifiDlMuBarBaSequence
                                                   ...
                                                   const WifiTxParameters& txParams)
{
    ...
    if (!psduInfo)
    {
        ...
        if (acknowledgment)
        {
            // txParams.m_acknowledgment points to an existing WifiDlMuAggregateTf
            // object. We have to return a copy of this object including the needed
            // changes
                                           ...
                                           const WifiTxParameters& txParams)
{
    ...
    if (trigger.IsBasic())
    {
        // the only supported ack method for now is through a multi-STA BlockAck
        // frame
        ...
        for (const auto& userInfo : trigger)
        {
            ...
            // we assume the Block Acknowledgment context is used for the multi-STA
            // BlockAck frame (since it requires the longest TX time due to the
            // presence of a bitmap)
