...
class CtrlBAckResponseHeader : public Header
{
  public:
    ...
    /**
     * For Block Ack variants other than Multi-STA Block Ack, set the TID_INFO
     * subfield of the BA Control field. For Multi-STA Block Acks, set the TID
     * subfield of the AID TID Info subfield of the Per AID TID Info subfield
     * identified by the given index.
     *
     * \param tid the traffic ID
     * \param index the index of the Per AID TID Info subfield (Multi-STA Block
     * Ack only)
     */
    void SetTidInfo(uint8_t tid, std::size_t index = 0);
    /**
     * For Block Ack variants other than Multi-STA Block Ack, set the starting
     * sequence number to the given value. For Multi-STA Block Acks, set the
     * starting sequence number in the Per AID TID Info subfield identified by the
    ...
    /**
     * For Block Ack variants other than Multi-STA Block Ack, get the TID_INFO
     * subfield of the BA Control field. For Multi-STA Block Acks, get the TID
     * subfield of the AID TID Info subfield of the Per AID TID Info subfield
     * identified by the given index.
     *
     * \param index the index of the Per AID TID Info subfield (Multi-STA Block
     * Ack only) \return the Traffic ID
     */
    uint8_t GetTidInfo(std::size_t index = 0) const;
    /**
     * For Block Ack variants other than Multi-STA Block Ack, get the starting
     * sequence number. For Multi-STA Block Acks, get the starting sequence number
     * in the Per AID TID Info subfield identified by the given index.
     *
     * \param index the index of the Per AID TID Info subfield (Multi-STA Block
     * Ack only) \return the starting sequence number
     */
     ...
     *
     * \return the number of Per AID TID Info subfields included in this Multi-STA
     * Block Ack
     */
     ...
     * \param index the index of the Per AID TID Info subfield (Multi-STA Block
     * Ack only) \return true if the packet with the given sequence number was
     * ACKed in this BlockAck response, false otherwise
     */
     ...
     *
     * \param index the index of the Per AID TID Info subfield (Multi-STA Block
     * Ack only) \return the value of the Starting Sequence Control subfield
     */
     ...
     * value. For Multi-STA Block Acks, <i>index</i> identifies the Per AID TID
     * Info subfield whose Starting Sequence Control subfield has to be set.
     *
     ...
     *
     * \param index the index of the Per AID TID Info subfield (Multi-STA Block
     * Ack only) \return a const reference to the bitmap from the BlockAck
     * response header
     */
  ...
  private:
    ...
    /**
     * Serialize bitmap to the given buffer. For Multi-STA Block Acks,
     * <i>index</i> identifies the Per AID TID Info subfield whose bitmap has to
     * be serialized.
     *
     * \param start iterator pointing to the beginning of the buffer to write
     * into. \param index the index of the Per AID TID Info subfield (Multi-STA
     * Block Ack only) \return Buffer::Iterator to the next available buffer
     */
    Buffer::Iterator SerializeBitmap(Buffer::Iterator start, std::size_t index = 0) const;
    /**
     * Deserialize bitmap from the given buffer. For Multi-STA Block Acks,
     * <i>index</i> identifies the Per AID TID Info subfield whose bitmap has to
     * be deserialized.
     *
     * \param start iterator pointing to the beginning of the buffer to read from.
     * \param index the index of the Per AID TID Info subfield (Multi-STA Block
     * Ack only) \return Buffer::Iterator to the next available buffer
     */
     ...
     * Multi-STA Block Acks, check if sequence number <i>seq</i> can be
     * acknowledged in the bitmap included in the Per AID TID Info subfield
     * identified by <i>index</i>.
     *
     * \param seq the sequence number
     * \param index the index of the Per AID TID Info subfield (Multi-STA Block
     * Ack only) \return true if the sequence number is concerned by the bitmap
     */
...
/**
 * \ingroup wifi
 * The different variants for Common Info field and User Info field of Trigger
 * Frames.
 */
 ...
 *
 * Trigger frames, introduced by 802.11ax amendment (see Section 9.3.1.23 of
 * D3.0), include one or more User Info fields, each of which carries
 * information about the HE TB PPDU that the addressed station sends in response
...
class CtrlTriggerUserInfoField
{
  public:
     ...
     * This method cannot be called on MU-RTS Trigger Frames (call
     * SetMuRtsRuAllocation instead).
     *
     ...
     *
     * B7–B1 of the RU Allocation subfield is set to indicate the primary 20 MHz
     * channel as follows:
     * - 61 if the primary 20 MHz channel is the only 20 MHz channel or the lowest
     * frequency 20 MHz channel in the primary 40 MHz channel or primary 80 MHz
     * channel
     * - 62 if the primary 20 MHz channel is the second lowest frequency 20 MHz
     * channel in the primary 40 MHz channel or primary 80 MHz channel
     * - 63 if the primary 20 MHz channel is the third lowest frequency 20 MHz
     * channel in the primary 80 MHz channel
     * - 64 if the primary 20 MHz channel is the fourth lowest frequency 20 MHz
     * channel in the primary 80 MHz channel
     *
     * B7–B1 of the RU Allocation subfield is set to indicate the primary 40 MHz
     * channel as follows:
     * - 65 if the primary 40 MHz channel is the only 40 MHz channel or the lowest
     * frequency 40 MHz channel in the primary 80 MHz channel
     * - 66 if the primary 40 MHz channel is the second lowest frequency 40 MHz
     * channel in the primary 80 MHz channel
     *
     * B7–B1 of the RU Allocation subfield is set to 67 to indicate the primary 80
     * MHz channel.
     *
     * B7–B1 of the RU Allocation subfield is set to 68 to indicate the primary
     * and secondary 80 MHz channel.
     *
     ...
     * This method can only be called on MU-RTS Trigger Frames.
     *
     * \return the value of B7–B1 of the RU Allocation subfield (\see
     * SetMuRtsRuAllocation)
     */
    uint8_t GetMuRtsRuAllocation() const;
    /**
     * Set the UL FEC Coding Type subfield, which indicates whether BCC or LDPC is
     * used
     *
    ...
    /**
     * Get the UL FEC Coding Type subfield, which indicates whether BCC or LDPC is
     * used
     *
    ...
    /**
     * Set the UL MCS subfield, which indicates the MCS of the solicited HE TB
     * PPDU
     *
    ...
    /**
     * Get the UL MCS subfield, which indicates the MCS of the solicited HE TB
     * PPDU
     *
    ...
    /**
     * Set the RA-RU Information subfield, which is present when the AID12
     * subfield is 0 or 2045. This method must be called after setting the AID12
     * subfield to 0 or 2045.
     *
     * \param nRaRu the number (from 1 to 32) of contiguous RUs allocated for
     * Random Access. \param moreRaRu whether RA-RUs are allocated in subsequent
     * Trigger frames
     */
    ...
    /**
     * Return true if the UL Target RSSI subfield indicates to the station to
     * transmit an HE TB PPDU response at its maximum transmit power for the
     * assigned MCS
     *
     * \return true if the UL Target RSSI subfield indicates to the station to
     * transmit an HE TB PPDU response at its maximum transmit power for the
  ...
  private:
    ...
    bool m_ulDcm;           //!< whether or not to use Dual Carrier Modulation (HE variant
                            //!< only)
    ...
    uint8_t m_basicTriggerDependentUserInfo;               //!< Basic Trigger variant of Trigger
                                                           //!< Dependent User Info subfield
    CtrlBAckRequestHeader m_muBarTriggerDependentUserInfo; //!< MU-BAR variant of Trigger Dependent
                                                           //!< User Info subfield
};
 ...
 *
 * 802.11ax amendment defines eight types of Trigger frames (see
 * Section 9.3.1.23 of D3.0):
 *   - Basic
 ...
 * minimum time requirement. The size in bytes of the Padding field is
 * configurable.
 */
class CtrlTriggerHeader : public Header
{
  public:
     ...
     *   - the UL Bandwidth, UL Length and GI And LTF Type subfields of the Common
     * Info field are set based on the values stored in the TX vector;
     *   - as many User Info fields as the number of entries in the
     * HeMuUserInfoMap of the TX vector are added to the Trigger Frame. The AID12,
     * RU Allocation, UL MCS and SS Allocation subfields of each User Info field
     * are set based on the values stored in the corresponding entry of the
     * HeMuUserInfoMap.
     *
     ...
     * forbid changing the Common Info field variant type after adding User Info
     * fields.
     *
     ...
     *
     * \return true if this is a Groupcast with Retries (GCR) MU-BAR Trigger
    ...
    /**
     * Get the guard interval duration (in nanoseconds) of the solicited HE TB
     * PPDU.
     *
     * \return the guard interval duration (in nanoseconds) of the solicited HE TB
     * PPDU
     */
     ...
     *
     * \return a Trigger frame including a copy of the Common Info field of this
     * frame.
     */
    ...
    /**
     * \brief Get a const iterator pointing to the first User Info field in the
     * list.
     *
    ...
    /**
     * \brief Get a const iterator indicating past-the-last User Info field in the
     * list.
     *
     * \return a const iterator indicating past-the-last User Info field in the
     * list
     */
    ...
    Iterator begin();
    /**
     * \brief Get an iterator indicating past-the-last User Info field in the
     * list.
     *
    ...
    /**
     * Get a const iterator pointing to the first User Info field found (starting
     * from the one pointed to by the given iterator) whose AID12 subfield is set
     * to the given value.
     *
     * \param start a const iterator pointing to the User Info field to start the
     * search from \param aid12 the value of the AID12 subfield to match \return a
     * const iterator pointing to the User Info field matching the specified
     * criterion, if any, or an iterator indicating past-the-last User Info field.
     */
    ConstIterator FindUserInfoWithAid(ConstIterator start, uint16_t aid12) const;
    /**
     * Get a const iterator pointing to the first User Info field found whose
     * AID12 subfield is set to the given value.
     *
     * \param aid12 the value of the AID12 subfield to match
     * \return a const iterator pointing to the User Info field matching the
     * specified criterion, if any, or an iterator indicating past-the-last User
     * Info field.
     */
    ConstIterator FindUserInfoWithAid(uint16_t aid12) const;
    /**
     * Get a const iterator pointing to the first User Info field found (starting
     * from the one pointed to by the given iterator) which allocates a Random
     * Access RU for associated stations.
     *
     * \param start a const iterator pointing to the User Info field to start the
     * search from \return a const iterator pointing to the User Info field
     * matching the specified criterion, if any, or an iterator indicating
     * past-the-last User Info field.
     */
    ConstIterator FindUserInfoWithRaRuAssociated(ConstIterator start) const;
    /**
     * Get a const iterator pointing to the first User Info field found which
     * allocates a Random Access RU for associated stations.
     *
     * \return a const iterator pointing to the User Info field matching the
     * specified criterion, if any, or an iterator indicating past-the-last User
     * Info field.
     ...
     * Get a const iterator pointing to the first User Info field found (starting
     * from the one pointed to by the given iterator) which allocates a Random
     * Access RU for unassociated stations.
     *
     * \param start a const iterator pointing to the User Info field to start the
     * search from \return a const iterator pointing to the User Info field
     * matching the specified criterion, if any, or an iterator indicating
    ...
    /**
     * Get a const iterator pointing to the first User Info field found which
     * allocates a Random Access RU for unassociated stations.
     *
     * \return a const iterator pointing to the User Info field matching the
     * specified criterion, if any, or an iterator indicating past-the-last User
     * Info field.
     ...
     * TODO Implement the checks listed in Section 27.5.3.2.3 of 802.11ax
     * amendment D3.0 (Allowed settings of the Trigger frame fields and TRS
     * Control subfield).
     *
