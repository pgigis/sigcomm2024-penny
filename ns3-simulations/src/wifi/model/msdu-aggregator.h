...
class MsduAggregator : public Object
{
  public:
     ...
     *
     * - the size of the A-MPDU resulting from the aggregation of the MPDU in
     * which the A-MSDU will be embedded and the current A-MPDU (as specified by
     * the given TX parameters) does not exceed the maximum A-MPDU size as
     * determined for the modulation class indicated by the given TxVector
     *
     ...
     * acknowledgment, as specified by the given TX parameters, does not exceed
     * the given available time (if distinct from Time::Min ())
     *
     * If aggregation succeeds (it was possible to aggregate at least an MSDU to
     * the given MSDU), all the aggregated MSDUs are dequeued and an MPDU
     * containing the A-MSDU is enqueued in the queue (replacing the given MPDU)
     * and returned. Otherwise, no MSDU is dequeued from the EDCA queue and a null
     * pointer is returned.
     *
     ...
     * \return the resulting A-MSDU, if aggregation is possible, a null pointer
     * otherwise.
     */
