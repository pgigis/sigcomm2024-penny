...
class Txop : public Object
{
  public:
    ...
    /**
     * Return the number of slots that make up an AIFS. For 11be multi-link
     * devices, return the number of slots that make up an AIFS on the first link.
     *
    ...
    /**
     * Called by the FrameExchangeManager to notify the completion of the
     * transmissions. This method generates a new backoff and restarts access if
     * needed.
     *
     ...
     * method is normally called by the WifiMac of a non-AP MLD upon completing ML
     * setup to have its link IDs match AP MLD's link IDs.
     *
