...
struct HeMuUserInfo
{
     ...
     * \return true if this user info compares equal to the given user info, false
     * otherwise
     */
     ...
     *
     * \param other the given user info
     * \return true if this user info differs from the given user info, false
     * otherwise
     */
    bool operator!=(const HeMuUserInfo& other) const;
};
...
class WifiTxVector
{
  public:
    ...
    /**
     * Sets the selected payload transmission mode for a given STA ID (for MU
     * only)
     *
     * \param mode
     * \param staId the station ID for MU
    ...
    /**
     * \returns the maximum number of Nss over all RUs of an HE MU (used for
     * OFDMA)
     */
    uint8_t GetNssMax() const;
    /**
     * \returns the total number of Nss for a given RU of an HE MU (used for full
     * bandwidth MU-MIMO)
     */
    ...
    /**
     * Return true if the Trigger Responding parameter is set to true, false
     * otherwise. \return true if the Trigger Responding parameter is set to true,
     * false otherwise
     */
    ...
    bool IsValid() const;
    /**
     * \return true if this TX vector is used for a multi-user (OFDMA and/or
     * MU-MIMO) transmission
     */
    bool IsMu() const;
    /**
     * \return true if this TX vector is used for a downlink multi-user (OFDMA
     * and/or MU-MIMO) transmission
     */
    bool IsDlMu() const;
    /**
     * \return true if this TX vector is used for an uplink multi-user (OFDMA
     * and/or MU-MIMO) transmission
     */
    bool IsUlMu() const;
    /**
     * Return true if this TX vector is used for a downlink multi-user
     ...
     *
     * \param staId the station ID
     * \return the HE MU user-specific transmission information for the given
     * STA-ID
     */
    ...
    /**
     * Get a const reference to the map HE MU user-specific transmission
     * information indexed by STA-ID. This is applicable only for HE MU.
     *
     * \return a const reference to the map of HE MU user-specific information
     * indexed by STA-ID
     */
    const HeMuUserInfoMap& GetHeMuUserInfoMap() const;
    /**
     * Get a reference to the map HE MU user-specific transmission information
     * indexed by STA-ID. This is applicable only for HE MU.
     *
     * \return a reference to the map of HE MU user-specific information indexed
     * by STA-ID
     */
    ...
    /**
     * Get the map of specific user info parameters ordered per increasing
     * frequency RUs.
     *
     * \param p20Index the index of the primary20 channel
     * \return the map of specific user info parameters ordered per increasing
     * frequency RUs
     */
     ...
     *
     * \return true if the Common field is present in the HE-SIG-B, false
     * otherwise
     */
     ...
     *
     * \param inactiveSubchannels the bitmap indexed by the 20 MHz subchannels in
     * ascending order, where each bit indicates whether the corresponding 20 MHz
     * subchannel is punctured or not within the transmission bandwidth
     */
     ...
     *         where each bit indicates whether the corresponding 20 MHz
     * subchannel is punctured or not within the transmission bandwidth
     */
  ...
  private:
     ...
     * This is valid only for allocations of RUs of the same size per 20 MHz
     * subchannel.
     *
     * \param p20Index the index of the primary20 channel
     * \return 8 bit RU_ALLOCATION per 20 MHz
    ...
    std::vector<bool> m_inactiveSubchannels; /**< Bitmap of inactive subchannels
                                                used for preamble puncturing */
    ...
    mutable std::optional<Center26ToneRuIndication>
        m_center26ToneRuIndication; /**< CENTER_26_TONE_RU field when format is
                                       HE_MU and when channel width is set to 80
                                       MHz or larger  (Table 27-1 802.11ax-2021)*/
