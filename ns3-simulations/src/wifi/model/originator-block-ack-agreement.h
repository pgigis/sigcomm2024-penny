...
class OriginatorBlockAckAgreement : public BlockAckAgreement
{
  ...
  public:
     ...
     *
     *  PENDING:
     *    If an agreement is in PENDING state it means that an ADDBARequest frame
     * was sent to recipient in order to setup the block ack and the originator is
     * waiting for the relative ADDBAResponse frame.
     *
     *  ESTABLISHED:
     *    The block ack is active and all packets relative to this agreement are
     * transmitted with Ack Policy set to Block Ack.
     *
     *  NO_REPLY
     *    No reply after an ADDBA request. In this state the originator will send
     * the rest of packets in queue using normal MPDU.
     *
     *  RESET
     *    A transient state to mark the agreement for reinitialization after
     * failed ADDBA request. Since it is a temporary state, it is not included in
     * the state diagram above. In this state the next transmission will be
     * treated as if the BA agreement is not created yet.
     *
     *  REJECTED (not used for now):
     *    The agreement's state becomes REJECTED if an ADDBAResponse frame is
     * received from recipient and the Status Code field is set to failure.
     */
    /// State enumeration
    enum State
    {
