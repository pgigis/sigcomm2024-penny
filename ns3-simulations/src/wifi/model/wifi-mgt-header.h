 ...
 *
 * Provides the type used to store Information Elements in the tuple held by
 * WifiMgtHeader:
 ...
 * - an Information Element of type T that can appear 0 or more times is stored
 * as std::vector\<T\>
 */
template <class T>
struct GetStoredIe
 ...
 * \tparam Tuple \explicit A tuple of the types of Information Elements included
 * in the mgt frame
 */
...
/**
 * \ingroup wifi
 * Base class for implementing management frame headers. This class adopts the
 * CRTP idiom, mainly to allow subclasses to specialize the method used to
 * initialize Information Elements before deserialization
 * (<i>InitForDeserialization</i>).
 *
 * The sorted list of Information Elements that can be included in the
 * management frame implemented as a subclass of this class is provided as the
 * template parameter pack. Specifically:
 * - the type of a mandatory Information Element IE is IE
 * - the type of an optional Information Element IE is std::optional\<IE\>
 * - the type of an Information Element IE that can appear zero or more times is
 * std::vector\<IE\>
 *
 * \tparam Derived \explicit the type of derived management frame
...
class WifiMgtHeader<Derived, std::tuple<Elems...>> : public Header
{
  ...
  protected:
    /**
     * \tparam IE \deduced the type of the Information Element to initialize for
     * deserialization \param optElem the object to initialize for deserializing
     * the information element into
     *
     * The Information Element object is constructed by calling the object's
     * default constructor.
     */
    ...
    /**
     * \param optElem the EhtCapabilities object to initialize for deserializing
     * the information element into
     */
...
/**
 * \ingroup wifi
 *  Inspect a type to deduce whether it is an Information Element that can be
 * included in a Per-STA Profile subelement of a Multi-Link Element. \tparam T
 * \explicit The type to inspect.
 */
template <class T>
    ...
    std::optional<EhtCapabilities>& optElem)
{
    ...
    const bool is2_4Ghz =
        rates.IsSupportedRate(1000000 /* 1 Mbit/s */); // TODO: use presence of VHT capabilities IE
                                                       // and HE 6 GHz Band Capabilities IE once the
 ...
 * \tparam T \deduced the type of the Information Element
 * \param elem the optional Information Element
 * \return the serialized size of the Information Element, if present, or 0,
 * otherwise
 */
template <typename T>
uint16_t
 ...
 * \return whether the given Information Element shall be serialized in a
 * Per-STA Profile subelement of the Multi-Link Element included in the
 * containing management frame
 */
...
MustBeSerializedInPerStaProfile(const std::optional<T>& elem, const Derived& frame)
{
    ...
    {
        // the IE is present both outside the Multi-Link Element and in the Per-STA
        // Profile, but they are different, hence the IE must be serialized in the
        // Per-STA Profile
        return true;
    }
    ...
    {
        // the IE is not present outside the Multi-Link Element and is present in
        // the Per-STA Profile, hence the IE must be serialized in the Per-STA
        // Profile
        return true;
    }
 ...
 * \param frame the containing management frame
 * \return whether the given Information Elements shall be serialized in a
 * Per-STA Profile subelement of the Multi-Link Element included in the
 * containing management frame
 */
template <typename T, typename Derived>
...
MustBeSerializedInPerStaProfile(const std::vector<T>& elems, const Derived& frame)
{
    ...
    {
        // the IEs are present both outside the Multi-Link Element and in the
        // Per-STA Profile, but they are different, hence the IEs must be serialized
        // in the Per-STA Profile
        return true;
    }

    if (frame.template Get<T>().empty() && !elems.empty())
    {
        // the IEs are not present outside the Multi-Link Element and is present in
        // the Per-STA Profile, hence the IEs must be serialized in the Per-STA
        // Profile
        return true;
    }
 ...
 * \param elem the given Information Element
 * \param frame the containing management frame
 * \return a pair (Element ID, Element ID Extension) if the given Information
 * Element shall be listed in the Non-Inheritance IE of the Per-STA Profile
 * subelement of the Multi-Link Element included in the containing management
 * frame
 */
template <typename T, typename Derived>
 ...
 * \param frame the containing management frame
 *
 * Copy the given Information Element from the containing frame to the Per-STA
 * Profile subelement of the Multi-Link Element, if the Information Element has
 * been inherited (i.e., it is present outside the Multi-Link Element and not
 * present in the Per-STA Profile subelement)
 */
template <typename T, typename Derived>
void
 ...
 *
 * Remove the given Information Element from this header, if it is present and
 * is listed in the given Non-Inheritance element.
 */
template <typename T>
void
 ...
 * \param nonInheritance the Non-Inheritance information element
 *
 * Remove the given Information Elements from this header, if they are present
 * and are listed in the given Non-Inheritance element.
 */
template <typename T>
void
...
MgtHeaderInPerStaProfile<Derived, std::tuple<Elems...>>::CopyIesFromContainingFrame(
    const Derived& frame)
{
    // copy inherited Information Elements that appear in the containing frame
    // after the MLE (those appearing before have been copied by
    // DeserializeFromPerStaProfileImpl)
    std::apply(
        [&](auto&... elems) { ((internal::DoCopyIeFromContainingFrame(elems, frame)), ...); },
        m_elements);

    // we have possibly deserialized a Non-Inheritance element; remove IEs listed
    // therein
    if (m_nonInheritance)
    {
        std::apply(
