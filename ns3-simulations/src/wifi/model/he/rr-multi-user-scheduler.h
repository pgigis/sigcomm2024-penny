 ...
 * \ingroup wifi
 *
 * RrMultiUserScheduler is a simple OFDMA scheduler that indicates to perform a
 * DL OFDMA transmission if the AP has frames to transmit to at least one
 * station. RrMultiUserScheduler assigns RUs of equal size (in terms of tones)
 * to stations to which the AP has frames to transmit belonging to the AC who
 * gained access to the channel or higher. The maximum number of stations that
 * can be granted an RU is configurable. Associated stations are served based on
 * their priority. The priority is determined by the credits/debits a station
 * gets when it is selected or not for transmission.
 *
 * \todo Take the supported channel width of the stations into account while
 * selecting stations and assigning RUs to them.
 */
class RrMultiUserScheduler : public MultiUserScheduler
{
  ...
  private:
     ...
     * time limits.
     *
     * \return UL_MU_TX if it is possible to send a Basic TF, DL_MU_TX if we can
     * try to send a DL MU PPDU and NO_TX if the remaining time is too short
     */
     ...
     *         can be transmitted (e.g., there are no HE stations associated or
     * sending a DL MU PPDU is not possible and m_forceDlOfdma is false) or NO_TX
     * otherwise
     */
     ...
     * transmissions from suitable stations, i.e., stations that have established
     * a BlockAck agreement with the AP and for which the given predicate returns
     * true.
     *
     * \tparam Func \deduced the type of the given predicate
     * \param canBeSolicited a predicate returning false for stations that shall
     * not be solicited \return a TXVECTOR that can be used to construct a Trigger
     * Frame to solicit transmissions from suitable stations
     */
    template <class Func>
     ...
     * TXVECTOR contains a subset of such HeMuUserInfo entries. The set of
     * candidate stations is also updated by removing stations that are not
     * allocated an RU.
     *
    ...
    /**
     * Update credits of the stations in the given list considering that a PPDU
     * having the given duration is being transmitted or solicited by using the
     * given TXVECTOR.
     *
     * \param staList the list of stations
     * \param txDuration the TX duration of the PPDU being transmitted or
     * solicited \param txVector the TXVECTOR for the PPDU being transmitted or
     * solicited
     */
