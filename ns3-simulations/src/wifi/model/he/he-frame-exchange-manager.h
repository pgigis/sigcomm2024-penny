 ...
 * \return true if the given PSDU map contains a single PSDU including a single
 * MPDU that carries a Trigger Frame
 */
...
class HeFrameExchangeManager : public VhtFrameExchangeManager
{
  public:
     ...
     *
     * \param muScheduler the Multi-user Scheduler associated with this Frame
     * Exchange Manager
     */
    void SetMultiUserScheduler(const Ptr<MultiUserScheduler> muScheduler);

    /**
     * Get the PSDU in the given PSDU map that is addressed to the given MAC
     * address, if any, or a null pointer, otherwise.
     *
    ...
    /**
     * Get the RSSI (in dBm) of the most recent packet received from the station
    ...
    /**
     * Return whether the received frame is classified as intra-BSS. It is assumed
     * that this station is already associated with an AP.
     *
     * \param psdu the received PSDU
     * \param txVector TX vector of the received PSDU
     * \return true if the received frame is classified as intra-BSS, false
     * otherwise (the received frame is classified as inter-BSS or it cannot be
     * classified as intra-BSS or inter-BSS)
     */
    bool IsIntraBssPpdu(Ptr<const WifiPsdu> psdu, const WifiTxVector& txVector) const;

    /**
     * This method is intended to be called a SIFS after the reception of a
     * Trigger Frame to determine whether the station is allowed to respond.
     *
     * \param trigger the Trigger Frame soliciting a response
     * \return true if CS is not required or the UL MU CS mechanism indicates that
     * the medium is idle, false otherwise
     */
  ...
  protected:
    ...
    /**
     * Clear the TXOP holder if the intra-BSS NAV counted down to zero (includes
     * the case of intra-BSS NAV reset).
     */
    ...
    /**
     * Compute how to set the Duration/ID field of an MU-RTS Trigger Frame to send
     * to protect a frame transmitted with the given TX vector.
     *
     ...
     * frame \return the computed Duration/ID value for the MU-RTS Trigger Frame
     */
     ...
     *
     * \param txParams the TX parameters for the data frame protected by the
     * MU-RTS TF.
     */
    ...
    /**
     * Get the TXVECTOR that the station having the given station ID has to use to
     * send a CTS frame after receiving an MU-RTS Trigger Frame from the AP it is
     * associated with.
     *
     ...
     * Note that both <i>psduMap</i> and <i>txParams</i> are moved to m_psduMap
     * and m_txParams, respectively, and hence are left in an undefined state.
     *
     ...
     * \param nSolicitedStations the number of stations solicited to send a TB
     * PPDU
     */
    ...
    /**
     * Return a TXVECTOR for the UL frame that the station will send in response
     * to the given Trigger frame, configured with the BSS color and transmit
     * power level to use for the consequent HE TB PPDU. Note that this method
     * should only be called by non-AP stations only.
     *
     ...
     * \param recipients the list of BlockAckReq headers indexed by the station's
     * AID \return the MPDU containing the built MU-BAR
     */
     ...
     *
     * \param txParams the TX parameters for the Trigger Frame that solicited the
     * TB PPDUs \param durationId the Duration/ID field of the HE TB PPDU that
     * elicited the Multi-STA Block Ack response
     */
    void SendMultiStaBlockAck(const WifiTxParameters& txParams, Time durationId);

    /**
     * Send QoS Null frames in response to a Basic or BSRP Trigger Frame. The
     * number of QoS Null frames that are actually aggregated depends on the
     * available time as indicated by the Trigger Frame and is at most 8 (one QoS
     * Null frame per TID).
     *
    ...
    EventId m_intraBssNavResetEvent; //!< the event to reset the intra-BSS NAV
                                     //!< after an RTS
  ...
  private:
     ...
     * \param durationId the Duration/ID field of the MPDU carrying the Trigger
     * Frame \param snr the receive SNR
     */
    ...
    bool m_triggerFrameInAmpdu;            //!< True if the received A-MPDU contains an
                                           //!< MU-BAR
};
