 ...
 * supporting at least VHT can use to determine the format of their next
 * transmission. VHT APs can only transmit DL MU PPDUs by using MU-MIMO, while
 * HE APs can transmit both DL MU PPDUs and UL MU PPDUs by using OFDMA in
 * addition to MU-MIMO.
 *
...
class MultiUserScheduler : public Object
{
  public:
     ...
     * access. The Multi-user Scheduler determines the format of the next
     * transmission.
     *
     ...
     * to determine whether the TXOP limit can be exceeded \param allowedWidth the
     * allowed width in MHz for the next transmission \param linkId the ID of the
     * link over which channel access was gained \return the format of the next
     * transmission
     */
    ...
    /**
     * Get the information required to perform a DL MU transmission on the given
     * link. Note that this method can only be called if GetTxFormat returns
     * DL_MU_TX on the given link.
     *
    ...
    /**
     * Get the information required to solicit an UL MU transmission on the given
     * link. Note that this method can only be called if GetTxFormat returns
     * UL_MU_TX on the given link.
     *
     * \param linkId the ID of the given link
    ...
    /**
     * Set the duration of the interval between two consecutive requests for
     * channel access made by the MultiUserScheduler.
     *
     * \param interval the duration of the interval between two consecutive
     * requests for channel access
     */
  ...
  protected:
     ...
     *
     * \param trigger the given Trigger Frame
     * \return the maximum size in bytes among the A-MPDUs containing QoS Null
     * frames and solicited by the given Trigger Frame
     */
    ...
    uint16_t m_allowedWidth; //!< the allowed width in MHz for the current
                             //!< transmission
    uint8_t m_linkId;        //!< the ID of the link over which channel access has been
                             //!< granted
  ...
  private:
    ...
    EventId m_accessReqTimer;                   ///< the timer controlling additional channel
                                                ///< access requests
    Time m_accessReqInterval;                   ///< duration of the interval between channel
                                                ///< access requests
    AcIndex m_accessReqAc;                      ///< AC we request channel access for
    bool m_restartTimerUponAccess;              ///< whether the channel access timer has to
                                                ///< be restarted upon channel access
};
