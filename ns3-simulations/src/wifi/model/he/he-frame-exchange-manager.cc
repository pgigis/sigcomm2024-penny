...
{
    NS_ASSERT(triggerSender != m_self); // TxPower information is used only by STAs, it is useless
    ...
    NS_ASSERT(optRssi);
    int8_t pathLossDb =
        trigger.GetApTxPower() - static_cast<int8_t>(*optRssi); // cast RSSI to be on equal footing
                                                                // with AP Tx power information
