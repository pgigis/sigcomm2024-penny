...
RrMultiUserScheduler::GetTxVectorForUlMu(Func canBeSolicited)
{
    ...
    // iterate over the associated stations until an enough number of stations is
    // identified
               ...
               std::min<std::size_t>(m_nStations, count + nCentral26TonesRus))
    {
        ...
        // if the first candidate STA is an EHT STA, we switch to soliciting EHT TB
        // PPDUs
        ...
        // prepare the MAC header of a frame that would be sent to the candidate
        // station, just for the purpose of retrieving the TXVECTOR used to transmit
        // to that station
...
RrMultiUserScheduler::TrySendingBasicTf()
{
    ...
    // Compute the time taken by each station to transmit a frame of maxBufferSize
    // size
    ...
    else
    {
        ...
        // transmit at least m_ulPsduSize bytes, give up the UL MU transmission for
        // now
    ...
    // maxDuration is the time to grant to the stations. Finalize the Trigger
    // Frame
    uint16_t ulLength;
        ...
        for (uint8_t tid : tids)
        {
            ...
            if (m_apMac->GetBaAgreementEstablishedAsOriginator(staIt->address, tid))
            {
                ...
                if (mpdu)
                {
                    ...
                    // candidate station to check if the MPDU meets the size and time
                    // limits. An RU of the computed size is tentatively assigned to the
                    // candidate station, so that the TX duration can be correctly
                    // computed.
                    WifiTxVector suTxVector =
                    ...
                    // the first candidate STA determines the preamble type for the DL MU
                    // PPDU
                                    ...
                                    const WifiTxVector& txVector)
{
    ...
    // Transmitting stations have to pay a number of credits equal to the TX
    // duration (in microseconds) times the allocated bandwidth share.
    double debitsPerMhz =
...
RrMultiUserScheduler::ComputeDlMuInfo()
{
    ...
    // Compute the TX params (again) by using the stored MPDUs and the final
    // TXVECTOR
    Time actualAvailableTime = (m_initialFrame ? Time::Min() : m_availableTime);
