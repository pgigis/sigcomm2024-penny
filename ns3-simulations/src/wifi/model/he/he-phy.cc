                            ...
                            Time rxDuration)
{
    ...
    {
        VhtPhy::StartReceivePreamble(ppdu,
                                     rxPowersW,
                                     ppdu->GetTxDuration()); // The actual duration of the PPDU
                                                             // should be used
    }
}
        ...
        ppdu->GetType() == WIFI_PPDU_TYPE_UL_MU || isResponseToTrigger)
    {
        ...
        const auto rxDuration =
            (ppdu->GetType() == WIFI_PPDU_TYPE_UL_MU)
                ? CalculateNonHeDurationForHeTb(txVector) // the HE portion of the transmission will
                                                          // be added later on
                : ppdu->GetTxDuration();
...
HePhy::ProcessSigA(Ptr<Event> event, PhyFieldRxStatus status)
{
    ...
    NotifyEndOfHeSigA(params); // if OBSS_PD CCA_RESET, set power restriction first and wait
