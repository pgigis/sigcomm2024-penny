...
class HeRu
{
  public:
     ...
     * subfield of the User Info field of Trigger frames (see 9.3.1.22.1 of
     * 802.11ax D8.0). Note that primary80MHz must be true if ruType is
     * RU_2x996_TONE. Internally, this class also stores the RU PHY index (ranging
     * from 1 to the number of RUs of the given type in a channel of the
     * considered width), so that this class contains all the information needed
     * to locate the RU in a 160 MHz channel.
     */
    class RuSpec
    {
      public:
         ...
         * \param primary80MHz whether the RU is allocated in the primary 80MHz
         * channel
         */
         ...
         *
         * \return true if the RU is in the primary 80 MHz channel and false
         * otherwise
         */
      ...
      private:
        ...
        bool m_primary80MHz; //!< true if the RU is allocated in the primary 80MHz
                             //!< channel
    };
     ...
     * the 160 MHz channel (i.e. -1012 to 1012). The PHY index parameter is used
     * to distinguish between lower and higher 80 MHz subchannels.
     *
     * \param bw the bandwidth (MHz) of the HE PPDU (20, 40, 80, 160)
     * \param ruType the RU type (number of tones)
     ...
     * assigned an RU, maximize the number of candidate stations that can be
     * assigned an RU subject to the constraint that all the stations must be
     * assigned an RU of the same size (in terms of number of tones).
     *
     ...
     * \param[out] nCentral26TonesRus the number of additional 26-tone RUs that
     * can be allocated if the returned RU size is greater than 26 tones \return
     * the RU type
     */
    ...
    /// Subcarrier groups for all RUs (with indices being applicable to primary 80
    /// MHz channel)
    ...
    /// \param ruType equal size RU type (generated by
    /// GetEqualSizedRusForStations) \param isOdd if number of stations is an odd
    /// number \return RU_ALLOCATION value
