...
class WifiPhy : public Object
{
  public:
    ...
    /**
     * For HE receptions only, check and possibly modify the transmit power
     * restriction state at the end of PPDU reception.
     */
    ...
    /**
     * This function is a wrapper for the Send variant that accepts a
     * WifiConstPsduMap as first argument. This function inserts the given PSDU in
     * a WifiConstPsduMap along with a STA-ID value that is determined based on
     * whether the given PSDU has to be transmitted as a DL or UL frame.
     *
    ...
    /**
     * \return true of the current state of the PHY layer is WifiPhy::IDLE, false
     * otherwise.
     */
    bool IsStateIdle() const;
    /**
     * \return true of the current state of the PHY layer is WifiPhy::CCA_BUSY,
     * false otherwise.
     */
    bool IsStateCcaBusy() const;
    /**
     * \return true of the current state of the PHY layer is WifiPhy::RX, false
     * otherwise.
     */
    bool IsStateRx() const;
    /**
     * \return true of the current state of the PHY layer is WifiPhy::TX, false
     * otherwise.
     */
    bool IsStateTx() const;
    /**
     * \return true of the current state of the PHY layer is WifiPhy::SWITCHING,
     * false otherwise.
     */
    bool IsStateSwitching() const;
    /**
     * \return true if the current state of the PHY layer is WifiPhy::SLEEP, false
     * otherwise.
     */
    bool IsStateSleep() const;
    /**
     * \return true if the current state of the PHY layer is WifiPhy::OFF, false
     * otherwise.
     */
    ...
    /**
     * This function is a wrapper for the CalculateTxDuration variant that accepts
     * a WifiConstPsduMap as first argument. This function inserts the given PSDU
     * in a WifiConstPsduMap along with a STA-ID value that is determined based on
     * whether the given PSDU has to be transmitted as a DL or UL frame, thus
     * allowing to properly calculate the TX duration in case the PSDU has to be
     * transmitted as an UL frame.
     *
    ...
    /**
     * Get the list of supported (non-MCS) modes for the given modulation class
     * (i.e. corresponding to a given PHY entity).
     *
     * \param modulation the modulation class
     * \return the list of supported (non-MCS) modes for the given modulation
     * class.
     *
     * \see GetModeList ()
     */
    ...
    /**
     * Check if the given MCS of the given modulation class is supported by the
     * PHY.
     *
     * \param modulation the modulation class
     ...
     * can support - a set of MCS indices which is stored by each HT PHY (and
     * above).
     *
     ...
     * Note that, in case a Spectrum PHY is used, this method must be called after
     * adding a spectrum channel covering the operating channel bandwidth.
     *
     ...
     *
     * \param psdus the PSDUs being transmitted (only one unless DL MU
     * transmission) \param txPowerW the transmit power in Watts
     */
     ...
     *        on a nearby channel.
     * \param txVector the TXVECTOR that holds RX parameters
     * \param aMpdu the type of the packet (0 is not A-MPDU, 1 is a MPDU that is
     * part of an A-MPDU and 2 is the last MPDU in an A-MPDU) and the A-MPDU
     * reference number (must be a different value for each A-MPDU but the same
     * for each subframe within one A-MPDU) \param signalNoise signal power and
     * noise power in dBm \param staId the STA-ID
     *
     ...
     * part of an A-MPDU and 2 is the last MPDU in an A-MPDU) and the A-MPDU
     * reference number (must be a different value for each A-MPDU but the same
     * for each subframe within one A-MPDU) \param staId the STA-ID
     *
     * \todo WifiTxVector should be passed by const reference because of its size.
     */
    ...
    /**
     * Sets the CCA energy detection threshold (dBm). The energy of a all received
     * signals should be higher than this threshold to allow the PHY layer to
     * declare CCA BUSY state.
     *
    ...
    /**
     * Sets the CCA sensitivity threshold (dBm). The energy of a received wifi
     * signal should be higher than this threshold to allow the PHY layer to
     * declare CCA BUSY state.
     *
    ...
    using ChannelTuple = std::tuple<uint8_t /* channel number */,
                                    ...
                                    int /* WifiPhyBand */,
                                    uint8_t /* primary20 index*/>; //!< Tuple identifying an
                                                                   //!< operating channel

    /**
     * If the standard for this object has not been set yet, store the given
     * channel settings. Otherwise, check if a channel switch can be performed
     * now. If not, schedule another call to this method when channel switch can
     * be performed. Otherwise, set the operating channel based on the given
     * channel settings and call ConfigureStandard if the PHY band has changed.
     *
     * Note that, in case a Spectrum PHY is used, a spectrum channel covering the
     * operating channel bandwidth must have been already added when actually
    ...
    /**
     * If the standard for this object has not been set yet, store the channel
     * settings corresponding to the given operating channel. Otherwise, check if
     * a channel switch can be performed now. If not, schedule another call to
     * this method when channel switch can be performed. Otherwise, set the given
     * operating channel and call ConfigureStandard if the PHY band has changed.
     *
     * Note that, in case a Spectrum PHY is used, a spectrum channel covering the
     * operating channel bandwidth must have been already added when actually
     * setting the operating channel.
    ...
    /**
     * Get the bandwidth for a transmission occurring on the current operating
     * channel and using the given WifiMode, subject to the constraint that the TX
     * bandwidth cannot exceed the given maximum allowed value.
     *
     ...
     * In SpectrumWifiPhy implementation, the power levels are equally spaced (in
     * dBm).
     *
    ...
    /**
     * Reset PHY to IDLE, with some potential TX power restrictions for the next
     * transmission.
     *
     * \param powerRestricted flag whether the transmit power is restricted for
     * the next transmission \param txPowerMaxSiso the SISO transmit power
     * restriction for the next transmission in dBm \param txPowerMaxMimo the MIMO
     * transmit power restriction for the next transmission in dBm
     */
    ...
    /**
     * This is a helper function to convert start and stop indices to start and
     * stop frequencies.
     *
     ...
     * This typically returns the entity corresponding to the modulation class
     * used to transmit the PPDU. If the modulation class used to transmit the
     * PPDU is not supported by the PHY, the latest PHY entity corresponding to
     * the configured standard is returned. If the modulation used to transmit the
     * PPDU is non-HT (duplicate), the latest PHY entity corresponding to the
    ...
    /**
     * \return the UID of the previously received PPDU (reset to UINT64_MAX upon
     * transmission)
     */
     ...
     *
     * \note This method shall only be used in exceptional circumstances, such as
     * when a PHY transmits a response to a Trigger Frame that was received by
     * another PHY. This is the case, e.g., when an aux PHY of an EMLSR client
     * receives an ICF but it is the main PHY that switches channel and transmits
     * the response to the ICF.
     */
  ...
  protected:
    ...
    Ptr<InterferenceHelper> m_interference; //!< Pointer to a helper responsible
                                            //!< for interference computations

    Ptr<UniformRandomVariable> m_random; //!< Provides uniform random variables.
    Ptr<WifiPhyStateHelper> m_state;     //!< Pointer to WifiPhyStateHelper

    uint32_t m_txMpduReferenceNumber; //!< A-MPDU reference number to identify
                                      //!< all transmitted subframes belonging to
                                      //!< the same received A-MPDU
    uint32_t m_rxMpduReferenceNumber; //!< A-MPDU reference number to identify
                                      //!< all received subframes belonging to
                                      //!< the same received A-MPDU
    ...
    std::map<std::pair<uint64_t /* UID*/, WifiPreamble>, Ptr<Event>>
        m_currentPreambleEvents; //!< store event associated to a PPDU (that has
                                 //!< a unique ID and preamble combination) whose
                                 //!< preamble is being received

    uint64_t m_previouslyRxPpduUid; //!< UID of the previously received PPDU,
                                    //!< reset to UINT64_MAX upon transmission
  ...
  private:
    ...
    ChannelTuple m_channelSettings;             //!< Store operating channel settings until
                                                //!< initialization
    WifiPhyOperatingChannel m_operatingChannel; //!< Operating channel
    bool m_fixedPhyBand;                        //!< True to prohibit changing PHY band after
                                                //!< initialization
    ...
    double m_ccaEdThresholdW;          //!< Clear channel assessment (CCA) energy
                                       //!< detection (ED) threshold in watts
    double m_ccaSensitivityThresholdW; //!< Clear channel assessment (CCA)
    ...
    bool m_powerRestricted;        //!< Flag whether transmit power is restricted by
                                   //!< OBSS PD SR
    double m_txPowerMaxSiso;       //!< SISO maximum transmit power due to OBSS PD SR
                                   //!< power restriction (dBm)
    double m_txPowerMaxMimo;       //!< MIMO maximum transmit power due to OBSS PD SR
                                   //!< power restriction (dBm)
    bool m_channelAccessRequested; //!< Flag if channels access has been
                                   //!< requested (used for OBSS_PD SR)
    ...
    Callback<void> m_capabilitiesChangedCallback; //!< Callback when PHY
