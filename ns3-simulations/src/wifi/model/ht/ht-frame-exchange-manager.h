...
class HtFrameExchangeManager : public QosFrameExchangeManager
{
  public:
    ...
    /**
     * Check if the PSDU obtained by aggregating the given MPDU to the PSDU
     * specified by the given TX parameters meets the constraints on the maximum
     * A-MPDU size and its transmission time does not exceed the given PPDU
     * duration limit (if different than Time::Min()).
     *
    ...
    /**
     * Check whether an A-MPDU of the given size meets the constraint on the
     * maximum size for A-MPDUs sent to the given receiver, belonging to the given
     * TID and transmitted using the given modulation class.
     *
     * \param ampduSize the size in bytes of the A-MPDU.
     * \param receiver the address of the station that is the receiver of the
     * A-MPDU \param tid the TID of the A-MPDU \param modulation the modulation
     * class used to transmit the A-MPDU \return true if the constraint on the max
     * A-MPDU size is met.
     */
     ...
     * \param txParams the TX parameters
     * \param availableTime the constraint on the TX time of the PSDU, if
     * different than Time::Min() \return true if aggregating an MSDU to the
     * current PSDU does not violate the size and time constraints
     */
    ...
    /**
     * Check if the PSDU obtained by aggregating the given MSDU to the PSDU
     * specified by the given TX parameters meets the constraints on the maximum
     * A-MSDU size and its transmission time does not exceed the given PPDU
     * duration limit (if different than Time::Min()).
     *
     * \param msdu the given MSDU
     * \param txParams the TX parameters
    ...
    /**
     * Get the next BlockAckRequest or MU-BAR Trigger Frame to send, if any. If
     * TID and recipient address are given, then only return a BlockAckRequest, if
     * any, addressed to that recipient and for the given TID.
     *
     * \param ac the AC whose queue is searched for BlockAckRequest or Trigger
     * Frames \param optTid the TID (optional) \param optAddress the recipient of
     * the BAR (optional)
     *
  ...
  protected:
    ...
    /**
     * Compute how to set the Duration/ID field of PSDUs that do not include
     * fragments.
     *
    ...
    /**
     * Dequeue the MPDUs of the given PSDU from the queue in which they are
     * stored.
     *
    ...
    /**
     * If the given MPDU contains a BlockAckReq frame (the duration of which plus
     * the response fits within the given available time, if the latter is not
     * Time::Min() and this is not the initial frame of a TXOP), transmit the
     * frame and return true. Otherwise, return false.
     *
     * \param mpdu the given MPDU
     * \param availableTime the amount of time allowed for the frame exchange.
     * Equals Time::Min() in case the TXOP limit is null \param initialFrame true
     * if the frame being transmitted is the initial frame of the TXOP. This is
     * used to determine whether the TXOP limit can be exceeded \return true if
     * frame is transmitted, false otherwise
     */
    virtual bool SendMpduFromBaManager(Ptr<WifiMpdu> mpdu, Time availableTime, bool initialFrame);

    /**
     * Given a non-broadcast QoS data frame, prepare the PSDU to transmit by
     * attempting A-MSDU and A-MPDU aggregation (if enabled), while making sure
     * that the frame exchange (possibly including protection and acknowledgment)
     * is completed within the given available time.
     *
    ...
    /**
     * A Block Ack agreement needs to be established with the given recipient for
     * the given TID if it does not already exist (or exists and is in state
     * RESET) and:
     *
     * - the number of packets in the queue reaches the BlockAckThreshold value OR
     * - MPDU aggregation is enabled and there is more than one packet in the
     * queue OR
     * - the station is a VHT station
     *
     * \param recipient address of the recipient.
     * \param tid traffic ID.
     * \return true if a Block Ack agreement needs to be established, false
     * otherwise.
     */
     ...
     * \param availableTime the amount of time allowed for the frame exchange.
     * Equals Time::Min() in case the TXOP limit is null \return true if ADDBA
     * Request frame is transmitted, false otherwise
     */
    ...
    /**
     * Create a BlockAck frame with header equal to <i>blockAck</i> and start its
     * transmission.
     *
     * \param agreement the agreement the Block Ack response belongs to
     * \param durationId the Duration/ID of the frame soliciting this Block Ack
     * response \param blockAckTxVector the transmit vector for the Block Ack
     * response \param rxSnr the receive SNR
     */
     ...
     *
     * \param psdu the PSDU (BlockAckReq or A-MPDU) that solicited a BlockAck
     * response \param txVector the TXVECTOR used to send the PSDU that solicited
     * a BlockAck response
     */
     ...
     * \param psdu the PSDU (BlockAckReq or A-MPDU) that solicited a BlockAck
     * response \param txVector the TXVECTOR used to send the PSDU that solicited
     * a BlockAck response \param[out] resetCw true if we shall stop
     * retransmitting the PSDU
     */
