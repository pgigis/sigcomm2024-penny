...
class WifiMacQueueScheduler : public Object
{
  public:
    ...
    /**
     * Get the next queue to serve, which is guaranteed to contain at least an
     * MPDU whose lifetime has not expired. Queues containing MPDUs that cannot be
     * sent over the given link (if any) are ignored.
     *
     ...
     * queue must be allowed to be sent \return the ID of the selected container
     * queue (if any)
     */
     ...
     * Queues containing MPDUs that cannot be sent over the given link (if any)
     * are ignored.
     *
     * \param ac the Access Category that we want to serve
     * \param linkId the ID of the link on which MPDUs contained in the returned
     * queue must be allowed to be sent \param prevQueueId the ID of the container
     * queue served previously \return the ID of the selected container queue (if
     * any)
     */
    ...
    /**
     * Block the given set of links for the container queues of the given types
     * and Access Category that hold frames having the given Receiver Address
     * (RA), Transmitter Address (TA) and TID (if needed) for the given reason,
     * such that frames in these queues are not transmitted on the given set of
     * links.
     *
    ...
    /**
     * Unblock the given set of links for the container queues of the given types
     * and Access Category that hold frames having the given Receiver Address
     * (RA), Transmitter Address (TA) and TID (if needed) for the given reason,
     * such that frames in these queues can be transmitted on the given set of
     * links.
     *
    ...
    /// Bitset identifying the reasons to block individual links for a container
    /// queue
    using Mask = std::bitset<static_cast<std::size_t>(WifiQueueBlockedReason::REASONS_COUNT)>;

    /**
     * Get the mask associated with the given container queue indicating whether
     * the given link is blocked and for which reason, provided that the given
     * container queue exists and has a mask for the given link.
     *
     * \param ac the given Access Category
     * \param queueId the ID of the given container queue
     * \param linkId the ID of the given link
     * \return the mask associated with the given container queue for the given
     * link
     */
