 ...
 *
 * Data frames transmitted by an 11be MLD must include link addresses as (RA,
 * TA) which are different than the MLD addresses seen by the upper layer. In
 * order to keep the original version of the data frame, we create an alias when
 * a data frame is sent over a link. Aliases are stored in the map of in-flight
 * MPDUs, which is indexed by the ID of the link over which the alias is
 * in-flight. For consistency, also data frame transmitted by non-MLDs have an
 * alias, which is simply a pointer to the original version of the data frame.
 */
