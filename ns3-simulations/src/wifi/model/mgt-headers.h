...
/**
 * Indicate which Information Elements cannot be included in a Per-STA Profile
 * subelement of a Basic Multi-Link Element (see Sec. 35.3.3.4 of 802.11be
 * D3.1):
 *
 * An AP affiliated with an AP MLD shall not include a Timestamp field, a Beacon
 * Interval field, an AID field, a BSS Max Idle Period element, a Neighbor
 * Report element, a Reduced Neighbor Report element, a Multiple BSSID element,
 * TIM element, Multiple BSSID-Index element, Multiple BSSID Configuration
 * element, TID-to-Link Mapping element, Multi-Link Traffic Indication element
 * or another Multi- Link element in the STA Profile field of the Basic
 * Multi-Link element.
 *
 * A non-AP STA affiliated with a non-AP MLD shall not include a Listen Interval
 * field, a Current AP Address field, an SSID element, BSS Max Idle Period
 * element or another Multi-Link element in the STA Profile field of the Basic
 * Multi-Link element.
 */
...
class WifiActionHeader : public Header
{
  public:
    ...
    enum CategoryValue // table 9-51 of IEEE 802.11-2020
    {
        ...
        // Since vendor specific action has no stationary Action value,the parse
    ...
    enum MeshActionValue
    {
        ...
        MDA_SETUP_REQUEST = 4,               // Action Value:4 in Category 13: Mesh
                                             // MCCA-Setup-Request (not used so far)
        MDA_SETUP_REPLY = 5,                 // Action Value:5 in Category 13: Mesh
                                             // MCCA-Setup-Reply (not used so far)
        MDAOP_ADVERTISEMENT_REQUEST = 6,     // Action Value:6 in Category 13: Mesh
     ...
     *
     * \param pkt the given packet
     * \return the category value and the action value in the removed Action
     * header
     */
...
class MgtDelBaHeader : public Header
{
  ...
  private:
    ...
    uint16_t m_reasonCode; //!< Not used for now. Always set to 1: "Unspecified
                           //!< reason"
};

/**
 * \ingroup wifi
 * Implement the header for Action frames of type EML Operating Mode
 * Notification.
 */
class MgtEmlOmn : public Header
{
  public:
    ...
    /**
     * \return the ID of the links whose bit position in the link bitmap is set to
     * 1
     */
