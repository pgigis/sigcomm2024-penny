...
class PhyEntity : public SimpleRefCount<PhyEntity>
{
  public:
    ...
    struct PhyFieldRxStatus
    {
        ...
        PhyRxFailureAction actionIfFailure{DROP}; //!< action to perform in case of failure \see
                                                  //!< PhyRxFailureAction
  ...
  protected:
    ...
    std::vector<EventId> m_endRxPayloadEvents; //!< the end of receive events (only one unless UL
                                               //!< MU reception)
    ...
    std::map<UidStaIdPair, std::vector<bool>>
        m_statusPerMpduMap; //!< Map of the current reception status per MPDU
                            //!< that is filled in as long as MPDUs are being
                            //!< processed by the PHY in case of an A-MPDU
