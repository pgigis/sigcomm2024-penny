...
class WifiRadioEnergyModelPhyListener : public WifiPhyListener
{
  public:
    /**
     * Callback type for updating the transmit current based on the nominal TX
     * power.
     */
  ...
  private:
    ...
    /**
     * Callback used to update the TX current stored in WifiRadioEnergyModel based
     * on the nominal TX power used to transmit the current frame.
     */
...
class WifiRadioEnergyModel : public DeviceEnergyModel
{
  public:
     ...
     *
     * \returns the time the radio can stay in that state based on the remaining
     * energy.
     */
  ...
  private:
    ...
    /// This variable keeps track of the total energy consumed by this model in
    /// watts.
