...
/**
 * \brief Handle packet fragmentation and retransmissions for QoS data frames as
 * well as MSDU aggregation (A-MSDU) and block ack sessions, for a given access
 * class. \ingroup wifi
 *
 ...
 * when to send a packet. Packets are stored in a ns3::WifiMacQueue until they
 * can be sent.
 *
...
class QosTxop : public Txop
{
  public:
     ...
     *
     * Returns the negotiated buffer size during ADDBA handshake with station
     ...
     * originator (WinStartO) of the block ack agreement established with the
     * given recipient for the given TID.
     */
     ...
     * corresponding to the given TID. A block ack agreement with the given
     * recipient for the given TID must have been established by such QosTxop.
     */
    ...
    /**
     * Stores an MPDU (part of an A-MPDU) in block ack agreement (i.e. the sender
     * is waiting for a BlockAck containing the sequence number of this MPDU).
     *
    ...
    /**
     * Return the next sequence number for the Traffic ID and destination, but do
     * not pick it (i.e. the current sequence number remains unchanged).
     *
     * \param hdr Wi-Fi header.
     *
     * \return the next sequence number.
    ...
    /**
     * Peek the next frame to transmit on the given link to the given receiver and
     * of the given TID from the EDCA queue. If <i>tid</i> is equal to 8 (invalid
     * value) and <i>recipient</i> is the broadcast address, the first available
     * frame is returned. If <i>mpdu</i> is not a null pointer, the search starts
     * from the MPDU following <i>mpdu</i> in the queue; otherwise, the search
     * starts from the head of the queue. Note that A-MSDU aggregation is never
     * attempted. If the frame has never been transmitted, it is assigned a
     * sequence number peeked from MacTxMiddle. Also note that multiple links are
     * only available since 802.11be.
     *
    ...
    /**
     * Prepare the frame to transmit on the given link starting from the MPDU that
     has been
     * previously peeked by calling PeekNextMpdu. A frame is only returned if it
     meets the
     ...
     * transmission time does not exceed the given PPDU duration limit (if
     distinct from
     *  Time::Min ()). If the peeked MPDU is a unicast QoS Data frame stored in
     the EDCA queue,
     * attempt to perform A-MSDU aggregation (while meeting the constraints
     mentioned
     ...
     * \return the frame to transmit or a null pointer if no frame meets the time
     constraints
     */
    ...
    /**
     * Return true if the MU EDCA Timer is running for the given link, false
     * otherwise.
     *
     * \param linkId the ID of the given link
    ...
    /**
     * For the given link, return the number of slots that make up an AIFS
     * according to the EDCA Parameter Set or the MU EDCA Parameter Set, depending
     * on whether the MU EDCA Timer is running or not.
     *
     * \param linkId the ID of the given link
  ...
  private:
    ...
    uint8_t m_blockAckThreshold;              /**< the block ack threshold (use BA mechanism if
                                                 number of packets in queue reaches this value.
                                                 If this value is 0, block ack is never used.
                                                 When A-MPDU is enabled, block ack mechanism is
                                                 used regardless of this value) */
    uint16_t m_blockAckInactivityTimeout;     //!< the BlockAck inactivity timeout
                                              //!< value (in TUs, i.e. blocks of 1024
                                              //!< microseconds)
    Time m_addBaResponseTimeout;              //!< timeout for ADDBA response
    Time m_failedAddBaTimeout;                //!< timeout after failed BA agreement
    bool m_useExplicitBarAfterMissedBlockAck; //!< flag whether explicit
                                              //!< BlockAckRequest should be sent
                                              //!< upon missed BlockAck Response
