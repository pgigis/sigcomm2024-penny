...
class WifiRemoteStationInfo
{
  public:
    ...
    /**
     * \brief Updates average frame error rate when data or RTS was transmitted
     * successfully.
     *
     * \param retryCounter is SLRC or SSRC value at the moment of success
     * transmission.
     */
    ...
    void NotifyTxFailed();
    /**
     * Return frame error rate (probability that frame is corrupted due to
     * transmission error). \returns the frame error rate
     */
    double GetFrameErrorRate() const;

  private:
    /**
     * \brief Calculate averaging coefficient for frame error rate. Depends on
