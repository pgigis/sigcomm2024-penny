...
/// generic value for aRxPHYStartDelay PHY characteristic (used when we do not
/// know the preamble type of the next frame)
...
EhtFrameExchangeManager::CreateAliasIfNeeded(Ptr<WifiMpdu> mpdu) const
{
    ...
    // alias needs only be created for non-broadcast QoS data frames exchanged
    // between two MLDs
     ...
     * 802.11be D2.0 ["the value of the Address 3 field and the Address 4 field
     * (if present) in the MAC header of a data frame shall be set based on Table
     * 9-30 (Address field contents) and the settings of the To DS and From DS
     * bits, where the BSSID is the MAC address of the AP affiliated with the AP
     * MLD corresponding to that link"].
     */
    if (hdr.IsQosAmsdu())
    {
...
EhtFrameExchangeManager::ForwardPsduDown(Ptr<const WifiPsdu> psdu, WifiTxVector& txVector)
{
    ...
    // EHT-SIG, the equivalent of HE-SIG-B, is present in EHT SU transmissions,
    // too
    ...
    if (m_apMac)
    {
        // check if the EMLSR clients shall switch back to listening operation at
        // the end of this PPDU
...
EhtFrameExchangeManager::NotifySwitchingEmlsrLink(Ptr<WifiPhy> phy, uint8_t linkId, Time delay)
{
    ...
    // TODO Shall we assert that there is no ongoing frame exchange sequence? Or
    // is it possible that there is an ongoing frame exchange sequence (in such a
    // case, we need to force a timeout, just like it is done in case of a normal
    // channel switch

    NS_ABORT_MSG_IF(!m_staMac, "This method can only be called on a STA");

    // if we receive the notification from a PHY that is not connected to us, it
    // means that we have been already connected to another PHY operating on this
    // link, hence we do not have to reset the connected PHY
    if (phy == m_phy)
    {
        ResetPhy();
    }
...
EhtFrameExchangeManager::SendEmlOmn(const Mac48Address& dest, const MgtEmlOmn& frame)
{
    ...
    // Use AC_VO to send management frame addressed to a QoS STA (Sec. 10.2.3.2 of
    // 802.11-2020)
                                                   ...
                                                   const Mac48Address& address) const
{
    ...
    // The non-AP MLD shall be switched back to the listening operation on the
    // EMLSR links after the EMLSR transition delay time if [...] the non-AP STA
    // affiliated with the non-AP MLD does not detect [...] any of the following
    // frames:
    // - an individually addressed frame with the RA equal to the MAC address of
    ...
    // - a Trigger frame that has one of the User Info fields addressed to the
    // non-AP STA affiliated with the non-AP MLD
    ...
    // - a Multi-STA BlockAck frame that has one of the Per AID TID Info fields
    // addressed to the non-AP STA affiliated with the non-AP MLD
    ...
    // - a NDP Announcement frame that has one of the STA Info fields addressed to
    // the non-AP STA affiliated with the non-AP MLD and a sounding NDP
...
EhtFrameExchangeManager::TransmissionFailed()
{
    ...
    for (const auto& address : m_txTimer.GetStasExpectedToRespond())
    {
        if (GetWifiRemoteStationManager()->GetEmlsrEnabled(address))
        {
            ...
            // The AP affiliated with the AP MLD should transmit before the TXNAV
            // timer expires another initial Control frame addressed to the non-AP STA
            // affiliated with the non-AP MLD if the AP intends to continue the frame
            // exchanges with the STA and did not receive the response frame from this
            // STA for the most recently transmitted frame that requires an immediate
            // response after a SIFS (Sec. 35.3.17 of 802.11be D3.1) We let the AP
            // continue the TXOP. TransmissionSucceeded() removes this client from
            // protected stations, hence next transmission to this client in this TXOP
            // will be protected by ICF
...
EhtFrameExchangeManager::NotifyChannelReleased(Ptr<Txop> txop)
{
    ...
    if (m_apMac)
    {
        // the channel has been released; all EMLSR clients will switch back to
        // listening operation after a timeout interval of aSIFSTime + aSlotTime +
        // aRxPHYStartDelay
...
EhtFrameExchangeManager::PostProcessFrame(Ptr<const WifiPsdu> psdu, const WifiTxVector& txVector)
{
        ...
        GetWifiRemoteStationManager()->GetEmlsrEnabled(*m_txopHolder))
    {
        ...
        {
            // an EMLSR client has started an UL TXOP. We may send a response after a
            // SIFS or we may receive another frame after a SIFS. Postpone the TXOP
            // end by considering the latter (which takes longer)
            auto delay =
                                     ...
                                     bool inAmpdu)
{
    ...
    if (hdr.IsTrigger())
    {
        ...
        if (trigger.IsMuRts() && m_staMac->IsEmlsrLink(m_linkId))
        {
            ...
            {
                // we received an ICF on a link that is blocked because another EMLSR
                // link is being used. This is likely because transmission on the other
                // EMLSR link started before the reception of the ICF ended. We drop
                // this ICF and let the UL TXOP continue.
            ...
            Simulator::ScheduleNow(&EmlsrManager::NotifyIcfReceived,
                                   ...
                                   m_linkId);
            // we just got involved in a DL TXOP. Check if we are still involved in
            // the TXOP in a SIFS (we are expected to reply by sending a CTS frame)
            m_ongoingTxopEnd.Cancel();
...
EhtFrameExchangeManager::UpdateTxopEndOnTxStart(Time txDuration)
{
    ...
    {
        // the TX timer is running, hence we are expecting a response. Postpone the
        // TXOP end to match the TX timer (which is long enough to get the
        // PHY-RXSTART.indication for the response)
        delay = m_txTimer.GetDelayLeft();
    }
    else
    {
        // the TX Timer is not running, hence no response is expected (e.g., we are
        // transmitting a CTS after ICS). The TXOP holder may transmit a frame a
        // SIFS after the end of this PPDU, hence we need to postpone the TXOP end
        // in order to get the PHY-RXSTART.indication
...
EhtFrameExchangeManager::UpdateTxopEndOnRxEnd()
{
    ...
    // we may send a response after a SIFS or we may receive another frame after a
    // SIFS. Postpone the TXOP end by considering the latter (which takes longer)
    auto delay = m_phy->GetSifs() + m_phy->GetSlot() + MicroSeconds(RX_PHY_START_DELAY_USEC);
    NS_LOG_DEBUG("Expected TXOP end=" << (Simulator::Now() + delay).As(Time::S));
    m_ongoingTxopEnd = Simulator::Schedule(delay, &EhtFrameExchangeManager::TxopEnd, this);
}
