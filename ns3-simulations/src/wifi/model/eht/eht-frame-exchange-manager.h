...
class EhtFrameExchangeManager : public HeFrameExchangeManager
{
  public:
     ...
     * having the given address. If there is no such information for the given
     * station and the station is affiliated with an MLD, return the RSSI (in dBm)
     * of the most recent packet received from another station of the same MLD.
     *
     ...
     * \return whether the EMLSR client having the given AID and MAC address shall
     * switch back to the listening operation when receiving the given PSDU
     */
    ...
    /**
     * Notify that the given PHY will switch channel to operate on another EMLSR
     * link after the given delay.
     *
  ...
  protected:
    ...
    /**
     * This method is intended to be called when an AP MLD detects that an EMLSR
     * client previously involved in the current TXOP will start waiting for the
     * transition delay interval (to switch back to listening operation) after the
     * given delay. This method blocks the transmissions on all the EMLSR links of
     * the given EMLSR client until the transition delay advertised by the EMLSR
     * client expires.
     *
  ...
  private:
    ...
    EventId m_ongoingTxopEnd; //!< event indicating the possible end of the
                              //!< current TXOP (of which we are not the holder)
};
