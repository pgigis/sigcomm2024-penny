...
class EmlsrManager : public Object
{
  public:
    ...
    /**
     * Set the Transition Timeout advertised by the associated AP with EMLSR
     * activated.
     *
    ...
    /**
     * Set the ID of main PHY (position in the vector of PHYs held by
     * WifiNetDevice). This method cannot be called during or after
     * initialization.
     *
    ...
    /**
     * \return the ID of main PHY (position in the vector of PHYs held by
     * WifiNetDevice)
     */
    uint8_t GetMainPhyId() const;

    /**
     * Take actions to enable EMLSR mode on the given set of links, if non-empty,
     * or disable EMLSR mode, otherwise.
     *
    ...
    /**
     * Set the member variable indicating whether the state of the CAM should be
     * reset when the main PHY switches channel and operates on the link
    ...
    /**
     * \return the value of the member variable indicating whether the state of
     * the CAM should be reset when the main PHY switches channel and operates on
     * the link associated with the CAM.
     */
     ...
     *
     * \param linkId the ID of the link on which the initial Control frame was
     * received
     */
  ...
  protected:
    ...
    /**
     * \param linkId the ID of the given link
     * \return the EHT FrameExchangeManager attached to the non-AP STA operating
     * on the given link
     */
    Ptr<EhtFrameExchangeManager> GetEhtFem(uint8_t linkId) const;

    /**
     * \return the ID of the link on which the EML Operating Mode Notification
     * frame has to be sent
     */
    virtual uint8_t GetLinkToSendEmlOmn() = 0;

    /**
     * A previous EML Operating Mode Notification frame was dropped. Ask the
     * subclass whether the frame needs to be re-sent on the given link (if any).
     *
     * \param mpdu the dropped MPDU that includes the EML Operating Mode
     * Notification frame \return the ID of the link over which to re-send the
     * frame, if needed
     */
    ...
    /**
     * \param linkId the ID of the given link
     * \return the operating channel the main PHY must switch to in order to
    ...
    uint8_t m_mainPhyId;         //!< ID of main PHY (position in the vector of PHYs held
                                 //!< by WifiNetDevice)
    uint16_t m_auxPhyMaxWidth;   //!< max channel width (MHz) supported by aux PHYs

  private:
    /**
     * Compute the operating channels that the main PHY and the aux PHY(s) must
     * switch to in order to operate on each of the setup links. The operating
     * channels may be different due to limited channel width capabilities of the
     * aux PHY(s). This method shall be called upon completion of ML setup.
     */
    ...
    /**
     * This method is called to make an EMLSR mode change effective after the
     * transition delay has elapsed or a notification response has been received
     * from the AP.
     */
    ...
    /**
     * Adjust the operating channel of all the aux PHYs to meet the constraint on
    ...
    /**
     * Notify subclass that the main PHY is switching channel to operate on
     * another link.
     *
     * \param currLinkId the ID of the link on which the main PHY is operating
     * \param nextLinkId the ID of the link on which the main PHY will be
     * operating
     */
    ...
    std::optional<Time> m_emlsrTransitionTimeout; /**< Transition timeout advertised by APs with
                                                       EMLSR activated */
    std::set<uint8_t> m_emlsrLinks;               //!< ID of the EMLSR links (empty if EMLSR
                                                  //!< mode is disabled)
    ...
    EventId m_transitionTimeoutEvent;                  /**< Timer started after the successful
                                                          transmission of an EML Operating Mode
                                                          Notification frame */
    bool m_resetCamState; //!< whether to reset the state of CAM when main PHY
                          //!< switches channel
    std::map<uint8_t, WifiPhyOperatingChannel>
        m_mainPhyChannels; //!< link ID-indexed map of operating channels for the
                           //!< main PHY
    std::map<uint8_t, WifiPhyOperatingChannel>
        m_auxPhyChannels; //!< link ID-indexed map of operating channels for the
                          //!< aux PHYs
};
