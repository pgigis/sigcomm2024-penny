...
class QosFrameExchangeManager : public FrameExchangeManager
{
  public:
    ...
    /**
     * Recompute the protection and acknowledgment methods to use if the given
     * MPDU is added to the frame being built (as described by the given TX
     * parameters) and check whether the duration of the frame exchange sequence
     * (including protection and acknowledgment) does not exceed the given
     * available time. The protection and acknowledgment methods held by the given
     * TX parameters are only updated if the given MPDU can be added.
     *
    ...
    /**
     * Check whether the given MPDU can be added to the frame being built (as
     * described by the given TX parameters) without violating the given
     * constraint on the PPDU transmission duration.
     *
     * \param mpdu the MPDU to add to the frame being built
     * \param txParams the TX parameters describing the frame being built
     ...
     * \return true if the constraints on the PPDU duration limit and the maximum
     * PSDU size are met
     */
    ...
    /**
     * Create an alias of the given MPDU for transmission by this Frame Exchange
     * Manager. This is required by 11be MLDs to support translation of MAC
     * addresses. For single link devices, the given MPDU is simply returned.
     *
  ...
  protected:
    ...
    /**
     * Start a frame exchange (including protection frames and acknowledgment
     * frames as needed) that fits within the given <i>availableTime</i> (if
     ...
     * Pass Time::Min() in case the TXOP limit is null \param initialFrame true if
     * the frame being transmitted is the initial frame of the TXOP. This is used
     * to determine whether the TXOP limit can be exceeded \return true if a frame
     * exchange is started, false otherwise
     */
    virtual bool StartFrameExchange(Ptr<QosTxop> edca, Time availableTime, bool initialFrame);

    /**
     * Perform a PIFS recovery as a response to transmission failure within a
     * TXOP. If the carrier sense indicates that the medium is idle, continue the
     * TXOP. Otherwise, release the channel.
     */
    void PifsRecovery();
    ...
    /**
     * Clear the TXOP holder if the NAV counted down to zero (includes the case of
     * NAV reset).
     */
  ...
  private:
    ...
    EventId m_pifsRecoveryEvent; //!< event associated with an attempt of PIFS
                                 //!< recovery
    Ptr<Txop> m_edcaBackingOff;  //!< channel access function that invoked backoff
                                 //!< during TXOP
};
