 ...
 * \ingroup wifi
 *
 * WifiProtectionManager is an abstract base class. Each subclass defines a
 * logic to select the protection method for a given frame.
 */
class WifiProtectionManager : public Object
{
  public:
    ...
    /**
     * Determine the protection method to use if the given MPDU is added to the
     * current frame. Return a null pointer if the protection method is unchanged
     * or the new protection method otherwise.
     *
    ...
    /**
     * Determine the protection method to use if the given MSDU is aggregated to
     * the current frame. Return a null pointer if the protection method is
     * unchanged or the new protection method otherwise.
     *
  ...
  protected:
    ...
    /**
     * Add a User Info field to the given MU-RTS Trigger Frame to solicit a CTS
     * from the station with the given MAC address. The MU-RTS is intended to
     * protect a data frame having the given TX width. The TX width of the
     * solicited CTS is the minimum between the TX width of the protected data
     * frame and the maximum width supported by the solicited station.
     *
