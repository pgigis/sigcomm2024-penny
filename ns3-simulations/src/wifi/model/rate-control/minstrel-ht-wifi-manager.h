...
struct MinstrelHtRateInfo
{
    ...
    uint8_t mcsIndex;            //!< The index in the operationalMcsSet of the
    ...
    double prob;                 //!< Current probability within last time interval. (# frame
                                 //!< success )/(# total frames)
     ...
     */
    double ewmaProb;
    double ewmsdProb;            //!< Exponential weighted moving standard deviation of
                                 //!< probability.
    uint32_t prevNumRateAttempt; //!< Number of transmission attempts with
                                 //!< previous rate.
    uint32_t prevNumRateSuccess; //!< Number of successful frames transmitted
                                 //!< with previous rate.
    uint32_t numSamplesSkipped;  //!< Number of times this rate statistics were not
                                 //!< updated because no attempts have been made.
...
{
    /**
     * MCS rates are divided into groups based on the number of streams and flags
     * that they use.
     */
    uint8_t m_col;               //!< Sample table column.
    uint8_t m_index;             //!< Sample table index.
    bool m_supported;            //!< If the rates of this group are supported by the
                                 //!< station.
    ...
    uint16_t m_maxProbRate;      //!< The highest success probability rate of this
                                 //!< group in bps.
...
static const uint8_t MAX_HT_STREAM_GROUPS =
    4; //!< Maximal number of groups per stream in HT (2 possible channel
       //!< widths and 2 possible GI configurations).
static const uint8_t MAX_VHT_STREAM_GROUPS =
    8; //!< Maximal number of groups per stream in VHT (4 possible channel
       //!< widths and 2 possible GI configurations).
static const uint8_t MAX_HE_STREAM_GROUPS =
    12; //!< Maximal number of groups per stream in HE (4 possible channel
        //!< widths and 3 possible GI configurations).
...
class MinstrelHtWifiManager : public WifiRemoteStationManager
{
  ...
  private:
    ...
    /**
     * Estimates the TxTime of a frame with a given mode and group (stream, guard
     * interval and channel width).
     *
     * \param phy pointer to the wifi PHY
     * \param streams the number of streams
    ...
    /**
     * Calculate the number of retransmissions to set for the (groupId, rateId)
     * rate.
     *
     * \param station the Minstrel-HT wifi remote station
     * \param groupId the group ID
    ...
    /**
     * Estimate the time to transmit the given packet with the given number of
     * retries. This function is "roughly" the function
     * "calc_usecs_unicast_packet" in minstrel.c in the madwifi implementation.
     *
     * The basic idea is that, we try to estimate the "average" time used to
     * transmit the packet for the given number of retries while also accounting
     * for the 802.11 congestion window change. The original code in the madwifi
     * seems to estimate the number of backoff slots as the half of the current CW
     * size.
     *
    ...
    /**
     * Perform EWMSD (Exponentially Weighted Moving Standard Deviation)
     * calculation.
     *
    ...
    /**
     * Returns the groupId of an HT MCS with the given number of streams, GI and
     * channel width used.
     *
    ...
    /**
     * Returns the groupId of a VHT MCS with the given number of streams, GI and
     * channel width used.
     *
     * \param txstreams the number of streams
     * \param gi guard interval duration (nanoseconds)
    ...
    /**
     * Returns the groupId of an HE MCS with the given number of streams, GI and
     * channel width used.
     *
     * \param txstreams the number of streams
     * \param gi guard interval duration (nanoseconds)
     ...
     * not greater than the given allowed width. If so, the index of the current
     * TX rate is returned. Otherwise, try halving the channel width and check if
     * the MCS group with the same number of streams and same GI is supported. If
     * a supported MCS group is found, return the index of the TX rate within such
     * a group with the same MCS as the given TX rate. If no supported MCS group
     * is found, the simulation aborts.
     *
    ...
    Time m_legacyUpdateStats;      //!< How frequent do we calculate the stats for
                                   //!< legacy MinstrelWifiManager.
    uint8_t m_lookAroundRate;      //!< The % to try other rates than our current rate.
    uint8_t m_ewmaLevel;           //!< Exponential weighted moving average level (or
                                   //!< coefficient).
    ...
    bool m_useLatestAmendmentOnly; //!< Flag if only the latest supported
                                   //!< amendment by both peers should be used.
