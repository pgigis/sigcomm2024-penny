 ...
 * \ingroup wifi
 *
 * This algorithm was first described in "Efficient Collision Detection for Auto
 * Rate Fallback Algorithm". The implementation available here was done by
 * Federico Maguolo for a very early development version of ns-3. Federico died
 * before merging this work in ns-3 itself so his code was ported to ns-3 later
...
class AarfcdWifiManager : public WifiRemoteStationManager
{
  ...
  private:
     ...
     * the first transmission after a rate increase and ends at the first
     * successful transmission. Specifically, recovery mode transcends
     * retransmissions boundaries. Fundamentally, ARF handles each data
     * transmission independently, whether it is the initial transmission of a
     * packet or the retransmission of a packet. The fundamental reason for this
     * is that there is a backoff between each data transmission, be it an initial
