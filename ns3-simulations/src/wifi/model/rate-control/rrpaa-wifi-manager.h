 ...
 *
 * This class implements the RRPAA algorithm as described in <i>Rate, Power and
 * Carrier-Sense Threshold Coordinated Management for High-Density IEEE 802.11
 * Networks</i> by Matías Richart; Jorge Visca and Javier Baliosian in
 * Integrated Network Management (IM), 2015 IFIP/IEEE International Symposium on
 * (pp. 139-146). IEEE. https://ieeexplore.ieee.org/document/7140286
 *
 * RRPAA adds power control to the RRAA mechanism. RRAA is described in
 * <i>Robust rate adaptation for 802.11 wireless networks</i> by Starsky H. Y.
 * Wong; Hao Yang; Songwu Lu and Vaduvur Bharghavan in Proceedings of the 12th
...
class RrpaaWifiManager : public WifiRemoteStationManager
{
  ...
  private:
    ...
    uint32_t m_frameLength; //!< Data frame length used to calculate mode TxTime
                            //!< (in bytes).
    uint32_t m_ackLength;   //!< Ack frame length used to calculate mode TxTime (in
                            //!< bytes).
    ...
    Ptr<UniformRandomVariable> m_uniformRandomVariable; //!< Provides uniform random variables for
