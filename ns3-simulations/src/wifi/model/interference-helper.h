...
class InterferenceHelper : public Object
{
  public:
     ...
     *
     * \return true if bands are tracked by this interference helper, false
     * otherwise
     */
    bool HasBands() const;

    /**
     * Update the frequency bands that belongs to a given frequency range when the
     * spectrum model is changed.
     *
     ...
     * start of the HE portion reception (only used for MU)
     *
     * \return Event
     */
     ...
     * this class.
     *
     * \param event the event corresponding to the first time the corresponding
     * PPDU arrives \param channelWidth the channel width used to transmit the
     * PSDU (in MHz) \param band identify the band used by the PSDU \param staId
     * the station ID of the PSDU (only used for MU) \param relativeMpduStartStop
     * the time window (pair of start and end times) of PHY payload to focus on
     *
     * \return struct of SNR and PER (with PER being evaluated over the provided
     * time window)
     */
     ...
     *
     * \param event the event corresponding to the first time the corresponding
     * PPDU arrives \param channelWidth the channel width (in MHz) \param nss the
     * number of spatial streams \param band identify the band used by the PSDU
     *
     ...
     *
     * \param event the event corresponding to the first time the corresponding
     * PPDU arrives \param channelWidth the channel width (in MHz) for header
     * measurement \param band identify the band used by the PSDU \param header
     * the PHY header to consider
     *
     ...
     * \param freqRange the frequency range in which the received signal event had
     * been detected
     */
     ...
     * \param rxPower the received power (W) per band to be added to the current
     * event
     */
  ...
  protected:
    ...
    /**
     * Calculate SNR (linear ratio) from the given signal power and
    ...
    double CalculateSnr(double signal,
                        ...
                        uint8_t nss) const;
    /**
     * Calculate the success rate of the chunk given the SINR, duration, and
     * TXVECTOR. The duration and TXVECTOR are used to calculate how many bits are
    ...
    /**
     * Calculate the success rate of the payload chunk given the SINR, duration,
     * and TXVECTOR. The duration and TXVECTOR are used to calculate how many bits
     * are present in the payload chunk.
     *
     * \param snir the SINR
     * \param duration the duration of the chunk
     * \param txVector the TXVECTOR
  ...
  private:
     ...
     * \return true if the band is already tracked by this interference helper,
     * false otherwise
     */
     ...
     * \param isStartHePortionRxing flag whether event corresponds to the start of
     * the HE portion reception (only used for MU)
     */
    ...
    /**
     * Calculate power of all other events preceding a given event that belong to
     * the same MU-MIMO transmission.
     *
     * \param event the event
     * \param band the band
     *
     * \return the power of all other events preceding the event that belong to
     * the same MU-MIMO transmission
     */
     ...
     * window (thus enabling per MPDU PER information). The PHY payload can be
     * divided into multiple chunks (e.g. due to interference from other
     * transmissions).
     *
     * \param event the event
     ...
     * \param staId the station ID of the PSDU (only used for MU)
     * \param window time window (pair of start and end times) of PHY payload to
     * focus on
     *
     ...
     * can be divided into multiple chunks (e.g. due to interference from other
     * transmissions).
     *
     * \param event the event
     * \param nis the NiChanges
    ...
    double CalculatePhyHeaderPer(Ptr<const Event> event,
                                 ...
                                 WifiPpduField header) const;
    /**
     * Calculate the success rate of the PHY header sections for the provided
     * event.
     *
     * \param event the event
     * \param nis the NiChanges
     * \param channelWidth the channel width (in MHz) for header measurement
     * \param band the band
     * \param phyHeaderSections the map of PHY header sections (\see
    ...
    uint8_t m_numRxAntennas;              //!< the number of RX antennas in the corresponding
                                          //!< receiver
    ...
    /**
     * Return whether another event is a MU-MIMO event that belongs to the same
     * transmission and to the same RU.
     *
     * \param currentEvent the current event that is being inspected
     * \param otherEvent the other event to compare against
     *
     * \return whether both events belong to the same transmission and to the same
     * RU
     */
