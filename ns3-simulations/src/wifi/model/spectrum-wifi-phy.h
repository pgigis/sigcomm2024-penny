...
class SpectrumWifiPhy : public WifiPhy
{
  public:
     ...
     * \param freqRange the frequency range, bounded by a minFrequency and a
     * maxFrequency in MHz
     */
     ...
     * \param interface the Spectrum PHY interface for which the signal has been
     * detected
     */
     ...
     *                SpectrumSignalParameters (in case any objects downstream of
     * the SpectrumWifiPhy wish to adjust signal properties based on the
    ...
    /**
     * Configure a non-active spectrum PHY interface to operate on a given
     * frequency with a given width. The function searches for the non-active PHY
     * interface that operates on the frequency range corresponding to the
     * spectrum portion specified by the caller. It takes care to configure the RX
     * spectrum model of the PHY interface and to update the bands tracked in
     * interference helper.
     *
    ...
    /**
     * Determine the WifiPpdu to be used by the RX PHY based on the WifiPpdu sent
     * by the TX PHY.
     *
  ...
  protected:
    ...
    Ptr<WifiSpectrumPhyInterface>
        m_currentSpectrumPhyInterface; //!< The current Spectrum PHY interface
                                       //!< (held for performance reasons)

  private:
    /**
     * Perform run-time spectrum model change
     * \param spectrumPhyInterface the spectrum PHY interface for which the
     * spectrum model should be changed \param centerFrequency the center
     * frequency in MHz the PHY interface should use \param channelWidth the
     * channel width in MHz the PHY interface should use
     */
    ...
    /**
     * This function is called to update the bands handled by the
     * InterferenceHelper. \param spectrumPhyInterface the spectrum PHY interface
     * for which the bands should be updated
     */
    void UpdateInterferenceHelperBands(Ptr<WifiSpectrumPhyInterface> spectrumPhyInterface);

    /**
     * This function computes the RU bands that belong to a given spectrum PHY
     * interface.
     *
     * \param spectrumPhyInterface the spectrum PHY interface to consider to
     * compute the RU bands \param guardBandwidth width of the guard band in MHz
    ...
    /**
     * This function computes the bands that belong to a given spectrum PHY
     * interface.
     *
     * \param spectrumPhyInterface the spectrum PHY interface to consider to
     * compute the bands \returns the computed bands for the spectrum PHY
     * interface
     */
     ...
     * stop frequencies.
     *
     * \param spectrumPhyInterface the spectrum PHY interface to consider for the
     * calculations \param indices the start/stop indices to convert \return the
     ...
     * \return the spectrum PHY interface that covers the indicated band of the RF
     * channel
     */
    ...
    bool m_disableWifiReception;           //!< forces this PHY to fail to sync on any
                                           //!< signal
    bool m_trackSignalsInactiveInterfaces; //!< flag whether signals coming from
                                           //!< inactive spectrum PHY interfaces
                                           //!< are tracked

    TracedCallback<bool, uint32_t, double, Time> m_signalCb; //!< Signal callback

    double m_txMaskInnerBandMinimumRejection; //!< The minimum rejection (in dBr)
                                              //!< for the inner band of the
                                              //!< transmit spectrum mask
    double m_txMaskOuterBandMinimumRejection; //!< The minimum rejection (in dBr)
                                              //!< for the outer band of the
                                              //!< transmit spectrum mask
    double m_txMaskOuterBandMaximumRejection; //!< The maximum rejection (in dBr)
                                              //!< for the outer band of the
                                              //!< transmit spectrum mask
