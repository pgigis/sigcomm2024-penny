    ...
    static TypeId tid =
        TypeId("ns3::WifiMac")
            ...
            .AddAttribute("ShortSlotTimeSupported",
                          "Whether or not short slot time is supported (only used by ERP "
                          "APs or STAs).",
                          BooleanValue(true),
            ...
            .AddAttribute("VO_MaxAmpduSize",
                          ...
                          "(capped to 65535 for HT PPDUs, 1048575 for VHT PPDUs, 6500631 "
                          "for HE PPDUs "
            ...
            .AddTraceSource("MacTxDrop",
                            ...
                            "This trace source is fired, e.g., when an AP's MAC "
                            "receives from the upper layer "
                            "a packet destined to a station that is not "
                            "associated with the AP or a STA's MAC "
                            "receives a packet from the upper layer while it is "
            ...
            .AddTraceSource("TxErrHeader",
                            ...
                            "DroppedMpdu trace or one of the traces associated with TX "
                            "timeouts.")
            ...
            .AddTraceSource("DroppedMpdu",
                            "An MPDU that was dropped for the given reason (see "
                            "WifiMacDropReason).",
                            MakeTraceSourceAccessor(&WifiMac::m_droppedMpduCallback),
                            "ns3::WifiMac::DroppedMpduCallback")
            .AddTraceSource("MpduResponseTimeout",
                            "An MPDU whose response was not received before the timeout, "
                            "along with "
                            "an identifier of the type of timeout (see WifiTxTimer::Reason) "
                            "and the "
                            "TXVECTOR used to transmit the MPDU. This trace source is fired "
                            "when a "
                            "CTS is missing after an RTS, when all CTS frames are missing "
                            "after an MU-RTS, "
                            "or when a Normal Ack is missing after an MPDU or after a DL MU "
                            "PPDU "
            ...
            .AddTraceSource("PsduResponseTimeout",
                            "A PSDU whose response was not received before the timeout, "
                            "along with "
                            "an identifier of the type of timeout (see WifiTxTimer::Reason) "
                            "and the "
                            "TXVECTOR used to transmit the PSDU. This trace source is fired "
                            "when a "
                            "BlockAck is missing after an A-MPDU, a BlockAckReq (possibly in "
                            "the "
                            "context of the acknowledgment of a DL MU PPDU in SU format) or "
                            "a TB PPDU "
                            "(in the latter case the missing BlockAck is a Multi-STA "
                            "BlockAck).",
            ...
            .AddTraceSource("PsduMapResponseTimeout",
                            "A PSDU map for which not all the responses were "
                            "received before the timeout, "
                            "along with an identifier of the type of timeout "
                            "(see WifiTxTimer::Reason), "
                            "the set of MAC addresses of the stations that did "
                            "not respond and the total "
                            "number of stations that had to respond. This trace "
                            "source is fired when not "
                            "all the addressed stations responded to an MU-BAR "
                            "Trigger frame (either sent as "
                            "a SU frame or aggregated to PSDUs in the DL MU "
                            "PPDU), a Basic Trigger Frame or "
...
WifiMac::ApplyTidLinkMapping(const Mac48Address& mldAddr, WifiDirection dir)
{
    ...
    NS_ABORT_MSG_IF(dir == WifiDirection::BOTH_DIRECTIONS,
                    "This method can be used to enforce TID-to-Link mapping for "
