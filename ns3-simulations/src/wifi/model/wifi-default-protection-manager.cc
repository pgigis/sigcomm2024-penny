...
WifiDefaultProtectionManager::TryAddMpdu(Ptr<const WifiMpdu> mpdu, const WifiTxParameters& txParams)
{
    ...
    // Call a separate method that handles MU-RTS/CTS protection in case of DL MU
    // PPDU containing more than one PSDU or in case the MPDU being added is
    // addressed to an EMLSR client or in case the protection method is already
    // MU-RTS/CTS. A DL MU PPDU contains more than one PSDU if either the TX
    // params' PSDU info map contains more than one entry or it contains one entry
    // but the MPDU being added is addressed to a different receiver (hence
    // generating a new entry if the MPDU is added)
    ...
    // No protection for TB PPDUs (the soliciting Trigger Frame can be protected
    // by an MU-RTS)
    ...
    // if the current protection method (if any) is already RTS/CTS or
    // CTS-to-Self, it will not change by adding an MPDU
                                               ...
                                               const WifiTxParameters& txParams)
{
    ...
    // if the current protection method is already RTS/CTS, CTS-to-Self or
    // MU-RTS/CTS, it will not change by aggregating an MSDU
                                                ...
                                                const WifiTxVector& txVector) const
{
    ...
    // a non-initial fragment does not need to be protected, unless it is being
    // retransmitted
    ...
    // no need to use protection if destination already received an RTS in this
    // TXOP
    ...
    // when an EMLSR client starts an UL TXOP on a link on which the main PHY is
    // not operating, the aux PHY sends an RTS frame
                                                 ...
                                                 const WifiTxParameters& txParams)
{
    ...
    if (txParams.GetPsduInfo(receiver) == nullptr)
    {
        ...
        {
            // txParams.m_protection points to an existing WifiMuRtsCtsProtection
            // object. We have to return a copy of this object including the needed
            // changes
        ...
        else
        {
            ...
            // The UL Length, GI And HE-LTF Type, MU-MIMO HE-LTF Mode, Number Of
            // HE-LTF Symbols, UL STBC, LDPC Extra Symbol Segment, AP TX Power,
            // Pre-FEC Padding Factor, PE Disambiguity, UL Spatial Reuse, Doppler and
            // UL HE-SIG-A2 Reserved subfields in the Common Info field are reserved.
            // (Sec. 9.3.1.22.5 of 802.11ax)
            protection->muRts.SetType(TriggerFrameType::MU_RTS_TRIGGER);
            protection->muRts.SetUlBandwidth(txWidth);

            // Add a User Info field for each of the receivers already in the TX
            // params
            ...
            // The transmitter of an MU-RTS Trigger frame shall not request a non-AP
            // STA to send a CTS frame response in a 20 MHz channel that is not
            // occupied by the PPDU that contains the MU-RTS Trigger frame.
            // (Sec. 26.2.6.2 of 802.11ax)
            protection->muRtsTxVector.SetChannelWidth(txWidth);
            // OFDM is needed to transmit the PPDU over a bandwidth that is a multiple
            // of 20 MHz
        ...
        // The initial Control frame of frame exchanges shall be sent in the non-HT
        // PPDU or non-HT duplicate PPDU format using a rate of 6 Mb/s, 12 Mb/s, or
        // 24 Mb/s. (Sec. 35.3.17 of 802.11be D3.0)
        ...
        // The UL HE-MCS, UL FEC Coding Type, UL DCM, SS Allocation and UL Target
        // RSSI fields in the User Info field are reserved (Sec. 9.3.1.22.5 of
        // 802.11ax)
                                                  ...
                                                  const WifiTxParameters& txParams)
{
    ...
    // initialize the MU-RTS Trigger Frame
    // The UL Length, GI And HE-LTF Type, MU-MIMO HE-LTF Mode, Number Of HE-LTF
    // Symbols, UL STBC, LDPC Extra Symbol Segment, AP TX Power, Pre-FEC Padding
    // Factor, PE Disambiguity, UL Spatial Reuse, Doppler and UL HE-SIG-A2
    // Reserved subfields in the Common Info field are reserved. (Sec. 9.3.1.22.5
    // of 802.11ax)
    protection->muRts.SetType(TriggerFrameType::MU_RTS_TRIGGER);
    protection->muRts.SetUlBandwidth(txWidth);
    ...
    // The transmitter of an MU-RTS Trigger frame shall not request a non-AP STA
    // to send a CTS frame response in a 20 MHz channel that is not occupied by
    // the PPDU that contains the MU-RTS Trigger frame. (Sec. 26.2.6.2 of
    // 802.11ax)
    protection->muRtsTxVector.SetChannelWidth(txWidth);
    // OFDM is needed to transmit the PPDU over a bandwidth that is a multiple of
    // 20 MHz
    const auto modulation = protection->muRtsTxVector.GetModulationClass();
    if (modulation == WIFI_MOD_CLASS_DSSS || modulation == WIFI_MOD_CLASS_HR_DSSS)
    {
    ...
    // The initial Control frame of frame exchanges shall be sent in the non-HT
    // PPDU or non-HT duplicate PPDU format using a rate of 6 Mb/s, 12 Mb/s, or 24
    // Mb/s. (Sec. 35.3.17 of 802.11be D3.0)
