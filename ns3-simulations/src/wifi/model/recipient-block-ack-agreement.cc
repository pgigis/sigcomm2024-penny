...
RecipientBlockAckAgreement::NotifyReceivedMpdu(Ptr<const WifiMpdu> mpdu)
{
    ...
    /* Update the receive reordering buffer (see Section 10.24.7.6.2 of
     * 802.11-2016) */
    if (distance < m_winSizeB)
    {
        // 1. Store the received MPDU in the buffer, if no MSDU with the same
    ...
    else if (distance < SEQNO_SPACE_HALF_SIZE)
    {
        ...
        // 4. Pass any complete MSDUs or A-MSDUs stored in the buffer with Sequence
        // Number subfield values that are lower than the new value of WinStartB up
        // to the next MAC process in order of increasing Sequence Number subfield
        // value. Gaps may exist in the Sequence Number subfield values of the MSDUs
        // or A-MSDUs that are passed up to the next MAC process.
        PassBufferedMpdusWithSeqNumberLessThan(mpdu->GetHeader().GetSequenceNumber() - m_winSizeB +
                                               1);

        // 5. Pass MSDUs or A-MSDUs stored in the buffer up to the next MAC process
        // in order of increasing value of the Sequence Number subfield starting
        // with WinStartB and proceeding sequentially until there is no buffered
        // MSDU or A-MSDU for the next sequential Sequence Number subfield value
        PassBufferedMpdusUntilFirstLost();
    }
}
