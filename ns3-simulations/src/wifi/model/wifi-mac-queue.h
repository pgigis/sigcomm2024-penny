...
class WifiMacQueue : public Queue<WifiMpdu, ns3::WifiMacQueueContainer>
{
  public:
    ...
    /**
     * Search and return the first packet present in the container queue
     * identified by the given queue ID. If <i>item</i> is a null pointer, the
     * search starts from the head of the container queue; MPDUs with expired
     * lifetime at the head of the container queue are ignored (and moved to the
     * container queue storing MPDUs with expired lifetime). If <i>item</i> is not
     * a null pointer, the search starts from the packet following <i>item</i> in
     * the container queue (and we do not check for expired lifetime because we
     * assume that a previous call was made with a null pointer as argument, which
    ...
    /**
     * Return first available packet for transmission on the given link. If
     * <i>item</i> is not a null pointer, the search starts from the packet
     * following <i>item</i> in the queue; otherwise, the search starts from the
     * head of the queue. The packet is not removed from queue.
     *
     ...
     * removed one, if any, or a null pointer otherwise. If <i>removeExpired</i>
     * is true, all the items in the queue from the head to the given position are
    ...
    /**
     * Replace the given current item with the given new item. Actually, the
     * current item is dequeued and the new item is enqueued in its place. In this
     * way, statistics about queue size (in terms of bytes) are correctly updated.
     *
     ...
     * container queue storing MPDUs with expired lifetime. Each MPDU that is
     * found to have an expired lifetime feeds the "Expired" trace source and is
     * notified to the scheduler. \note that such MPDUs are not removed from the
     * WifiMacQueue (and hence are still accounted for in the overall statistics
     * kept by the Queue base class) in order to make this method const.
     */
    ...
    /**
     * Unlike the GetOriginal() method of WifiMpdu, this method returns a
     * non-const pointer to the original copy of the given MPDU.
     *
     ...
     * \param mpdu the given MPDU
     * \param linkId the ID of the given link
     * \return the alias of the given MPDU that is inflight on the given link, if
  ...
  private:
    ...
    /**
     * Wrapper for the DoEnqueue method provided by the base class that
     * additionally sets the iterator field of the item and updates internal
     * statistics, if insertion succeeded.
     *
    ...
    /**
     * Wrapper for the DoDequeue method provided by the base class that
     * additionally resets the iterator field of the dequeued items and notifies
     * the scheduler, if any item was dequeued.
     *
    ...
    /**
     * Wrapper for the DoRemove method provided by the base class that
     * additionally resets the iterator field of the item and notifies the
     * scheduleer, if an item was dropped.
     *
