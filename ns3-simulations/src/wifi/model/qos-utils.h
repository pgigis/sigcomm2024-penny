...
class WifiAc
{
  public:
    ...
    /**
     * Given a TID belonging to this Access Category, get the other TID of this
     * AC.
     *
 ...
 * \return true if the AC on the left has higher priority than the AC on the
 * right
 */
...
/**
 * \ingroup wifi
 * Operator>= overload returning true if the AC on the left has higher or the
 * same priority than the AC on the right.
 *
 * \param left the AC on the left of operator>=
 * \param right the AC on the right of operator>=
 * \return true if the AC on the left has higher or the same priority than the
 * AC on the right
 */
bool operator>=(AcIndex left, AcIndex right);
 ...
 * \param left the AC on the left of operator<
 * \param right the AC on the right of operator<
 * \return true if the AC on the left has lower priority than the AC on the
 * right
 */
bool operator<(AcIndex left, AcIndex right);
...
/**
 * \ingroup wifi
 * Operator<= overload returning true if the AC on the left has lower or the
 * same priority than the AC on the right.
 *
 * \param left the AC on the left of operator<=
 * \param right the AC on the right of operator<=
 * \return true if the AC on the left has lower or the same priority than the AC
 * on the right
 */
bool operator<=(AcIndex left, AcIndex right);
 ...
 * (see section 9.10.3 in IEEE 802.11e) packets must be forwarded up before
 * "new" packets.
 *
...
/**
 * \ingroup wifi
 * This function checks if packet with sequence number <i>seqNumber</i> is an
 * "old" packet. The sequence number space is considered divided into two parts,
 * one of which is "old" and one of which is "new" by means of a boundary
 * created by adding half the sequence number range to the starting sequence
 * number <i>startingSeq</i>. So this function works fine also when
 * <i>seqNumber</i> is smaller than <i>startingSeq</i> and <i>startingSeq</i> +
 * 2048 is greater than 4096 because all comparison are circular modulo 2^12.
 ...
 * So this function, when specified a starting sequence and a sequence number,
 * returns true if that packet (with sequence number <i>numberSeq</i>)) belongs
 * to the section of the sequence number space marked with '-' characters. The
