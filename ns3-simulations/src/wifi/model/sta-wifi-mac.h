...
class StaWifiMac : public WifiMac
{
  public:
     ...
     *
     * \param bestAp the info about the best AP to associate with, if one was
     * found
     */
     ...
     *
     * \param enableLinkIdPair a pair indicating whether to enable or not power
     * save mode on the link with the given ID
     */
    void SetPowerSaveMode(const std::pair<bool, uint8_t>& enableLinkIdPair);

    /**
     * \param linkId the ID of the given link
     * \return the current Power Management mode of the STA operating on the given
     * link
     */
     ...
     *
     * \param linkIds the IDs of the links that are now EMLSR links (EMLSR mode is
     * disabled on other links)
     */
    ...
    /**
     * Notify that the given PHY switched channel to operate on another EMLSR
     * link.
     *
  ...
  private:
    ...
    /**
     * Get the (Re)Association Request frame to send on a given link. The returned
     ...
     * The standard is not clear on the correct queue for management frames if QoS
     * is supported. We always use the DCF.
     *
    ...
    /**
     * Try to ensure that we are associated with an AP by taking an appropriate
     * action depending on the current association status.
     */
     ...
     *
     * \return true if we are waiting for an association response from an AP,
     * false otherwise
     */
    ...
    /**
     * Check if any enabled link remains after the given link is disabled
     * (because, e.g., the maximum number of beacons is missed or the channel is
     * switched). If no enabled link remains, proceed with disassociation.
     *
     * \param linkId the ID of the given link
     */
    ...
    /**
     * Return the Multi-Link Element to include in the management frames
     * transmitted on the given link
     *
     * \param isReassoc whether the Multi-Link Element is included in a
     * Reassociation Request \param linkId the ID of the given link \return the
     * Multi-Link Element
     */
     ...
     * \return the TID-to-Link Mapping element(s) to include in Association
     * Request frame.
     */
