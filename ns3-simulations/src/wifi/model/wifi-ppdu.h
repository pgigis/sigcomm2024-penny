...
class WifiPpdu : public SimpleRefCount<WifiPpdu>
{
  ...
  protected:
    ...
    mutable std::optional<WifiTxVector> m_txVector; //!< the TXVECTOR at TX PHY or the reconstructed
                                                    //!< TXVECTOR at RX PHY (or std::nullopt if
                                                    //!< TXVECTOR has not been reconstructed yet)
  ...
  private:
    ...
    uint16_t m_txChannelWidth; /**< The channel width (MHz) used for the transmission
