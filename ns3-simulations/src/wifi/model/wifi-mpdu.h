 ...
 * Multi-Link Operation (MLO), a WifiMpdu variant, referred to as WifiMpdu
 * alias, is added. A WifiMpdu alias stores its own MAC header and a pointer to
 * the original copy of the WifiMpdu.
 */
class WifiMpdu : public SimpleRefCount<WifiMpdu>
{
  public:
     ...
     * header the Wifi MAC header included in the created item. \param stamp the
     * timestamp to associate with the MPDU
     */
    ...
    /**
     * \brief Return the size in bytes of the packet or control header or
     * management header stored by this item.
     *
     * \return the size in bytes of the packet or control header or management
     ...
     * \param tag a wifi MAC queue iterator tag (allows only WifiMacQueue to call
     * this method)
     */
    void SetQueueIt(std::optional<Iterator> queueIt, WmqIteratorTag tag);
    /**
     * \param tag a wifi MAC queue iterator tag (allows only WifiMacQueue to call
     * this method) \return the queue iterator stored by this object
     */
    ...
    /**
     * \return the set of IDs of the links on which this MPDU is currently in
     * flight
     */
    ...
    /**
     * Set the sequence number of this MPDU (and of the original copy, if this is
     * an alias) to the given value. Also, record that a sequence number has been
     * assigned to this MPDU.
     *
    ...
    /**
     * Create an alias for this MPDU (which must be an original copy) for
     * transmission on the link with the given ID. Aliases have their own copy of
     * the MAC header and cannot be used to perform non-const operations on the
     * frame body.
     *
     * \param linkId the ID of the given link
  ...
  private:
    ...
    /**
     * \return a reference to the information held by the original copy of the
     * MPDU.
     */
    OriginalInfo& GetOriginalInfo();
    /**
     * \return a const reference to the information held by the original copy of
     * the MPDU.
     */
