...
class ApWifiMac : public WifiMac
{
  public:
     ...
     * Each station is specified by an (association ID, MAC address) pair. Make
     * sure not to use the returned reference after that this object has been
     * deallocated.
     *
     ...
     * \return the Association ID allocated by the AP to the station, SU_STA_ID if
     * unallocated
     */
    uint16_t GetAssociationId(Mac48Address addr, uint8_t linkId) const;

    /**
     * Get the ID of a link (if any) that has been setup with the station having
     * the given MAC address. The address can be either a link address or an MLD
     * address. In the former case, the returned ID is the ID of the link
     * connecting the AP to the STA with the given address.
     *
     * \param address the given MAC address
     * \return the ID of a link (if any) that has been setup with the given
     * station
     */
     ...
     * \return the MLD address (in case of MLD) or link address (in case of single
     * link device) of the STA having the given AID, if any
     */
    std::optional<Mac48Address> GetMldOrLinkAddressByAid(uint16_t aid) const;

    /**
     * Return the value of the Queue Size subfield of the last QoS Data or QoS
     * Null frame received from the station with the given MAC address and
     * belonging to the given TID.
     *
     * The Queue Size value is the total size, rounded up to the nearest multiple
     * of 256 octets and expressed in units of 256 octets, of all  MSDUs and
     * A-MSDUs buffered at the STA (excluding the MSDU or A-MSDU of the present
     * QoS Data frame). A queue size value of 254 is used for all sizes greater
     * than 64 768 octets. A queue size value of 255 is used to indicate an
     * unspecified or unknown size. See Section 9.2.4.5.6 of 802.11-2016
     *
     ...
     * QoS Data or QoS Null frames received from the station with the given MAC
     * address and belonging to any TID.
     *
     * \param address the given MAC address
  ...
  protected:
    /**
     * Structure holding information specific to a single link. Here, the meaning
     * of "link" is that of the 11be amendment which introduced multi-link
     * devices. For previous amendments, only one link can be created.
     */
    ...
    std::map<uint16_t, Mac48Address> m_aidToMldOrLinkAddress; //!< Maps AIDs to MLD addresses (for
                                                              //!< MLDs) or link addresses (in case
                                                              //!< of single link devices)
  ...
  private:
    ...
    /**
     * Check whether the supported rate set included in the received
     * (Re)Association Request frame is compatible with our Basic Rate Set. If so,
     * record all the station's supported modes in its associated
     * WifiRemoteStation and return true. Otherwise, return false.
     *
     ...
     * \return true if the (Re)Association request can be accepted, false
     * otherwise
     */
     ...
     * check if a link can be setup with each of the reported stations (STA MAC
     * address and a (Re)Association Request frame body must be present, the Link
     * ID identifies a valid link other than the one the frame was received on and
     * the supported rates are compatible with our basic rate set).
     *
     * \param assoc the frame body of the received (Re)Association Request
     * \param from the Transmitter Address field of the frame
     * \param linkId the ID of the link on which the frame was received
    ...
    /**
     * Take necessary actions upon receiving the given EML Operating Mode
     * Notification frame from the given station on the given link.
     *
     ...
     *
     * \param timeoutReason the reason why the TX timer was started (\see
     * WifiTxTimer::Reason) \param mpdu the MPDU that we failed to sent
     */
    ...
    /**
     * Send a Probe Response in response to a Probe Request received from the STA
     * with the given address on the given link.
     *
    ...
    /**
     * Get the Association Response frame to send on a given link. The returned
     * frame never includes a Multi-Link Element.
     *
     ...
     * \param linkIdStaAddrMap a map of (link ID, remote STA address) of the links
     * to setup
     */
     ...
     * Information is taken from the given Association Response that is sent over
     * the given link to the given station.
     *
     * \param assoc the given Association Response frame
     * \param to the Receiver Address (RA) of the Association Response frame
     * \param linkId the ID of the link on which the Association Response frame is
     * sent \return a map of (link ID, remote STA address) of the links to setup
     */
     ...
     * \param linkId the ID of the link on which the association response must be
     * sent
     */
    ...
    /**
     * Perform the necessary actions when a given station switches from active
     * mode to powersave mode.
     *
    ...
    /**
     * Return the MU EDCA Parameter Set of the current AP, if one needs to be
     * advertised
     *
    ...
    /**
     * Return the Multi-Link Element that the current AP includes in the
     * management frames of the given type it transmits on the given link.
     *
     ...
     * \param to the Receiver Address of the frame containing the Multi-Link
     * Element \return the Multi-Link Element
     */
    ...
    /**
     * Return an instance of SupportedRates that contains all rates that we
     * support for the given link (including HT rates).
     *
     * \param linkId the ID of the given link
     ...
     * associated  to the AP, and that short slot time is supported by the AP and
     * by all other ERP stations that are associated to the AP. Otherwise, it is
     * disabled.
     *
     * \param linkId the ID of the given link
     */
    ...
    /**
     * \param linkIds the IDs of the links for which the next Association ID is
     * requested \return the next Association ID to be allocated by the AP on the
     * given links
     */
    ...
    Ptr<UniformRandomVariable> m_beaconJitter; //!< UniformRandomVariable used to randomize the time
                                               //!< of the first beacon
    bool m_enableBeaconJitter;                 //!< Flag whether the first beacon should be
                                               //!< generated at random time
    bool m_enableNonErpProtection;             //!< Flag whether protection mechanism is used
                                               //!< or not when non-ERP STAs are present
                                               //!< within the BSS
