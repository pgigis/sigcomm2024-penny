...
/**
 * @brief create AthstatsWifiTraceSink instances and connect them to wifi
 * devices
 *
 *
 */
 ...
 *
 * The AthstatsWifiTraceSink class is a trace sink to be connected to several of
 * the traces available within a wifi device. The purpose of
 * AthstatsWifiTraceSink is to mimic the behavior of the athstats tool
 * distributed with the madwifi driver. In particular, the reproduced behavior
 * is that obtained when executing athstats without parameters: a report written
 * in text format is produced every fixed interval, based on the events observed
 * by the wifi device.
 *
...
class AthstatsWifiTraceSink : public Object
{
  public:
     ...
     * layer  of the considered device resulted in an error due to a failure in
     * the CRC check of the frame
     *
     * @param context the calling context
     * @param packet the packet
     * @param snr the SNR in linear scale
