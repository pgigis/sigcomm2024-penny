...
class EmlOmnExchangeTest : public EmlsrOperationsTestBase
{
  ...
  private:
    std::size_t m_checkEmlsrLinksCount;        /**< counter for the number of times
                                                  CheckEmlsrLinks is called (should be two: when
                                                  the transition timeout expires and when the EML
                                                  Notification response from the AP MLD is
                                                  received */
    std::size_t m_emlNotificationDroppedCount; /**< counter for the number of times the
                                                    ...
                                                    uint8_t linkId)
{
    ...
    NS_TEST_ASSERT_MSG_EQ(mle->GetTransitionTimeout(),
                          m_transitionTimeout,
                                         ...
                                         uint8_t linkId)
{
    ...
    if (!sentbyNonApMld)
    {
        ...
        NS_TEST_ASSERT_MSG_EQ(+frame.m_emlControl.emlsrParamUpdateCtrl,
                              0,
...
EmlOmnExchangeTest::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ(m_emlNotificationDroppedCount,
                          1,
...
class EmlsrDlTxopTest : public EmlsrOperationsTestBase
{
  ...
  private:
    ...
    Time m_emlsrEnabledTime;          //!< when EMLSR mode has been enabled on all EMLSR
                                      //!< clients
    const Time m_fe2to3delay;         /**< time interval between 2nd and 3rd frame exchange
...
EmlsrDlTxopTest::CheckResults()
{
    ...
    if (m_nEmlsrStations == 2 && m_apMac->GetNLinks() == m_emlsrLinks.size())
    {
        ...
        NS_TEST_EXPECT_MSG_EQ(bAckRespTxEnd + m_apMac->GetWifiPhy(bAckRespIt->linkId)->GetSifs(),
                                ...
                                uint8_t linkId)
{
    ...
    case 2:
        ...
        // the EMLSR client
        Simulator::Schedule(txDuration + m_transitionDelay.at(secondClientId) - NanoSeconds(1),
                            [=]() {
                                for (uint8_t id = 0; id < m_apMac->GetNLinks(); id++)
                                {
                                    CheckBlockedLink(
