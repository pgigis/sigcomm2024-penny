...
WifiErrorRateModelsTestCaseMimo::DoRun()
{
    ...
    // MIMO 1x2:1: expect that SNR is increased by a factor of 3 dB (10 log 2/1)
    // compared to SISO thanks to RX diversity
    txVector.SetNss(1);
    txVector.SetNTx(1);
    ...
    // MIMO 2x2:1: expect that SNR is increased by a factor of 3 dB (10 log 2/1)
    // compared to SISO thanks to RX diversity
    txVector.SetNss(1);
    txVector.SetNTx(2);
    interference.SetNumberOfReceiveAntennas(2);
    ...
    // MIMO 3x3:1: expect that SNR is increased by a factor of 4.8 dB (10 log 3/1)
    // compared to SISO thanks to RX diversity
    txVector.SetNss(1);
    txVector.SetNTx(3);
    ...
    // MIMO 3x3:2: expect that SNR is increased by a factor of 1.8 dB (10 log 3/2)
    // compared to SISO thanks to RX diversity
    txVector.SetNss(2);
    txVector.SetNTx(3);
    interference.SetNumberOfReceiveAntennas(3);
    ...
    // MIMO 4x4:1: expect that SNR is increased by a factor of 6 dB (10 log 4/1)
    // compared to SISO thanks to RX diversity
    txVector.SetNss(1);
    txVector.SetNTx(4);
    ...
    // MIMO 4x4:2: expect that SNR is increased by a factor of 3 dB (10 log 4/2)
    // compared to SISO thanks to RX diversity
    txVector.SetNss(2);
    txVector.SetNTx(4);
    interference.SetNumberOfReceiveAntennas(4);
    ...
    // MIMO 4x4:3: expect that SNR is increased by a factor of 1.2 dB (10 log 4/3)
    // compared to SISO thanks to RX diversity
    txVector.SetNss(3);
    txVector.SetNTx(4);
    interference.SetNumberOfReceiveAntennas(4);
...
/**
 * map of PER values that have been manually computed for a given MCS, size (in
 * bytes) and SNR (in dB) in order to verify against the PER calculated by the
 * model
 */
