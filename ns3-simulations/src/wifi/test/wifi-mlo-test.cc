...
class MultiLinkOperationsTestBase : public TestCase
{
  public:
    ...
    struct BaseParams
    {
        std::vector<std::string> staChannels; //!< the strings specifying the operating channels for
                                              //!< the non-AP MLD
...
class MultiLinkSetupTest : public MultiLinkOperationsTestBase
{
  ...
  private:
    ...
    std::string m_ulTidLinkMappingStr;     //!< UL TID-to-Link Mapping for non-AP MLD
                                           //!< EHT configuration
    WifiTidLinkMapping m_dlTidLinkMapping; //!< expected DL TID-to-Link Mapping requested by
                                           //!< non-AP MLD and accepted by AP MLD
    WifiTidLinkMapping m_ulTidLinkMapping; //!< expected UL TID-to-Link Mapping requested by
                                           //!< non-AP MLD and accepted by AP MLD
    ...
    std::vector<std::size_t> m_qosFrames2; //!< indices of QoS frames of the optional set in the
                                           //!< vector of TX PSDUs
};
...
MultiLinkSetupTest::CheckAssocRequest(Ptr<WifiMpdu> mpdu, uint8_t linkId)
{
    ...
    NS_TEST_EXPECT_MSG_EQ(m_staMacs[0]->GetFrameExchangeManager(linkId)->GetAddress(),
                          mpdu->GetHeader().GetAddr2(),
    ...
    else
    {
        ...
        for (std::size_t i = 0; i < mle->GetNPerStaProfileSubelements(); i++)
        {
            ...
            NS_TEST_EXPECT_MSG_EQ(staLinkId.has_value(),
                                  true,
                                  "No link found with the STA MAC address advertised "
                                  "in Per-STA Profile");
            NS_TEST_EXPECT_MSG_NE(+staLinkId.value(),
                                  +linkId,
            ...
            NS_TEST_EXPECT_MSG_EQ(+staLinkId.value(),
...
MultiLinkSetupTest::CheckAssocResponse(Ptr<WifiMpdu> mpdu, uint8_t linkId)
{
    ...
    for (std::size_t i = 0; i < mle->GetNPerStaProfileSubelements(); i++)
    {
        ...
        NS_TEST_EXPECT_MSG_NE(+apLinkId.value(),
                              +linkId,
                               ...
                               uint8_t linkId)
{
    ...
    case 0: // first BlockAck frame (all traffic patterns)
        ...
        NS_TEST_EXPECT_MSG_EQ(blockAck.IsPacketReceived(1),
                              !isMpdu1corrupted,
...
MultiLinkTxTest::DoRun()
{
    ...
    {
        NS_TEST_EXPECT_MSG_EQ(m_inflightCount.size(),
                              m_nPackets / 2,
        ...
        NS_TEST_EXPECT_MSG_EQ(maxCount,
                              nMaxInflight,
