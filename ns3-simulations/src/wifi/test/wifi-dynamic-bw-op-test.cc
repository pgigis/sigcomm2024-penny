 ...
 * different channels. Specifically, the operating channel of BSS 1 is the
 * secondary<X> channel of BSS 0, where X is half the width of the channel used
 * by BSS 0. This test demonstrates that, if a transmission is ongoing on BSS 1,
 * we can have a transmission on BSS 0 on its primary<X> channel.
 */
class WifiUseAvailBwTest : public TestCase
{
  public:
     ...
     * \param bss0Width width (MHz) of the transmission in BSS 0 started when BSS
     * 1 is transmitting
     */
    ...
    /**
     * Function to trace packets received by the server application in the given
     * BSS \param bss the given BSS \param p the packet \param addr the address
     */
                             ...
                             double txPowerW)
{
    ...
    // Log all frames that are not management frames (we are only interested in
    // data frames and acknowledgments) and have been transmitted after 400ms (so
    // as to skip association requests/responses)
...
WifiUseAvailBwTest::CheckResults()
{
    ...
    // we expect that the AP of BSS 0 starts transmitting while the AP of BSS 1 is
    // transmitting
    NS_TEST_EXPECT_MSG_EQ(psduIt->header.IsQosData(), true, "Expected a QoS data frame");
    NS_TEST_EXPECT_MSG_EQ(+psduIt->bss, 0, "Expected a transmission in BSS 0");
    NS_TEST_EXPECT_MSG_EQ(psduIt->nMpdus,
