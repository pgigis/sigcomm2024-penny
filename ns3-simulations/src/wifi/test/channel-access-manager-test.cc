...
class TxopTest : public TxopType
{
  ...
  private:
    ...
    ExpectedBackoffs m_expectedInternalCollision; //!< expected backoff due to an
                                                  //!< internal collision
    ExpectedBackoffs m_expectedBackoff;           //!< expected backoff (not due to an
                                                  //!< internal collision)
...
class ChannelAccessManagerTest : public TestCase
{
  ...
  private:
     ...
     * \param per20MhzDurations vector that indicates for how long each 20 MHz
     * subchannel is busy
     */
                                              ...
                                              uint16_t chWidth)
{
    ...
    // the purpose of the following operations is to initialize the last busy
    // struct of the ChannelAccessManager. Indeed, InitLastBusyStructs(), which is
    // called by SetupPhyListener(), requires an attached PHY to determine the
    // channel types to initialize
...
ChannelAccessManagerTest<Txop>::DoRun()
{
    ...
    //
    //  20          60     66      70        74        78  80    100     106 110
    //  114 118 120
    //   |    rx     | sifs | aifsn | bslot0  | bslot1  |   | rx   | sifs  | aifsn
    //   | bslot2 | bslot3 | tx  |
    //        |
    ...
    // Test an EIFS
    //
    //  20          60     66           76             86       90       94 98 102
    //  106
    //   |    rx     | sifs | acktxttime | sifs + aifsn | bslot0 | bslot1 | bslot2
    //   | bslot3 | tx |
    //        |      | <------eifs------>|
    //       30 request access. backoff slots: 4
    StartTest(4, 6, 10);
    ...
    AddTxop(1);
    AddRxErrorEvt(20, 40,
                  20);    // At time 20, start reception for 40, but force error 20 into frame
    ...
    //  103
    //   |    rx     | sifs  |   |  rx  | sifs | aifsn | bslot0 | bslot1 | bslot2
    //   | bslot3 | tx |
    //        |      | <--eifs-->|
    //       30 request access. backoff slots: 4
    StartTest(4, 6, 10);
    ...
    // Test two DCFs which suffer an internal collision. the first DCF has a
    // higher priority than the second DCF.
    //
    //      20          60      66      70       74       78    88
    // DCF0  |    rx     | sifs  | aifsn | bslot0 | bslot1 | tx  |
    // DCF1  |    rx     | sifs  | aifsn | aifsn  | aifsn  |     | sifs | aifsn |
    // aifsn | aifsn | bslot |  tx  |
    //                                                                 94      98
    //                                                                 102     106
    //                                                                 110    112
    StartTest(4, 6, 10);
    ...
    // Test of AckTimeout handling: First queue requests access and ack procedure
    // fails, inside the Ack timeout second queue with higher priority requests
    // access.
    //
    //            20     26      34       54            74     80
    ...
    //
    // First queue requests access and Ack is 2 us delayed (got Ack interval at
    // the picture), inside this interval second queue with higher priority
    // requests access.
    //
    //            20     26      34           54        56     62
    ...
    // where the data rate used for the Ack is higher than expected by the Data
    // source so, the data exchange completes before the end of NAV.
    StartTest(4, 6, 10);
    AddTxop(1);
    AddRxOkEvt(20, 40);
    AddNavStart(60, 15);
    ...
    //  20     23      24      44             54          59     60     63      64
    //  65
    //   | sifs | aifsn |  tx   | Ack timeout  | switching | idle | sifs | aifsn |
    //   tx |
    //                                 |                          |
    //                                49 access request.          ^ access
    //                                request.
    //
    StartTest(1, 3, 10);
    AddTxop(1);
    ...
    //  20         60     66      70       74       78  80         100    101 107
    //  111  113
    //   |    rx    | sifs | aifsn | bslot0 | bslot1 |   | switching | idle | sifs
    //   | aifsn | tx |
    //        |                                                             |
    //       30 access request.                                             ^
    //       access request.
...
ChannelAccessManagerTest<QosTxop>::DoRun()
{
    ...
    // Also, check that CCA BUSY on a secondary channel does not affect channel
    // access:
    //    20     50     56      60     80
    //            |   cca_busy   |
    //     |  rx  | sifs | aifsn |  tx  |
    ...
    //  20           50     56      60         64         68         72         76
    //  96
    //   |     rx     | sifs | aifsn |   idle   |   idle   |   idle   |   idle   |
    //   tx  |
    //      |                        |          |          |          |
    ...
    // Check backoff decrement at slot boundaries. Medium becomes busy during
    // backoff
    //  20           50     56      60     61     71     77      81         85 87
    //  97    103 107    127
    //   |     rx     | sifs | aifsn | idle |  rx  | sifs | aifsn |   idle   |
    //   idle |  rx  | sifs | aifsn |  tx  |
    //      |                        |                            |          |
 ...
 * subsequently of the start of RX. The value returned by
 * GetLargestIdlePrimaryChannel() is checked at different times and for
 * different intervals. All the possible combinations of operating channel width
 * and busy channel type are tested.
 */
class LargestIdlePrimaryChannelTest : public TestCase
{
  ...
  private:
    ...
    /**
     * Test a specific combination of operating channel width and busy channel
     * type.
     *
...
LargestIdlePrimaryChannelTest::RunOne(uint16_t chWidth, WifiChannelListType busyChannel)
{
    ...
    // primary channel contiguous to the busy secondary channel, if the busy
    // channel is a secondary channel, or there is no idle channel, otherwise.
    ...
    // During any interval starting within CCA_BUSY period, the idle channel is
    ...
    // At RX end, we check the status of the channel during an interval
    // immediately preceding RX start and overlapping the CCA_BUSY period.
    ...
    // the CCA_BUSY period and preceding RX start. The entire operating channel is
    // idle.
