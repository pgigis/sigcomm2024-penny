...
class TestNonHtDuplicatePhyReception : public TestCase
{
  ...
  private:
    ...
    std::vector<Ptr<WaveformGenerator>> m_phyInterferers; ///< PHYs of interferers (1 interferer per
                                                          ///< 20 MHz subchannel)
};
...
class TestMultipleCtsResponsesFromMuRts : public TestCase
{
  ...
  private:
    ...
    std::size_t m_countApRxCtsFailure;  ///< count the number of unsuccessfully
                                        ///< received CTS frames by the AP
    std::size_t m_countStaRxCtsSuccess; ///< count the number of successfully received CTS
                                        ///< frames by the non-participating STA
    std::size_t m_countStaRxCtsFailure; ///< count the number of unsuccessfully received
...
TestMultipleCtsResponsesFromMuRts::TxNonHtDuplicateCts(std::size_t phyIndex)
{
    ...
    WifiTxVector txVector = WifiTxVector(OfdmPhy::GetOfdmRate54Mbps(), // use less robust modulation
                                                                       // for test purpose
                                         0,
                                         WIFI_PREAMBLE_LONG,
                                         800,
...
TestMultipleCtsResponsesFromMuRts::CheckResults()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_countStaRxCtsSuccess,
