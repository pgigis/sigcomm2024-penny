...
PowerRateAdaptationTest::ConfigureNode()
{
    ...
    /*
     * Create mac layer. We use Adhoc because association is not needed to get
     * supported rates.
     */
...
PowerRateAdaptationTest::TestParf()
{
    ...
    /*
     * After 10 consecutive successful transmissions parf increase rate or
     * decrease power. As we are at maximal rate, the power should be decreased.
     * recoveryPower=true.
     */
    for (int i = 0; i < 10; i++)
    {
    ...
    /*
     * After 10 consecutive successful transmissions parf increase rate or
     * decrease power. As we are not at maximal rate, the rate is increased again.
     * recoveryRate=true.
     */
    for (int i = 0; i < 10; i++)
    {
    ...
    /*
     * As we are using recovery rate, one failure make rate decrease.
     * recoveryRate=false.
     */
    ...
    /*
     * After 10 consecutive successful transmissions parf increase rate or
     * decrease power. As we are at maximal rate, the power is decreased.
     * recoveryRate=false, recoveryPower=true.
     */
    for (int i = 0; i < 10; i++)
    {
        manager->ReportDataOk(mpdu, 0, ackMode, 0, txVector);
    ...
    /*
     * One successful transmissions after a power decrease make
     * recoverPower=false. So we need two consecutive failures to increase power
     * again.
     */
    manager->ReportDataOk(mpdu, 0, ackMode, 0, txVector);
...
PowerRateAdaptationTest::TestAparf()
{
    ...
    /*
     * As Aparf starts in state High, after 3 consecutive successful transmissions
     * aparf increase rate or decrease power. As we are at maximal rate, the power
     * should be decreased. Change to state Spread.
     */
    for (int i = 0; i < 3; i++)
    {
        manager->ReportDataOk(mpdu, 0, ackMode, 0, txVector);
    ...
    /*
     * As we are in state Low we need 10 successful transmissions to increase rate
     * or decrease power. As we are at maximal rate, the power should be
     * decreased. Change to state Spread.
     */
    for (int i = 0; i < 10; i++)
    {
        manager->ReportDataOk(mpdu, 0, ackMode, 0, txVector);
    ...
    /*
     * As we are in state High we need 3 successful transmissions to increase rate
     * or decrease power. After 16*3 successful transmissions power is decreased
     * to zero.
     */
    ...
    /*
     * As we are in state High we need 3 successful transmissions to increase rate
     * or decrease power. As rate critical is set, after 3 successful
     * transmissions power is decreased.
     */
    for (int i = 0; i < 3; i++)
    {
        manager->ReportDataOk(mpdu, 0, ackMode, 0, txVector);
    ...
    /*
     * As we are in state High we need 3 successful transmissions to increase rate
     * or decrease power. After 10 power changes critical rate is reset. So after
     * 10*3 successful transmissions critical rate is set to 0. And 3 successful
     * transmissions more will make power increase to maximum and rate increase to
     * the critical rate.
     */
    for (int i = 0; i < 9 * 3; i++)
    {
...
PowerRateAdaptationTest::TestRrpaa()
{
     ...
     * - Opportunistic Rate Increase (and Power Decrease) Probability Threshold
     * (ORI)
     *
    ...
    /*
     * As RRPAA starts with the 6Mbps rate, 7 successful transmissions are needed
     * for RRPAA to increase rate. 1/8 = 0.125
     */
     ...
     * As we are at maximal rate, the power is decreased when it is sure that the
     * failures will not generate a rate decrease.
     */
