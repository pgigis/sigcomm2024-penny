...
class TxDurationTest : public TestCase
{
  ...
  private:
     ...
     *
     * @param size size of payload in octets (includes everything after the PHY
     * header)
                                  ...
                                  Time knownDuration)
{
    ...
    NS_ABORT_MSG_IF(channelWidth < std::accumulate(
                                       std::begin(userInfos),
                                       std::end(userInfos),
                                       0,
                    ...
                    "Cannot accommodate all the RUs in the provided band"); // MU-MIMO (for
                                                                            // which
                                                                            // allocations use
                                                                            // the same RU) is
                                                                            // not supported
    ...
    std::list<WifiPhyBand> testedBands{
        WIFI_PHY_BAND_5GHZ,
        WIFI_PHY_BAND_6GHZ,
        WIFI_PHY_BAND_2_4GHZ}; // Durations vary depending on frequency; test
                               // also 2.4 GHz (bug 1971)
    for (auto& testedBand : testedBands)
    {
        if (testedBand == WIFI_PHY_BAND_2_4GHZ)
        {
...
TxDurationTest::DoRun()
{
    ...
    // values from
    // http://mailman.isi.edu/pipermail/ns-developers/2009-July/006226.html
    ...
    // 802.11g durations are same as 802.11a durations but with 6 us signal
    // extension
    retval = retval &&
             CheckTxDuration(1536,
    ...
    retval = retval &&
             CheckMuTxDuration(std::list<uint32_t>{1536, 1536},
                               ...
                               800,
                               WIFI_PREAMBLE_HE_MU,
                               NanoSeconds(1493600)) // equivalent to HE_SU for 20 MHz with 2 extra
                                                     // HE-SIG-B (i.e. 8 us)
             && CheckMuTxDuration(std::list<uint32_t>{1536, 1536},
...
class HeSigBDurationTest : public TestCase
{
  public:
     ...
     * Constructor
     *
     * \param userInfos the HE MU specific per-user information to use for the
     * test \param sigBMode the mode to transmit HE-SIG-B for the test \param
     * channelWidth the channel width in MHz to select for the test \param
     * expectedMuType the expected MU type (OFDMA or MU-MIMO) \param
     * expectedRuAllocation the expected RU_ALLOCATION \param
...
PhyHeaderSectionsTest::DoRun()
{
    ...
    // -> long PPDU format if data rate is 1 Mbps (even if preamble is tagged
    // short)
    ...
    txVector.SetChannelWidth(20);
    txVector.SetNss(2); // HE-LTF duration assumed to be always 8 us for the time
                        // being (see note in HePhy::GetTrainingDuration)
    ...
    txVector.SetChannelWidth(20);
    txVector.SetNss(2); // EHT-LTF duration assumed to be always 8 us for the time
                        // being (see note in HePhy::GetTrainingDuration)
    ...
    : TestSuite("wifi-devices-tx-duration", UNIT)
{
    ...
    // 20 MHz band, HeSigBDurationTest::OFDMA, even number of users per HE-SIG-B
    // content channel
    ...
    // 40 MHz band, HeSigBDurationTest::OFDMA, even number of users per HE-SIG-B
    // content channel
    AddTestCase(
    ...
    // 40 MHz band, HeSigBDurationTest::OFDMA, odd number of users per HE-SIG-B
    // content channel
    AddTestCase(
        new HeSigBDurationTest({{{HeRu::RU_106_TONE, 1, true}, 11, 1},
                                {{HeRu::RU_106_TONE, 2, true}, 10, 4},
    ...
    // 40 MHz band, HeSigBDurationTest::OFDMA, unallocated RUs at the begin and at
    // the end of the first 20 MHz subband and in the middle of the second 20 MHz
    // subband
    AddTestCase(
    ...
    // 40 MHz band, HeSigBDurationTest::OFDMA, one unallocated RUs in the first 20
    // MHz subband and two unallocated RUs in second 20 MHz subband
    AddTestCase(
    ...
    AddTestCase(new HeSigBDurationTest({{{HeRu::RU_484_TONE, 1, true}, 4, 3},
                                       ...
                                       HeSigBDurationTest::MU_MIMO,
                                       {200, 200},
                                       std::make_pair(2, 1), // 2 users in content channel 1 and 1
                                                             // user in content channel 2
                                       MicroSeconds(4)),     // one OFDM symbol
                TestCase::QUICK);
    ...
    AddTestCase(new HeSigBDurationTest({{{HeRu::RU_996_TONE, 1, true}, 4, 3},
