...
std::ofstream socketSendTraceFile; ///< File that traces packets transmitted by
...
std::map<Mac48Address, uint64_t> bytesReceived;   ///< Map that stores the total bytes received per
                                                  ///< STA (and addressed to that STA)
std::map<Mac48Address, uint64_t> packetsTransmitted; ///< Map that stores the total packets
                                                     ///< transmitted per STA
std::map<Mac48Address, uint64_t>
    psduFailed; ///< Map that stores the total number of unsuccessfuly received
                ///< PSDUS (for which the PHY header was successfully received)
                ///< per STA (including PSDUs not addressed to that STA)
std::map<Mac48Address, uint64_t> psduSucceeded; ///< Map that stores the total number of
                                                ///< successfully received PSDUs per STA (including
                                                ///< PSDUs not addressed to that STA)
...
std::map<Mac48Address, uint64_t> rxEventWhileTxing; ///< Map that stores the number of reception
                                                    ///< events per STA that occurred while PHY was
                                                    ///< already transmitting a PPDU
std::map<Mac48Address, uint64_t> rxEventWhileRxing; ///< Map that stores the number of reception
                                                    ///< events per STA that occurred while PHY was
                                                    ///< already receiving a PPDU
std::map<Mac48Address, uint64_t>
    rxEventWhileDecodingPreamble;                    ///< Map that stores the number of reception
                                                     ///< events per STA that occurred while PHY
                                                     ///< was already decoding a preamble
std::map<Mac48Address, uint64_t> rxEventAbortedByTx; ///< Map that stores the number of reception
                                                     ///< events aborted per STA because the PHY has
                                                     ///< started to transmit

std::map<Mac48Address, Time> timeFirstReceived; ///< Map that stores the time at which the first
                                                ///< packet was received per STA (and the packet is
                                                ///< addressed to that STA)
std::map<Mac48Address, Time> timeLastReceived;  ///< Map that stores the time at which the last
                                                ///< packet was received per STA (and the packet is
                                                ///< addressed to that STA)
...
std::set<uint32_t> associated; ///< Contains the IDs of the STAs that successfully associated
                               ///< to the access point (in infrastructure mode only)
...
uint8_t maxMpdus = 0;    ///< The maximum number of MPDUs in A-MPDUs (0 to disable
                         ///< MPDU aggregation)
...
/**
 * Parse context strings of the form "/NodeList/x/DeviceList/x/..." to extract
 * the NodeId integer
 *
...
/**
 * Parse context strings of the form "/NodeList/x/DeviceList/x/..." and fetch
 * the Mac address
 *
 * \param context The context to parse.
...
class Experiment
{
  public:
     ...
     * \param infra flag to enable infrastructure model, ring adhoc network if not
     * set \param guardIntervalNs the guard interval in ns \param distanceM the
     * distance in meters \param apTxPowerDbm the AP transmit power in dBm \param
     * staTxPowerDbm the STA transmit power in dBm \param pktInterval the packet
     * interval \return 0 if all went well
     */
                ...
                Time pktInterval)
{
    ...
    if (infra)
    {
        ...
        uint64_t beaconInterval =
            std::min<uint64_t>((ceil((duration.GetSeconds() * 1000000) / 1024) * 1024),
                               (65535 * 1024)); // beacon interval needs to be a multiple of time
                                                // units (1024 us)
    ...
    if (infra)
    {
        Config::Connect("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/"
                        "$ns3::StaWifiMac/Assoc",
...
main(int argc, char* argv[])
{
    ...
    double duration = 100;      ///< duration (in seconds) of each simulation run (i.e.
                                ///< per trial and per number of stations)
    uint32_t trials = 1;        ///< Number of runs per point in the plot
    bool pcap = false;          ///< Flag to enable/disable PCAP files generation
    bool infra = false;         ///< Flag to enable infrastructure model, ring adhoc
                                ///< network if not set
    std::string workDir = "./"; ///< the working directory to store generated files
    std::string phyMode = "OfdmRate54Mbps"; ///< the constant PHY mode string used
                                            ///< to transmit frames
    std::string standard("11a");            ///< the 802.11 standard
    bool validate = false;                  ///< Flag used for regression in order to verify ns-3
                                            ///< results are in the expected boundaries
    uint16_t plotBianchiModel = 0x01;       ///< First bit corresponds to the DIFS
                                            ///< model, second bit to the EIFS model
    double maxRelativeError = 0.015; ///< Maximum relative error tolerated between ns-3 results and
                                     ///< the Bianchi model (used for regression, i.e. when the
    ...
    uint16_t pktInterval = 1000; ///< The socket packet interval in microseconds (a higher value is
                                 ///< needed to reach saturation conditions as the channel bandwidth
                                 ///< or the MCS increases)
    ...
    Config::SetDefault("ns3::WifiRemoteStationManager::MaxSsrc",
                       UintegerValue(std::numeric_limits<uint32_t>::max()));
    // Set maximum queue size to the largest value and set maximum queue delay to
    // be larger than the simulation time
    Config::SetDefault(
    ...
    cmd.AddValue("validate",
                 "Enable/disable validation of the ns-3 simulations against the "
                 "Bianchi model",
                 validate);
    cmd.AddValue("maxRelativeError",
                 "The maximum relative error tolerated between ns-3 results and "
                 "the Bianchi model "
    ...
    cmd.AddValue("guardIntervalNs",
                 "Set the the guard interval in nanoseconds (800 or 400 for "
                 "11n/ac, 800 or 1600 or "
                 "3200 for 11 ax)",
                 guardIntervalNs);
    cmd.AddValue("maxMpdus",
                 "Set the maximum number of MPDUs in A-MPDUs (0 to disable MPDU "
                 "aggregation)",
                 maxMpdus);
    ...
    for (uint32_t n = nMinStas; n <= nMaxStas; n += nStepSize)
    {
        ...
        if (itDifs != bianchiResultsDifs.end())
        {
            ...
            {
                NS_FATAL_ERROR("No Bianchi results (DIFS) calculated for that number "
                               "of stations!");
            }
        }
        ...
        if (itEifs != bianchiResultsEifs.end())
        {
            ...
            {
                NS_FATAL_ERROR("No Bianchi results (EIFS) calculated for that number "
                               "of stations!");
            }
        }
