...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("delay",
                 "Delay in microseconds between frame transmission from sender A "
                 "and frame "
    ...
    {
        std::cout << "Value of xA must be smaller than 0 and value of xB must be "
                     "bigger than 0!"
                  << std::endl;
        return 0;
    }
    ...
    {
        std::cout << "Preamble does not exist or is not compatible with the "
                     "selected standard!"
                  << std::endl;
        return 0;
    }
