...
class NixVectorRouting : public std::enable_if_t<std::is_same_v<Ipv4RoutingProtocol, T> ||
                                                 ...
                                                 T>
{
    /// Alias for determining whether the parent is Ipv4RoutingProtocol or
    /// Ipv6RoutingProtocol
  ...
  private:
     ...
     * \param [out] netDeviceContainer the NetDeviceContainer of the NetDevices in
     * the channel.
     */
     ...
     * \param [in] oif specific output interface to use from source node, if not
     * null \returns false if dest not found, true o.w.
     */
    ...
    static NetDeviceToIpInterfaceMap g_netdeviceToIpInterfaceMap; //!< NetDevice pointer to
                                                                  //!< IpInterface pointer map
};
