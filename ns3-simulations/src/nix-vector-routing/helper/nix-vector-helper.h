...
class NixVectorHelper : public std::enable_if_t<std::is_same_v<Ipv4RoutingHelper, T> ||
                                                ...
                                                T>
{
    /// Alias for determining whether the parent is Ipv4RoutingHelper or
    /// Ipv6RoutingHelper
  ...
  public:
    ...
    /**
     * \brief prints the routing path for a source and destination at a particular
     * time. If the routing path does not exist, it prints that the path does not
     * exist between the nodes in the ostream. \param printTime the time at which
     * the routing path is supposed to be printed. \param source the source node
     * pointer to start traversing \param dest the IP destination address \param
     * stream the output stream object to use \param unit the time unit to be used
     * in the report
     *
  ...
  private:
    ...
    /**
     * \brief prints the routing path for the source and destination. If the
     * routing path does not exist, it prints that the path does not exist between
     * the nodes in the ostream. \param source the source node pointer to start
     * traversing \param dest the IP destination address \param stream the output
     * stream object to use \param unit the time unit to be used in the report
     *
     * This method calls the PrintRoutingPath() method of the
     * NixVectorRouting for the source and destination to provide
     * the routing path.
 ...
 *
 * Note: This typedef enables also backwards compatibility with original
 * Ipv4RoutingHelper.
 */
