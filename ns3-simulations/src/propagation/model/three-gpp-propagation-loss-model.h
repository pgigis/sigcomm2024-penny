...
class ThreeGppPropagationLossModel : public PropagationLossModel
{
  public:
     ...
     * \param f the central frequency in the range in Hz, between 500.0e6 and
     * 100.0e9 Hz
     */
  ...
  private:
     ...
     *        according to the uniform distribution between the minimum and the
     * maximum value depending on the specific 3GPP scenario (UMa, UMi-Street
     * Canyon, RMa), i.e., between 0 and 25 m for UMa and UMi-Street Canyon, and
     * between 0 and 10 m for RMa. According to 3GPP TR 38.901 this 2D−in distance
     * shall be UT-specifically generated. 2D−in distance is used for the O2I
     * penetration losses calculation according to 3GPP TR 38.901 7.4.3. See
     * GetO2iLowPenetrationLoss/GetO2iHighPenetrationLoss functions. \return
     * Returns 02i 2D distance (in meters) used to calculate low/high losses.
     */
    virtual double GetO2iDistance2dIn() const = 0;

    /**
     * \brief Retrieves the o2i building penetration loss value by looking at
     * m_o2iLossMap. If not found or if the channel condition changed it generates
     * a new independent realization and stores it in the map, otherwise it
     * calculates a new value as defined in 3GPP TR 38.901 7.4.3.1.
     *
     *        Note that all child classes should implement this function to
     * support low losses calculation. As such, this function should be purely
     * virtual.
     *
     ...
     *
     *        Note that all child classes should implement this function to
     * support high losses calculation. As such, this function should be purely
     * virtual.
     *
     * \param a tx mobility model (used for the key calculation)
     * \param b rx mobility model (used for the key calculation)
     ...
     *        the tallest node is the BS and the smallest is the UT. The derived
     * classes can change the default behavior by overriding this method. \param
     * za the height of the first node in meters \param zb the height of the
     * second node in meters \return std::pair of heights in meters, the first
     * element is hUt and the second is hBs
     */
     ...
     *        independent realization and stores it in the map, otherwise it
     * correlates the new value with the previous one using the autocorrelation
     * function defined in 3GPP TR 38.901, Sec. 7.4.4. \param a tx mobility model
     * \param b rx mobility model
     * \param cond the LOS/NLOS channel condition
  ...
  protected:
    ...
    bool m_buildingPenLossesEnabled;                //!< enable/disable building penetration
                                                    //!< losses
    ...
    Ptr<NormalRandomVariable>
        m_normalO2iLowLossVar; //!< a normal random variable for the calculation
                               //!< of 02i low loss, see TR38.901 Table 7.4.3-2
    Ptr<NormalRandomVariable>
        m_normalO2iHighLossVar; //!< a normal random variable for the calculation
                                //!< of 02i high loss, see TR38.901 Table 7.4.3-2
};
