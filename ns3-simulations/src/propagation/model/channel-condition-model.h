 ...
 *
 * Additional information about the channel condition can be aggregated to
...
class ChannelCondition : public Object
{
  public:
     ...
     * \param o2iLowHighCondition the O2I Low-High Building Penetration loss
     * condition value (by default is set to LOW)
     */
     ...
     *
     * \param losCondition the LOS condition of the other channel condition
     * instance \param o2iCondition the 02I condition of the other channel
     * condition instance \return true if the channel LOS and O2i conditions of
     * the instance are equivalent to those passed as arguments
     */
    bool IsEqual(LosConditionValue losCondition, O2iConditionValue o2iCondition) const;

  private:
    LosConditionValue m_losCondition;               //!< contains the information about the LOS
                                                    //!< state of the channel
    O2iConditionValue m_o2iCondition;               //!< contains the information about the O2I
                                                    //!< state of the channel
...
class AlwaysLosChannelConditionModel : public ChannelConditionModel
{
  public:
    ...
    /**
     * Computes the condition of the channel between a and b, that will be always
     * LoS
     *
     * \param a mobility model
     * \param b mobility model
     * \return the condition of the channel between a and b, that will be always
     * LoS
     */
...
class ThreeGppChannelConditionModel : public ChannelConditionModel
{
  public:
     ...
     * ComputeChannelCondition and stores it in a local cache, that will be
     * updated following the "UpdatePeriod" parameter.
     *
     * \param a mobility model
     * \param b mobility model
     * \return the condition of the channel between a and b
  ...
  private:
    ...
    double m_o2iThreshold{0};        //!< the threshold for determining what is the
                                     //!< ratio of channels with O2I
    ...
    bool m_linkO2iConditionToAntennaHeight{
        false}; //!< the indicator that determines whether the O2I/O2O condition
                //!< is determined based on the UE height
    Ptr<UniformRandomVariable> m_uniformVarO2i; //!< uniform random variable that is used for the
                                                //!< generation of the O2i conditions
    Ptr<UniformRandomVariable> m_uniformO2iLowHighLossVar; //!< a uniform random variable for the
                                                           //!< calculation of the low/high losses,
