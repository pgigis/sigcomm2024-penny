 ...
 * \ingroup fading
 *
 * \brief Implementation for a single path Stationary Jakes propagation loss
 *model.
 *
 ...
 * \f[ X_c(t) = \frac{2}{\sqrt{M}}\sum_{n=1}^{M}\cos(\psi_n)\cos(\omega_d
 *t\cos(\alpha_n)+\phi_n)\f] \f[ X_s(t) =
 *\frac{2}{\sqrt{M}}\sum_{n=1}^{M}\sin(\psi_n)\cos(\omega_d
 *t\cos(\alpha_n)+\phi_n)\f] with \f[ \alpha_n = \frac{2\pi n - \pi +
 *\theta}{4M},  n=1,2, \ldots,M\f] where \f$\theta\f$, \f$\phi\f$, and
 *\f$\psi_n\f$ are statically independent and uniformly distributed over
 *\f$[-\pi, \pi)\f$ for all \f$n\f$.
 *
 *
...
class JakesProcess : public Object
{
  ...
  private:
    ...
    {
        /**
         * Initiate oscillator with complex amplitude, initial phase and rotation
         * speed \param amplitude initial complex amplitude \param initialPhase
         * initial phase \param omega rotation speed
         */
        ...
        std::complex<double> m_amplitude; //!< Complex number \f$Re=\cos(\psi_n),
                                          //!< Im = i\sin(\psi_n)]\f$
        double m_phase;                   //!< Phase \f$\phi_n\f$ of the oscillator
        double m_omega;                   //!< Rotation speed of the oscillator \f$\omega_d
                                          //!< \cos(\alpha_n)\f$
    };
