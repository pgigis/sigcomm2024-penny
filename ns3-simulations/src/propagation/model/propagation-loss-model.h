...
class PropagationLossModel : public Object
{
  public:
     ...
     * \returns the reception power after adding/multiplying propagation loss (in
     * dBm)
     */
 ...
 * so if, for \f$ d = 0 \f$, we use a fixed loss value, we end up with an
 * infinitely large discontinuity, which as we discussed can cause undesirable
 ...
 * Each field begins where the previous ends and all together form a continuous
function.
 *
 ...
 *
 * \f[ \underbrace{0 \cdots\cdots}_{=0} \underbrace{d_0 \cdots\cdots}_{n_0}
\underbrace{d_1 \cdots\cdots}_{n_1} \underbrace{d_2 \cdots\cdots}_{n_2} \infty
\f]
 *
...
L_0 + 10 \cdot n_0 \log_{10}(\frac{d_1}{d_0}) + 10 \cdot n_1
\log_{10}(\frac{d}{d_1}) & d_1 \leq d < d_2 \\ L_0 + 10 \cdot n_0
\log_{10}(\frac{d_1}{d_0}) + 10 \cdot n_1 \log_{10}(\frac{d_2}{d_1}) + 10 \cdot
n_2 \log_{10}(\frac{d}{d_2})& d_2 \leq d \end{cases}\f]
 *
 * where:
 *  - \f$ L \f$ : resulting path loss (dB)
 *  - \f$ d \f$ : distance (m)
 *  - \f$ d_0, d_1, d_2 \f$ : three distance fields (m)
 *  - \f$ n_0, n_1, n_2 \f$ : path loss distance exponent for each field
(unitless)
 *  - \f$ L_0 \f$ : path loss at reference distance (dB)
 *
 ...
 * ns3::FriisPropagationLossModel with 5.15 GHz and is thus \f$ L_0 \f$ = 46.67
dB.
 */
 ...
 * \f[ p(x; m, \omega) = \frac{2 m^m}{\Gamma(m) \omega^m} x^{2m - 1}
 * e^{-\frac{m}{\omega} x^2}  \f] with \f$ m \f$ the fading depth parameter and
 * \f$ \omega \f$ the average received power.
 *
 * It is implemented by either a ns3::GammaRandomVariable or a
 * ns3::ErlangRandomVariable random variable.
 *
 * The implementation of the model allows to specify different values of the m
 * parameter (and hence different fading profiles) for three different distance
 * ranges: \f[ \underbrace{0 \cdots\cdots}_{m_0} \underbrace{d_1
 * \cdots\cdots}_{m_1} \underbrace{d_2 \cdots\cdots}_{m_2} \infty \f]
 *
 ...
 * \ingroup propagation
 *
 * \brief The propagation loss is fixed for each pair of nodes and doesn't
 * depend on their actual positions.
 *
 * This is supposed to be used by synthetic tests. Note that by default
...
class MatrixPropagationLossModel : public PropagationLossModel
{
  public:
     ...
     * \param symmetric   If true (default), both a->b and b->a paths will be
     * affected
     */
    void SetLoss(Ptr<MobilityModel> a, Ptr<MobilityModel> b, double loss, bool symmetric = true);

    /**
     * Set the default propagation loss (in dB, positive) to be used, infinity if
     * not set \param defaultLoss the default proagation loss
     */
 ...
 * \ingroup propagation
 *
 * \brief The propagation loss depends only on the distance (range) between
