...
/**
 * \ingroup propagation
 * \brief Constructs a JakesPropagationlossModel and print the loss value as a
 * function of time into std::cout. Distribution and correlation statistics is
 * compared with a theoretical ones using R package (http://www.r-project.org/).
...
main(int argc, char* argv[])
{
     ...
     * R script for plotting a distribution:
     ...
     qqplot(10*log10(rayleigh), data$V2, main="QQ-plot for improved Jakes model",
     xlab="Reference Rayleigh distribution [power, dB]", ylab="Sum-of-sinusoids
     distribution [power, dB]", xlim=c(-45, 10), ylim=c(-45, 10)) lines (c(-50,
     50), c(-50, 50)) abline (v=-50:50*2, h=-50:50*2, col="light grey")
     */
     ...
     * R script to plot autocorrelation function:
     ...
     x<-1:2000/10
     acf (data, lag.max=200, main="Autocorrelation function of the improved Jakes
     model", xlab="Time x200 microseconds ", ylab="Autocorrelation") # If we have
     a delta T = 1/5000 part of the second and doppler freq = 80 Hz lines (x,
     besselJ(x*80*2*pi/5000, 0)^2) abline (h=0:10/10, col="light grey")
     */
    return 0;
}
