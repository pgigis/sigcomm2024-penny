...
// 3. Test for operation of count attribute and exit time after all pings were
// received
...
// 6. Test pinging to IPv4 broadcast address and IPv6 all nodes multicast
// address
...
PingTestCase::RttTraceSink(uint16_t seq, Time rttSample)
{
    ...
    if (m_sizeAttribute == 56)
    {
        if (!m_useIpv6)
        {
            // Each ping should be received with RTT of 20.0013 ms in this
            // configuration Check that each ping is within 20.0012 ms <= rttSample
            // <= 20.0014 ms
        ...
        {
            // Each ping should be received with RTT of 20.0017 ms in this
            // configuration Check that each ping is within 20.0016 ms <= rttSample
            // <= 20.0018 ms
            NS_TEST_ASSERT_MSG_EQ_TOL(
                rttSample.GetSeconds() * 1000,
                20.0017,
    ...
    : TestSuite("ping", UNIT)
{
    ...
    auto testcase1v4 = new PingTestCase("1. Unlimited pings, no losses, StopApplication () with "
    ...
    auto testcase1v6 = new PingTestCase("1. Unlimited pings, no losses, StopApplication () with "
    ...
    auto testcase2v4 = new PingTestCase("2. Unlimited pings, no losses, StopApplication () with "
    ...
    auto testcase2v6 = new PingTestCase("2. Unlimited pings, no losses, StopApplication () with "
    ...
    // 3. Test for operation of count attribute and the resulting StopApplication
    // time after all pings were received. Limited pings, no losses,
    // StopApplication () with no packet in flight Configuration:  Ping::Count =
    // 2,  Ping::Interval = 1s, Ping start time =
