...
class RadvdInterface : public SimpleRefCount<RadvdInterface>
{
  public:
     ...
     * \return true if advertisement interval option is added in RA, false
     * otherwise
     */
    ...
    /**
     * \brief Set the last RA send time. It also decrements the initial Rtr
     * Advertisements counter. \param now the last RA send time
     */
    void SetLastRaTxTime(Time now);

    /**
     * \brief Checks if the interface is subject to the initial Rtr Advertisements
     * rule. \returns true if the initial Rtr Advertisements counter is greater
     * than zero.
     */
  ...
  private:
    ...
    /**
     * \brief Other configuration flag. If true host use stateful protocol for
    ...
    /**
     * \brief Home agent lifetime in seconds. Ignored if home agent info is not
     * set.
     */
    ...
    /**
     * \brief Flag for HA to signals it supports Mobile Router registrations (NEMO
     * Basic).
     */
