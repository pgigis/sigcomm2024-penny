                                                              ...
                                                              Ptr<UniformRandomVariable> uniRand)
{
    ...
    // restricts maximum altitude from being less than zero (below earth's
    // surface). sets maximum altitude equal to zero if parameter is set to be
    // less than zero.
    if (maxAltitude < 0)
    {
    ...
    for (int i = 0; i < numPoints; i++)
    {
        ...
        // reference:
        // http://en.wikibooks.org/wiki/General_Astronomy/Coordinate_Systems
