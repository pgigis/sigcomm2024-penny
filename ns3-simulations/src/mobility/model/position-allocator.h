...
/**
 * \ingroup mobility
 * \brief Allocate a set of positions. The allocation strategy is implemented in
 * subclasses.
 *
...
class ListPositionAllocator : public PositionAllocator
{
  public:
     ...
     * \param v the position to append at the end of the list of positions to
     * return from GetNext.
     */
    void Add(Vector v);
...
class GridPositionAllocator : public PositionAllocator
{
  public:
    ...
    enum LayoutType
    {
         ...
         * N positions have been allocated. Then, the second row located a yMin +
         * yDelta is used to allocate positions.
         */
        ROW_FIRST,
        /**
         * In column-first mode, positions are allocated on the first column until
         * N positions have been allocated. Then, the second column located a xMin +
         * xDelta is used to allocate positions.
         */
        COLUMN_FIRST
    };
...
/**
 * \ingroup mobility
 * \brief Allocate random positions within a rectangle according to a pair of
 * random variables.
 */
...
/**
 * \ingroup mobility
 * \brief Allocate random positions within a 3D box according to a set of three
 * random variables.
 */
 ...
 *
 * \note With the default uniform distribution over \f$2 \pi\f$ in \c theta and
 * a uniform distribution for \c rho this position allocator will *not*
 * uniformly populate the disc.  The radial distribution will be proportional
 * to \f$\frac{1}{r^2}\f$.
 *
 * To get a uniform distribution over a circle use the
...
class RandomDiscPositionAllocator : public PositionAllocator
{
  public:
     ...
     * \param theta Random variable that represents the angle in radians of a
     * position in a random disc.
     */
    void SetTheta(Ptr<RandomVariableStream> theta);
    /**
     * \brief Set the random variable that generates position radius, in meters
     * \param rho Random variable that represents the radius of a position, in
     * meters, in a random disc.
     */
...
/**
 * \ingroup mobility
 * \brief Allocate the positions uniformly (with constant density) randomly
 * within a disc.
 *
 * UniformDiscPositionAllocator allocates the positions randomly within a disc
 * \f$ D \f$ lying on the plane \f$ z\f$ and having center at coordinates \f$
 * (x,y,z) \f$ and radius \f$ \rho \f$. The random positions are chosen such
 * that, for any subset \f$ S \subset D \f$, the expected value of the fraction
 * of points which fall into \f$ S \subset D \f$ corresponds to \f$
 * \frac{|S|}{|D|} \f$, i.e., to the ratio of the area of the subset to the area
 * of the whole disc.
 *
