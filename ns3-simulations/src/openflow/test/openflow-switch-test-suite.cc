...
{
    // Flow Table implementation is used by the OpenFlowSwitchNetDevice under the
    // chain_ methods we should test its implementation to verify the flow table
    // works.

    // Initialization
    time_init(); // OFSI requires this, otherwise we crash before we can do
                 // anything.
    ...
    // Create an sw_flow_key; in actual usage this is generated from the received
    // packet's headers.
    sw_flow_key key;
    key.wildcards = 0;
    ...
    // Testing Flow Modification; chain_modify should return 1, for 1 flow
    // modified. Create output-to-port action
    ...
    // Note: By providing chain_delete with output_port, the flow must have an
    // action that outputs on that port in order to delete the flow. This is how
    // we verify that our action was truly added via the flow modification.
