    ...
    static TypeId tid =
        TypeId("ns3::OpenFlowSwitchNetDevice")
            ...
            .AddAttribute("FlowTableLookupDelay",
                          ...
                          MakeTimeChecker())
            .AddAttribute("Flags", // Note: The Controller can configure this value,
            ...
            .AddAttribute("FlowTableMissSendLength", // Note: The Controller can configure
                                           ...
                                           PacketType packetType)
{
    ...
    if (now >= Seconds(m_lastExecute.GetSeconds() + 1)) // If a second or more has passed from the
                                                        // simulation time, execute.
    {
