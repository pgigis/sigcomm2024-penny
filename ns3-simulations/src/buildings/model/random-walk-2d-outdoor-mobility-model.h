 ...
 * Henderson, L.F., 1971. The statistics of crowd fluids. nature, 229(5284),
 * p.381.
 */
class RandomWalk2dOutdoorMobilityModel : public MobilityModel
{
  ...
  private:
    ...
    /**
     * Check if there is a building between two positions (or if the nextPosition
     * is inside a building). The code is taken from
     * MmWave3gppBuildingsPropagationLossModel from the NYU/UNIPD ns-3 mmWave
     * module \param currentPosition The current position of the node \param
     * nextPosition The position to check \return a pair with a boolean (true if
     * the line between the two position does not intersect building), and a
     * pointer which is 0 if the boolean is true, or it points to the building
     * which is intersected
     */
    std::pair<bool, Ptr<Building>> IsLineClearOfBuildings(Vector currentPosition,
                                                          Vector nextPosition) const;
    /**
     * Compute the intersecting point of the box represented by boundaries and the
     * line between current and next. Notice that we only consider a 2d plane.
    ...
    Vector m_prevPosition;                 //!< Store the previous position in case a step back
                                           //!< is needed
};
