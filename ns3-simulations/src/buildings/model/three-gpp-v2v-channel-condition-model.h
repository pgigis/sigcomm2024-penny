 ...
 * LOS, NLOS and NLOSv, the latter representing the case in which the LOS path
 * is blocked by other vehicles in the scenario. The document defines a
 * probabilistic model to determine if the channel state is LOS or NLOSv, while
 * the NLOS state is determined in a deterministic way based on the buildings
 * deployed in the scenario. For this reason, this class makes use of an
 * instance of BuildingsChannelConditionModel to determine if the LOS is
 * obstructed by buildings or not.
 */
class ThreeGppV2vUrbanChannelConditionModel : public ThreeGppChannelConditionModel
{
  ...
  private:
    /**
     * Compute the LOS probability as specified in Table Table 6.2-1 of 3GPP
     * TR 37.885 for the V2V Urban scenario.
     *
...
class ThreeGppV2vHighwayChannelConditionModel : public ThreeGppChannelConditionModel
{
  ...
  private:
    /**
     * Compute the LOS probability as specified in Table Table 6.2-1 of 3GPP
     * TR 37.885 for the V2V Highway scenario.
     *
     * \param a tx mobility model
     * \param b rx mobility model
     * \return the LOS probability
    ...
    /**
     * \brief The callback which is hooked to a method to compute channel
     * condition.
     *
