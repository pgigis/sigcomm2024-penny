      ...
      m_s5LinkMtu(3000)
{
    ...
    // create TUN device implementing tunneling of user data over GTP-U/UDP/IP in
    // the PGW
                                       ...
                                       EpsBearer bearer)
{
    ...
    NS_ASSERT_MSG(ueIpv4 || ueIpv6,
                  "UEs need to have IPv4/IPv6 installed before "
                                              ...
                                              const EpsBearer& bearer) const
{
    ...
    if (!ueLteDevice)
    {
        // You may wonder why this is not an assert. Well, take a look in
        // epc-test-s1u-downlink and -uplink: we are using CSMA to simulate UEs.
