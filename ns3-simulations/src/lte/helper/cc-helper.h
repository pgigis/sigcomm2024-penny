 ...
 * \ingroup lte
 *
 * Creation and configuration of Component Carrier entities. One CcHelper
 * instance is typically enough for an LTE simulation. To create it:
 *
 *     Ptr<CcHelper> ccHelper = CreateObject<CcHelper> ();
 *
 * The general responsibility of the helper is to create various Component
 * Carrier objects and arrange them together to set the eNodeB. The overall
 ...
 *
 *
 * This helper it is also used within the LteHelper in order to maintain
 * backwards compatibility with previous user simulation script.
 */
class CcHelper : public Object
{
  public:
     ...
     * > The Primary Component Carrier it is at the position 0 in the map The
     * number of Component Carrier created depend on m_noOfCcs Currently it is
     * limited to maximum 2 ComponentCarrier Since, only a LteEnbPhy object is
     * available just symmetric Carrier Aggregation scheme are allowed, i.e. 2
     * Uplink Component Carrier and 2 Downlink Component Carrier Using this
     * method, each CC will have the same characteristics (bandwidth) while they
     * are spaced by exactly the bandwidth. Hence, using this method, you will
     * create a intra-channel Carrier Aggregation scheme.
     *
  ...
  private:
     ...
     * \param isPrimary identify if this is the Primary Component Carrier (PCC) -
     * only one PCC is allowed \return the component carrier
     */
