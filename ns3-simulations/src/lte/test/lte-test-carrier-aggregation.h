 ...
 * \ingroup lte-test
 *
 * \brief This system test program creates different test cases with a single
 * eNB and several UEs, all having the same Radio Bearer specification. In each
 * test case, the UEs see the same SINR from the eNB; different test cases are
 * implemented obtained by using different SINR values and different numbers of
 * UEs. eNb and UEs are configured to use the secondary carrier and the
 * component carrier manager is configured to split the data equally between
 * primary and secondary carrier. The test consists of checking that the
 * throughput obtained over different carriers are equal within a given
 * tolerance.
 */
class CarrierAggregationTestCase : public TestCase
{
  public:
    static bool s_writeResults; ///< write results flag, determines whether to
                                ///< write results to outoput files
     ...
     * \param numberOfComponentCarriers number of component carriers to be used in
     * test configuration
     */
    ...
    /**
     * DL Scheduling function that is used in this test as callback function of DL
     * scheduling trace \param dlInfo the DL scheduling callback info
     */
    void DlScheduling(DlSchedulingCallbackInfo dlInfo);
    /**
     * UL Scheduling function that is used in this test as callback function of UL
     * sceduling trace \param frameNo the frame number \param subframeNo the
     * subframe number \param rnti the RNTI \param mcs the MCS \param sizeTb
