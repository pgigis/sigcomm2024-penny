 ...
 *        piecewise configuration and 120 ms report interval. During the
 * simulation the placement of UE is being changed several times. Four different
 ...
 *        when the signal from serving cell becomes weak, the measurements
 * should also start to correspond to the new channel conditions. Additionally,
 * it is checked if UE detects some neighboring signal, if it does the test
 *        fails because there is no neighbor in this configuration. Also, test
 *        checks if the reporting occurs at intervals that it should according
 * to the measurements configuration. If it occurs in some different time stamp
 * the test will fail.
 */
 ...
 *        Here is intenisvely test events A1 and A2. A1 event should be
 * triggered when the serving cell becomes better than a given threshold. A2
 * shall be triggered when the serving cell becomes worse than threshold. A3
 *        event is triggered when the neighbour becomes offset better than the
 * PCell. In this test UE is being transported several times during the
 * simulation duration. Then the test checks if measurement report contains
 * correct RSRP and RSRQ result, than if CQI info is correctly generated, if the
 *        reporting occurs at the correct time stamp in the simulation and
 * according to the measurements configuration. It also checks if the reported
 * values of RSRP and RSRQ are equal to expected reference values.
 *
 */
 ...
 *        send measurement reports after the RLF, even though all the
 * measurement events are properly cancelled upon detecting RLF.
 *
 ...
 * handover threshold configurations, A2 threshold difference, A3 offset
 * difference, A4 and A5 threshold difference. Finally, different handover is
 * tested for different time-to-trigger (TTT) configurations.
 *
 */
