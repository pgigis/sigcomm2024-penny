    ...
    : TestSuite("lte-tdbet-ff-mac-scheduler", SYSTEM)
{
    ...
    // bytes/sec UPLINK- DISTANCE 0 -> MCS 28 -> Itbs 26 (from table 7.1.7.2.1-1
    ...
    // of 36.213) 1 user -> 24 PRB at Itbs 20 -> 1383 -> 1383000 bytes/sec 3 users
    ...
    // bytes/sec UPLINK - DISTANCE 4800 -> MCS 14 -> Itbs 13 (from
    ...
    // of 36.213) 1 user -> 24 PRB at Itbs 18 -> 1191 -> 1191000 byte/sec 3 users
    ...
    // bytes/sec UPLINK - DISTANCE 6000 -> MCS 12 -> Itbs 11 (from
    ...
    // 4 PRB at Itbs 11 -> 97 -> 97000 bytes/sec 12 users -> 3 PRB at Itbs 11 ->
    // 73 bytes * 8/12 UE/TTI -> 48667 bytes/sec
    ...
    // of 36.213) 1 user -> 24 PRB at Itbs 13 -> 775 -> 775000 byte/sec 3 users ->
    // 775000 among 3 users -> 258333  bytes/sec 6 users -> 775000 among 6 users
    // -> 129166  bytes/sec 12 users -> 775000 among 12 users ->64583  bytes/sec
    // UPLINK - DISTANCE 9000 -> MCS 8 -> Itbs 8 (from table 7.1.7.2.1-1
    ...
    // DOWNLINK - DISTANCE 20000 -> MCS 8 -> Itbs 8 (from table 7.1.7.2.1-1
    // of 36.213) 1 user -> 24 PRB at Itbs 8 -> 421 -> 421000 bytes/sec 3 users ->
    // 421000 among 3 users ->140333  bytes/sec 6 users -> 421000 among 6 users
    // ->70166  bytes/sec 12 users -> 421000 among 12 users ->35083  bytes/sec
    // UPLINK - DISTANCE 20000 -> MCS 2 -> Itbs 2 (from table 7.1.7.2.1-1
    // of 36.213) 1 user -> 25 PRB at Itbs 2 -> 137 -> 137000 bytes/sec 3 users ->
    // 8 PRB at Itbs 2 -> 41 -> 41000 bytes/sec 6 users -> 4 PRB at Itbs 2 -> 22
    // -> 22000 bytes/sec after the patch enforcing min 3 PRBs per UE: 12 users ->
    // 3 PRB at Itbs 2 -> 18 bytes * 8/12 UE/TTI -> 12000 bytes/sec
...
LenaTdBetFfMacSchedulerTestCase1::DoRun()
{
    ...
    /**
     * Check that the downlink assignment is done in a "TD blind equal throughput"
     * manner
     */
    ...
    /**
     * Check that the assignment is done in a "TD blind equal throughput" manner
     * among users with equal SINRs: the bandwidth should be distributed according
     * to the ratio of the estimated throughput per TTI of each user; therefore
    ...
    /**
     * Check that the uplink assignment is done in a "TD blind equal throughput"
     * manner
     */
...
LenaTdBetFfMacSchedulerTestCase2::DoRun()
{
    ...
    /**
     * Check that the assignment is done in a "TD blind equal throughput" manner
     * among users with different SINRs: the bandwidth should be distributed
     * equally in long term
     */
    for (int i = 0; i < m_nUser; i++)
    {
