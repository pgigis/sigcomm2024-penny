...
class LteRadioLinkFailureTestCase : public TestCase
{
  public:
     ...
     * \param checkConnectedList the time at which UEs should have an active RRC
     * connection
     */
  ...
  private:
    ...
    /**
     * \brief This callback function is executed when UE context is removed at
     * eNodeB \param context the context string \param imsi the IMSI \param cellId
     * the cell ID \param rnti the RNTI
     */
    ...
    /**
     * \brief This callback function is executed when UE RRC receives an in-sync
     * or out-of-sync indication \param context the context string \param imsi the
     * IMSI \param rnti the RNTI \param cellId the cell ID \param type in-sync or
     * out-of-sync indication \param count the number of in-sync or out-of-sync
     * indications
     */
    ...
    /**
     * \brief This callback function is executed when radio link failure is
     * detected \param context the context string \param imsi the IMSI \param rnti
     * the RNTI \param cellId the cell ID
     */
    ...
    std::vector<Time> m_checkConnectedList; ///< the time at which UEs should
    ...
    uint32_t m_numOfOutOfSyncIndications; ///< number of out-of-sync indications
                                          ///< detected
