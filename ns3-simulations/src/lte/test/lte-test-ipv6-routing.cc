...
/* *
   * Scenario:  3 UEs, 2 ENBs, 1 Remote Host, UE0<-->ENB0, UE1<-->ENB0,
UE2<-->ENB1 Servers: UE1, UE2, Remote Host Client: UE0 (3 clients) UDP Echo
Packets transmitted between client and server

   * Pass criteria: 1) Every UDP Echo Request and Reply messages sent and
received respectively at UE0 must be matched by their UID, source address,
destination address, source port and destination port 2) Every request reply
must follow proper route (e.g. In case of UE0->UE1, packet must travel this
route: UE0->ENB0->PGW->ENB1->UE1->ENB1->PGW->ENB0->UE0) 3) The above check also
ensures no redundancy of the followed route for a packet
* */
...
class LteIpv6RoutingTestCase : public TestCase
{
  ...
  private:
    ...
    std::list<uint64_t> m_pgwUidRxFrmTun; //!< list of uids of packets received at
                                          //!< pgw from tunnel net device
...
LteIpv6RoutingTestCase::DoRun()
{
    ...
    // Set Callback at SgwPgWApplication of epc to get the packets from enb and
    // from tunnel net device
