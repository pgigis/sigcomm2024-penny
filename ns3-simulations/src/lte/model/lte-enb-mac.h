...
class LteEnbMac : public Object
{
  ...
  private:
     ...
     * Frame number, Subframe number, RNTI, MCS of TB, size of TB, component
     * carrier id
     */
    ...
    std::map<uint16_t, DlHarqProcessesBuffer_t>
        m_miDlHarqProcessesPackets; ///< Packet under transmission of the DL HARQ
                                    ///< process
    ...
    {
        uint16_t rnti;   ///< rnti previously allocated for this non-contention based
                         ///< RA procedure
        Time expiryTime; ///< value the expiration time of this allocation (so that
                         ///< stale preambles can be reused)
    };
