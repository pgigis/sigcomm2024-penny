 ...
 * \ingroup lte
 *
 * This application is installed inside eNBs and provides the bridge
 * functionality for user data plane packets between the radio interface and the
 * S1-U interface.
 */
class EpcEnbApplication : public Application
{
  ...
  public:
     ...
     * Constructor
     *
     * \param lteSocket the socket to be used to send/receive IPv4 packets to/from
     * the LTE radio interface \param lteSocket6 the socket to be used to
     * send/receive IPv6 packets to/from the LTE radio interface \param cellId the
     * identifier of the eNB
     */
     ...
     * S1-U interface connected with the SGW \param enbS1uAddress the IPv4 address
     * of the S1-U interface of this eNB \param sgwS1uAddress the IPv4 address at
     * which this eNB will be able to reach its SGW for S1-U communications
     */
    ...
    /**
     * Method to be assigned to the recv callback of the LTE socket. It is called
     * when the eNB receives a data packet from the radio interface that is to be
    ...
    /**
     * Method to be assigned to the recv callback of the S1-U socket. It is called
     * when the eNB receives a data packet from the SGW that is to be forwarded to
     * the UE.
     *
  ...
  private:
    ...
    /**
     * \brief This function accepts bearer id corresponding to a particular UE and
     * schedules indication of bearer release towards MME \param imsi maps to
     * mmeUeS1Id \param rnti maps to enbUeS1Id \param bearerId Bearer Identity
     * which is to be de-activated
     */
    ...
    /**
     * raw packet socket to send and receive the packets to and from the LTE radio
     * interface
     */
    ...
    /**
     * UDP socket to send and receive GTP-U the packets to and from the S1-U
     * interface
     */
