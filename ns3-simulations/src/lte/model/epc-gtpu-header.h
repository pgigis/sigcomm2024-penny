...
class GtpuHeader : public Header
{
  public:
    ...
    /**
     * Sets the flag that indicates the presence of a meaningful value of the
     * N-PDU Number field. \param nPduNumberFlag the NPDU number flag
     */
  ...
  private:
    /**
     * This field is used to determine the version of the GTPU-U protocol. The
     * version number shall be set to 1.
     */
    uint8_t m_version; // really a 3 uint3_t

    /**
     * This bit is used as a protocol discriminator between GTP (when PT is '1')
     * and GTP' (when PT is '0').
     */
    bool m_protocolType;

    /**
     * This flag indicates the presence of a meaningful value of the Next
     * Extension Header field. When it is set to '0', the Next Extension Header
     * field either is not present or, if present, shall not be interpreted. When
     * it is set to '1', the Next Extension Header field is present, and shall be
     * interpreted
     */
    bool m_extensionHeaderFlag;

    /**
     * This flag indicates the presence of a meaningful value of the Sequence
     * Number field. When it is set to '0', the Sequence Number field either is
     * not present or, if present, shall not be interpreted. When it is set to
     * '1', the Sequence Number field is present, and shall be interpreted
     */
    bool m_sequenceNumberFlag;
    /**
     * This flag indicates the presence of a meaningful value of the N-PDU Number
     * field. When it is set to '0', the N-PDU Number field either is not present,
     * or, if present, shall not be interpreted. When it is set to '1', the N-PDU
     * Number field is present, and shall be interpreted
     */
    ...
    /**
     *  This field indicates the length in octets of the payload, i.e. the rest of
     * the packet following the mandatory part of the GTP header (that is the
    ...
    /**
     * This field unambiguously identifies a tunnel endpoint in the receiving
     * GTP-U protocol entity. The receiving end side of a GTP tunnel locally
     * assigns the TEID value the transmitting side has to use.
     */
    uint32_t m_teid;
    /**
    ...
    /**
     * This field is used at the Inter SGSN Routeing Area Update procedure and
     * some inter-system handover procedures (e.g. between 2G and 3G radio access
     * networks). This field is used to co-ordinate the data transmission for
     * acknowledged mode of communication between the MS and the SGSN
     */
