...
class LteAmc : public Object
{
  public:
    ...
    /**
     * \brief Get the Transport Block Size for a selected MCS and number of PRB
     * (table 7.1.7.2.1-1 of 36.213) \param mcs the MCS index \param nprb the no.
     * of PRB \return the Transport Block Size in bits
     */
    int GetDlTbSizeFromMcs(int mcs, int nprb);

    /**
     * \brief Get the Transport Block Size for a selected MCS and number of PRB
     * (table 8.6.1-1 of 36.213) \param mcs the MCS index \param nprb the no. of
     * PRB \return the Transport Block Size in bits
     */
     ...
     * \param rbgSize size of RB group (in RBs) for evaluating subband/wideband
     * CQI \return a vector of CQI feedbacks
     */
     ...
     * In order to assure a lower block error rate, the AMC chooses the lower CQI
     * value for a given spectral efficiency \param s the spectral efficiency
