...
class LteUeComponentCarrierManager : public Object
{
  public:
    ...
    /**
     * \brief Exports the "provider" part of the ComponentCarrier Management SAP
     * interface. \return the reference to the "provider" part of the interface,
     * typically to be kept by an LteUeRrc instance
     */
    virtual LteUeCcmRrcSapProvider* GetLteCcmRrcSapProvider();

    /**
     * \brief Returns the MAC sap provider interface that if forwarding calls to
     * the instance of the LteUeComponentCarrierManager. \return the reference to
     * the "provider" part of the interface
     */
    virtual LteMacSapProvider* GetLteMacSapProvider() = 0;

    /**
     * \brief Sets a pointer to SAP interface of MAC instance for the specified
     * carrier. \param componentCarrierId the component carrier id \param sap the
     * pointer to the sap interface \return whether the settings of the sap
  ...
  protected:
    ...
    uint8_t m_noOfComponentCarriers; //!< The number of component carriers that
                                     //!< this UE can support.
    std::map<uint8_t, LteMacSapProvider*>
        m_macSapProvidersMap; //!< Map of pointers to SAP to interfaces of the
                              //!< MAC instance if the flows of this UE.
