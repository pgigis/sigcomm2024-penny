...
/**
 * \brief Service Access Point (SAP) offered by the Component Carrier Manager
 * (CCM) instance to the eNodeB RRC instance.
 *
 * This is the *Component Carrier Manager SAP Provider*, i.e., the part of the
 * SAP that contains the CCM methods called by the eNodeB RRC instance.
 */
class LteCcmRrcSapProvider
{
  ...
  public:
     ...
     *
     * The received measurement report is a result of the UE measurements
     * configuration previously configured by calling
     * LteCcmRrcSapProvider::AddUeMeasReportConfigForComponentCarrier. The report
     * may be stored and utilized for the purpose of making decision if and when
     ...
     * UE. \param state The current rrc state of the UE.
     */
     ...
     * \param msu a pointer to the LteMacSapUser, the
     * LteEnbComponentCarrierManager has to store a LteMacSapUser for each Rlc
     * instance, in order to properly redirect the packet \return vector of
     * LcsConfig contains the lc configuration for each Mac the size of the vector
     * is equal to the number of component carrier enabled.
     *
     * The Logical Channel configurations for each component carrier depend on the
     * algorithm used to split the traffic between the component carriers
     * themself.
     */
    ...
    /**
     * \brief Release an existing Data Radio Bearer for a Ue in the
     * LteEnbComponentCarrierManager \param rnti Radio Network Temporary Identity,
     * an integer identifying the UE where the report originates from \param lcid
     * the Logical Channel Id \return vector of integer the componentCarrierId of
     * the componentCarrier where the bearer is enabled
     */

    virtual std::vector<uint8_t> ReleaseDataRadioBearer(uint16_t rnti, uint8_t lcid) = 0;

    /**
     * \brief Add the Signal Bearer for a specific Ue in
     * LteEnbComponenCarrierManager \param lcInfo this structure it is hard-coded
     * in the LteEnbRrc \param rlcMacSapUser it is the MacSapUser of the Rlc
     * instance \return the LteMacSapUser of the ComponentCarrierManager
     *
     */
 ...
 *
 * This is the *Component Carrier Management SAP User*, i.e., the part of the
 * SAP that contains the eNodeB RRC methods called by the CCM.
 */
class LteCcmRrcSapUser
{
  ...
  public:
     ...
     *
     * The process to produce the decision is up to the implementation of
     * ComponentCarrier algorithm. It is typically based on the reported UE
     * measurements, which are received through the
     * LteCcmRrcSapProvider::ReportUeMeas function.
     */
     ...
     *
     * \param lcConfig is a single structure contains logical Channel Id, Logical
