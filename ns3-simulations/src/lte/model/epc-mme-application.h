 ...
 * the MME node and the SGW node. It supports the following functions and
 * messages:
 *
 ...
 *
 * Others functions enumerated in section 4.4.2 of 3GPP TS 23.401 are not
 * supported.
 */
class EpcMmeApplication : public Application
{
  ...
  private:
     ...
     * \param enbUeS1Id the ENB UE S1 ID
     * \param cgi the CGI
     * \param erabToBeSwitchedInDownlinkList the ERAB to be switched in downlink
     * list
     */
    ...
    uint16_t m_gtpcUdpPort;   ///< UDP port for GTP-C protocol. Fixed by the
                              ///< standard to port 2123
};
