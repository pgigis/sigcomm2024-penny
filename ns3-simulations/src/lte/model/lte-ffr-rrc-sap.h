 ...
 * contains the eNodeB RRC methods called by the Frequency Reuse algorithm
 * instance.
 */
class LteFfrRrcSapUser
{
  public:
    ...
    /**
     * \brief Instruct the eNodeB RRC entity to perform
     * RrcConnectionReconfiguration to inform UE about new PdschConfigDedicated
     * (i.e. P_a value). Also Downlink Power Allocation is done based on this
     * value. \param rnti Radio Network Temporary Identity, an integer identifying
     * the UE which shall perform the handover \param pdschConfigDedicated new
     * PdschConfigDedicated to be configured for UE
     *
     * This function is used by the Frequency Reuse algorithm entity when it
     * decides that PDSCH for this UE should be allocated with different transmit
     * power.
     *
     * The process to produce the decision is up to the implementation of
     * Frequency Reuse algorithm. It is typically based on the reported UE
     * measurements, which are received through the
     * LteFfrRrcSapProvider::ReportUeMeas function.
     */
