 ...
 *
 * \sa LteCcmRrcSapUser, LteCcmRrcSapProvider, LteCcmMacSapUser,
...
class LteEnbComponentCarrierManager : public Object
{
  public:
     ...
     * that this ComponentCarrier algorithm instance will interact with. \param s
     * a reference to the "user" part of the interface, typically a member of an
     * LteEnbRrc instance
     */
    virtual void SetLteCcmRrcSapUser(LteCcmRrcSapUser* s);

    /**
     * \brief Export the "provider" part of the ComponentCarrier Management SAP
     * interface. \return the reference to the "provider" part of the interface,
     * typically to be kept by an LteEnbRlc instance
     */
    virtual LteCcmRrcSapProvider* GetLteCcmRrcSapProvider();

    /**
     * \brief This function returns a pointer to the LteCcmMacSapUser interface,
     * which is used by MAC to communicate to CCM when e.g. UL buffer status
     * report is received, or to notify CCM about PRB occupancy, and similar.
     * Functions that are specific for the communication between MAC and CCM.
     *
     ...
     * the specified component carrier. Through this interface CCM communicates
     * with MAC, e.g. it notifies MAC of the specific carrier when to scheduler UL
     * BSR.
     *
     * \param componentCarrierId component carrier ID
     * \param sap the MAC SAP provider
  ...
  protected:
    ...
    struct UeInfo
    {
        ...
        std::map<uint8_t, LteEnbCmacSapProvider::LcInfo>
            m_rlcLcInstantiated;           //!< Logical channel configuration per flow Id
                                           //!< (rnti, lcid).
        uint8_t m_enabledComponentCarrier; //!< The number of enabled component
                                           //!< carriers.
    ...
    uint16_t m_noOfComponentCarriers;    //!< The number component of carriers that
                                         //!< are supported by this eNb.
    // pointer to RRC object for direct function calls, e.g. when CCM needs to
    // obtain a pointer to RLC object of a specific flow
    Ptr<LteEnbRrc> m_rrc; //!< A pointer to the RRC instance of this eNb.

    /*
     * This interface is used to receive API calls from the RLC instance that
     * through LteMacSapProvider interface. The component carrier manager acts a
     * proxy. This means that all RLC instances will see as in previous
     * architecture the LteMacSapProvider interface, but the actual provider in
     * new architecture will be some of child classes of
     * LteEnbComponentCarrierManager. So, LteEnbComponentCarrierManager class will
     * receive function calls that are meant for MAC, and will forward them to the
     * MAC of the component carriers based on the logic implemented in
     * LteComponentCarrierManager. This attribute will be initialized by using
     * class that implements LteMacSapProvider interface and class that implements
     * LteEnbComponentCarrierManager base class e.g.:EnbMacMemberLteMacSapProvider
     * <LteEnbComponentCarrierManagerImpl>
     */
    LteMacSapProvider* m_macSapProvider; //!< A pointer to main SAP interface of the MAC
                                         //!< instance, which is in this case handled by CCM.
    // This map is initialized in LteHelper when the Component Carrier Manager is
    // initialized, contains component carrier id and a pointer to the
    // corresponding LteMacSapProvider interface of the MAC instance
    std::map<uint8_t, LteMacSapProvider*> m_macSapProvidersMap; //!< A map of pointers to real SAP
                                                                //!< interfaces of MAC instances.
    ...
    std::map<uint8_t, LteCcmMacSapProvider*>
        m_ccmMacSapProviderMap;                //!< A map of pointers to the SAP interfaces of
                                               //!< CCM instance that provides the  CCM specific
                                               //!< functionalities to MAC, i.e.
                                               //!< ReportMacCeToScheduler.
    LteCcmMacSapUser* m_ccmMacSapUser;         //!< LteCcmMacSapUser is extended version of
                                               //!< LteMacSapUser interface. Contains functions that
                                               //!< allow reporting of UL BSR from MAC to CCM.
    LteCcmRrcSapUser* m_ccmRrcSapUser;         //!< A pointer to SAP interface of RRC instance, i.e.
                                               //!< to configure measurements reporting for CCM.
    LteCcmRrcSapProvider* m_ccmRrcSapProvider; //!< A pointer to the SAP interface of the CCM
                                               //!< instance to receive API calls from the eNodeB
                                               //!< RRC instance.
