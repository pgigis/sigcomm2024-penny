 ...
 * \ingroup lte
 *
 * This entity is installed inside an eNB and provides the functionality for the
 * X2 interface
 */
class EpcX2 : public Object
{
  ...
  public:
    ...
    /**
     * Method to be assigned to the recv callback of the X2-C (X2 Control Plane)
     * socket. It is called when the eNB receives a packet from the peer eNB of
     * the X2-C interface
     *
    ...
    /**
     * Method to be assigned to the recv callback of the X2-U (X2 User Plane)
     * socket. It is called when the eNB receives a packet from the peer eNB of
     * the X2-U interface
     *
  ...
  private:
    /**
     * Map the targetCellId to the corresponding (sourceSocket, remoteIpAddr) to
     * be used to send the X2 message
     */
    std::map<uint16_t, Ptr<X2IfaceInfo>> m_x2InterfaceSockets;

    /**
     * Map the localSocket (the one receiving the X2 message)
     * to the corresponding (sourceCellId, targetCellId) associated with the X2
     * interface
     */
