 ...
 *  to the UE RRC.
 *
 * This is the *Component Carrier Management SAP Provider*, i.e., the part of
 * the SAP that contains the component carrier manager methods called by the Ue
 * RRC instance.
 */
class LteUeCcmRrcSapProvider
{
  ...
  public:
     ...
     * Channel config and Component Carrier Id \param msu is the pointer to
     * LteMacSapUser related to the Rlc instance \return vector of LcsConfig
     * contains the lc configuration for each Mac the size of the vector is equal
     * to the number of component carrier enabled.
     *
    ...
    /**
     * \brief Remove an existing Logical Channel for a Ue in the
     * LteUeComponentCarrierManager \param lcid the Logical Channel Id \return
     * vector of integer the componentCarrierId of the componentCarrier where the
     * bearer is enabled
     */
     ...
     * LteUeComponenCarrierManager \param lcid the Logical Channel Id \param
     * lcConfig this structure it is hard-coded in the LteEnbRrc \param msu it is
     * the MacSapUser of the Rlc instance \return the LteMacSapUser of the
 ...
 * SAP that contains the UE RRC methods called by the UE CCM instance.
 */
class LteUeCcmRrcSapUser
{
  public:
     ...
     * that a reconfiguration message with Secondary component carrier (SCc)
     * arrived or not the method it is called only if the SCc wasn't set up \param
