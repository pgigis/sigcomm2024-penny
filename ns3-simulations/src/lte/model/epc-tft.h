...
class EpcTft : public SimpleRefCount<EpcTft>
{
  public:
     ...
     *  - IPSec filtering
     *  - filter precedence field is not evaluated, hence it is recommended to
     * setup the TFTs within a PDP context such that TFTs are mutually exclusive
     */
    struct PacketFilter
    {
        PacketFilter();
        ...
        /// Used to specify the precedence for the packet filter among all packet
        /// filters in the TFT; higher values will be evaluated last.
        uint8_t precedence;

        /// Whether the filter needs to be applied to uplink / downlink only, or in
        /// both cases
        Direction direction;
     ...
     *
     * \return the id( 0 <= id < 16) of the newly added filter, if the addition
     * was successful. Will fail if you try to add more than 15 filters. This is
     * to be compliant with TS 24.008.
     */
