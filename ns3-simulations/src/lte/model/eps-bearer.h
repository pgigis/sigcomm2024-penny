 ...
 * release. To change the release, change the attribute "Release". Please
 * remember that we must expose to all releases the most recent Qci. Asking for
 * Qci parameters for a release in which it has not been created will result in
 * a crash.
 *
 ...
 * we do not support intermediate types between releases 11 and 15: in other
 * words, you can select from Release 8 to Release 11, or Release 15 or 18. Any
 * other value will result in a program crash.
 *
...
class EpsBearer : public ObjectBase
{
  public:
    ...
    enum Qci : uint8_t
    {
        ...
        NGBR_VIDEO_TCP_OPERATOR = 6,  ///< Non-GBR TCP-based Video (Buffered
                                      ///< Streaming, e.g., www, e-mail...)
        NGBR_VOICE_VIDEO_GAMING = 7,  ///< Non-GBR Voice, Video, Interactive Streaming
        NGBR_VIDEO_TCP_PREMIUM = 8,   ///< Non-GBR TCP-based Video (Buffered
                                      ///< Streaming, e.g., www, e-mail...)
        NGBR_VIDEO_TCP_DEFAULT = 9,   ///< Non-GBR TCP-based Video (Buffered
                                      ///< Streaming, e.g., www, e-mail...)
        NGBR_MC_DELAY_SIGNAL = 69,    ///< Non-GBR Mission Critical Delay Sensitive
        ...
        DGBR_DISCRETE_AUT_SMALL = 82, ///< Delay-Critical GBR Discrete Automation
                                      ///< Small Packets (TS 22.261)
        DGBR_DISCRETE_AUT_LARGE = 83, ///< Delay-Critical GBR Discrete Automation
                                      ///< Large Packets (TS 22.261)
        DGBR_ITS = 84,           ///< Delay-Critical GBR Intelligent Transport Systems (TS 22.261)
        DGBR_ELECTRICITY = 85,   ///< Delay-Critical GBR Electricity Distribution
                                 ///< High Voltage (TS 22.261)
        DGBR_V2X = 86,           ///< Delay-Critical GBR V2X Messages (TS 23.501)
        DGBR_INTER_SERV_87 = 87, ///< Delay-Critical GBR Interactive Service -
                                 ///< Motion tracking data (TS 23.501)
        DGBR_INTER_SERV_88 = 88, ///< Delay-Critical GBR Interactive Service -
                                 ///< Motion tracking data (TS 23.501)
        DGBR_VISUAL_CONTENT_89 = 89, ///< Delay-Critical GBR Visual Content for
                                     ///< cloud/edge/split rendering (TS 23.501)
        DGBR_VISUAL_CONTENT_90 = 90, ///< Delay-Critical GBR Visual Content for
                                     ///< cloud/edge/split rendering (TS 23.501)
    };
    ...
    /**
     *
     * @return the priority associated with the QCI of this bearer as per
     ...
     *
     *
     * @return the packet delay budget associated with the QCI of this bearer as
     * per 3GPP 23.203 Section 6.1.7.2
     */
     ...
     *
     *
     * @return the packet error loss rate associated with the QCI of this bearer
     * as per 3GPP 23.203 Section 6.1.7.2
     */
  ...
  private:
     ...
     *
     * The tuple is formed by: resource type, priority, packet delay budget,
     * packet error rate, default maximum data burst, default averaging window (0
     * when does not apply)
     */
