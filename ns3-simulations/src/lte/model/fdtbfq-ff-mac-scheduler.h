...
struct fdtbfqsFlowPerf_t
{
    ...
    uint32_t burstCredit;         ///< the maximum number of tokens connection i can
                                  ///< borrow from the bank each time
    int debtLimit;                ///< counter threshold that the flow cannot further borrow
                                  ///< tokens from bank
    uint32_t creditableThreshold; ///< the flow cannot borrow token from bank
                                  ///< until the number of token it has deposited
                                  ///< to bank reaches this threshold
};

/**
 * \ingroup ff-api
 * \brief Implements the SCHED SAP and CSCHED SAP for a Frequency Domain Token
 * Bank Fair Queue scheduler
 *
 * This class implements the interface defined by the FfMacScheduler abstract
 * class
 */
...
class FdTbfqFfMacScheduler : public FfMacScheduler
{
  ...
  private:
    ...
    uint16_t m_nextRntiUl; ///< RNTI of the next user to be served next
                           ///< scheduling in UL

    uint32_t m_cqiTimersThreshold; ///< # of TTIs for which a CQI can be
                                   ///< considered valid
    ...
    // HARQ attributes
    bool m_harqOn; ///< m_harqOn when false inhibit the HARQ mechanisms (by
                   ///< default active)
