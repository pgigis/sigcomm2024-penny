...
RrcAsn1Header::SerializeDrbToAddModList(std::list<LteRrcSap::DrbToAddMod> drbToAddModList) const
{
    ...
    for (; it != drbToAddModList.end(); it++)
    {
        ...
        drbToAddModListOptionalFieldsPresent.set(3,
                                                 false);   // pdcp-Config not present
...
RrcAsn1Header::SerializeSrbToAddModList(std::list<LteRrcSap::SrbToAddMod> srbToAddModList) const
{
    ...
    for (; it != srbToAddModList.end(); it++)
    {
        ...
        srbToAddModListOptionalFieldsPresent.set(1,
                                                 false);   // rlc-Config not present
        ...
        // 2 options, selected option 0 (var "explicitValue", of type
    ...
    LteRrcSap::LogicalChannelConfig logicalChannelConfig) const
{
    ...
    // 1 optional field (ul-SpecificParameters), which is present. Extension
    // marker present.
    SerializeSequence(std::bitset<1>(1), true);

    // Serialize ul-SpecificParameters sequence
    // 1 optional field (logicalChannelGroup), which is present. No extension
    // marker.
    ...
    if (!nonCriticalExtension.sCellToAddModList.empty())
    {
        ...
        for (auto& it : nonCriticalExtension.sCellToAddModList)
        {
            ...
            sCellToAddMod_r10.set(
                0,
                it.haveRadioResourceConfigDedicatedSCell); // No nonCriticalExtension
                                                           // RRC
    ...
    LteRrcSap::PhysicalConfigDedicatedSCell pcdsc) const
{
    ...
    if (pcdsc.haveUlConfiguration)
    {
        ...
        ulOpt.set(2,
                  pcdsc.haveSoundingRsUlConfigDedicated); // soundingRS-UL-ConfigDedicated-r10
        ulOpt.set(1, false); // soundingRS-UL-ConfigDedicated-v1020 not present
        ulOpt.set(0,
                  false); // soundingRS-UL-ConfigDedicatedAperiodic-r10 not present
    ...
    Buffer::Iterator bIterator)
{
    ...
    for (int i = 0; i < numSchedulingInfo; i++)
    {
        ...
        int numSibType;
        bIterator = DeserializeSequenceOf(&numSibType,
                                          MAX_SIB - 1,
                                          0,
                                          bIterator); // sib-MappingInfo
                                      ...
                                      Buffer::Iterator bIterator)
{
    ...
    //    bIterator = DeserializeSequence
    //    (&measResultNeighCellsPresent,true,bIterator);
    ...
    else if (criticalExtensionChoice == 0)
    {
        ...
        else if (c1 == 0)
        {
            ...
            if (bitset1[0])
            {
                ...
                // Deserialization of lateR8NonCriticalExtension and
...
RrcConnectionReconfigurationHeader::PreSerialize() const
{
    ...
    options.set(0, m_haveNonCriticalExtension); // Implemented nonCriticalExtension
                                                // because compatibility with R10 - CA
    ...
    {
        // Serialize NonCriticalExtension RRCConnectionReconfiguration-v890-IEs
        // sequence: 2 optional fields. Extension marker not present.
        std::bitset<2> noncriticalExtension_v890;
        noncriticalExtension_v890.set(1, false); // No lateNonCriticalExtension
        noncriticalExtension_v890.set(
            0,
            m_haveNonCriticalExtension); // Implemented nonCriticalExtension because
                                         // compatibility with R10 - CA
        // Enable RRCCoonectionReconfiguration-v920-IEs
        SerializeSequence(noncriticalExtension_v890, false);

        // Serialize NonCriticalExtension RRCConnectionReconfiguration-v920-IEs
        // sequence: 3 optional fields. Extension marker not present.
        std::bitset<3> noncriticalExtension_v920;
        ...
        SerializeNonCriticalExtensionConfiguration(
            m_nonCriticalExtension); // Serializing
