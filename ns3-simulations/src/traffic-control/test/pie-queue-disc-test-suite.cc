...
class PieQueueDiscTestItem : public QueueDiscItem
{
  public:
    ...
    double m_maxDropProbDiff = 0.0; //!< Maximum difference between two
                                    //!< consecutive drop probability values
...
PieQueueDiscTestCase::RunPieTest(QueueSizeUnit mode)
{
    ...
    queue->Initialize();
    EnqueueWithDelay(queue, pktSize, 400, testAttributes);
    DequeueWithDelay(queue, 0.015,
                     400); // delay between two successive dequeue events is increased
    Simulator::Stop(Seconds(8.0));
    Simulator::Run();
    st = queue->GetStats();
    ...
    NS_TEST_ASSERT_MSG_EQ(st.GetNDroppedPackets(PieQueueDisc::FORCED_DROP),
                          0,
                          "There should be zero forced drops");
    NS_TEST_ASSERT_MSG_LT(testAttributes->m_maxDropProbDiff,
                          0.0200000000000001,
    ...
    // There are unforced drops because the value of m_maxDropProb goes beyond 0.3
    // in this test. PIE drops the packets even when they are ECN capable if drop
    // probability is more than 30%.
    NS_TEST_ASSERT_MSG_NE(st.GetNDroppedPackets(PieQueueDisc::UNFORCED_DROP),
                          0,
                          "There should be some unforced drops");
    ...
    // test 13: same as test 11 but with accumulated drop probability set below
    // the low threshold
    queue = CreateObject<PieQueueDisc>();
    // PieQueueDiscItem pointer for attributes
    testAttributes = Create<PieQueueDiscTestItem>(Create<Packet>(pktSize), dest, false);
    ...
    // Final value of accumulated drop probability to drop packet will be maximum
    // 0.84 while threshold to drop packet is 0.85
    ...
    // test 14: same as test 12 but with accumulated drop probability set above
    // the high threshold
    queue = CreateObject<PieQueueDisc>();
    // PieQueueDiscItem pointer for attributes
    testAttributes = Create<PieQueueDiscTestItem>(Create<Packet>(pktSize), dest, false);
    ...
    // Final value of accumulated drop probability to drop packet will be
    // minimum 8.6 while threshold to drop packet is 8.5
    ...
    // test 15: tests Active/Inactive feature, ActiveThreshold set to a high value
    // so PIE never starts and there should not be any drops
    queue = CreateObject<PieQueueDisc>();
    // PieQueueDiscItem pointer for attributes
    testAttributes = Create<PieQueueDiscTestItem>(Create<Packet>(pktSize), dest, false);
    queue->SetAttributeFailSafe("MaxSize", QueueSizeValue(QueueSize(mode, qSize)));
    ...
    // test 16: tests Active/Inactive feature, ActiveThreshold set to a low value
    // so PIE starts early and some packets should be dropped.
    queue = CreateObject<PieQueueDisc>();
    // PieQueueDiscItem pointer for attributes
    testAttributes = Create<PieQueueDiscTestItem>(Create<Packet>(pktSize), dest, false);
    queue->SetAttributeFailSafe("MaxSize", QueueSizeValue(QueueSize(mode, qSize)));
