...
CoDelQueueDisc::OkToDrop(Ptr<QueueDiscItem> item, uint32_t now)
{
    ...
    {
        NS_LOG_LOGIC("Sojourn time has been above target for at least q->interval; "
                     "it's OK to "
...
CoDelQueueDisc::DoDequeue()
{
    ...
    if (m_dropping)
    { // In the dropping state (sojourn time has gone above target
      // and hasn't come down yet)
        ...
        else if (CoDelTimeAfterEq(now, m_dropNext))
        {
            while (m_dropping && CoDelTimeAfterEq(now, m_dropNext))
            {
                ...
                NS_LOG_LOGIC("Sojourn time is still above target and it's time for "
                             "next drop; dropping "
                             << item);
