...
class PieQueueDisc : public QueueDisc
{
  ...
  private:
    ...
    double m_markEcnTh;         //!< ECN marking threshold (default 10% as suggested in
                                //!< RFC 8033)
    Time m_activeThreshold;     //!< Threshold for activating PIE (disabled by
                                //!< default)
    ...
    Time m_burstAllowance;           //!< Current max burst value in seconds that is
                                     //!< allowed before random drops kick in
    ...
    uint64_t m_dqCount;              //!< Number of bytes departed since current measurement
                                     //!< cycle starts
    EventId m_rtrsEvent;             //!< Event used to decide the decision of interval of
