...
class FqPieQueueDisc : public QueueDisc
{
  public:
     ...
     *
     * \param quantum The number of bytes each queue gets to dequeue on each round
     ...
     *
     * \returns The number of bytes each queue gets to dequeue on each round of
  ...
  private:
    ...
    /**
     * \brief Drop a packet from the head of the queue with the largest current
     * byte count \return the index of the queue with the largest current byte
     * count
     */
    uint32_t FqPieDrop();
    ...
    bool m_useEcn;              //!< True if ECN is used (packets are marked instead of being
                                //!< dropped)
    double m_markEcnTh;         //!< ECN marking threshold (default 10% as suggested in
                                //!< RFC 8033)
    Time m_ceThreshold;         //!< Threshold above which to CE mark
    bool m_useL4s;              //!< True if L4S is used (ECT1 packets are marked at CE
                                //!< threshold)
    ...
    Time m_maxBurst;            //!< Maximum burst allowed before random early dropping
                                //!< kicks in
    double m_a;                 //!< Parameter to pie controller
    double m_b;                 //!< Parameter to pie controller
    uint32_t m_dqThreshold;     //!< Minimum queue size in bytes before dequeue rate
                                //!< is measured
    bool m_useDqRateEstimator;  //!< Enable/Disable usage of dequeue rate
                                //!< estimator for queue delay calculation
    bool m_isCapDropAdjustment; //!< Enable/Disable Cap Drop Adjustment feature
                                //!< mentioned in RFC 8033
    bool m_useDerandomization;  //!< Enable Derandomization feature mentioned in
                                //!< RFC 8033
