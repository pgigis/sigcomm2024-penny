...
/**
 * The Traffic Control layer aims at introducing an equivalent of the Linux
 Traffic
 ...
 * In simple words, we use Callbacks to connect upper layer (which should
 register
 * their Receive callback through RegisterProtocolHandler) and NetDevices.
 *
 * An example of the IN connection between this layer and IP layer is the
 following:
 *\verbatim
  ...
  m_node->RegisterProtocolHandler (MakeCallback (&TrafficControlLayer::Receive,
 tc), Ipv4L3Protocol::PROT_NUMBER, device); m_node->RegisterProtocolHandler
 (MakeCallback (&TrafficControlLayer::Receive, tc), ArpL3Protocol::PROT_NUMBER,
 device);
 ...
 PeekPointer (GetObject<ArpL3Protocol> ())), ArpL3Protocol::PROT_NUMBER,
 device); \endverbatim
 * On the node, for IPv4 and ARP packet, is registered the
 * TrafficControlLayer::Receive callback. At the same time, on the
 TrafficControlLayer
 * object, is registered the callbacks associated to the upper layers (IPv4 or
 ARP).
 *
 ...
 *
 * Discrimination through callbacks (in other words: what is the right
 upper-layer
...
class TrafficControlLayer : public Object
{
  public:
    ...
    /**
     * \brief This method can be used to set the root queue disc installed on a
     * device
     *
    ...
    /**
     * \brief This method can be used to get the root queue disc installed on a
     * device
     *
     * \param device the device on which the requested root queue disc is
     * installed \return the root queue disc installed on the given device
     */
    virtual Ptr<QueueDisc> GetRootQueueDiscOnDevice(Ptr<NetDevice> device) const;

    /**
     * \brief This method can be used to remove the root queue disc (and
     * associated filters, classes and queues) installed on a device
     *
  ...
  private:
    ...
    Ptr<Node> m_node;
    /// Map storing the required information for each device with a queue disc
    /// installed
    std::map<Ptr<NetDevice>, NetDeviceInfo> m_netDevices;
    ProtocolHandlerList m_handlers; //!< List of upper-layer handlers

    /**
     * The trace source fired when the Traffic Control layer drops a packet
     * because no queue disc is installed on the device, the device supports flow
     * control and the device queue is stopped
     */
