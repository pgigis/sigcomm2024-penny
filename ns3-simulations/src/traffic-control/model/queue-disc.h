...
enum QueueDiscSizePolicy
{
    ...
    SINGLE_CHILD_QUEUE_DISC, //!< Used by queue discs with single child queue
                             //!< disc
    MULTIPLE_QUEUES,         //!< Used by queue discs with multiple internal queues/child
                             //!< queue discs
 ...
 * filter is only able to classify packets of a single protocol. Also, while in
 * Linux some queue discs (e.g., fq-codel) use an internal classifier and do not
 * make use of packet filters, in ns-3 every queue disc including multiple
 * queues or multiple classes needs an external filter to classify packets (this
 * is to avoid having the traffic-control module depend on other modules such as
 * internet).
 *
 * Queue disc configuration vary from queue disc to queue disc. A typical
 * taxonomy divides queue discs in classful (i.e., support classes) and
 * classless (i.e., do not support classes). More recently, after the appearance
 * of multi-queue devices (such as Wifi), some multi-queue aware queue discs
 * have been introduced. Multi-queue aware queue discs handle as many queues (or
 * queue discs -- without using classes) as the number of transmission queues
 * used by the device on which the queue disc is installed. An attempt is made,
 * also, to enqueue each packet in the "same" queue both within the queue disc
 * and within the device.
 *
 * The traffic control layer interacts with a queue disc in a simple manner:
 * after requesting to enqueue a packet, the traffic control layer requests the
 * qdisc to "run", i.e., to dequeue a set of packets, until a predefined number
 * ("quota") of packets is dequeued or the netdevice stops the queue disc. A
 * netdevice shall stop the queue disc when its transmission queue does not have
 * room for another packet. Also, a netdevice shall wake the queue disc when it
 * detects that there is room for another packet in its transmission queue, but
 * the transmission queue is stopped. Waking a queue disc is equivalent to make
 * it run.
 *
 * Every queue disc collects statistics about the total number of packets/bytes
 * received from the upper layers (in case of root queue disc) or from the
 * parent queue disc (in case of child queue disc), enqueued, dequeued,
 * requeued, dropped, dropped before enqueue, dropped after dequeue, queued in
 * the queue disc and sent to the netdevice or to the parent queue disc. Note
 * that packets that are dequeued may be requeued, i.e., retained by the traffic
 * control infrastructure, if the netdevice is not ready to receive them.
 * Requeued packets are not part of the queue disc. The following identities
 * hold:
 ...
 * When a packet is dropped by an internal queue, e.g., because the queue is
 * full, the reason is "Dropped by internal queue". When a packet is dropped by
 * a child queue disc, the reason is "(Dropped by child queue disc) " followed
 * by the reason why the child queue disc dropped the packet.
 *
 * The QueueDisc base class provides the SojournTime trace source, which
 * provides the sojourn time of every packet dequeued from a queue disc,
 * including packets that are dropped or requeued after being dequeued. The
 * sojourn time is taken when the packet is dequeued from the queue disc, hence
 * it does not account for the additional time the packet is retained within the
 * traffic control infrastructure in case it is requeued.
 *
...
class QueueDisc : public Object
{
  public:
    ...
    struct Stats
    {
        ...
        /// Total sent packets -- this value is not kept up to date, call GetStats
        /// first
        uint32_t nTotalSentPackets;
        /// Total sent bytes -- this value is not kept up to date, call GetStats
        /// first
    ...
    /// Callback invoked to send a packet to the receiving object when Run is
    /// called
    ...
    /**
     * \brief Set the maximum number of dequeue operations following a packet
     * enqueue \param quota the maximum number of dequeue operations following a
     * packet enqueue.
     */
    virtual void SetQuota(const uint32_t quota);

    /**
     * \brief Get the maximum number of dequeue operations following a packet
     * enqueue \return the maximum number of dequeue operations following a packet
     * enqueue.
     */
    ...
    /**
     * \brief Add a packet filter to the tail of the list of filters used to
     * classify packets. \param filter the packet filter to be added
     */
    ...
    /**
     * Classify a packet by calling the packet filters, one at a time, until
     * either a filter able to classify the packet is found or all the filters
     * have been processed. \param item item to classify \return -1 if no filter
     * able to classify the packet has been found, the value returned by first
     * filter found to be able to classify the packet otherwise.
     */
    ...
    /**
     * When setting up the wake callbacks on the netdevice queues, it is necessary
     * to determine which queue disc (the root queue disc or one of its children)
     * should be activated when the netdevice wakes one of its transmission
     * queues. The implementation of this method for the base class returns
     * WAKE_ROOT, i.e., the root queue disc is activated. Subclasses implementing
     * queue discs adopting a different strategy (e.g., multi-queue aware queue
     * discs such as mq) have to redefine this method.
     *
    ...
    static constexpr const char* CHILD_QUEUE_DISC_DROP =
        "(Dropped by child queue disc) "; //!< Packet dropped by a child queue
                                          //!< disc
  ...
  private:
    ...
    /**
     * Modelled after the Linux function qdisc_run_begin
     * (include/net/sch_generic.h). \return false if the qdisc is already running;
     * otherwise, set the qdisc as running and return true.
     */
    bool RunBegin();

    /**
     * Modelled after the Linux function qdisc_run_end
     * (include/net/sch_generic.h). Set the qdisc as not running.
     */
    void RunEnd();

    /**
     * Modelled after the Linux function qdisc_restart (net/sched/sch_generic.c)
     * Dequeue a packet (by calling DequeuePacket) and send it to the device (by
     * calling Transmit). \return true if a packet is successfully sent to the
     * device.
     */
    bool Restart();
     ...
     * \return the requeued packet, if any, or the packet dequeued by the queue
     * disc, otherwise.
     */
     ...
     * Sends a packet to the device if the device queue is not stopped, and
     * requeues it otherwise. \param item the packet to transmit \return true if
     * the device queue is not stopped and the queue disc is not empty
     */
    ...
    std::string m_childQueueDiscDropMsg; //!< Reason why a packet was dropped by
                                         //!< a child queue disc
    std::string m_childQueueDiscMarkMsg; //!< Reason why a packet was marked by a
                                         //!< child queue disc
    ...
    /// Type for the function objects notifying that a packet has been dropped by
    /// an internal queue
    ...
    /// Type for the function objects notifying that a packet has been marked by a
    /// child queue disc
    typedef std::function<void(Ptr<const QueueDiscItem>, const char*)> ChildQueueDiscMarkFunctor;

    /// Function object called when an internal queue dropped a packet before
    /// enqueue
    InternalQueueDropFunctor m_internalQueueDbeFunctor;
    /// Function object called when an internal queue dropped a packet after
    /// dequeue
    InternalQueueDropFunctor m_internalQueueDadFunctor;
    /// Function object called when a child queue disc dropped a packet before
    /// enqueue
    ChildQueueDiscDropFunctor m_childQueueDiscDbeFunctor;
    /// Function object called when a child queue disc dropped a packet after
    /// dequeue
