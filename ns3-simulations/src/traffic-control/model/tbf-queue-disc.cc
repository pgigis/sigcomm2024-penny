...
TbfQueueDisc::CheckConfig()
{
    ...
    if (GetNQueueDiscClasses() == 0)
    {
        ...
        {
            NS_LOG_ERROR("Cannot set the max size of the child queue disc to that of "
                         "TbfQueueDisc");
            return false;
        }
        qd->Initialize();
    ...
    {
        NS_LOG_ERROR("A non-null peak rate has been set, but the mtu is null. No "
                     "packet will be dequeued");
        return false;
    }
