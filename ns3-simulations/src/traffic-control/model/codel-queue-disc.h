...
class CoDelQueueDisc : public QueueDisc
{
  ...
  private:
     ...
     * \returns True if the packet can be added, False if the packet is dropped
     * due to full queue
     */
     ...
     * If we are not currently in dropping state, check if we need to enter the
     * state and drop the first packet
     *
    ...
    /**
     * \brief Calculate the reciprocal square root of m_count by using Newton's
     * method
     *  http://en.wikipedia.org/wiki/Methods_of_computing_square_roots#Iterative_methods_for_reciprocal_square_roots
     * m_recInvSqrt (new) = (m_recInvSqrt (old) / 2) * (3 - m_count *
     * m_recInvSqrt^2) \param recInvSqrt reciprocal value of sqrt (count) \param
     * count count value \return The new recInvSqrt value
     */
     ...
     * Here, we use m_recInvSqrt calculated by Newton's method in NewtonStep() to
     * avoid both sqrt() and divide operations
     *
     ...
     * \returns True if it is OK to drop the packet (sojourn time above target for
     * at least interval)
     */
    ...
    TracedValue<uint32_t> m_lastCount; //!< Last number of packets dropped since
                                       //!< entering drop state
