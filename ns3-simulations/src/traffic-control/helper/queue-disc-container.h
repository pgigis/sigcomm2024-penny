 ...
 *
 * Typically ns-3 QueueDiscs are installed on net devices using a traffic
 * control helper. The helper Install method takes a NetDeviceContainer which
 * holds some number of Ptr<NetDevice>. For each of the net devices in the
...
class QueueDiscContainer
{
  public:
     ...
     * \endcode
     *
     * \returns a const iterator which refers to the first QueueDisc in the
     * container.
     */
