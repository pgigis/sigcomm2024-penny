...
class QueueDiscFactory
{
  ...
  private:
    ...
    /// Map storing the associations between class IDs and child queue disc
    /// handles
...
class TrafficControlHelper
{
  public:
     ...
     *
     * The default configuration is an FqCoDelQueueDisc, if the device has a
     * single queue, or an MqQueueDisc with as many FqCoDelQueueDiscs as the
     * number of device queues, otherwise.
     */
    static TrafficControlHelper Default(std::size_t nTxQueues = 1);

    /**
     * Helper function used to set a root queue disc of the given type and with
     * the given attributes. To set the InternalQueueList, PacketFilterList and
     * ChildQueueDiscList attributes, use the AddInternalQueue, AddPacketFilter
     * and AddChildQueueDisc methods.
     *
     * \tparam Args \deduced Template type parameter pack for the sequence of
     * name-value pairs. \param type the type of queue disc \param args A sequence
     * of name-value pairs of the attributes to set. \return the handle of the
     * root queue disc (zero)
     */
    template <typename... Args>
    uint16_t SetRootQueueDisc(const std::string& type, Args&&... args);

    /**
     * Helper function used to add the given number of internal queues (of the
     * given type and with the given attributes) to the queue disc having the
     * given handle.
     *
     * \tparam Args \deduced Template type parameter pack for the sequence of
     * name-value pairs. \param handle the handle of the parent queue disc \param
     * count the number of queues to add \param type the type of queue \param args
     * A sequence of name-value pairs of the attributes to set.
     */
    template <typename... Args>
     ...
     * \tparam Args \deduced Template type parameter pack for the sequence of
     * name-value pairs. \param handle the handle of the parent queue disc \param
     * type the type of packet filter \param args A sequence of name-value pairs
     * of the attributes to set.
     */
    template <typename... Args>
    ...
    /**
     * Helper function used to add the given number of queue disc classes (of the
     * given type and with the given attributes) to the queue disc having the
     * given handle.
     *
     * \tparam Args \deduced Template type parameter pack for the sequence of
     * name-value pairs. \param handle the handle of the parent queue disc \param
     * count the number of queue disc classes to add \param type the type of queue
     * disc class \param args A sequence of name-value pairs of the attributes to
     * set. \return the list of class IDs
     */
    template <typename... Args>
    ...
    /**
     * Helper function used to attach a child queue disc (of the given type and
     * with the given attributes) to a given class (included in the queue disc
     * having the given handle).
     *
     * \tparam Args \deduced Template type parameter pack for the sequence of
     * name-value pairs. \param handle the handle of the parent queue disc \param
     * classId the class ID of the class to attach the queue disc to \param type
     * the type of queue disc \param args A sequence of name-value pairs of the
     * attributes to set. \return the handle of the created child queue disc
     */
    template <typename... Args>
    ...
    /**
     * Helper function used to attach a child queue disc (of the given type and
     * with the given attributes) to each of the given classes (included in the
     * queue disc having the given handle).
     *
     * \tparam Args \deduced Template type parameter pack for the sequence of
     * name-value pairs. \param handle the handle of the parent queue disc \param
     * classes the class IDs of the classes to attach a queue disc to \param type
     * the type of queue disc \param args A sequence of name-value pairs of the
     * attributes to set. \return the list of handles of the created child queue
     * discs
     */
    template <typename... Args>
     ...
     *
     * \tparam Args \deduced Template type parameter pack for the sequence of
     * name-value pairs. \param type the type of queue \param args A sequence of
     * name-value pairs of the attributes to set.
     */
    template <typename... Args>
     ...
     * \returns a QueueDisc container with the root queue discs installed on the
     * devices
     *
     ...
     * class and installs them on each device in the given container.
     * Additionally, if configured, a queue limits object is installed on each
    ...
    /**
     * \param d device
     * \returns a QueueDisc container with the root queue disc installed on the
     * device
     *
     * This method creates the queue discs (along with their packet filters,
     * internal queues, classes) configured with the methods provided by this
     * class and installs them on the given device. Additionally, if configured,
     * a queue limits object is installed on each transmission queue of the
     * device.
     */
