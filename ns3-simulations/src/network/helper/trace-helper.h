...
class PcapHelper
{
  public:
    /**
     * This enumeration holds the data link types that will be written to the pcap
     * file.
     *
     * We don't include pcap-bpf.h to avoid an explicit dependency on the real
     * pcap and we don't make an enumeration of all of the values to make it easy
     * to pass new values in.
     *
     ...
     * @param tzCorrection time zone correction to be applied to timestamps of
     * packets
  ...
  private:
    ...
    /**
     * This trace sink passes a header separately from the packet to prevent
     * creating a new packet (for performance reasons)
     *
     * @param file the file to write to
...
class AsciiTraceHelper
{
  public:
     ...
     *
     * The tempting ns-3 idiom when tracing to a file is to create a bound
     * callback with an ofstream as the bound object.  Unfortunately, this implies
     * a copy construction in order to get the ofstream object into the callback.
     * This operation, as mentioned above, is forbidden by the STL.  You could use
     * a global ostream and pass a pointer to it, but that is pretty ugly.  You
...
class PcapHelperForDevice
{
  public:
    ...
    /**
     * @brief Enable pcap output the indicated net device using a device
     * previously named using the ns-3 object name service.
     *
     * @param prefix filename prefix to use for pcap files.
     * @param ndName The name of the net device in which you want to enable
     * tracing.
     * @param promiscuous If true capture all possible packets available at the
     * device.
     * @param explicitFilename Treat the prefix as an explicit filename if true
 ...
 * helpers representing net devices.
 */
class AsciiTraceHelperForDevice
{
  public:
     ...
     *               ascii tracing
     * @param deviceid The device identifier/index of the device on which to
