...
#define PACKET_METADATA_DATA_M_DATA_SIZE 8
    ...
    struct Data
    {
        ...
        uint16_t m_size;
        /** max of the m_used field over all objects which reference this struct
         * Data instance */
        uint16_t m_dirtyEnd;
     ...
     * \param item pointer to where we should store the data to return to the
     * caller \param extraItem pointer to where we should store the data to return
     * to the caller \returns the number of bytes read.
     */
