...
class Socket : public Object
{
  public:
    ...
    /**
     * \brief Ask if the socket is currently passing information about IPv6
     * Traffic Class up the stack
     *
    ...
    /**
     * \brief Ask if the socket is currently passing information about IP_TTL up
     * the stack
     *
     ...
     *
     * Mind that a socket can join only one multicast group. Any attempt to join
     ...
     * \param sourceAddresses All the source addresses on which socket is
     ...
     *
     * A socket can join only one multicast group. Any attempt to join another
     * group will remove the old one.
     *
  ...
  protected:
    ...
    /**
     * \brief Notify through the callback (if set) that the connection has not
     * been established due to an error.
     */
    ...
    Ptr<NetDevice> m_boundnetdevice;         //!< the device this socket is bound to
                                             //!< (might be null).
    bool m_recvPktInfo;                      //!< if the socket should add packet info tags to the
                                             //!< packet forwarded to L4.
