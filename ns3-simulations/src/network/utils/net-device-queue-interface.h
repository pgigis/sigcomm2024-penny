...
class NetDeviceQueue : public Object
{
  public:
     ...
     * This is the analogous to the netif_tx_start_queue function of the Linux
     * kernel.
     */
    virtual void Start();
     ...
     * This is the analogous to the netif_tx_stop_queue function of the Linux
     * kernel.
     */
    virtual void Stop();
     ...
     * This is the analogous to the netif_tx_wake_queue function of the Linux
     * kernel.
     */
    virtual void Wake();
     ...
     *
     * Called by queue discs to enquire about the status of a given transmission
     * queue. This is the analogous to the netif_xmit_stopped function of the
     * Linux kernel.
     */
     ...
     *
     * This NetDeviceQueue stores a pointer to the NetDevice the
     * NetDeviceQueueInterface was aggregated to.
     */
     ...
     *
     * Called by the traffic control layer to set the wake callback. The wake
     * callback is invoked by the device whenever it is needed to "wake" the upper
     * layers (i.e., solicitate the queue disc associated with this transmission
     * queue (in case of multi-queue aware queue discs) or to the network device
     * (otherwise) to send packets down to the device).
     */
    virtual void SetWakeCallback(WakeCallback cb);

    /**
     * \brief Called by the netdevice to report the number of bytes queued to the
     * device queue \param bytes number of bytes queued to the device queue
     */
    virtual void NotifyQueuedBytes(uint32_t bytes);

    /**
     * \brief Called by the netdevice to report the number of bytes it is going to
     * transmit \param bytes number of bytes the device is going to transmit
     */
    ...
    /**
     * \brief Connect the traced callbacks of a queue to the methods providing
     * support for flow control and dynamic queue limits. A queue can be any
     * object providing:
  ...
  private:
    ...
    bool m_stoppedByQueueLimits;    //!< True if the queue has been stopped by a
                                    //!< queue limits object
...
NetDeviceQueue::PacketDiscarded(QueueType* queue, Ptr<const typename QueueType::ItemType> item)
{
    ...
    // This method is called when a packet is discarded before being enqueued in
    // the device queue, likely because the queue is full. This should not happen
    // if the device correctly stops the queue. Anyway, stop the tx queue, so that
