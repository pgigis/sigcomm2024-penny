...
class PacketSocket : public Socket
{
  ...
  private:
    ...
    /**
     * \brief Get the minimum MTU supported by the NetDevices bound to a specific
     * address \param ad the socket address to check for \returns The minimum MTU
     */
...
/**
 * \brief  This class implements a tag that carries the dest address of a packet
 * and the packet type.
 */
...
/**
 * \brief  This class implements a tag that carries the ns3 device name from
 * where a packet is coming.
 */
