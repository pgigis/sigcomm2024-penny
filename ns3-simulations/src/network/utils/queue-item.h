...
class QueueItem : public SimpleRefCount<QueueItem>
{
  public:
    ...
    /**
     * \brief Use this method (instead of GetPacket ()->GetSize ()) to get the
     * packet size
     *
...
class QueueDiscItem : public QueueItem
{
  public:
     ...
     *
     * Subclasses may keep header and payload separate to allow manipulating the
     * header, so this method allows to add the header to the packet before
     * sending the packet to the device.
     */
    ...
    /**
     * \brief Marks the packet as a substitute for dropping it, such as for
