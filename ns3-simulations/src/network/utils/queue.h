 ...
 *
 * This class defines the subset of the base APIs for packet queues in the ns-3
 * system that is independent of the type of enqueued objects
 */
class QueueBase : public Object
{
  public:
     ...
     * Note: the check is performed according to the queue's operating mode (bytes
     * or packets). \param nPackets number of additional packets \param nBytes
  ...
  protected:
    ...
    uint32_t m_nTotalDroppedBytesBeforeEnqueue;   //!< Total dropped bytes before
                                                  //!< enqueue
    ...
    uint32_t m_nTotalDroppedPacketsBeforeEnqueue; //!< Total dropped packets
                                                  //!< before enqueue
    uint32_t m_nTotalDroppedPacketsAfterDequeue;  //!< Total dropped packets after
                                                  //!< dequeue
 ...
 *
 * This class defines the subset of the base APIs for packet queues in the ns-3
 * system that is dependent on the type of enqueued objects.
 *
 ...
 * container used internally to store queue items. The container type must
 * provide the methods insert(), erase() and clear() and define the iterator and
 * const_iterator types, following the usual syntax of C++ containers. The
 * default container type is std::list (as defined in queue-fwd.h). In case the
 * container is such that an object stored within the queue is obtained from a
 * container element through an operation other than dereferencing an iterator
 * pointing to the container element, the container has to provide a public
 * method named GetItem that returns the object stored within the queue that is
 * included in the container element pointed to by a given const iterator.
 *
 * Users of the Queue template class usually hold a queue through a smart
 * pointer, hence forward declaration is recommended to avoid pulling the
 * implementation of the templates included in this file. Thus, include
 * queue-fwd.h, which provides a forward declaration for the Queue class that
 * defines the default value for the template template parameter, instead of
 * queue.h in your .h file. Then, include queue.h in the corresponding .cc file.
 *
...
class Queue : public QueueBase
{
  ...
  protected:
     ...
     * \param item item that was dropped
     *
     * This method is called by the base class when a Remove operation is
     * requested and by the subclasses to notify parent (this class) that a packet
     * has been dropped for other reasons after being dequeued.
     */
  ...
  private:
    /**
     * Struct providing a static method returning the object stored within the
     * queue that is included in the container element pointed to by the given
     * const iterator. This method is used when the container does not define a
     * GetItem method and assumes that an object stored in the queue can be
     * obtained by dereferencing the iterator pointing to the container element
     * that includes such an object.
     */
     ...
     * Struct providing a static method returning the object stored within the
     * queue that is included in the container element pointed to by the given
     * const iterator. This method is used when the container defines a GetItem
     * method and is simply a wrapper to invoke such a method.
     */
    template <class T>
    struct MakeGetItem<
        T,
...
// NS_OBJECT_TEMPLATE_CLASS_DEFINE (Queue,QueueDiscItem), which are included in
// queue.cc
