...
/**
 * \ingroup channel
 * \brief A Error channel, introducing deterministic delays on even/odd packets.
 * Used for testing
 */
class ErrorChannel : public SimpleChannel
{
  public:
    ...
    /**
     * \brief Set the delay for the odd duplicate packets (even ones are not
     * duplicated) \param delay Delay for the odd packets.
     */
    void SetDuplicateTime(Time delay);

    /**
     * \brief Set if the odd packets are duplicated (even ones are not duplicated
     * ever) \param mode true if the odd packets should be duplicated.
     */
