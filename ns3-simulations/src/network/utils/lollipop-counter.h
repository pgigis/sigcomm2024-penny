 ...
 *
 * \brief Template class implementing a Lollipop counter as defined in
 * \RFC{8505}, \RFC{6550}, and [Perlman83].
 *
 ...
 *
 * The counter is split in two regions, an initializing region, and a circular
 * region, having the same size. Assuming a counter using an uint8_t (max value
 * 255), values from 128 and greater are used as a linear sequence to indicate a
 * restart and bootstrap the counter, and the values less than or equal to 127
 * are used as a circular sequence number space of size 128 as mentioned in
 * \RFC{1982}.
 *
 * In both regions, the comparison between two counters is allowed only if both
 * counters are inside a Sequence Window. The default value for the Sequence
 * Window is equal to 2^N where N is half the number of digits of the underlying
 * type. For an uint8_t the Sequence Window is 16.
 *
 * The counter, by default, is initialized to the maximum counter value minus
 * the Sequence Window plus one, e.g., in case of a uint8_t, to 240.
 *
 ...
 *
 * Warning: two Lollipop counters can be compared only if they are of the same
 * type (same underlying type, and same Sequence Window).
 *
 * References:
 * [Perlman83] Perlman, R., "Fault-Tolerant Broadcast of Routing Information",
 * North-Holland Computer Networks 7: pp. 395-405,
 * DOI 10.1016/0376-5075(83)90034-X, 1983,
...
class LollipopCounter
{
  public:
     ...
     * The initial value is set to the maximum counter value minus the Sequence
     * Window plus one.
     */
    LollipopCounter()
    {
    ...
    /**
     * Checks if the counter is comparable with another counter (i.e., not
     * desynchronized).
     *
    ...
    friend bool operator>(const LollipopCounter& lhs, const LollipopCounter& rhs)
    {
        ...
        // One counter is in the "high" region and the other is in the in the
        // "lower" region
        bool lhsIsHigher;
        T difference;
        ...
        T distance = (m_maxValue - difference) + 1; // this is guaranteed to be positive and
                                                    // between [1...m_lollipopMaxValue].
     ...
     * \param [in] val LollipopCounter to be incremented
     * \param [in] noop ignored argument (used to mark it as a postfix, blame
     * c++). \return The result of the Postfix increment.
     */
  ...
  private:
    ...
    {
        // useless because it is computed always on counters on their respective
        // regions. Left (commented) for debugging purposes in case there is a code
        // change. NS_ASSERT_MSG ((m_value <= m_circularRegion && val.m_value <=
        // m_circularRegion) ||
        //                (m_value > m_circularRegion && val.m_value >
        //                m_circularRegion), "Absolute Magnitude Of Difference can
        //                be computed only on two values in the circular region " <<
        //                +m_value << " - " << +val.m_value);
