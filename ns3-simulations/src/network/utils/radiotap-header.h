...
class RadiotapHeader : public Header
{
  public:
     ...
     * its output as it wishes, it is recommended to follow a few rules to
     * integrate with the packet pretty printer: start with flags, small field
    ...
    enum FrameFlag
    {
        ...
        FRAME_FLAG_DATA_PADDING = 0x20,   /**< Frame has padding between 802.11 header
                                             and payload (to 32-bit boundary) */
    ...
    enum McsKnown
    {
        ...
        MCS_KNOWN_NESS_BIT_1 = 0x80,     /**< Ness data - bit 1 (MSB) of Number of
    ...
    enum McsFlags
    {
        MCS_FLAGS_NONE = 0x00,           /**< Default: 20 MHz, long guard interval, mixed HT
                                            format and BCC FEC type */
    ...
    enum AmpduFlags
    {
        ...
        A_MPDU_STATUS_LAST_KNOWN = 0x04,          /**< Last subframe is known (should be set
                                                     for all subframes in an A-MPDU) */
        A_MPDU_STATUS_LAST = 0x08,                /**< This frame is the last subframe */
        A_MPDU_STATUS_DELIMITER_CRC_ERROR = 0x10, /**< Delimiter CRC error */
        A_MPDU_STATUS_DELIMITER_CRC_KNOWN = 0x20  /**< Delimiter CRC value known: the delimiter CRC
     ...
     *
     * @param referenceNumber The A-MPDU reference number to identify all
     * subframes belonging to the same A-MPDU.
     * @param flags The flags to set.
    ...
    enum VhtKnown
    {
        ...
        VHT_KNOWN_STBC = 0x0001,                /**< Space-time block coding (1 if all spatial
                                                   streams of all users have STBC, 0 otherwise). */
        ...
        VHT_KNOWN_BEAMFORMED = 0x0020,                   /**< Beamformed known/applicable (this flag
                                                            should be set to  zero for MU PPDUs). */
    ...
    enum VhtFlags
    {
        ...
        VHT_FLAGS_STBC = 0x01,                /**< Set if all spatial streams of all users have
                                                 space-time block coding */
        VHT_FLAGS_TXOP_PS_NOT_ALLOWED = 0x02, /**< Set if STAs may not doze during TXOP (valid only
                                                 for AP transmitters). */
        ...
        VHT_FLAGS_LDPC_EXTRA_OFDM_SYMBOL =
            0x10,                    /**< Set if one or more users are using LDPC and the encoding
    ...
    enum HeData1
    {
        ...
        HE_DATA1_SPTL_REUSE_KNOWN = 0x0400,   /**< Spatial Reuse known (Spatial Reuse
                                                 1 for HE TRIG PPDU format) */
    ...
    enum HeMuFlags2
    {
        ...
        HE_MU_FLAGS2_NUM_SIGB_SYMBOLS_FROM_SIGA = 0x00f0, /**< # of HE-SIG-B Symbols - 1 or # of
                                                             MU-MIMO Users - 1 from SIG-A */
  ...
  private:
    ...
    uint64_t m_tsft;         //!< Time Synchronization Function Timer (when the first bit
                             //!< of the MPDU arrived at the MAC)
    ...
    int8_t m_antennaSignal;  //!< RF signal power at the antenna, dB difference
                             //!< from an arbitrary, fixed reference.
    int8_t m_antennaNoise;   //!< RF noise power at the antenna, dB difference from
    ...
    uint8_t m_ampduStatusPad;    //!< A-MPDU Status Flags, padding before A-MPDU
                                 //!< Status Field.
    uint32_t m_ampduStatusRef;   //!< A-MPDU Status Flags, reference number.
    uint16_t m_ampduStatusFlags; //!< A-MPDU Status Flags, information about the
                                 //!< received A-MPDU.
