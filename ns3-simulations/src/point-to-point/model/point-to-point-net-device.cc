...
void
PointToPointNetDevice::EnablePacketDrop()
{
    NS_LOG_FUNCTION(this);
    is_pkt_drop_enabled = true;
}

void
PointToPointNetDevice::DisablePacketDrop()
{
    NS_LOG_FUNCTION(this);
    is_pkt_drop_enabled = false;
}

void
PointToPointNetDevice::SetDropPacketUid(uint64_t pkt_drop_uid)
{
    NS_LOG_FUNCTION(this);
    m_pkt_drop_uids.insert(pkt_drop_uid);
}

bool
PointToPointNetDevice::ProbabilisticPacketLinkLoss()
{
    return Random::get<bool>(link_loss_perc);
}

void
PointToPointNetDevice::EnableLinkLoss(double loss_perc)
{
    link_loss_perc = loss_perc;
    is_random_loss_enabled = true;
}

void
PointToPointNetDevice::DisableLinkLoss()
{
    is_random_loss_enabled = false;
}

void
PointToPointNetDevice::EnablePacketDropAndLossLogging()
{
    log_pkt_drops = true;
}

void
PointToPointNetDevice::SetPacketDropAndLossLoggingPath(std::string path)
{
    log_pkt_drops_path = path;
}

void
PointToPointNetDevice::DisablePacketDropAndLossLogging()
{
    log_pkt_drops = false;
}

void
PointToPointNetDevice::LogPacketDrop(uint64_t uid, std::string reason)
{
    std::ofstream outfile;
    outfile.open(log_pkt_drops_path + "/" + "linkDrops.txt", std::ios_base::app);
    outfile << "[" << Simulator::Now().GetSeconds() << "] Packet drop - reason: " << reason
            << " packet UID:" << uid << std::endl;
    outfile.close();
}
...
PointToPointNetDevice::TransmitStart(Ptr<Packet> p)
{
    ...
    */
    ...
    /* Return False if the packet was dropped or lost. */
    ...
    if (is_random_loss_enabled)
    {
        if (ProbabilisticPacketLinkLoss())
        {
            if (log_pkt_drops)
            {
                LogPacketDrop(p->GetUid(), "random loss");
            }
            m_phyTxDropTrace(p);
            return false;
        }
    ...
    if (is_pkt_drop_enabled)
    {
        const bool is_in = m_pkt_drop_uids.find(p->GetUid()) != m_pkt_drop_uids.end();
        if (is_in)
        {
            std::set<std::uint64_t>::iterator it = m_pkt_drop_uids.find(p->GetUid());
            m_pkt_drop_uids.erase(it);

            if (log_pkt_drops)
            {
                LogPacketDrop(p->GetUid(), "penny drop");
            }
            m_phyTxDropTrace(p);
            return false;
        }
