...
class Ipv6RoutingHelper
{
  public:
     ...
     * \param printTime the time at which the routing table is supposed to be
     * printed. \param stream The output stream object to use \param unit The time
     * unit to be used in the report
     *
    ...
    /**
     * \brief prints the routing tables of all nodes at regular intervals
     * specified by user. \param printInterval the time interval for which the
     * routing table is supposed to be printed. \param stream The output stream
     * object to use \param unit The time unit to be used in the report
     *
     * This method calls the PrintRoutingTable() method of the
     * Ipv6RoutingProtocol stored in the Ipv6 object, for all nodes at the
     ...
     * \param printTime the time at which the routing table is supposed to be
     * printed. \param node The node ptr for which we need the routing table to be
     * printed \param stream The output stream object to use \param unit The time
     * unit to be used in the report
     *
     * This method calls the PrintRoutingTable() method of the
    ...
    /**
     * \brief prints the routing tables of a node at regular intervals specified
     * by user. \param printInterval the time interval for which the routing table
     * is supposed to be printed. \param node The node ptr for which we need the
     * routing table to be printed \param stream The output stream object to use
     ...
     * \param printTime the time at which the neighbor cache is supposed to be
     printed.
     * \param stream The output stream object to use
     * \param unit The time unit to be used in the report
     *
     * This method calls the PrintNdiscCache() method of the
     * NdiscCache associated with each Ipv6Interface stored in the Ipv6 object,
     for all nodes at the
    ...
    /**
     * \brief prints the neighbor cache of all nodes at regular intervals
     specified by user.
     * \param printInterval the time interval for which the neighbor cache is
    ...
    /**
     * \brief prints the neighbor cache of a node at regular intervals specified
     by user.
     * \param printInterval the time interval for which the neighbor cache is
     supposed to be printed.
     * \param node The node ptr for which we need the neighbor cache to be printed
    ...
    /**
     * \brief Request a specified routing protocol &lt;T&gt; from
     * Ipv6RoutingProtocol protocol
     *
     ...
     * \return a Smart Pointer to the requested protocol (zero if the protocol
     * can't be found)
     */
    template <class T>
    ...
    if (!ret)
    {
        ...
        if (lrp)
        {
            for (uint32_t i = 0; i < lrp->GetNRoutingProtocols(); i++)
            {
                int16_t priority;
                ret = GetRouting<T>(
                    lrp->GetRoutingProtocol(i,
                                            priority)); // potential recursion, if inside
                                                        // ListRouting is ListRouting
                if (ret)
                {
                    break;
                }
