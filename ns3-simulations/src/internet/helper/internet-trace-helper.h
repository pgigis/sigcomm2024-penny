...
class AsciiTraceHelperForIpv4
{
  public:
     ...
     *
     * If the prefix is provided, there will be one file per Ipv4 and interface
     * pair created.  In this case, adding a trace context to the file would be
     * pointless, so the helper implementation is expected to
    ...
    /**
     * @brief Enable ascii trace output on the Ipv4 and interface pair specified
     * by a global node-id (of a previously created node) and interface.  Since
     * there can be only one Ipv4 aggregated to a node, the node-id unambiguously
     * determines the Ipv4.
     *
  ...
  private:
     ...
     * @param nodeid The node identifier/number of the node on which to enable
     *               ascii tracing
     * @param interface The device identifier/index of the device on which to
     * enable ascii tracing
     * @param explicitFilename Treat the prefix as an explicit filename if true.
     */
...
class AsciiTraceHelperForIpv6
{
  public:
     ...
     * each net device using the rules described in the class overview.
     *
     * If the prefix is provided, there will be one file per Ipv6 and interface
     * pair created.  In this case, adding a trace context to the file would be
     * pointless, so the helper implementation is expected to
     * TraceConnectWithoutContext.
     *
    ...
    /**
     * @brief Enable ascii trace output on the Ipv6 and interface pair specified
     * by a global node-id (of a previously created node) and interface.  Since
     * there can be only one Ipv6 aggregated to a node, the node-id unambiguously
     * determines the Ipv6.
     *
     * @param prefix Filename prefix to use when creating ascii trace files
     * @param nodeid The node identifier/number of the node on which to enable
