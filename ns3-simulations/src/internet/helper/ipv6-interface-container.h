...
class Ipv6InterfaceContainer
{
  public:
    /**
     * \brief Container Const Iterator for pairs of Ipv6 smart pointer / Interface
     * Index.
     */
     ...
     * \param i index
     * \return the link-local address, or "::" if the interface has no link local
     * address.
     */
    Ipv6Address GetLinkLocalAddress(uint32_t i);

    /**
     * \brief Get the link-local address for the node with the specified global
     * address. \param address the address to find. \return the link-local
     * address, or "::" if the interface has no link local address.
     */
     ...
     *   Ptr<Ipv6Interface> iface =  DynamicCast<Ipv6L3Protocol>
     * (ipv6)->GetInterface (index); \endcode
     */
    std::pair<Ptr<Ipv6>, uint32_t> Get(uint32_t i) const;

    /**
     * \brief Set the state of the stack (act as a router or as an host) for the
     * specified index. This automatically sets all the node's interfaces to the
     * same forwarding state. \param i index \param state true : is a router,
     * false : is an host
     */
    void SetForwarding(uint32_t i, bool state);

    /**
     * \brief Set the default route for all the devices (except the router
     * itself). \param router the default router index
     */
    void SetDefaultRouteInAllNodes(uint32_t router);

    /**
     * \brief Set the default route for all the devices (except the router
     * itself). Note that the route will be set to the link-local address of the
     * node with the specified address. \param routerAddr the default router
     * address
     */
    ...
    /**
     * \brief Set the default route for the specified index.
     * Note that the route will be set to the link-local address of the node with
     * the specified address. \param i index \param routerAddr the default router
     * address
     */
