 ...
 * Copyright (c) 2019 Caliola Engineering, LLC : RFC 6621 multicast packet
 * de-duplication
 *
...
Ipv4DeduplicationTest::DoRun()
{
    ...
    for (auto end = nodes.End(), iter = nodes.Begin(); iter != end; ++iter)
    {
        ...
        // route for host
        // Use host routing entry according to note in
        // Ipv4StaticRouting::RouteOutput:
...
{
    // a priori determined packet receptions based on initial TTL of 4, disabled
    // de-dup
    ...
    // a priori determined packet receptions based on initial TTL of 4, degenerate
    // de-dup There are TTL (4) rounds of packets.  Each round a node will
    // register a received packet if another connected node transmits.  A misses
    // the 1st round since it is the only one transmitting.  D is not connected to
    // A in 1st round either.  E only hears a packet in the 3rd and 4th rounds.
...
Ipv4DeduplicationTest::CheckDrops(const std::string& name)
{
    ...
    {
    case ENABLED:
        // a priori determined packet drops based on initial TTL of 4, enabled
        // de-dup; A hears from B & C -- > 2 drops D hears from B, C, AND E B (C)
        // hears from A, C (B), D,
        drops = {{"A", 2}, {"B", 2}, {"C", 2}, {"D", 2}, {"E", 0}};
        break;
    case DISABLED:
        // a priori determined packet drops based on initial TTL of 4, disabled
        // de-dup
        drops = {{"A", 10}, {"B", 9}, {"C", 9}, {"D", 12}, {"E", 2}};
        break;
    case DEGENERATE:
        // a priori determined packet drops based on initial TTL of 4, degenerate
        // de-dup There are TTL (4) rounds of transmissions.  Since all transmitters
        // are synchronized, multiple packets are received each round when there are
        ...
        // in check packets, but with only a 1 for each node in each round when
        // packets are received.  Drops are the sum of receptions using these
        // indicator receive vectors subtracting 1 for each node (for the delivered
        // packet) and adding 1 at all nodes for TTL expiry.
...
static Ipv4DeduplicationPerformanceTestSuite
    g_ipv4DeduplicationPerformanceTestSuite; //!< Static variable for test
                                             //!< initialization
