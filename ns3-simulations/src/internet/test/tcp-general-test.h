 ...
 *
 * \brief Class for inserting callbacks special points of the flow of TCP
 * sockets
 *
 ...
 * directions.
 *
 * There isn't the necessity to fill TcpSocketBase of TracedCallbacks; the
 * rationale is to maintain the base class as clean as possible.
 *
 * To be fair with testing, this class should NOT modify the behavior of
 * TcpSocketBase.
 *
 * \see SetRcvAckCb
...
class TcpSocketMsgBase : public ns3::TcpSocketBase
{
  public:
    ...
    /**
     * \brief Set the callback invoked after the processing of a retransmit
     * timeout
     *
     * \param cb callback
     */
    ...
    /**
     * \brief Set the callback invoked before the processing of a retransmit
     * timeout
     *
     * \param cb callback
     */
 ...
 * (e.g. two ACKs, one for "x/2", and the other for "x+1") ? A TCP
 * implementation should avoid to artificially increase the congestion window,
 * thinking of having ACKed 2 segments instead of 1.
 *
 ...
 * - Ack received (RcvAck) and Ack processed (ProcessedAck). The first is used
 * to signal that an ACK has been received; after the processing of it, the
 * Second is called
 * - A packet is transmitted to IP layer or received from IP layer (Tx and Rx)
 * - The RTO expires (RTOExpired)
 *
 * The default version of such methods is empty; implement their behavior
 * differently, based on what you want to test. Default is empty to avoid the
 * need to implement useless pure virtual function.
 *
 * If you need values from TcpSocketBase, thanks to the friend relationship
 * between this class and TcpSocketBase itself you can get it. Remember that
 * friendship isn't passed by inheritance, so the way to go is to write a
 * Getters (like GetSegSize) and call it in the subclass.
 *
 * \see DoRun
...
class TcpGeneralTest : public TestCase
{
  ...
  protected:
     ...
     *
     * As environment, two socket are connected through a SimpleChannel. Each
     * device has an MTU of 1500 bytes, and the application starts to send packet
     * at 10s of simulated time, through SendPacket.
     *
     ...
     * that should be applied after socket have been created) use
     * ConfigureProperties.
     *
    ...
    /**
     * \brief Set the initial time at which the application sends the first data
     * packet
     *
  ...
  private:
    ...
    Time m_interPacketInterval; //!< Time between sending application packet down
                                //!< to tcp socket
