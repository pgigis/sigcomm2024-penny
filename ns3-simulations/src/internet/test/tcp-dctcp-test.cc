    ...
    if (who == SENDER && m_testCase == 3)
    {
        ...
        if (m_senderReceived == 2 && m_testCase == 3)
        {
            NS_TEST_ASSERT_MSG_NE(
                ...
                0,
                "The flag ECE should be set in TCP header of the packet sent by the "
                "receiver when "
...
TcpDctcpCongestedRouter::SendDataPacket(SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
    ...
    {
        NS_LOG_INFO("Backoff mechanism by reducing CWND  by half because we've "
                    "received ECN Echo");
        ...
        : TestSuite("tcp-dctcp-test", UNIT)
    {
        ...
        AddTestCase(
            new TcpDctcpCodePointsTest(2,
                                       "ECT Test : Check if ECT is not set on Syn, Syn+Ack and "
                                       "Ack but set on "
