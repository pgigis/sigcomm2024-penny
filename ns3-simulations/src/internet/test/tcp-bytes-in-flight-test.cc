 ...
 * in theory, the possibility to know the real amount of bytes in flight.
 * However this value is useless, since the sender bases its guess on the
 * received ACK.
 *
 * \see Tx
        ...
        { // New ack
            ...
            { // Previously we got some ACKs
                ...
                else
                {
                    ...
                    // During fast recovery the TCP data sender respond to a partial
                    // acknowledgment by inferring that the next in-sequence packet has
                    // been lost (RFC5681)
            ...
            { // received the ACK for the FIN (which includes 1 spurious
              // byte)
                diff -= 1;
            }
