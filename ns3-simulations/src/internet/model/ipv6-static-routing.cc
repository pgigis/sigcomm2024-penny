...
Ipv6StaticRouting::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
    ...
    *os << "Node: " << m_ipv6->GetObject<Node>()->GetId() << ", Time: " << Now().As(unit)
    ...
    if (GetNRoutes() > 0)
    {
        *os << "Destination                    Next Hop                   Flag Met "
               "Ref Use If"
            << std::endl;
...
Ipv6StaticRouting::LookupStatic(Ipv6Address dst, Ptr<NetDevice> interface)
{
    ...
    if (dst.IsLinkLocalMulticast())
    {
        NS_ASSERT_MSG(interface,
                      "Try to send on link-local multicast address, and "
...
Ipv6StaticRouting::LookupStatic(Ipv6Address origin, Ipv6Address group, uint32_t interface)
{
    ...
    for (auto i = m_multicastRoutes.begin(); i != m_multicastRoutes.end(); i++)
    {
        ...
        /*
           ...
           matches.  This picks up exact routes during forwarded and exact routes
           from the local node (in which case the ifIndex is a wildcard).
           */
                                  ...
                                  Ipv6Address prefixToUse)
{
    ...
    {
        /* this case is mainly used by configuring default route following RA
         * processing, in case of multiple prefix in RA, the first will configured
         * default route
         */
