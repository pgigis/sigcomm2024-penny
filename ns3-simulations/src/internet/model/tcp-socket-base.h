 ...
 * interface for upper layers to call. This class provides connection
 * orientation and sliding window flow control; congestion control is delegated
 * to subclasses of TcpCongestionOps. Part of TcpSocketBase is modified from the
 * original NS-3 TCP socket implementation (TcpSocketImpl) by Raj Bhattacharjea
 * <raj.b@gatech.edu> of Georgia Tech.
 *
 ...
 * "Congestion state machine", which keeps track of the phase we are in.
 * Currently, ns-3 manages the states:
 *
 * - CA_OPEN
 * - CA_DISORDER
 * - CA_RECOVERY
 ...
 * used, which consists in chaining callbacks from TcpSocketState to
 * TcpSocketBase (see for example cWnd trace source).
 *
 * Fast retransmit
 * ----------------
 *
 * The fast retransmit enhancement is introduced in RFC 2581 and updated in RFC
 * 5681. It reduces the time a sender waits before retransmitting a lost
 * segment, through the assumption that if it receives a certain number of
 * duplicate ACKs, a segment has been lost and it can be retransmitted. Usually,
 * it is coupled with the Limited Transmit algorithm, defined in RFC 3042. These
 * algorithms are included in this class, and they are implemented inside the
 * ProcessAck method. With the SACK option enabled, the LimitedTransmit
 * algorithm will be always on, as a consequence of how the information in the
 * received SACK block is managed.
 *
 ...
 * the SACK block) are processed and a particular data structure, called
 * Scoreboard, is filled. Please take a look at TcpTxBuffer and TcpRxBuffer
 * documentation if you need more information. The reference paper is
...
class TcpSocketBase : public TcpSocket
{
  public:
    ...
    /**
     * Clone a TCP socket, for use upon receiving a connection request in LISTEN
     * state
     *
    ...
    /**
     * \brief Set the retransmission threshold (dup ack threshold for a fast
     * retransmit) \param retxThresh the threshold
     */
    void SetRetxThresh(uint32_t retxThresh);

    /**
     * \brief Get the retransmission threshold (dup ack threshold for a fast
     * retransmit) \return the threshold
     */
    ...
    /**
     * \brief Callback function to hook to TcpSocketState inflated congestion
     * window \param oldValue old cWndInfl value \param newValue new cWndInfl
     * value
     */
    ...
    int Listen() override;                // Verify the socket is in a correct state and call
                                          // ProcessAction() to listen
    ...
    int Send(Ptr<Packet> p,
             uint32_t flags) override; // Call by app to send data to network
  ...
  protected:
    ...
    /**
     * \brief Perform the real connection tasks: Send SYN if allowed, RST if
     * invalid
     *
    ...
    /**
     * \brief Configure the endpoint to a local address. Called by Connect() if
    ...
    /**
     * \brief Configure the endpoint v6 to a local address. Called by Connect() if
     * Bind() didn't specify one.
     *
     * \returns 0 on success
     */
    ...
    /**
     * \brief Called by the L3 protocol when it received a packet to pass on to
     * TCP.
     *
    ...
    /**
     * \brief Extract at most maxSize bytes from the TxBuffer at sequence seq, add
     * the TCP header, and send to TcpL4Protocol
     *
    ...
    /**
     * \brief Close a socket by sending RST, FIN, or FIN+ACK, depend on the
     * current state
     *
     * \returns 0 on success
     */
     ...
     *
     * This function is mimicking the role of tcp_rcv_established() in tcp_input.c
     * in Linux kernel.
     *
    ...
    /**
     * Get the current value of the receiver's highest (in-sequence) sequence
     * number acked. \note This method exists to expose the value to the
     * TcpTxBuffer \return value of receiver's highest sequence number acked.
     */
  ...
  protected:
    ...
    EventId m_persistEvent{};  //!< Persist event: Send 1 byte to probe for a
                               //!< non-zero Rx window
    EventId m_timewaitEvent{}; //!< TIME_WAIT expiration event: Move this socket
                               //!< to CLOSED state
    ...
    SequenceNumber32 m_recover{0}; //!< Previous highest Tx seqnum for fast
                                   //!< recovery (set it to initial seq number)
