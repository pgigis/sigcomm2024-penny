    ...
    static TypeId tid =
        TypeId("ns3::TcpSocketBase")
            ...
            .AddAttribute("MaxSegLifetime",
                          "Maximum segment lifetime in seconds, use for "
                          "TIME_WAIT state transition "
                          "to CLOSED state",
            ...
            .AddAttribute("IcmpCallback",
                          "Callback invoked whenever an icmp error is received "
                          "on this socket.",
                          CallbackValue(),
                          MakeCallbackAccessor(&TcpSocketBase::m_icmpCallback),
                          MakeCallbackChecker())
            .AddAttribute("IcmpCallback6",
                          "Callback invoked whenever an icmpv6 error is received "
                          "on this socket.",
                          CallbackValue(),
...
/* Inherit from Socket class: Bind socket (with specific address) to an
 * end-point in TcpL4Protocol
 */
int
...
TcpSocketBase::SetInitialSSThresh(uint32_t threshold)
{
    NS_ABORT_MSG_UNLESS((m_state == CLOSED) || threshold == m_tcb->m_initialSsThresh,
                        "TcpSocketBase::SetSSThresh() cannot change initial "
                        "ssThresh after connection started.");
...
TcpSocketBase::SetInitialCwnd(uint32_t cwnd)
{
    NS_ABORT_MSG_UNLESS((m_state == CLOSED) || cwnd == m_tcb->m_initialCWnd,
                        "TcpSocketBase::SetInitialCwnd() cannot change initial "
...
TcpSocketBase::Send(Ptr<Packet> p, uint32_t flags)
{
    ...
    if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
    {
        ...
        { // Try to send the data out: Add a little step
...
/* Inherit from Socket class: Return data to upper-layer application. Parameter
   flags is not used. Data is returned as a packet of size no larger than
   maxSize */
Ptr<Packet>
...
TcpSocketBase::DoConnect()
{
    ...
    { // In states SYN_RCVD, ESTABLISHED, FIN_WAIT_1,
      // FIN_WAIT_2, and CLOSING, an connection
...
/* Peacefully close the socket by notifying the upper layer and deallocate end
 * point */
void
...
TcpSocketBase::OutOfRange(SequenceNumber32 head, SequenceNumber32 tail) const
{
    ...
    { // In LAST_ACK and CLOSING states, it only wait
      // for an ACK and the
                                 ...
                                 const uint32_t tcpPayloadSize)
{
    ...
    else if (tcpPayloadSize > 0 && OutOfRange(seq, seq + tcpPayloadSize))
    {
        ...
        // Acknowledgement should be sent for all unacceptable packets (RFC793,
        // p.69)
...
TcpSocketBase::DoForwardUp(Ptr<Packet> packet, const Address& fromAddress, const Address& toAddress)
{
    ...
    {
        // Check if the sender has responded to ECN echo by reducing the Congestion
        // Window
        if (tcpHeader.GetFlags() & TcpHeader::CWR)
        {
            // Check if a packet with CE bit set is received. If there is no CE bit
            // set, then change the state to ECN_IDLE to stop sending ECN Echo
            // messages. If there is CE bit set, the packet should continue sending
            // ECN Echo messages
            //
    ...
    { // Zero window: Enter persist state to send
      // 1 byte to probe
    ...
    case CLOSED:
        ...
        { // Since m_endPoint is not configured yet, we cannot
          // use SendRST here
            TcpHeader h;
    ...
    { // persist probes end, the other end has
      // increased the window
...
TcpSocketBase::ProcessEstablished(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    if (tcpflags == TcpHeader::ACK)
    {
        ...
        {
            // Case 1:  If the ACK is a duplicate (SEG.ACK < SND.UNA), it can be
            // ignored. Pag. 72 RFC 793
        ...
        else if (tcpHeader.GetAckNumber() > m_tcb->m_highTxMark)
        {
            ...
            // Receiver sets ECE flags when it receives a packet with CE bit on or
    ...
    { // Received SYN, old NS-3 behaviour is to set
      // state to SYN_RCVD and respond with a SYN+ACK.
      // But it is not a legal state transition as of
      // RFC793. Thus this is ignored.
    }
    else if (tcpflags == (TcpHeader::SYN | TcpHeader::ACK))
    { // No action for received SYN+ACK, it is
      // probably a duplicated packet
    }
    else if (tcpflags == TcpHeader::FIN || tcpflags == (TcpHeader::FIN | TcpHeader::ACK))
    { // Received FIN or FIN+ACK, bring
      // down this socket nicely
    ...
    { // Received RST or the TCP flags is invalid, in either case,
...
TcpSocketBase::DupAck(uint32_t currentDelivered)
{
    ...
    // of a real packet loss. With SACK, it is easy to know, but we do not
    // consider dupacks. Without SACK, there are some heuristics in the RFC 6582,
    // but for now, we do not implement it, leading to ignoring the dupacks.
...
TcpSocketBase::ReceivedAck(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    // Update bytes in flight before processing the ACK for proper calculation of
    // congestion window
    ...
    // RFC 6675, Section 5, point (C), try to send more data. NB: (C) is
    // implemented inside SendPendingData
    SendPendingData(m_connected);
}
                          ...
                          const SequenceNumber32& oldHeadSequence)
{
    ...
    {
        // DupAck. Artificially call PktsAcked: after all, one segment has been
        // ACKed.
...
TcpSocketBase::ProcessSynSent(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    if (tcpflags == 0)
    { // Bare data, accept it and move to ESTABLISHED state.
      // This is not a normal behaviour. Remove this?
    ...
    { // Received SYN, move to SYN_RCVD
      // state and respond with SYN+ACK
        NS_LOG_DEBUG("SYN_SENT -> SYN_RCVD");
        m_state = SYN_RCVD;
        m_synCount = m_synRetries;
        m_tcb->m_rxBuffer->SetNextRxSequence(tcpHeader.GetSequenceNumber() + SequenceNumber32(1));
        /* Check if we received an ECN SYN packet. Change the ECN state of receiver
         * to ECN_IDLE if the traffic is ECN capable and sender has sent ECN SYN
         * packet
         */
    ...
    { // Handshake completed
        ...
        // Before sending packets, update the pacing rate based on RTT measurement
        // so far
        UpdatePacingRate();
        SendEmptyPacket(TcpHeader::ACK);

        /* Check if we received an ECN SYN-ACK packet. Change the ECN state of
         * sender to ECN_IDLE if receiver has sent an ECN SYN-ACK packet and the
         * traffic is ECN Capable
         */
        if (m_tcb->m_useEcn != TcpSocketState::Off &&
    ...
    { // Other in-sequence input
        if (!(tcpflags & TcpHeader::RST))
        { // When (1) rx of FIN+ACK; (2) rx of
          // FIN; (3) rx of bad flags
                              ...
                              const Address& /* toAddress */)
{
    ...
    if (tcpflags == 0 ||
        (tcpflags == TcpHeader::ACK &&
         m_tcb->m_nextTxSequence + SequenceNumber32(1) == tcpHeader.GetAckNumber()))
    { // If it is bare data, accept it and
      // move to ESTABLISHED state. This is
        ...
        UpdatePacingRate();
        // As this connection is established, the socket is available to send data
        // now
        if (GetTxAvailable() > 0)
        {
    ...
    { // Probably the peer lost my SYN+ACK
        m_tcb->m_rxBuffer->SetNextRxSequence(tcpHeader.GetSequenceNumber() + SequenceNumber32(1));
        /* Check if we received an ECN SYN packet. Change the ECN state of receiver
         * to ECN_IDLE if sender has sent an ECN SYN packet and the  traffic is ECN
         * Capable
         */
        if (m_tcb->m_useEcn != TcpSocketState::Off &&
    ...
    else if (tcpflags == (TcpHeader::FIN | TcpHeader::ACK))
    {
        ...
        { // In-sequence FIN before connection complete.
          // Set up connection and close.
            m_connected = true;
            m_retxEvent.Cancel();
            m_tcb->m_highTxMark = ++m_tcb->m_nextTxSequence;
            m_txBuffer->SetHeadSequence(m_tcb->m_nextTxSequence);
    ...
    { // Other in-sequence input
        if (tcpflags != TcpHeader::RST)
        { // When (1) rx of SYN+ACK; (2) rx of FIN;
          // (3) rx of bad flags
            NS_LOG_LOGIC("Illegal flag " << TcpHeader::FlagsToString(tcpflags)
                                         << " received. Reset packet is sent.");
            if (m_endPoint)
...
TcpSocketBase::ProcessWait(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    { // Process the ACK, and if in FIN_WAIT_1,
      // conditionally move to FIN_WAIT_2
        ReceivedAck(packet, tcpHeader);
    ...
    { // Duplicated SYN or SYN+ACK, possibly due
...
TcpSocketBase::ProcessClosing(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    { // CLOSING state means simultaneous close, i.e. no one is sending
      // data to
        ...
        { // FIN from the peer as well.
          // We can close immediately.
            SendEmptyPacket(TcpHeader::ACK);
        }
        else if (tcpflags != TcpHeader::RST)
        { // Receive of SYN or SYN+ACK or bad
          // flags or pure data
            NS_LOG_LOGIC("Illegal flag " << TcpHeader::FlagsToString(tcpflags)
                                         << " received. Reset packet is sent.");
            SendRST();
...
TcpSocketBase::ProcessLastAck(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    {
        if (tcpHeader.GetSequenceNumber() == m_tcb->m_rxBuffer->NextRxSequence())
        { // This ACK corresponds to the FIN sent. This
          // socket closed peacefully.
            CloseAndNotify();
        }
    }
    else if (tcpflags == TcpHeader::FIN)
    { // Received FIN again, the peer
      // probably lost the FIN+ACK
        SendEmptyPacket(TcpHeader::FIN | TcpHeader::ACK);
    }
...
TcpSocketBase::PeerClose(Ptr<Packet> p, const TcpHeader& tcpHeader)
{
    ...
    // Return if FIN is out of sequence, otherwise move to CLOSE_WAIT state by
    // DoPeerClose
    if (!m_tcb->m_rxBuffer->Finished())
    {
        return;
    ...
    // Simultaneous close: Application invoked Close() when we are processing this
    // FIN packet
    if (m_state == FIN_WAIT_1)
    {
        NS_LOG_DEBUG("FIN_WAIT_1 -> CLOSING");
...
TcpSocketBase::DoPeerClose()
{
    ...
    if (!m_closeNotified)
    {
        // The normal behaviour for an application is that, when the peer sent a
        // in-sequence FIN, the app should prepare to close. The app has two choices
        // at this point: either respond with ShutdownSend() call to declare that it
        // has nothing more to send and the socket can be closed immediately; or
        // remember the peer's close request, wait until all its existing data are
        // pushed into the TCP socket, then call Close() explicitly.
        NS_LOG_LOGIC("TCP " << this << " calling NotifyNormalClose");
        NotifyNormalClose();
        m_closeNotified = true;
    }
    if (m_shutdownSend)
    { // The application declares that it would not sent any
      // more, close this socket
        Close();
    }
    else
...
TcpSocketBase::SendEmptyPacket(uint8_t flags)
{
    ...
    if (hasSyn)
    {
        ...
        { // The window scaling option is set only on SYN
          // packets
    ...
    { // Retransmit SYN / SYN+ACK / FIN / FIN+ACK to guard against
      // lost
...
{
    // note: it shouldn't be necessary to invalidate the callback and manually
    // call TcpL4Protocol::RemoveSocket. Alas, if one relies on the endpoint
    // destruction callback, there's a weird memory access to a free'd area.
    // Harmless, but valgrind considers it an error.
...
/* Configure the endpoint to a local address. Called by Connect() if Bind()
 * didn't specify one. */
int
                            ...
                            const Address& toAddress)
{
    ...
    /* Check if we received an ECN SYN packet. Change the ECN state of receiver to
     * ECN_IDLE if sender has sent an ECN SYN packet and the traffic is ECN
     * Capable
     */
    if (m_tcb->m_useEcn != TcpSocketState::Off &&
...
{ // Wrapper to protected function
  // NotifyConnectionSucceeded() so
  // that it can
...
TcpSocketBase::SendPendingData(bool withAck)
{
    ...
    while (availableWindow > 0)
    {
        ...
        else
        {
            ...
            // window syndrome) but continue if we don't have data
...
TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader& tcpHeader)
{
    ...
    { // A gap exists in the buffer, or we
      // filled a gap: Always ACK
...
TcpSocketBase::ReTxTimeout()
{
    ...
    // If all data are received (non-closing socket and nothing to send), just
    // return
    ...
    bool resetSack = !m_sackEnabled; // Reset SACK information if SACK is not
                                     // enabled. The information in the
                                     // TcpTxBuffer is guessed, in this case.
