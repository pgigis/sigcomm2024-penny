 ...
 *                             \ betaMax            otherwise
 *
 *  where the calculations of k1, k2, k3, and k4 are shown in Equations (3),
 * (4), (5), and (6).
 *
 ...
 *
 *  with da the current average queueing delay calculated in Equation  (7)
 * where: Ta is the average RTT (sumRtt / cntRtt in the implementation) and Tmin
 * (baseRtt in the implementation) is the minimum RTT ever seen dm the maximum
 * (average) queueing delay calculated in Equation (8) where Tmax (maxRtt in the
 ...
 *
 * Following the implementation of Illinois in the Linux kernel, we use the
 * following default parameter settings:
 *
...
class TcpIllinois : public TcpNewReno
{
  ...
  private:
     ...
     *
     * If the current average queueing delay is <= 10% of max. (average) queueing
     * delay, beta is set to betaMin, which equals to 1/8 by default. If the
     * current average queueing delay is >= 80% of max. (average) queueing delay,
