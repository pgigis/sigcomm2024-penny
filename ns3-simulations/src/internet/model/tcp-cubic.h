 ...
 * that the window growth function is defined in real time in order to be
 * independent from the RTT. More specifically, the congestion window of Cubic
 * is determined by a function of the elapsed time from the last window
 * reduction.
 *
...
class TcpCubic : public TcpCongestionOps
{
  ...
  private:
    ...
    uint32_t m_hystartLowWindow;           //!< Lower bound cWnd for hybrid slow start
                                           //!< (segments)
    ...
    uint8_t m_hystartMinSamples;           //!< Number of delay samples for detecting the
                                           //!< increase of delay
