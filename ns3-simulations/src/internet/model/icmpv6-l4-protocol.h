...
class Icmpv6L4Protocol : public IpL4Protocol
{
  public:
    ...
    /**
     * \brief This method is called by AggregateObject and completes the
     * aggregation by setting the node in the ICMPv6 stack and adding ICMPv6
     * factory to IPv6 stack connected to the node.
     */
    ...
    /**
     * \brief Send a packet via ICMPv6, note that packet already contains ICMPv6
     * header. \param packet the packet to send which contains ICMPv6 header
    ...
    /**
     * \brief Helper function used during delayed solicitation. Calls SendMessage
     * internally \param packet the packet to send which contains ICMPv6 header
     * \param src source address
     * \param dst destination address
     * \param ttl next hop limit
     */
     ...
     * \return true if the address is in the ND cache, the hardwareDestination is
     * updated.
     */
    ...
    /**
     * \brief Lookup in the ND cache for the IPv6 address (similar as ARP
     * protocol).
     *
  ...
  protected:
    ...
    /**
     * \brief Maximum time between multicast RS retransmissions [\RFC{7559}]. Zero
     * means unbound.
     */
    Time m_rsMaxRetransmissionTime;

    /**
     * \brief Maximum number of multicast RS retransmissions [\RFC{7559}]. Zero
     * means unbound.
     */
    uint32_t m_rsMaxRetransmissionCount;

    /**
     * \brief Maximum duration of multicast RS retransmissions [\RFC{7559}]. Zero
     * means unbound.
     */
     ...
     *
     * Zero indicate a first transmission, greater than zero means
     * retranmsisisons.
     */
