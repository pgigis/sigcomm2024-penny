...
TcpRxBuffer::Add(Ptr<Packet> p, const TcpHeader& tcph)
{
    ...
    while (i != m_data.end() && i->first <= tailSeq)
    {
        ...
        if (lastByteSeq > headSeq)
        {
            ...
            { // Rare case: Existing packet is embedded
              // fully in the new packet
...
TcpRxBuffer::UpdateSackList(const SequenceNumber32& head, const SequenceNumber32& tail)
{
    ...
    // This is perfectly fine for the RFC point (a), given that we do not report
    // any overlapping blocks shortly after.
}
