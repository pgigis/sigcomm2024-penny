 ...
 *
 * Authors: Siddharth Gangadhar <siddharth@ittc.ku.edu>, Truc Anh N. Nguyen
 * <annguyen@ittc.ku.edu>, and Greeshma Umapathi
 *
 ...
 * TCPHeader&) and the EstimateBW (int, const, Time). The CountAck method
 * calculates the number of acknowledged segments on the receipt of an ACK. The
 * EstimateBW estimates the bandwidth based on the value returned by CountAck
 * and the sampling interval (last RTT).
 *
 * WARNING: this TCP model lacks validation and regression tests; use with
 * caution.
 */
class TcpWestwoodPlus : public TcpNewReno
{
  ...
  private:
     ...
     *
     * \param [in] acked the number of packets the currently received ACK
     * acknowledges
     */
  ...
  protected:
    ...
    bool m_IsCount;            //!< Start keeping track of m_ackedSegments for Westwood+ if
                               //!< TRUE
