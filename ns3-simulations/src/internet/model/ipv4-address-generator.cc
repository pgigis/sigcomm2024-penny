...
Ipv4AddressGeneratorImpl::AddAllocated(const Ipv4Address address)
{
    ...
    NS_ABORT_MSG_UNLESS(addr,
                        "Ipv4AddressGeneratorImpl::Add(): Allocating the "
    ...
    for (i = m_entries.begin(); i != m_entries.end(); ++i)
    {
        ...
        // completely filled network ranges will be a fairly rare occurrence, so we
        // don't worry about collapsing address range blocks.
        //
...
Ipv4AddressGeneratorImpl::IsAddressAllocated(const Ipv4Address address)
{
    ...
    NS_ABORT_MSG_UNLESS(addr,
                        "Ipv4AddressGeneratorImpl::IsAddressAllocated(): "
...
Ipv4AddressGeneratorImpl::MaskToIndex(Ipv4Mask mask) const
{
    ...
    // network number.  In order to translate this mask into an index, we just
    // need to count the number of zero bits in the mask.  We do this in a loop in
    // which we shift the mask right until we find the first nonzero bit.  This
    // tells us the number of zero bits, and from this we infer the number of
    // nonzero bits which is the number of bits in the mask.
    //
