...
class Ipv4L3Protocol : public Ipv4
{
  public:
     ...
     *
     * This function checks all the node's interfaces and the respective subnet
     * masks. An address is considered unicast if it's not broadcast,
     * subnet-broadcast or multicast.
     *
     * \param ad address
     *
  ...
  protected:
    ...
    /**
     * This function will notify other components connected to the node that a new
     * stack member is now connected This will be used to notify Layer 3 protocol
     * of layer 4 protocol stack to connect them together.
     */
  ...
  private:
    ...
    /**
     * \brief Container of NetDevices registered to IPv4 and their interface
     * indexes.
     */
    ...
    Ipv4InterfaceReverseContainer m_reverseInterfacesContainer; //!< Container of NetDevice /
                                                                //!< Interface index associations.
    ...
    // <ip-header, payload, reason, ifindex> (ifindex not valid if reason is
    // DROP_NO_ROUTE)
    ...
    /**
     * \brief A Set of Fragment belonging to the same packet (src, dst,
     * identification and proto)
     */
    ...
    /// Container of fragments, stored as pairs(src+dst addr, src+dst port) /
    /// fragment
    ...
    /// RFC 6621 recommended duplicate packet tuple: {IPV hash, IP protocol, IP
