 ...
 *
 *  YeAH-TCP (Yet Another HighSpeed TCP) is a heuristic designed to balance
 * various requirements of a state-of-the-art congestion control algorithm: 1)
 * fully exploit the link capacity of high BDP networks while inducing a small
...
class TcpYeah : public TcpNewReno
{
  public:
     ...
     *
     * We only start a YeAH cycle when we are in normal congestion state (CA_OPEN
     * state).
     *
     * \param tcb internal congestion state
  ...
  private:
    uint32_t m_alpha;   //!< Maximum backlog allowed at the bottleneck queue; Q_max
                        //!< in the paper
    uint32_t m_gamma;   //!< Fraction of queue to be removed per RTT when
                        //!< precautionary decongestion executed
    ...
    uint32_t m_rho;     //!< Minimum number of consecutive RTT to consider
                        //!< competition with Reno flows on loss
    ...
    Time m_baseRtt;               //!< Minimum of all YeAH RTT measurements seen during
                                  //!< connection
