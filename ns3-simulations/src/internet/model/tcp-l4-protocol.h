 ...
 * is done through the SendPacket function, which sends the packet down the
 * stack.
 *
 * Moreover, this class allocates "endpoint" objects (ns3::Ipv4EndPoint) for
 * TCP, and SHOULD checksum packets its receives from the socket layer going
 * down the stack, but currently checksumming is disabled.
 *
 * \see CreateSocket
...
class TcpL4Protocol : public IpL4Protocol
{
  public:
    ...
    // NOTE: API from here should not be removed, only added. Be
    // backward-compatible!
  ...
  protected:
    ...
    /**
     * \brief Get the tcp header of the incoming packet and checks its checksum if
     * needed
     *
     ...
     * \param destination Destination address (an underlying Ipv4Address or
     * Ipv6Address)
     *
