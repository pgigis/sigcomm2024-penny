...
class GlobalRoutingLinkRecord
{
  public:
     ...
     * network mask
     *
     * @returns The Ipv4Address corresponding to the Link Data field of the
     * record.
     */
     ...
     *
     * @see LinkType
     * @param linkType The new LinkType for the current Global Routing Link
     * Record.
     */
...
class GlobalRoutingLSA
{
  public:
    ...
    /**
     * @brief Check to see if the list of Global Routing Link Records present in
     * the Global Routing Link State Advertisement is empty.
     *
    ...
    /**
     * @brief Print the contents of the Global Routing Link State Advertisement
     * and any Global Routing Link Records present in the list.  Quite verbose.
...
class GlobalRouter : public Object
{
  public:
     ...
     * @return whether the operation succeeded (will return false if no such
     * route)
     *
  ...
  private:
    ...
    /**
     * \brief Link through the given channel and find the net device that's on the
     * other end.
     *
     ...
     *
     * Given a local net device, we need to walk the channel to which the net
     * device is attached and look for nodes with GlobalRouter interfaces on them
     * (one of them will be us).  Of these, the router with the lowest IP address
     * on the net device connecting to the channel becomes the designated router
     * for the link.
     *
    ...
    /**
     * \brief Build one NetworkLSA for each net device talking to a network that
     * we are the designated router for.
     *
     * \param c the devices.
     */
    ...
    /**
     * \brief Decide whether or not a given net device is being bridged by a
     * BridgeNetDevice.
     *
     * \param nd the NetDevice
    ...
    typedef std::list<Ipv4RoutingTableEntry*>::const_iterator
        InjectedRoutesCI; //!< Const Iterator to container of
