...
class TcpTxBuffer : public Object
{
  public:
     ...
     *
     * \returns number of segments that have been transmitted more than once,
    ...
    /**
     * \brief Get the number of segments that have been explicitly sacked by the
     * receiver. \return the number of sacked segment.
     */
    ...
    /**
     * \brief Returns the number of bytes from the buffer in the range [seq,
     * tailSequence)
     *
     ...
     *
     * In the following, we refer to the block [seq, seq+numBytes) simply as
     * "block". We check the boundary of the block, and divide the possibilities
     * in three cases:
     *
     * - the block have already been transmitted (managed in
     ...
     * not transmitted. We trick this case by requesting the portion not
     * transmitted from GetNewSegment, and then calling GetTransmittedSegment with
     ...
     * Please do not delete the pointer, nor modify Packet data or sequence
     * numbers.
     */
     ...
     * \param sackedCb Callback invoked, if it is not null, when a segment has
     * been SACKed by the receiver. \returns the number of bytes newly sacked by
     * the list of blocks
     */
     ...
     *
     * It does a check on the flags to determine if the segment has to be
     * considered as lost for an external class
     *
     ...
     *
     * \param seq Next sequence number to transmit, based on the scoreboard
     * information \param seqHigh Maximum sequence number to transmit, based on
     * SMSS and/or receiver window \param isRecovery true if the socket congestion
     * state is in recovery mode \return true is seq is updated, false otherwise
     */
     ...
     *
     * sentsize is SND.NXT-SND.UNA, retrans is the number of retransmitted
     * segments. leftOut is the number of segment that left the network without
     * being ACKed:
     *
     ...
     *
     * Used to set all the sent list as lost, so the bytes in flight is not
     * counting them as in flight, but we will continue to use SACK information
     * for recovering the timeout.
     *
     ...
     * flag on the discarded item. As example, if the implementation discard an
     * item that is marked as sacked, the sackedOut count is decreased
     * accordingly.
     */
    void AddRenoSack();
  ...
  private:
     ...
     *
     * - RFC 6675 algorithm, which says that if more than "Dupack thresh" (e.g.,
     * 3) sacked segments above the sequence, then we can consider the sequence
     * lost;
     ...
     * \param size size to remove (can be different from pktSize because of
     * fragmentation)
     */
     ...
     *
     * After these operations, the requested block is exactly the resulting
    packet.
     * Merge operation is required when the requested block span over two (or
    more)
     * existing packets.
     *
     ...
     * \param listEdited output parameter which indicates if the list has been
    edited
     * \return the item that contains the right packet
     */
