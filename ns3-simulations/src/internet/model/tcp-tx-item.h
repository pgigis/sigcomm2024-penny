...
class TcpTxItem
{
  public:
     ...
     *
     * \return 1 if the packet size is 0 or there's no packet, otherwise the size
     * of the packet
     */
    ...
    struct RateInformation
    {
        ...
        Time m_deliveredTime{Time::Max()}; //!< Connection's delivered time at the
                                           //!< time the packet was sent
        Time m_firstSent{Time::Max()};     //!< Connection's first sent time at the
                                           //!< time the packet was sent
  ...
  private:
    ...
    Time m_lastSent{Time::Max()};   //!< Timestamp of the time at which the segment
                                    //!< has been sent last time
