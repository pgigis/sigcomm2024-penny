 ...
 * backlog at the bottleneck queue.
 *
 * Vegas continuously measures the actual throughput a connection achieves as
 * shown in Equation (1) and compares it with the expected throughput calculated
 * in Equation (2). The difference between these 2 sending rates in Equation (3)
 * reflects the amount of extra packets being queued at the bottleneck.
 *
 *              actual = cwnd / RTT        (1)
 ...
 * default values of alpha, beta, and gamma, respectively.
 *
...
class TcpVegas : public TcpNewReno
{
  public:
     ...
     *
     * We only start a Vegas cycle when we are in normal congestion state (CA_OPEN
     * state).
     *
     * \param tcb internal congestion state
  ...
  private:
    ...
    Time m_baseRtt;               //!< Minimum of all Vegas RTT measurements seen during
                                  //!< connection
