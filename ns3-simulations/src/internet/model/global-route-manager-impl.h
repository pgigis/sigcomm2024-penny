...
class SPFVertex
{
  public:
     ...
     *
     * @returns The distance, in hops, from the root SPFVertex to "this"
     * SPFVertex.
     */
     ...
     *
     * In this method we are telling the root node which exit direction it should
     * send should I send a packet to the network or host represented by 'this'
     * SPFVertex.
     *
     * @see GlobalRouter
     * @see GlobalRoutingLSA
     ...
     * @see GlobalRoutingLSA
     * @see GlobalRoutingLinkRecord
     * @param exit The pair of next-hop-IP and outgoing-interface-index to use
     * when forwarding packets to the host or network represented by "this"
     * SPFVertex.
     */
    ...
    /**
     * \brief Inherit all root exit directions from a given vertex to 'this'
     * vertex \param vertex The vertex from which all root exit directions are to
     * be inherited
     *
    ...
    /**
     * \brief Get the number of exit directions from root for reaching 'this'
     * vertex \return The number of exit directions from root
     */
  ...
  private:
    ...
    bool m_vertexProcessed; //!< Flag to note whether vertex has been processed
                            //!< in stage two of SPF computation
...
class GlobalRouteManagerLSDB
{
  public:
     ...
     * prior to each SPF calculation to reset the state of the SPFVertex
     * structures that will reference the LSAs during the calculation.
     *
     * @see GlobalRoutingLSA
     * @see SPFVertex
     */
    ...
    /**
     * @brief Look up the External Link State Advertisement associated with the
     * given index.
     *
...
class GlobalRouteManagerImpl
{
  ...
  private:
    ...
    /**
     * \brief Examine the links in v's LSA and update the list of candidates with
     * any vertices not already on the list
     *
     * \internal
     *
     ...
     * SPF candidate queue, which is a priority queue containing the shortest
     * paths to the networks we know about.
     *
    ...
    /**
     * \brief Calculate nexthop from root through V (parent) to vertex W
    ...
    /**
     * \brief Return the interface number corresponding to a given IP address and
     * mask
     *
