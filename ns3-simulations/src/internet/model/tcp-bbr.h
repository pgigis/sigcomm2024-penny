 ...
 *
 * This class implement the BBR (Bottleneck Bandwidth and Round-trip propagation
 * time) congestion control type.
 */
class TcpBbr : public TcpCongestionOps
{
  ...
  protected:
    ...
    /**
     * \brief Advances pacing gain using cycle gain algorithm, while in
     * BBR_PROBE_BW state
     */
    ...
    /**
     * \brief Called on exiting from BBR_PROBE_RTT state, it eithers invoke
     * EnterProbeBW () or EnterStartup ()
     */
    void ExitProbeRTT();
    ...
    /**
     * \brief Checks whether to move to next value of pacing gain while in
     * BBR_PROBE_BW. \param tcb the socket state. \param rs  rate sample. \returns
     ...
     *        the latest congestion window unmodulated by loss recovery or
     * ProbeRTT. \param tcb the socket state.
     */
    ...
    /**
     * \brief Updates control parameters congestion windowm, pacing rate, send
     * quantum. \param tcb the socket state. \param rs rate sample.
     */
  ...
  private:
    ...
    uint32_t m_minPipeCwnd{0};  //!< The minimal congestion window value BBR tries
                                //!< to target, default 4 Segment size
    uint32_t m_roundCount{0};   //!< Count of packet-timed round trips
    bool m_roundStart{false};   //!< A boolean that BBR sets to true once per
    ...
    Time m_probeRtPropStamp{Seconds(0)};  //!< The wall clock time at which the current BBR.RTProp
                                          //!< sample was obtained.
    ...
    Time m_rtProp{Time::Max()};          //!< Estimated two-way round-trip propagation delay of the
                                         //!< path, estimated from the windowed minimum recent
                                         //!< round-trip delay sample.
    uint32_t m_sendQuantum{0};           //!< The maximum size of a data aggregate
    ...
    Time m_rtPropStamp{Seconds(0)};      //!< The wall clock time at which the current
    ...
    uint32_t m_appLimited{0};             //!< The index of the last transmitted packet
                                          //!< marked as application-limited
    uint32_t m_txItemDelivered{0};        //!< The number of bytes already delivered at
                                          //!< the time of new packet transmission
    ...
    uint32_t m_ackEpochAckedResetThresh{1 << 17}; //!< Max allowed val for m_ackEpochAcked, after
                                                  //!< which sampling epoch is reset
