 ...
 *
 * The RIPng protocol (\RFC{2080}) is a unicast-only IPv6 IGP (Interior Gateway
 * Protocol). Its convergence time is rather long. As a consequence, it is
 * suggested to carefully check the network topology and the route status before
 * sending data flows.
 *
 * RIPng implements the Bellman-Ford algorithm (although the RFC does not state
 * it). Bellman-Ford algorithm convergence time is O(|V|*|E|) where |V| and |E|
 * are the number of vertices (routers) and edges (links) respectively. Since
 * unsolicited updates are exchanged every 30 seconds, the convergence might
 * require a long time.
 *
 ...
 * it a router will announce to be the default router *and* more RTEs, which is
 * silly.
 */
...
class RipNgRoutingTableEntry : public Ipv6RoutingTableEntry
{
  public:
     ...
     * \param prefixToUse prefix that should be used for source address for this
     * destination
     */
...
class RipNg : public Ipv6RoutingProtocol
{
  public:
    ...
    /**
     * \brief Add a default route to the router through the nextHop located on
     * interface.
     *
  ...
  private:
    /// Container for the network routes - pair RipNgRoutingTableEntry *, EventId
    /// (update event)
    ...
    Time m_minTriggeredUpdateDelay; //!< Min cooldown delay after a Triggered
                                    //!< Update.
    Time m_maxTriggeredUpdateDelay; //!< Max cooldown delay after a Triggered
                                    //!< Update.
    ...
    // note: we can not trust the result of socket->GetBoundNetDevice
    // ()->GetIfIndex (); it is dependent on the interface initialization (i.e.,
    // if the loopback is already up).
    /// Socket list type
    ...
    SocketList m_unicastSocketList;    //!< list of sockets for unicast messages
                                       //!< (socket, interface index)
