...
class Ipv4GlobalRouting : public Ipv4RoutingProtocol
{
  public:
     ...
     * n entries.  The one subtlety of note is that if a default route has been
     * set it will appear as the zeroth entry in the table.  This means that if
     * you add only a default route, the table will have one entry that can be
     * accessed either by explicitly calling GetDefaultRoute () or by calling
     * GetRoute (0).
     *
     ...
     * n entries.  The one subtlety of note is that if a default route has been
     * set it will appear as the zeroth entry in the table.  This means that if
     * the default route has been set, calling RemoveRoute (0) will remove the
     * default route.
     *
  ...
  private:
    ...
    /// Set to true if this interface should respond to interface events by
    /// globallly recomputing routes
    ...
    /// const iterator of container of Ipv4RoutingTableEntry (routes to external
    /// AS)
