 ...
 *
 * Only three methods have been implemented right now; however, Linux has many
 * others, which can be added later in ns-3.
 *
 * \see IncreaseWindow
 * \see PktsAcked
 */
class TcpCongestionOps : public Object
{
  public:
     ...
     * instantly recover from a loss event (e.g. when there is a network with an
     * high reordering factor).
     *
     * \param tcb internal congestion state
     ...
     *
     * Mimic the function \pname{cong_avoid} in Linux. New segments have been
     * ACKed, and the congestion control duty is to update the window.
     *
     ...
     * TcpSocketState::CA_RECOVERY); m_tcb->m_congState =
     * TcpSocketState::CA_RECOVERY; \endcode
     *
     * \param tcb internal congestion state
    ...
    /**
     * \brief Returns true when Congestion Control Algorithm implements
     * CongControl
     *
    ...
    //     void (*pkts_acked)(struct sock *sk, u32 ext, int *attr, union
    //     tcp_cc_info *info);
