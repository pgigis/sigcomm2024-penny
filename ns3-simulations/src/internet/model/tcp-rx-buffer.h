 ...
 *
 * The first useful sequence that this class is waiting is returned by the
 * method NextRxSequence, and could be set at the beginning through
 * MaxRxSequence.
 *
 * The max. size of this buffer is managed through SetMaxBufferSize, and could
 * be retrieved using MaxBufferSize. The current size instead is returned by
...
class TcpRxBuffer : public Object
{
  public:
    ...
    /**
     * \brief Check if the buffer did receive all the data (and the connection is
     * closed) \returns true if all data have been received
     */
    bool Finished();
  ...
  private:
     ...
     *
     * In fact, the maximum amount of blocks is 4, and if we consider the
     * timestamp (or other) options, it is even less. For more detail about this
     * function, please see the source code and in-line comments.
     *
    ...
    uint32_t m_size;           //!< Number of total data bytes in the buffer, not
                               //!< necessarily contiguous
    uint32_t m_maxBuffer;      //!< Upper bound of the number of data bytes in buffer
                               //!< (RCV.WND)
    uint32_t m_availBytes;     //!< Number of bytes available to read, i.e.
