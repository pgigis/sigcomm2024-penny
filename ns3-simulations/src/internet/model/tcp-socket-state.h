...
class TcpSocketState : public Object
{
  public:
     ...
     *
     * These states represent the situation from a congestion control point of
     * view: in fact, apart the CA_OPEN state, the other states represent a
     * situation in which there is a congestion, and different actions should be
    ...
    enum TcpCongState_t
    {
        ...
        CA_DISORDER,  //!< In all the respects it is "Open",
                      //!< but requires a bit more attention. It is entered when we
                      //!< see some SACKs or dupacks. It is split of "Open".
        CA_CWR,       //!< cWnd was reduced due to some congestion notification event,
                      //!< such as ECN, ICMP source quench, local device congestion.
    ...
    enum EcnMode_t
    {
        ...
        DctcpEcn,   //!< ECN functionality as described in RFC 8257. Note: this mode
                    //!< is specific to DCTCP.
    };
    ...
    enum EcnState_t
    {
        ...
        ECN_CWR_SENT      //!< Sender has reduced the congestion window, and sent a
                          //!< packet with CWR bit set in TCP header. This state is used
                          //!< for tracing.
    };
    ...
    TracedValue<uint32_t> m_cWndInfl{0}; //!< Inflated congestion window trace (used only for
    ...
    // Recovery
    // This variable is used for implementing following flag of Linux:
    // FLAG_RETRANS_DATA_ACKED and is used only during a recovery phase to keep
    // track of acknowledgement of retransmitted packet.
    ...
    TracedValue<EcnState_t> m_ecnState{ECN_DISABLED}; //!< Current ECN State, represented as
                                                      //!< combination of EcnState values
    ...
    uint32_t m_lastAckedSackedBytes{
        0}; //!< The number of bytes acked and sacked as indicated by the current
            //!< ACK received. This is similar to acked_sacked variable in Linux
