...
class Ipv6L3Protocol : public Ipv6
{
  public:
     ...
     *
     * This function is used by Fragment Timeout handling to signal a fragment
     * drop.
     *
    ...
    /**
     * Removes a multicast address from the list of addresses to pass to local
     * deliver. \param address the address.
     */
    void RemoveMulticastAddress(Ipv6Address address);

    /**
     * Removes a multicast address from the list of addresses to pass to local
     * deliver. \param address the address. \param interface the incoming
     * interface.
     */
     ...
     *
     * This function shall be called by L4-L7 protocols when an address is
     * confirmed to be reachable (i.e., at least a packet send and a reply
     * received). The net effect is to extend the NCE reachability time if the NCE
     * is in REACHABLE state, and to mark the NCE as REACHABLE if it is in STALE,
     * PROBE, or DELAY states. NCEs in INCOMPLETE state are not changed.
     *
     * Note that the IP interface index might not be the same as the NetDevice
     * index. The correct way to check the IP interface index is by using
  ...
  protected:
    ...
    /**
     * \brief Notify other components connected to the node that a new stack
     * member is now connected.
     *
     * This will be used to notify Layer 3 protocol of layer 4 protocol stack to
  ...
  private:
    ...
    /**
     * \brief Container of NetDevices registered to IPv6 and their interface
     * indexes.
     */
    ...
    /**
     * \brief Make a copy of the packet, add the header and invoke the TX trace
     * callback \param ipHeader the IP header that will be added to the packet
    ...
    /**
     * \brief Rejects packets directed to an interface with wrong address
     * (\RFC{1222}).
     */
