...
class LrWpanNetDevice : public NetDevice
{
  public:
     ...
     * which the device or coordinator is associated. When assigning a short
     * address the extended address must also be present.
     *
     * \param panId The id of the PAN used by the coordinator device.
     *
     * \param coordExtAddr The coordinator extended address (EUI-64) through which
     * this device or coordinator is associated.
     *
     * \param coordShortAddr The coordinator assigned short address through which
     * this device or coordinator is associated. [FF:FF] address indicates that
     * the value is unknown. [FF:FE] indicates that the associated coordinator is
     * using only its extended address.
     *
     *
     * \param assignedShortAddr The assigned short address for this device.
     *                          [FF:FF] address indicates that the device have no
     * short address and is not associated. [FF:FE] address indicates that the
     * devices has associated but has not been allocated a short address.
     *
     */
     ...
     *
     * \param params 802.15.4 specific parameters, including source and
     * destination addresses \param pkt the packet do be delivered
     */
  ...
  private:
     ...
     * However, the U/L bit flipping is performed when the IPv6 address is
     * created.
     *
     ...
     *
     * According to \RFC{4944} the pseudo-MAC is YYYY:0000:XXXX (with U/L bit set
     * to local) According to \RFC{6282} the pseudo-MAC is 0200:0000:XXXX
     */
