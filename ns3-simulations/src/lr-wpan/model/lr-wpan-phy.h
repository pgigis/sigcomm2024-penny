 ...
 * \ingroup lr-wpan
 *
 * This data structure provides number of symbols for the PPDU headers: SHR and
 * PHR See IEEE802.15.4-2006 Figure 16, Table 19 and 20 in section 6.3
 */
...
struct LrWpanPhyPibAttributes : public SimpleRefCount<LrWpanPhyPibAttributes>
{
    ...
    uint32_t phyChannelsSupported[32]; //!< BitField representing the available
                                       //!< channels supported by a channel page.
    uint8_t phyTransmitPower;          //!< 2 MSB: tolerance on the transmit power, 6 LSB:
                                       //!< Tx power in dBm relative to 1mW (signed int in
                                       //!< 2-complement format)
    ...
    uint32_t phySHRDuration;           //!< The duration of the synchronization header
                                       //!< (SHR) in symbols
...
class LrWpanPhy : public SpectrumPhy
{
  public:
     ...
     *
     * In ns-3 , rx sensitivity is only checked to be at least what is specified
     * by the standard (-85dBm or -92dBm for BPSK bands). Most vendors provide
     * better sensitivity options and exceed the minimum values proposed by the
     * standard. Default sensitivity is -106.58 dBm (This does not include any
     * noise figure). A sensitivity of -95dBm or less is considered by many
     * vendors a high sensitivity.
     *
     ...
     *
     * @param params the SpectrumSignalParameters associated with the incoming
     * waveform
     */
    ...
    /**
     * Get The current channel page number in use in this PHY from the PIB
     * attributes.
     *
  ...
  private:
     ...
     *
     * \param packet the packet for which the transmission time should be
     * calculated \return the time required for transmitting the packet
     */
    ...
    /**
     * Calculates the nominal transmit power of the device in decibels relative to
     * 1 mW according to the representation of the PIB attribute phyTransmitPower.
     *
     * The phyTransmitPower (table 23 of IEEE 802.15.4-2006) is a 8-bit bitmap,
     * stored as a `uint8_t`. The bitmap has the following meaning: The 2 MSBs
     * represent the tolerance on the transmit power. The 6 LSBs represent a
     * signed integer in twos-complement format, corresponding to the nominal
     * transmit power of the device in decibels relative to 1 mW.
     *
     * This function extracts the 6 LSBs corresponding to the nominal transmit
     * power and converts it to a `int8_t`.
     *
    ...
    /**
     * This callback is used to report packet transmission status to the MAC
     * layer. See IEEE 802.15.4-2006, section 6.2.1.2.
     */
    ...
    /**
     * Timestamp of the last calculation of the PER of a packet currently
     * received.
     */
    Time m_rxLastUpdate;
