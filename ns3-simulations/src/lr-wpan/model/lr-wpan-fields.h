...
class SuperframeField
{
  public:
     ...
     * \param battLifeExt Sets true or false the value of the Battery Life
     * Extension flag of the superframe field.
     */
    void SetBattLifeExt(bool battLifeExt);
    /**
     * Set the Superframe Specification PAN coordinator field.
     * \param panCoor set true or false the value for the PAN Coordinator flag of
     * the superframe field.
     */
    void SetPanCoor(bool panCoor);
    /**
     * Set the Superframe Specification Association Permit field.
     * \param assocPermit set true or false the value of the Association Permit
     * flag of the superframe field.
     */
     ...
     * \param i an iterator which points to where the superframe specification
     * field should be written. \return an iterator.
     */
    Buffer::Iterator Serialize(Buffer::Iterator i) const;
    /**
     * Deserialize the entire superframe specification field.
     * \param i an iterator which points to where the superframe specification
     * field should be read. \return an iterator.
     */
  ...
  private:
    ...
    uint8_t m_sspecBcnOrder;    //!< Superframe Specification field Beacon Order
                                //!< (Bit 0-3)
    uint8_t m_sspecSprFrmOrder; //!< Superframe Specification field Superframe
                                //!< Order (Bit 4-7)
    uint8_t m_sspecFnlCapSlot;  //!< Superframe Specification field Final CAP slot
                                //!< (Bit 8-11)
    bool m_sspecBatLifeExt;     //!< Superframe Specification field Battery Life
                                //!< Extension (Bit 12) Superframe Specification
                                //!< field Reserved (not necessary) (Bit 13)
    bool m_sspecPanCoor;        //!< Superframe Specification field PAN Coordinator (Bit
                                //!< 14)
    bool m_sspecAssocPermit;    //!< Superframe Specification field Association
                                //!< Permit (Bit 15)
};
...
class GtsFields
{
  public:
    ...
    /**
     * Get the GTS Specification Permit. TRUE if coordinator is accepting GTS
     * requests. \return True if the coordinator is accepting GTS request.
     */
  ...
  private:
    ...
    {
        Mac16Address m_gtsDescDevShortAddr; //!< GTS Descriptor Device Short
                                            //!< Address (Bit 0-15)
    ...
    uint8_t m_gtsSpecDescCount; //!< GTS specification field Descriptor Count
                                //!< (Bit 0-2)
...
class PendingAddrFields
{
  public:
    ...
    /**
     * Get the whole Pending Address Specification Field from the Pending Address
     * Fields. \return The Pending Address Specification Field.
     */
    uint8_t GetPndAddrSpecField() const;
    /**
     * Get the number of Short Pending Address indicated in the Pending Address
     * Specification Field. \return The number Short Pending Address.
     */
    uint8_t GetNumShortAddr() const;
    /**
     * Get the number of Extended Pending Address indicated in the Pending Address
     * Specification Field. \return The number Short Pending Address.
     */
    uint8_t GetNumExtAddr() const;

    /**
     * Set the whole Pending Address Specification field. This field is part of
     * the Pending Address Fields header. \param pndAddrSpecField The Pending
     ...
     * \param i an iterator which points to where the Pending Address Fields
     * should be written. \return an iterator.
     */
    Buffer::Iterator Serialize(Buffer::Iterator i) const;
    /**
     * Deserialize the all the Pending Address Fields.
     * \param i an iterator which points to where the Pending Address Fields
     * should be read. \return an iterator.
     */
    Buffer::Iterator Deserialize(Buffer::Iterator i);

  private:
    // Pending Address Specification Field
    uint8_t m_pndAddrSpecNumShortAddr; //!< Pending Address Specification field
                                       //!< Number of Short Address (Bits 0-2)
                                       //!< Pending Address Specification field
                                       //!< Reserved (Not Necessary)(Bit  3)
    uint8_t m_pndAddrSpecNumExtAddr;   //!< Pending Address Specification field
                                       //!< Number of Extended Address (Bits 4-6)
                                       //!< Pending Address Specification field
...
class CapabilityField
{
  public:
    ...
    /**
     * True if the device type is a Full Functional Device (FFD) false if is a
     * Reduced Functional Device (RFD).
     *
     * \return True if the device type is a Full Functional Device (FFD) false if
     * is a Reduced Functional Device (RFD).
     */
     ...
     *
     * \return True if the device is receiving power from alternating current
     * mains.
     */
    bool IsPowSrcAvailable() const;

    /**
     * True if the device does not disable its receiver to conserve power during
     * idle periods.
     *
     * \return True if the device does not disable its receiver to conserve power
     * during idle periods.
     */
    bool IsReceiverOnWhenIdle() const;

    /**
     * True if the device is capable of sending and receiving cryptographically
     * protected MAC frames.
     *
     * \return True if the device is capable of sending and receiving
     * cryptographically protected MAC frames.
     */
    bool IsSecurityCapability() const;

    /**
     * True if the device wishes the coordinator to allocate a short address as
     * result of the association procedure.
     *
     * \return True if the device wishes the coordinator to allocate a short
     ...
     * True = full functional device (FFD)  False = reduced functional device
     * (RFD).
     *
     * \param devType The device type described in the Capability Information
     * Field.
     */
     ...
     *
     * \param pow Set true if a Power Source is available in the Capability
     * Information Field.
     */
     ...
     *
     * \param addrAlloc Describes whether or not the coordinator should allocate a
     * short address in the association process.
     */
  ...
  private:
    ...
    bool m_receiverOnWhenIdle; //!< Capability Information Field, Receiver On
                               //!< When Idle (bit 3)
    uint8_t m_reservedBit45;   //!< Capability Information Field, Reserved (bit 4 & 5)
    bool m_securityCap;        //!< Capability Information Field, Security Capability
                               //!< (bit 6)
