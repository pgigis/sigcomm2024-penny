 ...
 * \ingroup lr-wpan
 *
 * This method informs the transaction cost in a slotted CSMA-CA data
 * transmission. i.e. Reports number of symbols (time) it would take slotted
 * CSMA-CA to process the current transaction. 1 Transaction = 2 CCA + frame
 * transmission (PPDU) + turnaroudtime or Ack time (optional) + IFS See IEEE
 * 802.15.4-2011 (Sections 5.1.1.1 and 5.1.1.4)
 */
...
class LrWpanCsmaCa : public Object
{
  public:
    ...
    /**
     * Start CSMA-CA algorithm (step 1), initialize NB, BE for both slotted and
     * unslotted CSMA-CA. For slotted CSMA-CA initializes CW and starts the
     * backoff slot count.
     */
    void Start();
    /**
    ...
    void Cancel();
    /**
     * In step 2 of the CSMA-CA, perform a random backoff in the range of 0 to
     * 2^BE -1
     */
    ...
    void RequestCCA();
    /**
     * The CSMA algorithm call this function at the end of the CAP to return the
     * MAC state back to to IDLE after a transmission was deferred due to the lack
     * of time in the CAP.
     */
     ...
     *
     * When Phy has completed CCA, it calls back here which in turn execute the
     * final steps of the CSMA-CA algorithm. It checks to see if the Channel is
     * idle, if so check the Contention window  before permitting transmission
     * (step 5). If channel is busy, either backoff and perform CCA again or treat
     * as channel access failure (step 4).
     */
    void PlmeCcaConfirm(LrWpanPhyEnumeration status);
    /**
     * Set the callback function to report a transaction cost in slotted CSMA-CA.
     * The callback is triggered in CanProceed() after calculating the transaction
     * cost (2 CCA checks,transmission cost, turnAroundTime, ifs) in the boundary
     * of an Active Period.
     *
    ...
    /**
     * Set the callback function to the MAC. Used at the end of a Channel
     * Assessment, as part of the interconnections between the CSMA-CA and the
     * MAC. The callback lets MAC know a channel is either idle or busy.
     *
     ...
     *
     * \param batteryLifeExtension the Battery Life Extension value active or
     * inactive
     */
  ...
  private:
    ...
    /**
     * \brief Get the time left in the CAP portion of the Outgoing or Incoming
     * superframe. \return the time left in the CAP
     */
    ...
    /**
     * Indicates whether the CSMA procedure is targeted for a message to be sent
     * to the coordinator. Used to run slotted CSMA/CA on the incoming or outgoing
     * superframe according to the target.
     */
    bool m_coorDest;
};
