...
class BeaconPayloadHeader : public Header
{
  public:
    ...
    /**
     * Set the superframe Guaranteed Time Slot (GTS) fields to the beacon payload
     * header. \param gtsFields The GTS fields.
     */
...
class CommandPayloadHeader : public Header
{
  public:
    ...
    /**
     * Set the Capability Information Field to the command payload header
     * (Association Request Command). \param cap The capability Information field
     */
    ...
    /**
     * Set status resulting from the association attempt (Association Response
     * Command). \param status The status resulting from the association attempt
     */
    ...
    /**
     * Get the status resulting from an association request (Association Response
     * Command). \return The resulting status from an association request
     */
  ...
  private:
    ...
    CapabilityField m_capabilityInfo; //!< Capability Information Field
                                      //!< (Association Request Command)
    Mac16Address m_shortAddr;         //!< Contains the short address assigned by the
                                      //!< coordinator (Association Response and
                                      //!< Coordinator Realiagment Command)
    ...
    uint8_t m_logChPage;              //!< The channel page number (Coordinator realigment
                                      //!< command)
