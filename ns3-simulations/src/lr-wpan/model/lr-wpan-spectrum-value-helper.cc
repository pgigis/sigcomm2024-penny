...
Ptr<SpectrumModel> g_LrWpanSpectrumModel; //!< Global object used to initialize
                                          //!< the LrWpan Spectrum Model

/**
 * \ingroup lr-wpan
 * \brief Helper class used to automatically initialize the LrWpan Spectrum
 * Model objects
 */
...
} g_LrWpanSpectrumModelInitializerInstance; //!< Global object used to
                                            //!< initialize the LrWpan Spectrum
                                            //!< Model
...
LrWpanSpectrumValueHelper::CreateNoisePowerSpectralDensity(uint32_t channel)
{
    ...
    // noise Floor (W) which accounts for thermal noise and non-idealities of the
    // receiver
