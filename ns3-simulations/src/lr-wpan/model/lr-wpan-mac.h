 ...
 *
 * This section documents the API of the IEEE 802.15.4-related models.  For a
...
struct PanDescriptor
{
    ...
    Mac16Address m_coorShortAddr;                 //!< The coordinator short address as specified
                                                  //!< in the coordinator address mode.
    Mac64Address m_coorExtAddr;                   //!< The coordinator extended address as
                                                  //!< specified in the coordinator address mode.
    ...
    SuperframeField m_superframeSpec; //!< The superframe specification as
    ...
    uint8_t m_linkQuality{0};         //!< The LQI at which the network beacon was
                                      //!< received. Lower values represent lower LQI.
    Time m_timeStamp;                 //!< Beacon frame reception time. Used as Time data type in
                                      //!< ns-3 to avoid precision problems.
};
...
struct McpsDataConfirmParams
{
    ...
    LrWpanMcpsDataConfirmStatus m_status{
        IEEE_802_15_4_INVALID_PARAMETER}; //!< The status of the last MSDU
                                          //!< transmission
};
...
{
    Mac64Address m_extDevAddr;      //!< The extended address of the device requesting
                                    //!< association
    CapabilityField capabilityInfo; //!< The operational capabilities of
                                    //!< the device requesting association.
    uint8_t lqi{0};                 //!< The link quality indicator of the received associate
                                    //!< request command (Not officially supported in the
                                    //!< standard but found in implementations)
};
...
struct MlmeAssociateResponseParams
{
    Mac64Address m_extDevAddr;     //!< The extended address of the device requesting
                                   //!< association
    Mac16Address m_assocShortAddr; //!< The short address allocated by the coordinator on
                                   //!< successful assoc. FF:FF = Unsuccessful
...
struct MlmeStartRequestParams
{
    ...
    uint32_t m_logChPage{0};   //!< Logical channel page on which to start using
                               //!< the new superframe configuration.
    ...
    uint8_t m_bcnOrd{15};      //!< Beacon Order, Used to calculate the beacon interval, a value of
                               //!< 15 indicates no periodic beacons will be transmitted.
    uint8_t m_sfrmOrd{15};     //!< Superframe Order, indicates the length of the CAP
                               //!< in time slots.
    bool m_panCoor{false};     //!< On true this device will become coordinator.
    bool m_battLifeExt{false}; //!< Flag indicating whether or not the Battery
                               //!< life extension (BLE) features are used.
    bool m_coorRealgn{false};  //!< True if a realignment request command is to be
                               //!< transmitted prior changing the superframe.
};
...
{
    uint8_t m_logCh{11};    //!< The channel number on which to attempt coordinator
                            //!< synchronization.
    bool m_trackBcn{false}; //!< True if the mlme sync with the next beacon and
                            //!< attempts to track future beacons. False if mlme
                            //!< sync only the next beacon.
};
...
{
    LrWpanAddressMode m_coorAddrMode{SHORT_ADDR}; //!< The addressing mode of the coordinator to
                                                  //!< which the pool is intended.
...
struct MlmeAssociateRequestParams
{
    ...
    uint8_t m_coordAddrMode{SHORT_ADDR}; //!< The coordinator addressing mode for
                                         //!< this primitive and subsequent MPDU.
    uint16_t m_coordPanId{0};            //!< The identifier of the PAN with which to associate.
    Mac16Address m_coordShortAddr;       //!< The short address of the coordinator with
                                         //!< which to associate.
    Mac64Address m_coordExtAddr;         //!< The extended address of the coordinator
                                         //!< with which to associate.
    CapabilityField m_capabilityInfo;    //!< Specifies the operational capabilities
 ...
 * \ingroup lr-wpan
 *
 * MLME-BEACON-NOTIFY.indication params. See  802.15.4-2011   Section 6.2.4.1,
 * Table 16
 */
 ...
 * \ingroup lr-wpan
 *
 * MLME-SYNC-LOSS.indication params. See  802.15.4-2011   Section 6.2.13.2,
 * Table 37
 */
struct MlmeSyncLossIndicationParams
{
    LrWpanSyncLossReason m_lossReason{MLMESYNCLOSS_PAN_ID_CONFLICT}; //!< The reason for the lost of
                                                                     //!< synchronization.
    uint16_t m_panId{0}; //!< The PAN identifier with which the device lost
                         //!< synchronization or to which it was realigned.
    uint8_t m_logCh{11}; //!< The channel number on which the device lost
                         //!< synchronization or to which it was realigned.
};
 ...
 * \ingroup lr-wpan
 *
 * MLME-COMM-STATUS.indication params. See  802.15.4-2011   Section 6.2.4.2
 * Table 18
 */
struct MlmeCommStatusIndicationParams
{
    uint16_t m_panId{0}; //!< The PAN identifier of the device from which the frame was
    ...
    Mac16Address m_srcShortAddr;       //!< The short address of the entity from which
                                       //!< the frame causing the error originated.
    Mac64Address m_srcExtAddr;         //!< The extended address of the entity from which
                                       //!< the frame causing the error originated.
...
struct MlmePollConfirmParams
{
    LrWpanMlmePollConfirmStatus m_status{
        MLMEPOLL_INVALID_PARAMETER}; //!< The confirmation status resulting from
                                     //!< a MLME-poll.request.
};
 ...
 * \ingroup lr-wpan
 *
 * This callback is called by the MLME and issued to its next higher layer
 * following a transmission instigated through a response primitive.
 *
 *  Security related parameters and not handle.
 ...
 *
 * This callback is called by the MLME and issued to its next higher layer
 * following the reception of a orphan notification.
 *
 *  Security related parameters and not handle.
...
class LrWpanMac : public Object
{
  public:
     ...
     *
     * \return true, if the receiver is enabled during idle periods, false
     * otherwise
     */
     ...
     * MLME-SCAN.request
     * Request primitive used to initiate a channel scan over a given list of
     * channels.
     *
    ...
    /**
     * Set the callback for the indication of an incoming associate request
     * command. The callback implements MLME-ASSOCIATE.indication SAP of IEEE
     * 802.15.4-2011, section 6.2.2.2.
     *
     * \param c the callback
     */
     ...
     * The callback implements MLME-COMM-STATUS.indication SAP of IEEE
     * 802.15.4-2011, section 6.2.4.2.
     *
     * \param c the callback
     */
    ...
    /**
     * Set the callback for the indication to the reception of an orphan
     * notification. The callback implements MLME-ORPHAN.indication SAP of IEEE
     * 802.15.4-2011, section 6.2.7.1.
     *
     * \param c the callback
     */
     ...
     * The callback implements MLME-BEACON-NOTIFY.indication SAP of IEEE
     * 802.15.4-2011, section 6.2.4.1.
     *
     * \param c the callback
     */
     ...
     * The callback implements MLME-BEACON-NOTIFY.indication SAP of IEEE
     * 802.15.4-2011, section 6.2.13.2.
     *
     * \param c the callback
     */
     ...
     * \param status SUCCESS, UNSUPPORTED_ATTRIBUTE, INVALID_PARAMETER, or
     * READ_ONLY \param id the attributed identifier
     */
     ...
     * It also indicates the start of the Active Period in the Outgoing
     * superframe. See IEEE 802.15.4-2011, section 6.4.2, Table 52.
     */
    Time m_macBeaconTxTime;

    /**
     * The time that the device received its last bit of the beacon frame.
     * It does not indicate the start of the Active Period in the Incoming
     * superframe. Not explicitly listed by the standard but its use is implied.
    ...
    /**
     * The maximum wait time for an association response command after the
     * reception of data request command ACK during the association process. Not
     * explicitly listed by the standard but its use is required for a device to
     * react to the lost of the association response (failure of the association:
     * NO_DATA)
     */
     ...
     * (outgoing superframe). Range 0 - 15 with 15 meaning no beacons are being
     * sent. See IEEE 802.15.4-2011, section 6.4.2, Table 52.
     */
     ...
     * coordinator and indicated in its beacon. This value establish the
     * expiration time of the packets stored in the pending transaction list
     * (indirect transmissions). 1 Unit Period: Beacon-enabled =
     * aBaseSuperframeDuration * 2^BO Non-beacon enabled = aBaseSuperframeDuration
     * See IEEE 802.15.4-2006, section 7.4.2, Table 86.
     */
    ...
    /**
     * Indication of the Slot where the CAP portion of the OUTGOING Superframe
     * ends.
     */
    uint8_t m_fnlCapSlot;

    /**
     * The beaconOrder value of the INCOMING frame. Used by all devices that have
     * a parent. Specification of how often the parent coordinator transmits its
     * beacon. 0 - 15 with 15 means the parent is not currently transmitting
     * beacons.
     */
    ...
    /**
     * Indication of the Slot where the CAP portion of the INCOMING Superframe
     * ends.
     */
    ...
    /**
     * Temporally stores the value of the current m_macPanId when a
     * MLME-SCAN.request is performed. See IEEE 802.15.4-2011, section 5.1.2.1.2.
     */
    ...
    /**
     * The value of the necessary InterFrame Space after the transmission of a
     * packet.
     */
    uint32_t m_ifs;
    ...
    /**
     * Indication of the Interval used by the coordinator to transmit beacon
     * frames expressed in symbols.
     */
     ...
     *
     * \return True if m_txPkt (packet awaiting to be sent) destination is its
     * coordinator
     */
    bool isCoordDest();
    ...
    /**
     * Obtain the number of symbols in the packet which is currently being sent by
     *the MAC layer.
     *
  ...
  private:
    ...
    /**
     * Helper structure for managing pending transaction list elements (Indirect
     * transmissions).
     */
    struct IndTxQueueElement : public SimpleRefCount<IndTxQueueElement>
    {
        ...
        Ptr<Packet> txQPkt;           //!< Queued packet.
        Time expireTime;              //!< The expiration time of the packet in the indirect
                                      //!< transmission queue.
    };
    ...
    /**
     * Used to send a data request command (i.e. Request the coordinator to send
     ...
     *
     * \param rxDataReqPkt The received data request pkt that instigated the
     * Association response command.
     */
    void SendAssocResponseCommand(Ptr<Packet> rxDataReqPkt);

    /**
     * Called after m_assocRespCmdWaitTime timeout while waiting for an
     * association response command.
     */
    ...
    /**
     * Called to send a orphan notification command. This is used by an associated
     * device that has lost synchronization with its coordinator. As described in
     * IEEE 802.15.4-2011 (Section 5.3.6)
     */
    void SendOrphanNotificationCommand();

    /**
     * Called to end a MLME-START.request after changing the page and channel
     * number.
     */
    void EndStartRequest();

    /**
     * Called at the end of the current channel scan (Active or Passive) for a
     * given duration.
     */
    void EndChannelScan();
    ...
    /**
     * Called to end an MLME-ASSOCIATE.request after changing the page and channel
     * number.
     */
    ...
    /**
     * Called if the device is unable to locate a beacon in the time set by
     * MLME-SYNC.request.
     */
    ...
    /**
     * After a successful transmission of a frame (beacon, data) or an ack frame
     * reception, the mac layer wait an Interframe Space (IFS) time and triggers
     * this function to continue with the MAC flow.
     *
     ...
     * \param dst The extended address used an index to obtain an element from the
     * pending transaction list. \param entry The dequeued element from the
     * pending transaction list. \return The status of the dequeue
     */
    ...
    /**
     * This callback is used to notify incoming beacon packets to the upper
     * layers. See IEEE 802.15.4-2011, section 6.2.4.1.
     */
    MlmeBeaconNotifyIndicationCallback m_mlmeBeaconNotifyIndicationCallback;

    /**
     * This callback is used to indicate the loss of synchronization with a
     * coordinator. See IEEE 802.15.4-2011, section 6.2.13.2.
     */
    MlmeSyncLossIndicationCallback m_mlmeSyncLossIndicationCallback;

    /**
     * This callback is used to report the result of a scan on a group of channels
     * for the selected channel page. See IEEE 802.15.4-2011, section 6.2.10.2.
     */
    MlmeScanConfirmCallback m_mlmeScanConfirmCallback;

    /**
     * This callback is used to report the status after a device request an
     * association with a coordinator. See IEEE 802.15.4-2011, section 6.2.2.4.
     */
    MlmeAssociateConfirmCallback m_mlmeAssociateConfirmCallback;

    /**
     * This callback is used to report the status after a device send data command
     * request to the coordinator to transmit data. See IEEE 802.15.4-2011,
     * section 6.2.14.2.
     */
    ...
    /**
     * This callback is used to indicate the reception of an association request
     * command. See IEEE 802.15.4-2011, section 6.2.2.2
     */
    ...
    /**
     * This callback is used to indicate the reception of a orphan notification
     * command. See IEEE 802.15.4-2011, section 6.2.7.1
     */
    ...
    /**
     * The command request packet received. Briefly stored to proceed with
     * operations that take place after ACK messages.
     */
    Ptr<Packet> m_rxPkt;
    ...
    /**
     * The indirect transmit queue used by the MAC pending messages (The pending
     * transaction list).
     */
    ...
    /**
     * The maximum size of the indirect transmit queue (The pending transaction
     * list).
     */
    uint32_t m_maxIndTxQueueSize;

    /**
     * The list of PAN descriptors accumulated during channel scans, used to
     * select a PAN to associate.
     */
    std::vector<PanDescriptor> m_panDescriptorList;

    /**
     * The list of energy measurements, one for each channel searched during an ED
     * scan.
     */
    ...
    /**
     * The parameters used during a MLME-SCAN.request. These parameters are stored
     * here while PLME-SET (set channel page, set channel number) and other
     * operations take place.
     */
    MlmeScanRequestParams m_scanParams;

    /**
     * The parameters used during a MLME-START.request. These parameters are
     * stored here while PLME-SET operations (set channel page, set channel
     * number) take place.
     */
    MlmeStartRequestParams m_startParams;

    /**
     * The parameters used during a MLME-ASSOCIATE.request. These parameters are
     * stored here while PLME-SET operations (set channel page, set channel
     * number) take place.
     */
    ...
    /**
     * Indicates the pending primitive when PLME.SET operation (page or channel
     * switch) is called from within another MLME primitive (e.g. Association,
     * Scan, Sync, Start).
     */
