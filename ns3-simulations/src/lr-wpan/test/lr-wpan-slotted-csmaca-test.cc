...
class LrWpanSlottedCsmacaTestCase : public TestCase
{
  ...
  private:
    ...
    /**
     * \brief Function called to indicated the calculated transaction cost in
     * slotted CSMA-CA \param testcase The TestCase. \param dev The
     * LrWpanNetDevice. \param trans The transaction cost in symbols.
     */
    ...
    Time m_startCap;      //!< The time of the start of the Contention Access Period
                          //!< (CAP).
    Time m_apBoundary;    //!< Indicates the time after the calculation of the
                          //!< transaction cost (A boundary of an Active Period in
                          //!< the CAP)
...
LrWpanSlottedCsmacaTestCase::TransEndIndication(LrWpanSlottedCsmacaTestCase* testcase,
                                                ...
                                                McpsDataConfirmParams params)
{
    // In the case of transmissions with the acknowledgment flag activated, the
    // transmission is only successful if the acknowledgment was received.
...
LrWpanSlottedCsmacaTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ(boundary,
                          0,
                          "Error, the transaction is not calculated on a "
                          "boundary of an Active Period in the CAP");

    // Slotted CSMA-CA needs to precalculate the cost of the transaction to ensure
    // there is enough time in the CAP to complete the transmission. The following
    // checks that such pre-calculation matches the time it took to complete the
    // transmission.

    // The calculated transaction includes the IFS time, so we need to subtract
    // its value to compare it. MPDU = MAC Header + MSDU (payload) Mac Header = 13
    // bytes If the MPDU is > aMaxSIFSFrameSize (18 bytes) then IFS = LIFS (40
    // symbols), else  IFS = SIFS (12 symbols)
    ...
    // The transaction cost here includes the ifsSize and the turnAroundTime
    // (Tx->Rx) therefore we subtract these before the final comparison
    //
