...
/*         [00:01]                 [00:02] [00:03] PAN 5 Coordinator End Device
 * PAN 7 Coordinator N0 (dev0)   ----------------N1 (dev1)
 * ------------------------------ N2 (dev2) Channel 12             ED Scan
 * Channels 11-14                         Channel 14
 *
 *       |--------10 m----------------|----------30 m -----------------------|
 *
 ...
 *
 * At the beginning of the simulation, PAN coordinators N0 (Channel 12) and N2
 * (Channel 14) start transmitting beacon frames on their respective channels.
 * At the same time, end device N1 will scan through channels  (11,12,13 and 14)
 * looking for energy. i.e. multiple energy scans on each channel (PLME-ED
 * calls). The results of the Max energy read registered for each channel is
 * shown after the last channel scan is finished.
 *
 * The radio uses the default Sensibility and Rx Power provided by the
 * LogDistancePropagationLossModel. The simulation might take a few seconds to
 * complete.
 *
 * ED range: 0 - 255
 */
...
main(int argc, char* argv[])
{
    ...
    // Device 1 initiate channels scan on channels 11, 12, 13, and 14 looking for
    // energy Scan Channels represented by bits 0-26  (27 LSB)
    //                       ch 14  ch 11
    //                           |  |
