...
class RawTextConfigLoad : public FileConfig
{
  ...
  private:
     ...
     * '#'), and incomplete entries. An entry is considered complete when a type
     * and name have been found and the value contains two delimiting quotation
     * marks '"'. \param line the config file line \param type the config type
     * {default, global, value} \param name the config attribute name \param value
     * the value to set \returns true if all of type, name, and value parsed;
     * false otherwise
     *
     */
