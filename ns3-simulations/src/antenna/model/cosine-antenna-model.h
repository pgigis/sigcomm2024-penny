 ...
 *
 * Differently from the source, the response is defined for azimuth and
 elevation angles
 * between –180 and 180 degrees and is always positive.
 * There is no response at the backside of a cosine antenna.
 * The cosine response pattern achieves a maximum value of 1 (0 dB) at 0 degrees
 azimuth
 * and 90 degrees inclination.
 * An extra settable gain is added to the original model, to improve its
 generality.
 */
class CosineAntennaModel : public AntennaModel
{
  ...
  private:
    ...
    double m_orientationRadians; //!< orientation in radians in the horizontal
                                 //!< direction (bearing)
