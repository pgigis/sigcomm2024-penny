...
/**
 * \brief  Antenna model based on a parabolic approximation of the main lobe
 * radiation pattern.
 *
 * This class implements the parabolic model as described in 3GPP TR 38.901
 * v15.0.0
 */
class ThreeGppAntennaModel : public AntennaModel
{
  public:
    ...
    /**
     * Get the side-lobe attenuation in the vertical direction of the antenna
     * element. \return side-lobe attenuation in the vertical direction in dB
     */
  ...
  private:
    double m_verticalBeamwidthDegrees;   //!< beamwidth in the vertical direction
                                         //!< \f$(\theta_{3dB})\f$ [deg]
    double m_horizontalBeamwidthDegrees; //!< beamwidth in the horizontal
                                         //!< direction \f$(\phi_{3dB})\f$ [deg]
    double m_aMax;                       //!< maximum attenuation (A_{max}) [dB]
    double m_slaV;  //!< side-lobe attenuation in the vertical direction (SLA_V) [dB]
    double m_geMax; //!< maximum directional gain of the antenna element
                    //!< (G_{E,max}) [dBi]
};
