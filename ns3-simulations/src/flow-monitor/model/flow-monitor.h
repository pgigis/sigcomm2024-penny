 ...
 * \brief An object that monitors and reports back packet flows observed during
 * a simulation
 *
...
{
  public:
    /// \brief Structure that represents the measured metrics of an individual
    /// packet flow
    struct FlowStats
    {
        ...
        /// Delay\left\{P_{N-1}\right\}\right|\f$. This definition is in accordance
        /// with the Type-P-One-way-ipdv as defined in IETF \RFC{3393}.
        ...
        /// had to be dropped).
        std::vector<uint32_t> packetsDropped; // packetsDropped[reasonCode] =>
        ...
        Histogram flowInterruptionsHistogram; //!< histogram of durations of flow
                                              //!< interruptions
    };
    ...
    /// Set the time, counting from the current time, from which to start
    /// monitoring flows. This method overwrites any previous calls to Start()
    /// \param time delta time to start
    void Start(const Time& time);
    /// Set the time, counting from the current time, from which to stop
    /// monitoring flows. This method overwrites any previous calls to Stop()
    ...
    /// \param enableProbes if true, include also the per-probe/flow pair
    ...
    /// \param enableHistograms if true, include also the histograms in the output
    /// \param enableProbes if true, include also the per-probe/flow pair
    /// statistics in the output \return the XML output as string
  ...
  private:
    ...
    {
        Time firstSeenTime;      //!< absolute time when the packet was first seen by a
                                 //!< probe
        Time lastSeenTime;       //!< absolute time when the packet was last seen by a
                                 //!< probe
        uint32_t timesForwarded; //!< number of times the packet was reportedly
                                 //!< forwarded
    };
    ...
    double m_flowInterruptionsBinWidth; //!< Flow interruptions bin width (for
                                        //!< histograms)
