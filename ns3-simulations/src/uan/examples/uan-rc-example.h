...
class Experiment
{
  public:
    ...
    uint32_t m_numRates;  //!< Number of divided rates ( (NumberRates+1)%TotalRate
                          //!< should be 0).
    ...
    bool m_doNode;        //!< 1 for do max nodes simulation (invalidates AMin and AMax
                          //!< values).
     ...
     *
     * \f[{\rm{rate, bandwidth}} =
     * {\rm{kass}}\frac{{{\rm{m\_totalRate}}}}{{{\rm{m\_numRates}} + 1}}\f]
     *
     * The center frequency is set to
     *
     * \f[{f_{center}} = {\rm{fc}} + \frac{{ \pm {\rm{m\_totalRate}} \mp
     * {\rm{rate}}}}{2}\f]
     *
