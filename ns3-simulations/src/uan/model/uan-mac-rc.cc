...
UanMacRc::Enqueue(Ptr<Packet> packet, uint16_t protocolNumber, const Address& dest)
{
    if (protocolNumber > 0)
    {
        NS_LOG_WARN("Warning: UanMacRc does not support multiple protocols.  "
                    "protocolNumber "
                       ...
                       uint32_t ctsBytes)
{
    ...
    for (uint8_t i = 0; i < it->GetNoFrames(); i++, pit++)
    {
        ...
        if (eventTime < Time(0))
        {
            NS_FATAL_ERROR("Scheduling error resulted in very negative data "
                           "transmission time! eventTime = "
                           << eventTime.As(Time::S));
        }
        NS_LOG_DEBUG(Now().As(Time::S)
                     << " Node " << Mac8Address::ConvertFrom(GetAddress())
