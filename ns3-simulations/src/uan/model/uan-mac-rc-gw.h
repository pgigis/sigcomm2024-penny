...
class UanMacRcGw : public UanMac
{
  ...
  private:
    ...
    Time m_maxDelta;             //!< Maximum propagation delay between gateway and
                                 //!< non-gateway nodes .
    Time m_sifs;                 //!< Spacing between frames to account for timing error and
                                 //!< processing delay.
    ...
    uint32_t m_currentRateNum;   //!< Rate number corresponding to data rate of
                                 //!< current cycle.
    uint32_t m_numNodes;         //!< Number of non-gateway nodes in this gateway's
                                 //!< neighborhood.
    uint32_t m_totalRate;        //!< Total available channel rate in bps (for a single
                                 //!< channel, without splitting reservation channel).
