 ...
 * all packet in the queue. The UGS flows can transmit the quota of date equal
 *to the "Grant Size" (that is one of service flows QoS parameters) according to
 *their requested MaxLatency (another QoS parameter). All rtPS flows are able to
 *transmit all packet in the queue according to the available bandwidth. The
 *bandwidth saturation control has been implemented to redistribute the
 *effective available bandwidth to all rtPS flows that have at least one packet
 * to transmit. The remaining bandwidth is allocated to rntPS and BE
 *Connections.
 */
class BSSchedulerRtps : public BSScheduler
{
  public:
    ...
    /**
     * \brief Check for IR and Broadcast connections that have packets to
     * transmit. \param connection will point to a initial ranging or broadcast
     * connection which have packets to transmit \returns false if no initial
     * ranging or broadcast connection has packets to transmit, true otherwise
     */
    bool SelectIRandBCConnection(Ptr<WimaxConnection>& connection);
    /**
     * \brief Check for Basic and Primary connections that have packets to
     * transmit. \param connection will point to a basic or primary connection
    ...
    /**
     * \brief Selects a connection from the list of connections having packets to
     * be sent . \param connection will point to a connection that has packets to
     * be sent \returns false if no connection has packets to be sent, true
     * otherwise
     */
     ...
     * \param availableSymbols the remaining free OFDM symbols in the current
     * subframe
     */
     ...
     * \param availableSymbols maximum number of OFDM symbols to be used by the
     * burst \returns a Burst (list of packets)
     */
