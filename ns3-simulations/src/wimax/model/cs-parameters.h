...
class CsParameters
{
  public:
    ...
    /**
     * \brief creates a convergence sub-layer parameters from an ipcs classifier
     * record \param classifierDscAction the DCS action type \param classifier the
     * IPCS classifier record
     */
    CsParameters(Action classifierDscAction, IpcsClassifierRecord classifier);
    /**
     * \brief sets the dynamic service classifier action to ADD, Change or delete.
     * Only ADD is supported \param action the action enumeration
     */
