...
/**
 * \ingroup wimax
 * \brief this class implements the link manager of subscriber station net
 * device. An ss link manager is responsible for link scanning and ranging
 */
class SSLinkManager : public Object
{
  ...
  private:
    ...
    uint16_t m_eirXPIrMax; ///< initial ranging maximum equivalent isotropic
                           ///< received power at BS
    uint16_t m_pTxIrMax;   ///< maximum transmit signal strength for initial
                           ///< ranging calculated by SS

    uint8_t m_initRangOppNumber;        ///< Initial Ranging opportunity (1–255) in
                                        ///< which SS transmitted the RNG_REQ
    ...
    uint8_t m_dlChnlNr;          ///< indicates the channel/frequency currently the SS is
                                 ///< scanning
    uint64_t m_frequency;        ///< frequency on which it is currently operating,
                                 ///< i.e., where scanning was successful
