...
BSLinkManager::ProcessRangingRequest(Cid cid, RngReq rngreq)
{
    NS_ASSERT_MSG(m_bs->GetState() == BaseStationNetDevice::BS_STATE_UL_SUB_FRAME,
                  "Base station: Error while processing ranging request: "
    ...
    else
    {
        NS_ASSERT_MSG(m_bs->GetUplinkScheduler()->GetIsIrIntrvlAllocated(),
                      "Base station: Error while processing ranging request: IR "
                      "interval not allocated");
...
BSLinkManager::PerformInitialRanging(Cid cid, RngReq* rngreq, RngRsp* rngrsp)
{
    ...
    if (isOldSS)
    {
        // CIDs already assigned, e.g., RNG-REQ was lost and resent after timeout.
        // reusing old CIDs
    ...
    else
    {
        ...
        // add SS (Basic CID) to poll list for invited ranging intervals, see Table
        // 115
...
BSLinkManager::AcceptRanging(Cid cid, RngRsp* rngrsp, SSRecord* ssRecord)
{
    ...
    /*Shall not be set until the SS receives the RNG-RSP, as it may be lost etc.
     may be state field is also added to SSRecord which then set to
     SS_STATE_REGISTERED once confirmed that SS has received this RNG-RSP, but how
     to determine that, may be as a data packet is received by the SS*/
...
BSLinkManager::SelectDlChannel()
{
    ...
    // profR10_3 :
    // channels: 5000 + n â‹… 5 MHz, âˆ€n âˆˆ { 147, 149, 151, 153, 155, 157, 159,
    // 161, 163, 165, 167 } temporarily set to 1 for quick scanning. To be
    // standard compliant, use a value in the list above
