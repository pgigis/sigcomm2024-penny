...
/**
 * \ingroup wimax
 * \brief This class implements  the DL Frame Prefix IE as described by
 * IEEE-802.16 standard \brief The DL Frame Prefix IE is contained in DLFP
 * (Downlink Frame Prefix) in OFDM PHY
 */
...
class OfdmDownlinkFramePrefix : public Header
{
  ...
  private:
    ...
    uint32_t m_frameNumber;             ///< shall actually be only 4 LSB of the same field
                                        ///< in OFDM DCD Channel Encodings
    uint8_t m_configurationChangeCount; ///< shall actually be only 4 LSB of the
                                        ///< same field in DCD
