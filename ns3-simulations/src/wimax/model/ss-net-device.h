...
class SubscriberStationNetDevice : public WimaxNetDevice
{
  public:
    ...
    /**
     * \brief initializes the net device and sets the parameters to the default
     * values
     */
    void InitSubscriberStationNetDevice();
    /**
     * \param lostDlMapInterval time since last received DL-MAP message before
     * downlink synchronization is considered lost
     */
    void SetLostDlMapInterval(Time lostDlMapInterval);
    /**
     * \returns the time since last received DL-MAP message before downlink
    ...
    /**
     * \returns the time since last received UL-MAP before uplink synchronization
     * is considered lost
     */
    ...
    /**
     * \param interval2 the wait for broadcast ranging timeout, i.e., wait for
     * initial ranging opportunity
     */
    void SetIntervalT2(Time interval2);
    /**
     * \returns the wait for broadcast ranging timeout, i.e., wait for initial
     * ranging opportunity
     */
    Time GetIntervalT2() const;
    /**
     * \param interval3 the ranging Response reception timeout following the
     * transmission of a ranging request
     */
    void SetIntervalT3(Time interval3);
    /**
     * \returns the ranging Response reception timeout following the transmission
     * of a ranging request
     */
    ...
    /**
     * \param interval21 the time the SS searches for (decodable) DL-MAP on a
     * given channel
     */
    ...
    /**
     * \param maxContentionRangingRetries the Number of retries on contention
     * Ranging Requests
     */
    ...
    /**
     * \brief Set the most efficient modulation and coding scheme (MCS) supported
     * by the device \param modulationType the most robust MCS supported
     */
    void SetModulationType(WimaxPhy::ModulationType modulationType);
    /**
     * \returns the most efficient modulation and coding scheme (MCS) supported by
     * the device
     */
    WimaxPhy::ModulationType GetModulationType() const;
    /**
     * \param areManagementConnectionsAllocated true if the management connections
    ...
    /**
     * \param areServiceFlowsAllocated true if the service flows are allocated,
     * false otherwise
     */
    ...
    /**
     * \returns true if the device has at least one active service flow, false
     * otherwise
     */
  ...
  private:
    ...
    Time m_lostDlMapInterval; ///< in milliseconds, time since last received
                              ///< DL-MAP before downlink synchronization is
                              ///< considered lost, maximum is 600
    Time m_lostUlMapInterval; ///< in milliseconds, time since last received
                              ///< UL-MAP before uplink synchronization is
                              ///< considered lost, maximum is 600
    Time m_maxDcdInterval;    ///< in seconds, maximum time between transmission of
                              ///< DCD messages
    Time m_maxUcdInterval;    ///< in seconds, maximum time between transmission of
                              ///< UCD messages
    Time m_intervalT1;        ///< in seconds, wait for DCD timeout
    Time m_intervalT2;        ///< in seconds, wait for broadcast ranging timeout, i.e.,
                              ///< wait for initial ranging opportunity
    Time m_intervalT3;        ///< in milliseconds, ranging Response reception timeout
                              ///< following the transmission of a ranging request
    Time m_intervalT7;        ///< in seconds, wait for DSA/DSC/DSD Response timeout
    Time m_intervalT12;       ///< in seconds, wait for UCD descriptor
    Time m_intervalT20;       ///< in seconds, time the SS searches for preambles on a
                              ///< given channel
    Time m_intervalT21;       ///< in seconds, time the SS searches for (decodable)
    ...
    // to keep the number of DL-MAP/UL-MAP IEs found in the last DL-MAP/U-MAP
    // messages
    ...
    /*represents the (least robust) modulation type of the SS which it then
     requests in RNG-REQ and if accepted by BS uses it for receiving and
     transmitting. currently it is set by user in simulation script, shall
     actually be determined based on SS's distance, power, signal etc*/
    WimaxPhy::ModulationType m_modulationType; ///< modulation type

    bool m_areManagementConnectionsAllocated; ///< are management connections
                                              ///< allocated
