...
class SNRToBlockErrorRateManager
{
  public:
    ...
    /**
     * \brief returns a record of type SNRToBlockErrorRateRecord corresponding
     * to a given modulation and SNR value \param SNR the SNR value \param
     * modulation one of the seven MCS \return the Block Error Rate
     */
    GetSNRToBlockErrorRateRecord(double SNR, uint8_t modulation);
    /**
     * \brief Loads the traces form the repository specified in the constructor or
     * set by SetTraceFilePath function. If no repository is provided, default
     * traces will be loaded from default-traces.h file
     */
    ...
    void ReLoadTraces();
    /**
     * \brief If activate loss is called with false, all the returned BlcER will
     * be 0 (no losses) \param loss true to activates losses
     */
