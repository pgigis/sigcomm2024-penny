...
class WimaxMacQueue : public Object
{
  public:
     ...
     * \return the first packet in the queue if its size is lower than
    ...
    struct QueueElement
    {
        ...
        uint32_t m_fragmentOffset; ///< tracks the start of the next fragment into
                                   ///< the packet
  ...
  private:
    /**
     In the case of non-UGS service flows at the SS side the queue will store both
     data packets and bandwidth request packets. The two are distinguished by
     their headers. The below two functions are for this purpose exclusively. The
     Front function returns the first packet of a specific packet type from the
     queue (which may not necessarily be at the front of the queue), and the Pop
     function pops that elements.
     * \param packetType the type of the packet
