...
class WimaxConnection : public Object
{
  public:
    ...
    /**
     * \return true if the connection has at least one packet in its queue, false
     * otherwise
     */
    bool HasPackets() const;
    /**
     * \return true if the connection has at least one packet of type packetType
     * in its queue, false otherwise \param packetType type of packet to check in
     * the queue \return true if packets available
     */
    ...
    /**
     * \brief enqueue a received packet (that is a fragment) into the fragments
     * queue \param fragment received fragment
     */
