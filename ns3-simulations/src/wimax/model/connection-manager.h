...
/**
 * \ingroup wimax
 * The same connection manager class serves both for BS and SS though some
 * functions are exclusive to only one of them.
 */
...
class ConnectionManager : public Object
{
  public:
    ...
    /**
     * \brief allocates the management connection for an ss record. This method is
     * only used by BS \param ssRecord the ss record to which the management
     * connection will be allocated \param rngrsp the ranging response message
     */
    ...
    /**
     * \return true if one of the managed connection has at least one packet to
