...
/**
 * \ingroup wimax
 * \brief this class implements a structure to manage some parameters and
 * statistics related to a service flow
 */
class ServiceFlowRecord
{
  public:
    ...
    /**
     * \brief Set the grant time stamp. Used for data allocation for ugs flows,
     * and unicast poll (bw request) for non-UGS flows \param grantTimeStamp the
     * grant time stamp to set
     */
    void SetGrantTimeStamp(Time grantTimeStamp);
    /**
     * \return the grant time stamp. Used for data allocation for ugs flows, and
     * unicast poll (bw request) for non-UGS flows
     */
    ...
    /**
     * \return the DlTimeStamp: time when this service flow's traffic was last
     * sent
     */
  ...
  private:
    ...
    Time m_grantTimeStamp; ///< allocation (for data) for UGS flows and unicast
                           ///< poll (for bandwidth requests) for non-UGS flows
    ...
    uint32_t m_grantedBandwidthTemp; ///< Temporary variable used to sort list.
                                     ///< Necessary to keep original order

    /** bandwidth granted since last expiry of minimum reserved traffic rate
     * interval, only for nrtPS, to make sure minimum reserved traffic rate is
     * maintained */
