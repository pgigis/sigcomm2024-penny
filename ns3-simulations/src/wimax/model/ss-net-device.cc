    ...
    static TypeId tid =
        TypeId("ns3::SubscriberStationNetDevice")
            ...
            .AddAttribute("LostDlMapInterval",
                          "Time since last received DL-MAP message before "
            ...
            .AddAttribute("MaxDcdInterval",
                          "Maximum time between transmission of DCD messages. Maximum is "
                          "10s",
            ...
            .AddAttribute("MaxUcdInterval",
                          "Maximum time between transmission of UCD messages. Maximum is "
                          "10s",
                          TimeValue(Seconds(10)),
            ...
            .AddAttribute("IntervalT2",
                          "Wait for broadcast ranging timeout, i.e., wait for initial "
                          "ranging "
            ...
            .AddAttribute("IntervalT3",
                          "ranging Response reception timeout following the transmission "
                          "of a "
            ...
            .AddAttribute("IntervalT20",
                          "Time the SS searches for preambles on a given channel. Minimum "
                          "is 2 MAC frames",
                          TimeValue(Seconds(0.5)),
...
SubscriberStationNetDevice::SetPrimaryConnection(Ptr<WimaxConnection> primaryConnection)
{
    ...
    if (!m_asciiTxQueueEnqueueCb.IsNull())
    {
        std::ostringstream oss;
        oss << "/NodeList/" << nodeId << "/DeviceList/" << ifaceId
            << "/$ns3::SubscriberStationNetDevice/PrimaryConnection/TxQueue/"
               "Enqueue";
        Config::Connect(oss.str(), m_asciiTxQueueEnqueueCb);
    }
    if (!m_asciiTxQueueDequeueCb.IsNull())
                                    ...
                                    Ptr<WimaxConnection> connection)
{
    NS_ASSERT_MSG(connection,
                  "SS: Can not enqueue the packet: the selected "
