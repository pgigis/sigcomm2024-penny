...
class WimaxPhy : public Object
{
  public:
     ...
     * \param frequency  the frequency to be used for reception and transmission
     * process
     */
     ...
     * \param state the state to be set (PHY_STATE_IDLE, PHY_STATE_SCANNING,
     * PHY_STATE_TX, PHY_STATE_RX)
     */
    void SetState(PhyState state);
    /**
     * Get the state of the device
     * \return the state of the device (PHY_STATE_IDLE, PHY_STATE_SCANNING,
     * PHY_STATE_TX, PHY_STATE_RX)
     */
    PhyState GetState() const;
    /**
     * \brief scan a frequency for maximum timeout seconds and call the callback
     * if the frequency can be used \param frequency the frequency to scan \param
     * timeout the timeout before considering the channel as unusable \param
    ...
    /**
     * \brief calculates the data rate of each modulation and save them for future
     * use
     */
    void SetDataRates();
    /**
     ...
     * \param modulationType the modulation that will be used to transmit the
     * bytes
     */
    Time GetTransmissionTime(uint32_t size, ModulationType modulationType) const;
    /**
     * Get the number of symbols needed to transmit size bytes using the
     * modulation modulationType \return the number of symbols needed \param size
     * the number of byte to transmit \param modulationType the modulation that
     * will be used to transmit the bytes
     */
    uint64_t GetNrSymbols(uint32_t size, ModulationType modulationType) const;
    /**
     * Get the maximum number of bytes that could be carried by symbols symbols
     * using the modulation modulationType \param symbols the number of symbols to
     * use \param modulationType the modulation that will be used \return the
