...
/**
 * \ingroup wimax
 * This class implements the ranging response message described by "IEEE
 * Standard for Local and metropolitan area networks Part 16: Air Interface for
 * Fixed Broadband Wireless Access Systems" 6.3.2.3.6 Ranging response (RNG-RSP)
 * message, page 50
 */
class RngRsp : public Header
{
  public:
    ...
    /**
     * \brief set the relative change in transmission power level that the SS
     * should make in order that transmissions arrive at the BS at the desired
     * power. When subchannelization is employed, the subscriber shall interpret
     * the power offset adjustment as a required change to the transmitted power
     * density. \param powerLevelAdjust the relative change in transmission power
     * level
     */
    void SetPowerLevelAdjust(uint8_t powerLevelAdjust);
    /**
     * \brief set the relative change in transmission frequency that the SS should
     * take in order to better match the BS. This is fine-frequency adjustment
     * within a channel, not reassignment to a different channel \param
    ...
    /**
     * \brief set the Center frequency, in kHz, of new downlink channel where the
     * SS should redo initial ranging. \param dlFreqOverride the Center frequency
     * in kHz
     */
    void SetDlFreqOverride(uint32_t dlFreqOverride);
    /**
     * \brief set the identifier of the uplink channel with which the SS is to
     * redo initial ranging \param ulChnlIdOverride the uplink channel index
     */
    ...
    /**
     * \return Tx timing offset adjustment (signed 32-bit). The time required to
     * advance SS transmission so frames arrive at the expected time instance at
     * the BS.
     */
    uint32_t GetTimingAdjust() const;
    /**
     * \return the relative change in transmission power level that the SS should
     * take in order that transmissions arrive at the BS at the desired power.
     * When subchannelization is employed, the subscriber shall interpret the
     * power offset adjustment as a required change to the transmitted power
     * density.
     */
    uint8_t GetPowerLevelAdjust() const;
    /**
     * \return the relative change in transmission frequency that the SS should
     * take in order to better match the BS. This is fine-frequency adjustment
     * within a channel, not reassignment to a different channel.
     */
    ...
    /**
     * \return Center frequency, in kHz, of new downlink channel where the SS
     * should redo initial ranging.
     */
    uint32_t GetDlFreqOverride() const;
    /**
     * \return The identifier of the uplink channel with which the SS is to redo
     * initial ranging
     */
    uint8_t GetUlChnlIdOverride() const;
    /**
     * \return DlOperBurstProfile: This parameter is sent in response to the
     * RNG-REQ Requested Downlink Burst Profile parameter
     */
  ...
  private:
    ...
    /**
     * Tx timing offset adjustment (signed 32-bit). The time required to advance
     * SS transmission so frames arrive at the expected time instance at the BS.
     */
    uint32_t m_timingAdjust;

    /**
     * Specifies the relative change in transmission power level that the SS is to
     * make in order that transmissions arrive at the BS at the desired power.
     * When subchannelization is employed, the subscriber shall interpret the
     * power offset adjustment as a required change to the transmitted power
     * density.
     */
    ...
    /**
     * Specifies the relative change in transmission frequency that the SS is to
     * make in order to better match the BS. This is fine-frequency adjustment
     * within a channel, not reassignment to a different channel.
     */
    ...
    /// Center frequency, in kHz, of new downlink channel where the SS should redo
    /// initial ranging.
    uint32_t m_dlFreqOverride;

    /**
     * Licensed bands: The identifier of the uplink channel with which the SS is
     * to redo initial ranging (not used with PHYs without channelized uplinks).
     */
    uint8_t m_ulChnlIdOverride;

    /**
     * This parameter is sent in response to the RNG-REQ Requested Downlink Burst
     * Profile parameter. Byte 0: Specifies the least robust DIUC that may be used
     * by the BS for transmissions to the SS. Byte 1: Configuration Change Count
     * value of DCD defining the burst profile associated with DIUC.
     */
    ...
    /**
     * Initial Ranging opportunity (1–255) in which the associated RNG_REQ message
     * was detected by the BS. Usage is mutually exclusive with SS MAC Address
     */
    uint8_t m_initRangOppNumber;

    /**
     * Used to indicate the OFDM subchannel reference that was used to transmit
     * the initial ranging message (OFDM with subchannelization).
     */
 ...
 * Broadband Wireless Access Systems" 6.3.2.3.10 DSA-REQ message, page 62
 */
 ...
 * Local and metropolitan area networks Part 16: Air Interface for Fixed
 * Broadband Wireless Access Systems" 6.3.2.3.11 DSA-RSP message, page 63
 *
 * \verbatim
 *   0             7             15            23
 *  +-------------+-------------+-------------+
 ...
 * Local and metropolitan area networks Part 16: Air Interface for Fixed
 * Broadband Wireless Access Systems" 6.3.2.3.12 DSA-ACK message, page 64
 */
...
/**
 * \ingroup wimax
 * This class implements the ranging request message described by "IEEE Standard
