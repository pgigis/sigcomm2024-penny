...
class BSLinkManager : public Object
{
  public:
     ...
     * \param cid the connection identifier in which the ranging message was
     * received \param rngreq the ranging request message
     */

    void ProcessRangingRequest(Cid cid, RngReq rngreq);
    /**
     * \brief Verifies at the end of an invited ranging interval if SS sent
     * ranging message in it or not \param cid the connection identifier in which
     * the ranging message was received \param uiuc the ranging
     */
  ...
  private:
    ...
    int tries;                        ///< variable to test multiple RNG-REQ by the SS for example
                                      ///< undecodable at BS or lost RNG-RSP
};
