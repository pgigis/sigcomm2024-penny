...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  ...
  private:
    // Protocol parameters.
    /// Holdtimes is the multiplicative factor of PeriodicUpdateInterval for which
    /// the node waits since the last update before flushing a route from the
    /// routing table. If PeriodicUpdateInterval is 8s and Holdtimes is 3, the
    /// node waits for 24s since the last update to flush this route from its
    /// routing table.
    uint32_t Holdtimes;
    ...
    /// an update. It waits for this time interval in hope of receiving an update
    ...
    /// packet for.
    Time m_maxQueueTime;
    /// A "drop front on full" queue used by the routing layer to buffer packets
    /// to which it does not have a route.
    PacketQueue m_queue;
    ...
    /// This is a flag to enable route aggregation. Route aggregation will
    /// aggregate all routes for 'RouteAggregationTime' from the time an update is
    /// received by a node and sends them as a single update .
  ...
  private:
    ...
    /// Timer used by the trigger updates in case of Weighted Settling Time is
    /// used
