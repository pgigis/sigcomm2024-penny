    ...
    static TypeId tid =
        TypeId("ns3::dsdv::RoutingProtocol")
            ...
            .AddAttribute("PeriodicUpdateInterval",
                          "Periodic interval between exchange of full routing tables among "
                          "nodes.",
                          TimeValue(Seconds(15)),
                          MakeTimeAccessor(&RoutingProtocol::m_periodicUpdateInterval),
                          MakeTimeChecker())
            .AddAttribute("SettlingTime",
                          "Minimum time an update is to be stored in adv table "
                          "before sending out "
            ...
            .AddAttribute("MaxQueuedPacketsPerDst",
                          "Maximum number of packets that we allow per destination to "
                          "buffer.",
                          UintegerValue(5),
            ...
            .AddAttribute("EnableBuffering",
                          "Enables buffering of data packets if no route to destination is "
                          "available",
                          BooleanValue(true),
            ...
            .AddAttribute(
                "EnableWST",
                "Enables Weighted Settling Time for the updates before "
                "advertising",
                BooleanValue(true),
            ...
            .AddAttribute("WeightedFactor",
                          "WeightedFactor for the settling time if Weighted "
                          "Settling Time is enabled",
                          DoubleValue(0.875),
        ...
        else
        {
            ...
            if (dsdvHeader.GetDstSeqno() % 2 != 1)
            {
                ...
                else
                {
                    ...
                    NS_LOG_DEBUG(dsdvHeader.GetDst() << " : Received update with old seq "
...
{
    NS_LOG_FUNCTION("Merging advertised table changes with main table before "
                    "sending out periodic update");
    std::map<Ipv4Address, RoutingTableEntry> allRoutes;
    m_advRoutingTable.GetListOfAllRoutes(allRoutes);
    if (!allRoutes.empty())
    {
