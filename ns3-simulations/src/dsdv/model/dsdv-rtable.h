...
class RoutingTableEntry
{
  ...
  private:
    ...
    RouteFlags m_flag;
    /// Time for which the node retains an update with changed metric before
    /// broadcasting it. A node does that in hope of receiving a better update.
    Time m_settlingTime;
    /// Flag to show if any of the routing table entries were changed with the
    /// routing update.
...
class RoutingTable
{
  public:
     ...
     * \param allRoutes is the list that will hold all these addresses present in
    ...
    /**
     * Add an event for a destination address so that the update to for that
     * destination is sent after the event is completed. \param address
     * destination address for which this event is running. \param id unique
     * eventid that was generated. \return true on success
     */
    ...
    /**
     * Force delete an update waiting for settling time to complete as a better
     * update to same destination was received. \param address destination address
     * for which this event is running. \return true on success
     */
    bool AnyRunningEvent(Ipv4Address address);
    /**
     * Force delete an update waiting for settling time to complete as a better
     * update to same destination was received. \param address destination address
     * for which this event is running. \return true on finding out that an event
     * is already running for that destination address.
     */
