 ...
 *
 * When a route is not available, the packets are queued. Every node can buffer
 * up to 5 packets per destination. We have implemented a "drop front on full"
 * queue where the first queued packet will be dropped to accommodate newer
 * packets.
 */
class PacketQueue
{
  public:
     ...
     * destination address in queue. \param entry QueueEntry to compare \return
     * true if successful
     */
