 ...
 * These characteristics are based on a legacy 3GPP specification. The
 * description can be found in the following references:
 ...
 * connection to the server is maintained and used for transmitting and
 * receiving all objects.
 *
 * Each request by default has a constant size of 350 bytes. A
 * ThreeGppHttpHeader is attached to each request packet. The header contains
 * information such as the content type requested (either main object or
 * embedded object) and the timestamp when the packet is transmitted (which will
 * be used to compute the delay and RTT of the packet).
 */
class ThreeGppHttpClient : public Application
{
  public:
    ...
    enum State_t
    {
        ...
        NOT_STARTED = 0,
        /// Sent the server a connection request and waiting for the server to be
        /// accept it.
        CONNECTING,
        /// Sent the server a request for a main object and waiting to receive the
        /// packets.
        EXPECTING_MAIN_OBJECT,
        /// Parsing a main object that has just been received.
        PARSING_MAIN_OBJECT,
        /// Sent the server a request for an embedded object and waiting to receive
        /// the packets.
  ...
  private:
     ...
     * ThreeGppHttpVariables and is assumed to be smaller than 536 bytes. Fires
     ...
     * comparing the content length specified in the ThreeGppHttpHeader of the
     * packet and the actual packet size. #m_objectBytesToBeReceived keeps track
     * of the number of bytes that has been received.
     *
     ...
     * by comparing the content length specified in the TheeGppHttpHeader of the
     * packet and the actual packet size. #m_objectBytesToBeReceived keeps track
     * of the number of bytes that has been received.
     *
     * If the received packet is not the last packet of the object, then the
    ...
    /// The client time stamp of the ThreeGppHttpHeader from the last received
    /// packet.
    Time m_objectClientTs;
    /// The server time stamp of the ThreeGppHttpHeader from the last received
    /// packet.
