 ...
 * determined (see ThreeGppHttpVariables). Each object may be sent as multiple
 * packets due to limited socket buffer space.
 *
 * To assist with the transmission, the application maintains several instances
 * of ThreeGppHttpServerTxBuffer. Each instance keeps track of the object type
 * to be served and the number of bytes left to be sent.
 *
...
class ThreeGppHttpServer : public Application
{
  public:
     ...
     * \param httpServer Pointer to this instance of ThreeGppHttpServer, which is
     * where the trace originated. \param socket Pointer to the socket where the
 ...
 *
 * Types of data are expressed using the ThreeGppHttpHeader::ContentType_t type.
 * Only one type of data can be active for one client at a time, i.e., the
 * current content of a buffer has to be removed before a different type of data
 * can be added.
 */
class ThreeGppHttpServerTxBuffer : public SimpleRefCount<ThreeGppHttpServerTxBuffer>
{
  public:
    ...
    /**
     * Returns ThreeGppHttpHeader::NOT_SET when the buffer is new and never been
     * filled with any data before. Otherwise, returns either
     * ThreeGppHttpHeader::MAIN_OBJECT or ThreeGppHttpHeader::EMBEDDED_OBJECT.
     * \param socket Pointer to the socket which is associated with the
     *               transmission buffer of interest
  ...
  private:
    ...
    struct TxBuffer_t
    {
         ...
         * will be set in ThreeGppHttpHeader of every corresponding response packet
         * sent, to be used by the client to compute round trip delay time (RTT).
         */
        Time clientTs;
        /**
