...
TapBridge::Start(Time tStart)
{
    ...
    //
    // Cancel any pending start event and schedule a new one at some relative time
    // in the future.
    //
...
TapBridge::Stop(Time tStop)
{
    ...
    //
    // Cancel any pending stop event and schedule a new one at some relative time
    // in the future.
    //
...
TapBridge::CreateTap()
{
    ...
    if (pid == 0)
    {
        ...
        // -o<operating mode> The operating mode of the bridge (1=ConfigureLocal,
        // 2=UseLocal, 3=UseBridge) -p<path> the path to the unix socket described
        // above.
        //
        // Example tap-creator -dnewdev -g1.2.3.2 -i1.2.3.1 -m08:00:2e:00:01:23
        // -n255.255.255.0 -o1 -pblah
        //
        ...
        //
        // If the execlp successfully completes, it never returns.  If it returns it
        // failed or the OS is broken.  In either case, we bail.
        //
    ...
    else
    {
        ...
        // an "ancillary element" but the msghdr uses the control message
        // termimology so we call it "control."
        //
...
TapBridge::ForwardToBridgedDevice(uint8_t* buf, ssize_t len)
{
    ...
    //
    // If we are operating in CONFIGURE_LOCAL mode, we simply simply take all
    // packets that come from "Tap Device" and ask "ns3 Device" to send them down
    // its directly connected network.  A normal bridge would need to call
    // SendFrom in order to preserve the original from address, but in
    // CONFIGURE_LOCAL mode the tap device and the ns-3 device have the same MAC
    // address by definition so we can call Send.
    //
