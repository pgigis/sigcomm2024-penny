...
class TapBridge : public NetDevice
{
  public:
    ...
    enum Mode
    {
        ...
        USE_BRIDGE,      //!< ns-3 uses a pre-created tap, and bridges to a bridging net
                         //!< device
    };
     ...
     *
     * The bridged net device is the ns-3 device to which this bridge is
     * connected,
     *
  ...
  private:
     ...
     * be received forwarded up the IP stack on the ghost node through this
     * device.
     */
    NetDevice::ReceiveCallback m_rxCallback;

    /**
     * Callback used to hook the promiscuous packet receive callback of the
     * TapBridge ns-3 net device.  This is never called, and therefore no packets
     * will ever be received forwarded up the IP stack on the ghost node through
     * this device.
     *
     * Note that we intercept the similar callback in the bridged device in order
     * to do the actual bridging between the bridged ns-3 net device and the Tap
     * device on the host.
     */
