...
main(int argc, char* argv[])
{
...

    {
        ...
        plot.AppendExtra("NormalDist(x,m,s) = 1 / (s * sqrt(2*pi)) * exp(-1.0 / "
                         "2.0 * ((x-m) / s)**2)");
...

    {
        ...
        plot.AppendExtra("set label 1 '{/Symbol g}(x,{/Symbol a},{/Symbol b}) = "
                         "x^{/Symbol a-1} e^{-x {/Symbol "
                         "b}^{-1}} ( {/Symbol b}^{/Symbol a} {/Symbol G}({/Symbol "
                         "a}) )^{-1}' at 0.7, 0.9");
...

    {
        ...
        plot.AppendExtra("set label 1 'Erlang(x,k,{/Symbol l}) = x^{k-1} e^{-x "
                         "{/Symbol l}^{-1}} ( "
