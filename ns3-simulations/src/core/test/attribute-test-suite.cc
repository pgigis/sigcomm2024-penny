 ...
 * \ingroup attribute-tests
 *
 * \brief Test case template used for generic Attribute Value types -- used to
 * make sure that Attributes work as expected.
 */
template <typename T>
...
AttributeTestCase<BooleanValue>::DoRun()
{
    ...
    //
    // Set the default value of the BooleanValue the other way and create an
    // object. The new default value should stick.
    //
    ...
    // attribute is declared differently in the object.  First make sure we can
    // set to true.
    //
...
AttributeTestCase<EnumValue>::DoRun()
{
    ...
    //  NS_TEST_ASSERT_MSG_EQ (ok, false, "Unexpectedly could
    //  SetAttributeFailSafe() to TEST_D"); //

    ok = CheckGetCodePaths(p, "TestEnum", "TestB", EnumValue(AttributeObjectTest::TEST_B));
    NS_TEST_ASSERT_MSG_EQ(ok, true, "Error in SetAttributeFailSafe() but value changes");

    //
    // Try to set the Attribute to a bogus enum using an integer implicit
    // conversion and make sure the underlying value doesn't change.
    //
...
class RandomVariableStreamAttributeTestCase : public TestCase
{
  ...
  private:
    ...
    int16_t m_gotCbValue; //!< Value used to verify that the callback has been
                          //!< invoked.
};
...
IntegerTraceSourceTestCase::DoRun()
{
    ...
    // could set it using an IntegerValue through its IntegerTraceSource1
    // "persona." We should also be able to hook a trace source to the underlying
    // variable.
    //
...
PointerAttributeTestCase::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_EQ((bool)derived,
                          false,
...
CallbackValueTestCase::DoRun()
{
    ...
    // Attribute named "Callback" also points to m_cbValue and allows us to set
    // the callback using that Attribute.
    //
    // NotifyCallbackValue is going to be the target of the callback and will just
    // set m_gotCbValue to its single parameter.  This will be the parameter from
    // the callback invocation.  The method InvokeCbValue() just invokes the
    // m_cbValue callback if it is non-null.
    //
    m_gotCbValue = 1;

    //
    // If we invoke the callback (which has not been set) nothing should happen.
    // Further, nothing should happen when we initialize the callback (it
    // shouldn't accidentally fire).
    //
    p->InvokeCbValue(2);
