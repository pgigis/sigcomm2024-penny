...
CreateObjectTestCase::DoRun()
{
    ...
    //
    // Since BaseA is a BaseA and not a DerivedA, we must not find a DerivedA if
    // we look.
    //
    ...
    //
    // Since baseA is not a BaseA, we must not be able to ask for a DerivedA even
    // if we try an implied cast back to a BaseA.
    //
    ...
    //
    // If we create a DerivedA and cast it to a BaseA, then if we do a GetObject
    // for that BaseA we should get the same address (same Object).
    //
...
AggregateObjectTestCase::DoRun()
{
    ...
    //
    // We should be able to ask the aggregation (through baseA) for the DerivedB
    // part
    //
    NS_TEST_ASSERT_MSG_NE(baseA->GetObject<DerivedB>(),
                          nullptr,
    ...
    //
    // Since the DerivedB is also a BaseB, we should be able to ask the
    // aggregation (through baseA) for the BaseB part
    //
    NS_TEST_ASSERT_MSG_NE(baseA->GetObject<BaseB>(),
                          nullptr,
                          "Cannot GetObject (through baseA) for BaseB Object");
    ...
    //
    // We should be able to ask the aggregation (through baseB) for the DerivedA
    // part
    //
    NS_TEST_ASSERT_MSG_NE(baseB->GetObject<DerivedA>(),
                          nullptr,
    ...
    //
    // Since the DerivedA is also a BaseA, we should be able to ask the
    // aggregation (through baseB) for the BaseA part
    //
    NS_TEST_ASSERT_MSG_NE(baseB->GetObject<BaseA>(),
                          nullptr,
                          "Cannot GetObject (through baseB) for BaseA Object");
