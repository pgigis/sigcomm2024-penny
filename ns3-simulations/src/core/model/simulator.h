...
class Simulator
{
  public:
    ...
    /**
     * @name Schedule events (in a different context) to run now or at a future
     * time.
     *
    ...
    static void ScheduleWithContext(uint32_t context,
                                    ...
                                    void (*f)(Us...),
                                    Ts&&... args);
    /** @} */ // Schedule events (in a different context) to run now or at a
              // future time.
    ...
    /**
     * @name Schedule events to run at the end of the simulation, when
     * Simulator:Destroy() is called.
     */
    /** @{ */
    /**
     * Schedule an event to run at the end of the simulation, when
     * Simulator::Destroy() is called. All events scheduled to expire at "Destroy"
     * time are scheduled FIFO, after all normal events have expired and only when
