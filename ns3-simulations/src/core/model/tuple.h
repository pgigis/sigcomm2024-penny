 ...
 * \tparam Args \explicit The list of AttributeValues to be held by this
 * TupleValue
 */
template <class... Args>
class TupleValue : public AttributeValue
{
  public:
     ...
     * type of a set method or the type of a data member) \return true if the
     * given variable was set
     */
    template <typename T>
 ...
 *
 * TupleValue<UintegerValue, DoubleValue> t = MakeTupleValue<Pack> (Tuple
 * {10, 1.5}); \endcode
 *
 * \tparam T1 \explicit A std::tuple of the AttributeValue types included in
 * TupleValue \tparam T2 \deduced A std::tuple of the type of elements stored by
 * TupleValue \param t the tuple of elements stored by TupleValue \return a
 * TupleValue object
 */
...
/**
 * Create a TupleChecker from AttributeCheckers associated with TupleValue
 * elements.
 *
...
// in MakeTupleChecker. The non-templated base ns3::TupleChecker is returned in
// that function. This is the same pattern as ObjectPtrContainer.
namespace internal
{
...
/**
 * Helper class defining static methods for MakeTupleChecker and
 * MakeTupleAccessor that are called when user specifies the list of
