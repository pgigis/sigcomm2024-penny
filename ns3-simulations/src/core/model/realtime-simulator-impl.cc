...
RealtimeSimulatorImpl::ProcessOneEvent()
{
    ...
    //
    // We need to be able to have external events (such as a packet reception
    // event) cause us to re-evaluate our state.  The way this works is that the
    // synchronizer gets interrupted and returns.  So, there is a possibility that
    // things may change out from under us dynamically.  In this case, we need to
    // re-evaluate how long to wait in a for-loop until we have waited
    // successfully (until a timeout) for the event at the head of the event list.
    //
    // m_synchronizer->Synchronize will return true if the wait was completed
    // without interruption, otherwise it will return false indicating that
    // something has changed out from under us.  If we sit in the for-loop trying
    // to synchronize until Synchronize() returns true, we will have successfully
    // synchronized the execution time of the next event with the wall clock time
    // of the synchronizer.
    //
    ...
    for (;;)
    {
        ...
        // simulator, so the last event is, by definition, executed entirely at a
        // single
        //  discrete time.  This is the definition of m_currentTs.  It really has
        // nothing to do with the current real time, except that we are trying to
        // arrange that at the instant of the beginning of event execution, the
        // current real time and m_currentTs coincide.
        //
...

        {
            ...
            // event.  Since m_currentTs is actually the timestamp of the last event
            // we executed, it's not particularly meaningful for us here since real
            // time has certainly elapsed since it was last updated.
            //
            // It is possible that the current realtime has drifted past the next
            // event time so we need to be careful about that and not delay in that
            // case.
            //
            ...
            //
            // tsNow is set to the normalized current real time.  When the simulation
            // was started, the current real time was effectively set to zero; so
            // tsNow is the current "real" simulation time.
            //
            ...
            //
            // tsDelay is therefore the real time we need to delay in order to bring
            // the real time in sync with the simulation time.  If we wait for this
            // amount of real time, we will accomplish moving the simulation time at
            // the same rate as the real time.  This is typically called "pacing" the
            // simulation time.
            //
            // We do have to be careful if we are falling behind.  If so, tsDelay must
            // be zero.  If we're late, don't dawdle.
            //
            if (tsNext <= tsNow)
            {
                tsDelay = 0;
        ...
        // until the condition variable becomes true.  A true condition indicates
        // that the wait should stop.  The condition is set to true by one of the
        // Schedule methods of the simulator; so if we are in a wait down in
        // Synchronize, and a Simulator::ScheduleReal is done, the wait down in
        // Synchronize will exit and Synchronize will return false.  This means we
        // have not actually synchronized to the event expiration time.  If no
        // real-time schedule operation is done while down in Synchronize, the wait
        // will time out and Synchronize will return true.  This indicates that we
        // have synchronized to the event time.
        //
        // So we need to stay in this for loop, looking for the next event timestamp
        // and attempting to sleep until its due.  If we've slept until the
        // timestamp is due, Synchronize returns true and we break out of the sync
        // loop.  If an external event happens that requires a re-schedule,
        // Synchronize returns false and we re-evaluate our timing by continuing in
        // the loop.
        //
        // It is expected that tsDelay become shorter as external events interrupt
        // our waits.
        //
        ...
        //
        // If we get to this point, we have been interrupted during a wait by a
        // real-time schedule operation.  This means all bets are off regarding
        // tsDelay and we need to re-evaluate what it is we want to do.  We'll loop
        // back around in the for-loop and start again from scratch.
        //
    }

    //
    // If we break out of the for-loop above, we have waited until the time
    // specified by the event that was at the head of the event list when we
    // started the process. Since there is a bunch of code that was executed
    // outside a critical section (the Synchronize call) we cannot be sure that
    // the event at the head of the event list is the one we think it is.  What we
    // can be sure of is that it is time to execute whatever event is at the head
    // of this list if the list is in time order.
    //
...

    {
        ...
        //
        // We do know we're waiting for an event, so there had better be an event on
        // the event queue.  Let's pull it off.  When we release the critical
        // section, the event we're working on won't be on the list and so
        // subsequent operations won't mess with us.
        //
        ...
        //
        // We cannot make any assumption that "next" is the same event we originally
        // waited for.  We can only assume that only that it must be due and cannot
        // cause time to move backward.
        //
        ...
        // executing.  From the rest of the simulation's point of view, simulation
        // time is frozen until the next event is executed.
        //
        ...
        // we have to decide if we've done a good enough job and if we haven't,
        // we've been asked to commit ritual suicide.
        //
    ...
    //
    // We have got the event we're about to execute completely disentangled from
    // the event list so we can execute it outside a critical section without fear
    // of someone changing things out from under us.
...
RealtimeSimulatorImpl::Schedule(const Time& delay, EventImpl* impl)
{
    ...
    Scheduler::Event ev;
    {
        std::unique_lock lock{m_mutex};
        //
        // This is the reason we had to bring the absolute time calculation in from
        // the simulator.h into the implementation.  Since the implementations may
        // be multi-threaded, we need this calculation to be atomic.  You can see it
        // is here since we are running in a CriticalSection.
        //
