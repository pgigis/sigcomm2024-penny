 ...
 * use Version to output a version string.  command-line-example has been
 * updated to include CommandLine::PrintVersion() in its output
 *
 ...
 * | Major              | 3        |       |
 * | Minor              | 32       |       |
 * | Patch              | 0        | This version tag does not have a patch
 * field | | ReleaseCandidate   | ""       | This version tag does not have a
 * release candidate field | | TagDistance        | 1        |       | |
 * CommitHash         | g6ad7f05 | g at front of hash indicates a git hash | |
 * DirtyWorkingTree   | Variable | Depends on status of git working and stage
 * areas | | BuildProfile       | Variable | Depends on the value of
 * --build-profile option of ns3 configure
 * |
 */
class Version
{
  public:
     ...
     * \endverbatim
     *
     * [patch], [rc], [closest-tag], and [dirty] will only be present under
