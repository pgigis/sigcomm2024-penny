 ...
 * additional linear algebra operations. The objective of this class is to
 * two-fold:
 *
 * - First, it provides simple interfaces to perform matrix operations in
 * parallel on all pages without the need to write an outer loop over all the
 * pages.
 *
...
class MatrixArray : public ValArray<T>
{
  public:
    ...
    /**
     * \brief Constructor "pages" number of matrices that are of size
     * "numRows"x"numCols", and are initialized with all-zero elements. If only 1
     * parameter, numRows, is provided then a single 1D array is being created.
    ...
    /**
     * \brief Constructor creates a single array of values.size () elements and 1
     * column, and uses std::valarray<T> values to initialize the elements. \param
     * values std::valarray<T> that will be used to initialize elements of 1D
     * array
     */
    explicit MatrixArray<T>(const std::valarray<T>& values);
    /**
     * \brief Constructor creates a single array of values.size () elements and 1
     * column, and moves std::valarray<T> values to initialize the elements.
    ...
    /**
     * \brief Constructor creates a single array of values.size () elements and 1
     * column, and uses std::vector<T> values to initialize the elements. \param
    ...
    /**
     * \brief Constructor creates a single matrix of numRows and numCols, and
     ...
     * std::valarray<T> that will be moved to initialize elements of 3D array
     */
    MatrixArray<T>(size_t numRows, size_t numCols, std::valarray<T>&& values);
    /**
     * \brief Constructor creates the array of numPages matrices of numRows x
     * numCols dimensions, and uses std::valarray<T> values to initialize all the
     * elements. \param numRows the number of rows \param numCols the number of
     * columns \param numPages the number of pages \param values std::valarray<T>
     * that will be used to initialize elements of 3D array
     */
    MatrixArray<T>(size_t numRows, size_t numCols, size_t numPages, const std::valarray<T>& values);
    /**
     * \brief Constructor creates the array of numPages matrices of numRows x
     * numCols dimensions, and moves std::valarray<T> values to initialize all the
     * elements. \param numRows the number of rows \param numCols the number of
     * columns \param numPages the number of pages \param values std::valarray<T>
     * that will be used to initialize elements of 3D array
     */
     ...
     * operator. \return a reference to the result of the assignment
     */
     ...
     * rows in rhs, and rhs must have the same number of pages as this
     * MatrixArray. \param rhs is another MatrixArray instance \returns The array
     * of results of matrix multiplications.
     */
    MatrixArray<T> operator*(const MatrixArray<T>& rhs) const;
    /**
     * \brief This operator interprets the 3D array as an array of matrices,
     * and performs a linear algebra operation on each of the matrices (pages),
     * i.e., transposes the matrix or array of matrices definition for
     * MatrixArray<T>. \return The resulting MatrixArray composed of the array of
     * transposed matrices.
     */
     ...
     *
     * This operation is not possible when using the multiplication operator
     *because the number of pages does not match.
     *
     ...
     * This function assumes that the matrix contains complex numbers, because of
     * that it is only available for the <std::complex<double>> specialization of
     * MatrixArray. \return Returns a new matrix that is the result of the
     * Hermitian transpose
     */
  ...
  protected:
    // To simplify functions in MatrixArray that are using members from the
    // template base class
