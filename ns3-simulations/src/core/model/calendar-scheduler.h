...
class CalendarScheduler : public Scheduler
{
  ...
  private:
    ...
    /**
     * Ordering function to identify the insertion point, according to \c
     * m_reverse. \param [in] newEvent The new event being inserted. \param [in]
     * it The current position in the bucket being examined. \return \c true if
     * the \c newEvent belongs before \c it.
     */
