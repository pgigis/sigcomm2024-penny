...
WallClockSynchronizer::WallClockSynchronizer()
{
    ...
    // requires a delay less than a jiffy.  This is on the order of one
    // millisecond (999848 ns) on the ns-regression machine.
    //
...
WallClockSynchronizer::DoGetDrift(uint64_t ns)
{
    ...
    {
        //
        // Real time (nsNow) is smaller/earlier than the simulator time (ns).  We
        // are ahead of real time and the difference (drift) is negative.
        //
