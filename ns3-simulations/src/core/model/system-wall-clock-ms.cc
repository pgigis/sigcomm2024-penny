...
SystemWallClockMsPrivate::End()
{
    ...
    // a millisecond is measured, the function will work as expected.  If an
    // elapsed time is measured that turns out to be less than a millisecond,
    // we'll just return zero which would, I think, also will be expected.
    //
    NS_LOG_FUNCTION(this);
