 ...
 * ECT1 flows are classified into different flows. Sub Test 1 70 packets are
 * enqueued into both the flows with the delay of 0.5ms between two enqueues,
 * and dequeued with the delay of 1ms between two dequeues. Sub Test 2 140(70
 * ECT0 + 70 ECT1) packets are enqueued such that ECT1 packets are enqueued at
 * 0.5ms, 1.5ms, 2.5ms and so on, and ECT0 packets are enqueued are enqueued at
 * 1ms, 2ms, 3ms and so on Any future classifier options (e.g.
