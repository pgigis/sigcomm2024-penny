...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  ...
  private:
    ...
    uint16_t m_ttlThreshold;   ///< Maximum TTL value for expanding ring search,
                               ///< TTL = NetDiameter is used beyond this value.
    ...
    uint32_t m_allowedHelloLoss; ///< Number of hello messages which may be loss
                                 ///< for valid link
    /**
    ...
    Time m_maxQueueTime;     ///< The maximum period of time that a routing protocol
                             ///< is allowed to buffer a packet for.
  ...
  private:
     ...
     * destination use the expanding ring search technique. \param dst the
     * destination IP address
     */
    void ScheduleRreqRetry(Ipv4Address dst);
    /**
     * Set lifetime field in routing table entry to the maximum of existing
     * lifetime and lt, if the entry exists \param addr destination address \param
     * lt proposed time for lifetime field in routing table entry for destination
     * with address addr. \return true if route to destination address addr exist
     */
