 ...
 * \ingroup aodv
 *
 * \brief Unique packets identification cache used for simple duplicate
 * detection.
 */
class IdCache
{
  public:
    ...
    /**
     * Check that entry (addr, id) exists in cache. Add entry, if it doesn't
     * exist. \param addr the IP address \param id the cache entry ID \returns
  ...
  private:
    ...
    struct UniqueId
    {
        /// ID is supposed to be unique in single address context (e.g. sender
        /// address)
