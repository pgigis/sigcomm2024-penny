...
struct BatteryPresets
{
    ...
    double cuttoffVoltage;     //!< The threshold voltage where the battery is
                               //!< considered depleted
};
...
/**
 * \ingroup energy
 * \brief A generic battery model for  Li-Ion, NiCd, NiMh and Lead acid
 * batteries
 *
...
class GenericBatteryModel : public EnergySource
{
  public:
    ...
    /**
     * Implements GetInitialEnergy. It returns the amount of energy in Joules
     * stored in the battery when fully charged. This energy is different to the
     * total amount of usable energy in the battery. This is because the battery
     * cannot be used until Voltage = 0, only until it reaches the cutoff voltage.
     *
    ...
    /**
     * Implements GetEnergyFraction. For the generic battery model, energy
     * fraction is equivalent to the remaining usable capacity (i.e. The SoC).
     *
  ...
  private:
     ...
     * battery has being depleted. Each energy model is then responsible for its
     * own handler.
     */
  ...
  private:
    ...
    Time m_energyUpdateLapseTime;           //!< The lapse of time between the last battery
                                            //!< energy update and the current time.
    ...
    double m_cutoffVoltage;      //!< The threshold voltage where the battery is
                                 //!< considered depleted
