 ...
 *
 * \deprecated The LiIonEnergySource was deprecated in ns-3.40 in favor of
 * GenericBatteryModel, and will be removed in a future release.
 *
 * The model can be fitted to any type of Li-Ion Battery, simply changing the
 * model parameters.
 * The default values are fitted for the Panasonic CGR18650DA Li-Ion Battery
 * [3].
 *
 * The energy is drained as defined from the EnergySource interface but, this
 * class consider the non-linear behaviour of Li-Ion cell. Each time energy is
 * drained from the cell, the class evaluates the discharge curve to get the
 * actual cell's voltage, accordingly to State of Charge (SOC) and current's
 * drain.
 *
 * If the actual voltage of the cell goes below the minimum threshold voltage,
 * the cell is considered depleted and the energy drained event fired up.
 *
 *
 ...
 * - NominalCellVoltage, nominal cell's voltage, is used to determine the end of
 * the nominal zone.
 ...
 * - TypCurrent, typical discharge current value, used during the fitting
 * process, in Ah
 * - ThresholdVoltage, minimum threshold voltage below which the cell is
 * considered depleted
 *
 * For a complete reference of the energy source model and model's fitting
 * please refer to <a href="http://www.nsnam.org/wiki/GSOC2010UANFramework">UAN
 * Framework</a> page and <a
 * href="http://www.nsnam.org/wiki/Li-Ion_model_fitting">Li-Ion model
 * fitting</a> page.
 *
 * References:
 * [1] C. M. Shepherd, "Design of Primary and Secondary Cells - Part 3. Battery
 * discharge equation," U.S. Naval Research Laboratory, 1963 [2] Tremblay, O.;
 * Dessaint, L.-A.; Dekkiche, A.-I., "A Generic Battery Model for the Dynamic
 * Simulation of Hybrid Electric Vehicles," Ecole de Technologie Superieure,
 * Universite du Quebec, 2007 [3]
...
class LiIonEnergySource : public EnergySource
{
  public:
    ...
    /**
     * \param supplyVoltageV Initial Supply voltage at the energy source, in
     * Volts.
     *
  ...
  private:
    ...
    double m_lowBatteryTh;                  //!< low battery threshold, as a fraction of the
                                            //!< initial energy
    ...
    double m_minVoltTh;          //!< minimum threshold voltage to consider the battery
                                 //!< depleted
};
