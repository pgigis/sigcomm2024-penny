...
class PeerLink : public Object
{
  public:
     ...
     *
     * \param cb is a callback, which notifies, that on interface (uint32_t), peer
     * link with address (Mac48Address) was opened (bool is true) or closed (bool
     * is false)
     */
  ...
  private:
     ...
     *
     * \attention In all this methods {local/peer}LinkID correspond to _peer_
     * station, as written in received frame, e.g. I am peerLinkID and peer link
     * is localLinkID .
     *
     */
    ///@{
  ...
  private:
    ...
    uint16_t m_maxBeaconLoss;  //!< Maximum number of lost beacons before link
                               //!< will be closed
    uint16_t m_maxPacketFail;  //!< Maximum number of failed packets before link
                               //!< will be closed
    ///@}
