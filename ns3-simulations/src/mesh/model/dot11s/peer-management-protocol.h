...
class PeerManagementProtocol : public Object
{
  public:
     ...
     * element
     * \return IeBeaconTiming is a beacon timing element that should be present in
     * beacon \param interface is a interface sending a beacon
     */
     ...
     * \param interface is a interface ID of a given MAC (interfaceID rather than
     * MAC address, because many interfaces may have the same MAC) \param
     * peerAddress is address of peer \param peerMeshPointAddress is address of
     * peer mesh point device (equal to peer address when only one interface)
    ...
    /**
     * Get mesh point address. \todo this used by plugins only. Now MAC plugins
     * can ask MP address directly from main MAC
     *
     * \return Mac48Address
     */
  ...
  private:
    ...
    // Private structures
    /// Keeps information about beacon of peer station: beacon interval,
    /// association ID, last time we have received a beacon
    struct BeaconInfo
    {
        uint16_t aid;        ///< Assoc ID
        Time referenceTbtt;  ///< When one of my station's beacons was put into a
                             ///< beacon queue;
    ...
    /// This map keeps relationship between peer address and its beacon
    /// information
  ...
  private:
     ...
     * bool is status - true when new link has appeared, false - when link was
     * closed,
     */
    Callback<void, Mac48Address, Mac48Address, uint32_t, bool> m_peerStatusCallback;

    /// Simple link open/close trace source type. Addresses are: src interface,
    /// dst interface
