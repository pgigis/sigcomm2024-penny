...
class HwmpProtocol : public MeshL2RoutingProtocol
{
  public:
    ...
    void PeerLinkStatus(Mac48Address meshPointAddress,
                        ...
                        bool status);
    /**
     * \brief This callback is used to obtain active neighbours on a given
     * interface \param cb is a callback, which returns a list of addresses on
     * given interface (uint32_t)
     */
  ...
  private:
    ...
    struct QueuedPacket
    {
        ...
        uint32_t inInterface;     ///< incoming device interface ID. (if packet has
                                  ///< come from upper layers, this is Mesh point ID)
    ...
    void SendPrep(Mac48Address src,
                  ...
                  uint32_t interface);
    /**
     * \brief forms a path error information element when list of destination
     * fails on a given interface \attention removes all entries from routing
     * table!
     *
    ...
    /**
     * \brief MAC-plugin asks whether the frame can be dropped. Protocol
    ...
    /**
     * Signal the protocol that the reactive path toward a destination is now
     * available \param dst the destination
     */
    ...
    ///@{
    /**
     * \brief checks when the last path discovery procedure was started for a
     * given destination. \return true if should send PREQ \param dst is the
     * destination address
     *
     * If the retry counter has not achieved the maximum level - preq should not
     * be sent
     */
    bool ShouldSendPreq(Mac48Address dst);

    /**
     * \brief Generates PREQ retry when retry timeout has expired and route is
     * still unresolved. \param dst is the destination address \param numOfRetry
     * is the number of retries
     *
     * When PREQ retry has achieved the maximum level - retry mechanism should be
     * canceled
     */
  ...
  private:
    ...
    /// keeps HWMP seqno (first in pair) and HWMP metric (second in pair) for each
    /// address
    ...
    ///@{
    uint16_t m_maxQueueSize;                      //!< Maximum number of packets we can store when
                                                  //!< resolving route
    uint8_t m_dot11MeshHWMPmaxPREQretries;        //!< Maximum number of retries before
                                                  //!< we suppose the destination to be
                                                  //!< unreachable
    Time m_dot11MeshHWMPnetDiameterTraversalTime; //!< Time we suppose the packet
                                                  //!< to go from one edge of the
                                                  //!< network to another
    Time m_dot11MeshHWMPpreqMinInterval;          //!< Minimal interval between to
                                                  //!< successive PREQs
    Time m_dot11MeshHWMPperrMinInterval;          //!< Minimal interval between to
                                                  //!< successive PREQs
    Time m_dot11MeshHWMPactiveRootTimeout;        //!< Lifetime of proactive routing
                                                  //!< information
    Time m_dot11MeshHWMPactivePathTimeout;        //!< Lifetime of reactive routing
                                                  //!< information
    Time m_dot11MeshHWMPpathToRootInterval;       //!< Interval between two successive
                                                  //!< proactive PREQs
    Time m_dot11MeshHWMPrannInterval;             //!< Lifetime of proactive routing
                                                  //!< information
    bool m_isRoot;                                //!< True if the node is a root
    uint8_t m_maxTtl;                             //!< Initial value of Time To Live field
    uint8_t m_unicastPerrThreshold;               //!< Maximum number of PERR receivers, when
                                                  //!< we send a PERR as a chain of unicasts
    uint8_t m_unicastPreqThreshold;               //!< Maximum number of PREQ receivers, when
                                                  //!< we send a PREQ as a chain of unicasts
    uint8_t m_unicastDataThreshold;               //!< Maximum number of broadcast receivers, when
                                                  //!< we send a broadcast as a chain of unicasts
