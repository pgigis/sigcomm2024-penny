    ...
    static TypeId tid =
        TypeId("ns3::MeshPointDevice")
            ...
            .AddAttribute("ForwardingDelay",
                          "A random variable to account for processing time (microseconds) "
                          "to "
                          "forward a frame.",
...
MeshPointDevice::AddInterface(Ptr<NetDevice> iface)
{
    ...
    {
        NS_FATAL_ERROR("Device does not support eui 48 addresses: cannot be used "
                       "as a mesh point interface.");
    }
    if (!iface->SupportsSendFrom())
    {
        NS_FATAL_ERROR("Device does not support SendFrom: cannot be used as a mesh "
                       "point interface.");
    }
    ...
    if (!ifaceMac)
    {
        NS_FATAL_ERROR("WiFi device doesn't have correct MAC installed: cannot be "
                       "used as a mesh "
                       "point interface.");
    }
