 ...
 *   - Aggregating and coordinating 1..* real devices -- mesh interfaces, see
 * MeshInterfaceDevice class.
 *   - Hosting all mesh-related level 2 protocols.
 *
 * One of hosted L2 protocols must implement L2RoutingProtocol interface and is
 * used for packets forwarding.
 *
 * From the level 3 point of view MeshPointDevice is similar to BridgeNetDevice,
...
class MeshPointDevice : public NetDevice
{
  public:
    ...
    ///@{
    /**
     * \brief Attach new interface to the station. Interface must support 48-bit
     * MAC address and SendFrom method. \param port the port used
     *
     * \attention Only MeshPointDevice can have IP address, but not individual
     * interfaces.
     */
    ...
    ///@{
    /**
     * Register routing protocol to be used. Protocol must be already installed on
     * this mesh point.
     *
  ...
  private:
    ...
    /**
     * \brief Response callback for L2 routing protocol. This will be executed
     * when routing information is ready.
     *
     ...
     * \param iface       Interface to use (ID) for send (decided by routing
     * protocol). All interfaces will be used if outIface = 0xffffffff \todo
  ...
  private:
    ...
    /// Mesh point MAC address, supposed to be the address of the first added
    /// interface
