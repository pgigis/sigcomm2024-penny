...
/**
 * The request entry for intermediate nodes to check if they have received this
 * request or not This is used to control the duplication request from being
 * processed
 */
...
class DsrRreqTable : public Object
{
  public:
     ...
     * Check for duplicate ids and save new entries if the id is not present in
     * the table.
     *
    ...
    void Invalidate();
    /**
     * \brief Verify if entry is unidirectional or not(e.g. add this neighbor to
     * "blacklist" for blacklistTimeout period) \param neighbor neighbor address
     * link to which assumed to be unidirectional \return true on success
     */
    BlackList* FindUnidirectional(Ipv4Address neighbor);
    /**
     * \brief Mark entry as unidirectional (e.g. add this neighbor to "blacklist"
     * for blacklistTimeout period) \param neighbor neighbor address link to which
     * assumed to be unidirectional \param blacklistTimeout time for which the
     * neighboring node is put into the blacklist \return true on success
     */
     ...
     * not found \param src the source address we just received the source request
  ...
  private:
    ...
    /// The id cache to ensure all the ids are unique, it is used when sending out
    /// route request
    std::map<Ipv4Address, uint32_t> m_rreqIdCache;
    /// The cache to save route request table entries indexed with destination
    /// address
