 ...
 * using dijsktra algorithm to get the best route)
 *
 ...
 * The route cache structure
  \verbatim
  +-+-+-+-+-+-+-+-+-+-+-+-         +-+-+-+-+-+-+-+-+-+-+-
 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+- |  Destination Address |---------| Route Cache
 Entry  | ---------- | IP_VECTOR | dst | exp time |
  +-+-+-+-+-+-+-+-+-+-+-+-   Map   +-+-+-+-+-+-+-+-+-+-+-  Contains
 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
                                   +-+-+-+-+-+-+-+-+-+-+-
 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+- | Route Cache Entry  | ---------- | IP_VECTOR |
 dst | exp time |
                                   +-+-+-+-+-+-+-+-+-+-+-  Contains
 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+- .                                       . . . .
 . .                                       .
...
class DsrLinkStab
{
  ...
  private:
    /**
     * The link stability lifetime expected, when the time is due, the link
     * expires the expiration happens when purge the node and link cache before
     * update them when receiving new information
     */
...
class DsrRouteCache : public Object
{
  public:
     ...
     * successfully delivered the data packet \param dst destination address of
     * the route \return true in success
     */
     ...
     * that has just been notified as unreachable
     *
    ...
    //------------------------------------------------------------------------------------------
    /**
     * \brief Check for duplicate ids and save new entries if the id is not
     * present in the table \param nextHop to check for in cache \return ack ID
     */
    ...
    /**
     * \brief Return expire time for neighbor node with address addr, if exists,
     * else return 0. \param addr IP address \return expire time
     */
    ...
    /**
     * \brief Update expire time for entry with address addr, if it exists, else
     * add new entry \param nodeList list of addresses \param expire expiration
     * time
     */
  ...
  private:
    DsrRouteCacheEntry::IP_VECTOR m_vector; ///< The route vector to save the ip
                                            ///< addresses for intermediate nodes.
    uint32_t m_maxCacheLen;                 ///< The maximum number of packets that we allow a
                                            ///< routing protocol to buffer.
    Time RouteCacheTimeout;                 ///< The maximum period of time that dsr is allowed
                                            ///< to for an unused route.
    Time m_badLinkLifetime;                 ///< The time for which the neighboring node is put
                                            ///< into the blacklist.
    /*
...
/**
 * The link cache to update all the link status, bi-link is two link for link is
 * a struct when the weight is calculated we normalized them: 100*weight/max of
 * Weight
 */
#define MAXWEIGHT 0xFFFF;
    /**
     * Current network graph state for this node, double is weight, which is
     * calculated by the node information and link information, any time some
     * changes of link cache and node cache change the weight and then recompute
     * the best choice for each node
     */
  ...
  public:
    /**
     * \brief Dijsktra algorithm to get the best route from m_netGraph and update
     * the m_bestRoutesTable_link when current graph information has changed
    ...
    void PurgeLinkNode();
    /**
     * When a link from the Route Cache is used in routing a packet originated or
     * salvaged by that node, the stability metric for each of the two endpoint
     * nodes of that link is incremented by the amount of time since that link was
     * last used. When a link is used in a route chosen for a packet originated or
     * salvaged by this node, the link's lifetime is set to be at least UseExtends
     * into the future \param rt cache entry
     */
    ...
    std::vector<Ptr<ArpCache>> m_arp; ///< list of ARP cached to be used for
                                      ///< layer 2 notifications processing
