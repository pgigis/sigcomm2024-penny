...
{
/**
 * The maintenance buffer is responsible for maintaining packet next hop
 * delivery The data packet is saved in maintenance buffer whenever the data
 * packet is sent out of send buffer
 */
struct LinkKey
{
...
class DsrMaintainBuffer
{
  public:
    ...
    /// Push entry in queue, if there is no entry with the same packet and
    /// destination address in queue. \param entry Maintain Buffer Entry \return
    ...
    /// \note For real this function checks if at most one entry is equal. If it
    /// is, that entry is removed. Further entries are NOT checked. This could be
    /// a bug. \param entry The Entry to check \return true if an Entry was found
    /// and removed.
    bool AllEqual(DsrMaintainBuffEntry& entry);
    /// Verify if the maintain buffer entry is the same in every field for link
    /// ack \param entry The Entry to check \return true if an Entry was found and
    /// removed.
    bool LinkEqual(DsrMaintainBuffEntry& entry);
    /// Verify if the maintain buffer entry is the same in every field for network
    /// ack \param entry The Entry to check \return true if an Entry was found and
    /// removed.
    bool NetworkEqual(DsrMaintainBuffEntry& entry);
    /// Verify if the maintain buffer entry is the same in every field for
    /// promiscuous ack \param entry The Entry to check \return true if an Entry
