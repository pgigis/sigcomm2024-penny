...
class ThreeGppChannelModel : public MatrixBasedChannelModel
{
  public:
    ...
    /**
     * Looks for the channel matrix associated to the aMob and bMob pair in
     * m_channelMatrixMap. If found, it checks if it has to be updated. If not
     * found or if it has to be updated, it generates a new uncorrelated channel
     * matrix using the method GetNewChannel and updates m_channelMap.
     *
  ...
  protected:
     ...
     * rotated by M_PI.
     * This methods aims specifically at solving the problem of generating angles
     * at the boundaries of the angle domain, specifically, generating angle
     * distributions close to inclinationRad=0 and inclinationRad=M_PI.
     *
    ...
    {
        ChannelCondition::LosConditionValue m_losCondition; //!< contains the information about the
                                                            //!< LOS state of the channel
        ChannelCondition::O2iConditionValue m_o2iCondition; //!< contains the information about the
                                                            //!< O2I state of the channel
        // TODO these are not currently used, they have to be correctly set when
        // including the spatial consistent update procedure
        /*The following parameters are stored for spatial consistent updating. The
        notation is that of 3GPP technical reports, but it can apply also to other
        ...
        MatrixBasedChannelModel::Double2DVector
            m_norRvAngles;              //!< stores the normal variable for random angles
                                        //!< angle[cluster][id] generated for equation
                                        //!< (7.6-11)-(7.6-14), where id =
        ...
        DoubleVector m_attenuation_dB;      //!< vector that stores the attenuation of
                                            //!< the blockage
    ...
    struct ParamsTable : public SimpleRefCount<ParamsTable>
    {
        ...
        double m_uLgASD = 0;    //!< Mean value of 10-base logarithm of azimuth angle
                                //!< spread of departure
        double m_sigLgASD = 0;  //!< Standard deviation of 10-base logarithm of
                                //!< azimuth angle spread of departure
        double m_uLgASA = 0;    //!< Mean value of 10-base logarithm of azimuth angle
                                //!< spread of arrival
        double m_sigLgASA = 0;  //!< Standard deviation of 10-base logarithm of
                                //!< azimuth angle spread of arrival
        double m_uLgZSA = 0;    //!< Mean value of 10-base logarithm of zenith angle
                                //!< spread of arrival
        double m_sigLgZSA = 0;  //!< Standard deviation of 10-base logarithm of
                                //!< zenith angle spread of arrival
        double m_uLgZSD = 0;    //!< Mean value of 10-base logarithm of zenith angle
                                //!< spread of departure
        double m_sigLgZSD = 0;  //!< Standard deviation of 10-base logarithm of
        ...
        double m_sqrtC[7][7];                //!< The square root matrix and follows the order of
     ...
     * \return ThreeGppChannelParams structure with all the channel parameters
     * generated according 38.901 steps from 4 to 10.
     */
     ...
     * described in 3GPP TR 38.901
     * \param channelParams the channel parameters previously generated for the
     * pair of nodes a and b \param table3gpp the 3gpp parameters table \param
     * sMob the mobility model of node s \param uMob the mobility model of node u
     ...
     * \param clusterAOA vector containing the azimuth angle of arrival for each
     * cluster \param clusterZOA vector containing the zenith angle of arrival for
     * each cluster \return vector containing the power attenuation for each
     * cluster
     */
    ...
    /**
     * Check if the channel matrix has to be updated (it needs update when the
     * channel params generation time is more recent than channel matrix
     * generation time \param channelParams channel params structure \param
     * channelMatrix channel matrix structure \return true if the channel matrix
     * has to be updated, false otherwise
     */
    ...
    std::unordered_map<uint64_t, Ptr<ChannelMatrix>>
        m_channelMatrixMap; //!< map containing the channel realizations per pair
                            //!< of PhasedAntennaArray instances, the key of this
                            //!< map is reciprocal uniquely identifies a pair of
                            //!< PhasedAntennaArrays
    std::unordered_map<uint64_t, Ptr<ThreeGppChannelParams>>
        m_channelParamsMap; //!< map containing the common channel parameters per
                            //!< pair of nodes, the key of this map is reciprocal
                            //!< and uniquely identifies a pair of nodes
    ...
    Ptr<UniformRandomVariable> m_uniformRvShuffle;      //!< uniform random variable used to shuffle
                                                        //!< array in GetNewChannel

    // Variable used to compute the additional Doppler contribution for the
    // delayed (reflected) paths, as described in 3GPP TR 37.885 v15.3.0,
    // Sec. 6.2.3.
    double m_vScatt;                               //!< value used to compute the additional Doppler
                                                   //!< contribution for the delayed paths
