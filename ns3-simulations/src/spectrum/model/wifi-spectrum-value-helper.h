...
class WifiSpectrumValueHelper
{
  public:
     ...
     * \param guardBandwidth width of the guard band (MHz)
     * \returns a pointer to a newly allocated SpectrumValue representing the DSSS
     * Transmit Power Spectral Density in W/Hz
     */
     ...
     * \param txPowerW  transmit power (W) to allocate
     * \param guardBandwidth width of the guard band (MHz)
     * \param minInnerBandDbr the minimum relative power in the inner band (in
     * dBr) \param minOuterbandDbr the minimum relative power in the outer band
     * (in dBr) \param lowestPointDbr maximum relative power of the outermost
     * subcarriers of the guard band (in dBr) \return a pointer to a newly
     * allocated SpectrumValue representing the OFDM Transmit Power Spectral
    ...
    /**
     * Create a transmit power spectral density corresponding to OFDM duplicated
     * over multiple 20 MHz subchannels. Channel width may vary between 20, 40,
     * 80, and 160 MHz.
     *
     * \param centerFrequency center frequency (MHz)
     * \param channelWidth channel width (MHz)
     ...
     * dBr) \param minOuterbandDbr the minimum relative power in the outer band
     * (in dBr) \param lowestPointDbr maximum relative power of the outermost
     * subcarriers of the guard band (in dBr) \param puncturedSubchannels bitmap
     * indicating whether a 20 MHz subchannel is punctured or not \return a
     * pointer to a newly allocated SpectrumValue representing the duplicated 20
     * MHz OFDM Transmit Power Spectral Density in W/Hz for each Band
     */
     ...
     * (in dBr) \param lowestPointDbr maximum relative power of the outermost
     * subcarriers of the guard band (in dBr) \return a pointer to a newly
     * allocated SpectrumValue representing the HT OFDM Transmit Power Spectral
     * Density in W/Hz for each Band
     */
     ...
     * subcarriers of the guard band (in dBr) \param puncturedSubchannels bitmap
     * indicating whether a 20 MHz subchannel is punctured or not \return a
     * pointer to a newly allocated SpectrumValue representing the HE OFDM
     ...
     * \return a pointer to a newly allocated SpectrumValue representing the HE
     * OFDM Transmit Power Spectral Density on the RU used by the STA in W/Hz for
     * each Band
     */
     ...
     * \param channelWidth channel width (MHz)
     * \param carrierSpacing carrier spacing (Hz)
     * \param noiseFigure the noise figure in dB w.r.t. a reference temperature of
     * 290K \param guardBandwidth width of the guard band (MHz) \return a pointer
     * to a newly allocated SpectrumValue representing the noise Power Spectral
     * Density in W/Hz for each Band
     */
     ...
     * \return a pointer to a newly allocated SpectrumValue representing the noise
     * Power Spectral Density in W/Hz corresponding to thermal noise, for each
     * Band
     */
     ...
     * dBr) \param puncturedSubBands vector of start and stop subcarrier indexes
     * of the punctured sub bands \param puncturedSlopeWidth size (in number of
     * subcarriers) of the punctured band slope
     */
     ...
     * Taken from wifi-utils since the original method couldn't be called from
     * here due to resulting circular dependencies of spectrum and wifi modules.
     *
    ...
    /**
     * Calculate the power of the specified band composed of uniformly-sized
     * sub-bands.
     *
...
/**
 * \ingroup spectrum
 * Struct defining a frequency range between minFrequency (MHz) and maxFrequency
 * (MHz).
 */
 ...
 *
 * \return true if minFrequency of left is less than minFrequency of right,
 * false otherwise
 */
 ...
 * \param rhs the FrequencyRange value on the right of operator
 *
 * \return true if both minFrequency and maxFrequency of left are equal to
 * minFrequency and maxFrequency of right respectively, false otherwise
 */
bool operator==(const FrequencyRange& lhs, const FrequencyRange& rhs);
 ...
 * \param rhs the FrequencyRange value on the right of operator
 *
 * \return true if either minFrequency or maxFrequency of left different from
 * minFrequency or maxFrequency of right respectively, false otherwise
 */
bool operator!=(const FrequencyRange& lhs, const FrequencyRange& rhs);
...
/// Identifier for the frequency range covering the wifi spectrum in the 2.4 GHz
/// band
constexpr FrequencyRange WIFI_SPECTRUM_2_4_GHZ = {2401, 2483};

/// Identifier for the frequency range covering the wifi spectrum in the 5 GHz
/// band
constexpr FrequencyRange WIFI_SPECTRUM_5_GHZ = {5170, 5915};

/// Identifier for the frequency range covering the wifi spectrum in the 6 GHz
/// band
