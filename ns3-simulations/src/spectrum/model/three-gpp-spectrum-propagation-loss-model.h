 ...
 * described by 3GPP TR 38.901 document. The main method is
 * DoCalcRxPowerSpectralDensity, which takes as input the power spectral density
 * (PSD) of the transmitted signal, the mobility models of the transmitting node
 * and receiving node, and returns the PSD of the received signal.
 *
...
class ThreeGppSpectrumPropagationLossModel : public PhasedArraySpectrumPropagationLossModel
{
  public:
     ...
     * \param channel a pointer to an object implementing the
     ...
     * \return a pointer to the object implementing the MatrixBasedChannelModel
     * interface
     */
     ...
     * a certain channel is cached and recomputed only when the channel
     * realization is updated, or when the beamforming vectors change.
     *
  ...
  private:
    ...
    {
        PhasedArrayModel::ComplexVector m_longTerm; //!< vector containing the long term component
                                                    //!< for each cluster
        Ptr<const MatrixBasedChannelModel::ChannelMatrix>
            m_channel; //!< pointer to the channel matrix used to compute the long
                       //!< term
        PhasedArrayModel::ComplexVector m_sW; //!< the beamforming vector for the node s used to
                                              //!< compute the long term
        PhasedArrayModel::ComplexVector m_uW; //!< the beamforming vector for the node u used to
                                              //!< compute the long term
    };
