...
operator<(const WifiSpectrumModelId& a, const WifiSpectrumModelId& b)
{
    return (
        ...
        || ((a.m_centerFrequency == b.m_centerFrequency) &&
            ...
            (a.m_guardBandwidth < b.m_guardBandwidth))); // to cover 2.4 GHz case, where DSSS
                                                         // coexists with OFDM
}
    ...
    const std::vector<bool>& puncturedSubchannels)
{
    ...
    auto innerSlopeWidth =
        static_cast<uint32_t>((2e6 / carrierSpacing) + 0.5); // size in number of subcarriers of the
    ...
    uint32_t puncturedSlopeWidth)
{
    ...
    WifiSpectrumBandIndices outerBandRight(maskBand.second - outerSlopeWidth + 1,
                                           maskBand.second); // start from outer edge to be able to
                                                             // compute flat junction width
