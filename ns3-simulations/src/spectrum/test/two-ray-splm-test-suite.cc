...
class FtrFadingModelAverageTest : public TestCase
{
  ...
  private:
     ...
     * \f$ \sigma^2 \f$ is the variance of the Gaussian distributed random
     * variables which model the real and complex component of the specular terms
     * and \f$ V_1 \f$ and \f$ V_2 \f$ are the constant amplitudes of the
     * reflected components.ù In turn, this equals to \f$ 2*\sigma^2 \left( 1 + K
     * \right) \f$.
     *
     ...
     * Globecom Workshops (GC Wkshps) for further details.
     *
...
FtrFadingModelAverageTest::DoRun()
{
    ...
    // Check the consistency of the empirical average for a set of values of the
    // FTR model parameters
...
class ArrayResponseTest : public TestCase
{
  public:
     ...
     * \param expectedGain the theoretically expected gain for the above
     * parameters
     */
  ...
  private:
    ...
    uint16_t m_txNumAntennas;      //!< the number of antenna elements of the TX
                                   //!< antenna panel
    uint16_t m_rxNumAntennas;      //!< the number of antenna elements of the RX
                                   //!< antenna panel
...
ArrayResponseTest::ArrayResponseTest(Ptr<AntennaModel> txAntElem,
                                     ...
                                     double rxBearing,
                                     double expectedGain)
    // TODO: Create a string with the test parameters as the test case name like
    // in test-uniform-planar-array ?
...
class OverallGainAverageTest : public TestCase
{
  ...
  private:
    ...
    /**
     * Tolerance for testing average channel gain produced by the Two Ray model
     * with respect to the 3GPP model, which serves as the reference.
     */
    ...
    /**
     * The width of a RB, which in turn specifies the resolution of the generated
     * PSDs. Results are independent from this parameter.
     */
...
OverallGainAverageTest::DoRun()
{
    ...
    // Disable blockage in order to prevent unwanted variations around the mean
    // value
    ...
    for (uint32_t i = 0; i < N_MEASUREMENTS; ++i)
    {
        // Re-create the RX array at each iteration, to force resampling of the 3GPP
        // channel
        auto rxArray = CreateObject<UniformPlanarArray>();
...
{
    // Test the GetFtrFastFading function of the
    // TwoRaySpectrumPropagationLossModel class
    AddTestCase(new FtrFadingModelAverageTest, TestCase::QUICK);

    // Test the CalcBeamformingGain function of the
    // TwoRaySpectrumPropagationLossModel class
    ...
    // Deploy 2 squared antenna arrays which face each other. Steer their beam
    // towards the boresight and check the resulting array gain. SE = single
    // element radiation pattern, N = number of radiating elements, Phi = bearing
    // angle
    //                                 SE tx,   SE rx, N tx, N rx, position tx,
    //                                 position rx,  Phi tx,  Phi rx, expected
    //                                 gain
    ...
    // Deploy 2 squared antenna arrays which face each other. Steer their beam
    // towards the boresight and check the overall resulting channel gain. SE =
    // single element radiation pattern, N = number of radiating elements, Fc =
    // carrier frequency, Scen = 3GPP scenario
