...
class VirtualNetDevice : public NetDevice
{
  public:
    /**
     * Callback the be invoked when the VirtualNetDevice is asked to
     * queue/transmit a packet. For more information, consult the documentation of
     * NetDevice::SendFrom().
     */
    ...
    /**
     * \brief Set the user callback to be called when a L2 packet is to be
     * transmitted \param transmitCb the new transmit callback
     */
     ...
     * (broadcast/multicast/unicast/otherhost) \returns true if the packet was
     * forwarded successfully, false otherwise.
     *
