...
class SQLiteOutput : public SimpleRefCount<SQLiteOutput>
{
  public:
    ...
    /**
     * \brief Instruct SQLite to store the journal in memory. May lead to data
     * losses in case of unexpected program exits.
     */
     ...
     *
     * Ignore errors due to concurrency, the method will repeat the command
     * instead
     *
     * \param cmd Command
    ...
    /**
     * \brief Execute a step operation on a statement until the result is ok or an
     * error
     *
  ...
  protected:
    ...
    /**
     * \brief Execute a Prepared Statement Object ignoring concurrency problems,
     * retrying instead \param db Database \param stmt Prepared Statement Object
     * \return Sqlite error code
     */
