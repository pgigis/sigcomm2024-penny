 ...
 * \brief Performs 6LoWPAN compression of IPv6 packets as specified by
 *\RFC{4944} and \RFC{6282}
 *
 * This module acts as a shim between IPv6 and a generic NetDevice.
 *
 * The module implements \RFC{4944} and \RFC{6282}, with the following
 *exceptions: <ul> <li> MESH and LOWPAN_BC0 dispatch types are not supported
 *</li> <li> HC2 encoding is not supported </li> <li> IPHC's SAC and DAC are not
 *supported </li>
 *</ul>
 */
 ...
 * performing packet compression, decompression and fragmentation in a
 * transparent way. To this end, the class pretend to be a normal NetDevice,
 * masquerading some functions of the underlying NetDevice.
 */
class SixLowPanNetDevice : public NetDevice
{
  public:
    ...
    enum DropReason
    {
        ...
        DROP_SATETFUL_DECOMPRESSION_PROBLEM, //!< Decompression failed due to
     ...
     * \param [in] contextPrefix context prefix to be used in
     * compression/decompression. \param [in] compressionAllowed compression and
     * decompression allowed (true), decompression only (false). \param [in]
     * validLifetime validity time (relative to the actual time).
     *
     */
     ...
     *
     * \param [in] contextId context id (most be between 0 and 15 included).
     * \param [out] contextPrefix context prefix to be used in
     * compression/decompression. \param [out] compressionAllowed compression and
     * decompression allowed (true), decompression only (false). \param [out]
     * validLifetime validity time (relative to the actual time).
     *
     ...
     *
     * The context will have its lifetime extended and its validity for
     * compression re-enabled.
     *
     * \param [in] contextId context id (most be between 0 and 15 included).
     ...
     *
     * An invalid context will not be used for compression but it will be used for
     * decompression.
     *
     * \param [in] contextId context id (most be between 0 and 15 included).
     */
  ...
  private:
     ...
     * \param [in] source Source mac address (only used if doSendFrom is true,
     * i.e., "MAC spoofing"). \param [in] dest Mac address of the destination
     * (already resolved). \param [in] protocolNumber Identifies the type of
     * payload contained in this packet. Used to call the right L3Protocol when
     * the packet is received. \param [in] doSendFrom Perform a SendFrom instead
     * of a Send.
     *
     ...
     * \param [in] src The MAC source address.
     * \param [in] dst The MAC destination address.
     * \return true if the packet can not be decompressed due to wrong context
     * information.
     */
     ...
     * \return A std::pair containing the decompressed header type and a flag -
     * true if the packet can not be decompressed due to wrong context
     * information.
     */
     ...
     * \param [in] packet the packet to be fragmented (with headers already
     * compressed with 6LoWPAN). \param [in] origPacketSize the size of the IP
     * packet before the 6LoWPAN header compression, including the IP/L4 headers.
     * \param [in] origHdrSize the size of the IP header before the 6LoWPAN header
     * compression. \param [in] extraHdrSize the sum of the sizes of BC0 header
     * and MESH header if mesh routing is used or 0. \param [out] listFragments A
     * reference to the list of the resulting packets, all with the proper headers
     * in place.
     */
    ...
    Ptr<RandomVariableStream> m_meshUnderJitter; //!< Random variable for the mesh-under packet
                                                 //!< retransmission.
    std::map<Address /* OriginatorAddress */, std::list<uint8_t /* SequenceNumber */>>
        m_seenPkts; //!< Seen packets, memorized by OriginatorAddress,
                    //!< SequenceNumber.
     ...
     * Also implying that the underlying NetDevice is using 48-bit Addresses,
     * e.g., Ethernet, Wi-Fi, etc.
     */
    bool m_forceEtherType;

    uint16_t m_etherType;   //!< EtherType number (used only if m_forceEtherType is
                            //!< true).
    ...
    /**
     * Structure holding the information for a context (used in compression and
     * decompression)
     */
    struct ContextEntry
    {
        Ipv6Prefix contextPrefix; //!< context prefix to be used in
                                  //!< compression/decompression
        bool compressionAllowed;  //!< compression and decompression allowed (true),
                                  //!< decompression only (false)
        Time validLifetime;       //!< validity period
    };

    std::map<uint8_t, ContextEntry> m_contextTable; //!< Table of the contexts used in
                                                    //!< compression/decompression
    ...
    /**
     * \brief Finds if the given multicast address matches a context for
     * compression
     *
     * \param[in] address the address to check
     * \param[out] contextId the context found
     ...
     *
     * This function is used to find the relevant bits to be sent in stateful IPHC
     * compression. Only the prefix length is used - the address prefix is assumed
     * to be matching the prefix.
     *
