    ...
    static TypeId tid =
        TypeId("ns3::SixLowPanNetDevice")
            ...
            .AddAttribute("OmitUdpChecksum",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("FragmentReassemblyListSize",
                          "The maximum size of the reassembly buffer (in "
                          "packets). Zero meaning infinite.",
                          UintegerValue(0),
                          MakeUintegerAccessor(&SixLowPanNetDevice::m_fragmentReassemblyListSize),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("FragmentExpirationTimeout",
                          "When this timeout expires, the fragments will be "
                                      ...
                                      PacketType packetType)
{
    ...
    if (hasMesh)
    {
        ...
        NS_ABORT_MSG_IF(!Mac16Address::IsMatchingType(meshHdr.GetFinalDst()),
                        "SixLowPan mesh-under flooding can not currently handle "
                        "extended address "
                        "final destinations: "
                            << meshHdr.GetFinalDst());
...
SixLowPanNetDevice::DecompressLowPanHc1(Ptr<Packet> packet, const Address& src, const Address& dst)
{
    ...
    NS_ASSERT_MSG(encoding.IsHc2HeaderPresent() == false,
                  "6LoWPAN: error in decompressing HC1 encoding, unsupported L4 "
    ...
    if (headerType == Ipv6Header::IPV6_EXT_HOP_BY_HOP)
    {
        ...
        if (hopHeader.GetLength() >= 0xff)
        {
            NS_LOG_DEBUG("LOWPAN_NHC MUST NOT be used to encode IPv6 Extension Headers "
                         "that have more than 255 octets following the Length field after "
                         "compression. "
                         "Packet uncompressed.");
            return 0;
        }
