 ...
 * \ingroup fd-net-device
 *
 * \brief a NetDevice to read/write network traffic from/into a netmap file
 * descriptor.
 *
 * A NetmapNetDevice object will read and write packets from/to a netmap file
 * descriptor.
 *
 */
class NetmapNetDevice : public FdNetDevice
{
  public:
    ...
    /**
     * \brief Get the number of slots currently available in the netmap
     * transmission ring. \return the number of slots currently available in the
  ...
  private:
    ...
    std::atomic<bool> m_syncAndNotifyQueueThreadRun; //!< Running flag of the
                                                     //!< flow control thread
    uint8_t m_syncAndNotifyQueuePeriod;              //!< The period of time in us after which
                                                     //!< the device syncs the netmap ring and
                                                     //!< notifies queue status
};
