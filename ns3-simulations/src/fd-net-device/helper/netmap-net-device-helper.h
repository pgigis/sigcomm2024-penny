...
class NetmapNetDeviceHelper : public FdNetDeviceHelper
{
  ...
  protected:
    /**
     * \brief This method creates an ns3::FdNetDevice attached to a physical
     * network interface \param node The node to install the device in \returns A
    ...
    /**
     * \brief Call out to a separate process running as suid root in order to get
     * a raw socket.  We do this to avoid having the entire simulation running as
     * root. \return the rawSocket number
     */
    ...
    std::string m_deviceName; //!< The unix/linux name of the underlying device
                              //!< (e.g., eth0)
};
