...
TapFdNetDeviceHelper::SetFileDescriptor(Ptr<FdNetDevice> device) const
{
    ...
    // TAP device.  We do this to avoid having the entire simulation running as
    // root.
    //
...
TapFdNetDeviceHelper::CreateFileDescriptor() const
{
    ...
    if (pid == 0)
    {
        ...
        // -n<network-IPv4-mask> The network IPv4 mask to assign to the new tap
        // device; -N<network-IPv6-mask> The network IPv6 mask to assign to the new
        // tap device; -t Set the IFF_TAP flag -h Set the IFF_NO_PI flag -p<path>
        // the path to the unix socket described above.
        //
        // Example tap-creator -dnewdev -i1.2.3.1 -m08:00:2e:00:01:23
        // -n255.255.255.0 -t -h -pblah
        //
