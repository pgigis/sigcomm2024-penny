...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  ...
  private:
    ...
    Ptr<Ipv4StaticRouting> m_routingTableAssociation; //!< Associations from an Ipv4StaticRouting
                                                      //!< instance
  ...
  public:
     ...
     * local HNA associations to be sent by the node via HNA messages.
     * If this tuple does not exist, nothing is done (see
     * "OlsrState::EraseAssociation()").
     *
     * \param networkAddr The network address.
     * \param netmask The network mask.
     */
  ...
  private:
    ...
    Willingness m_willingness; //!< Willingness for forwarding packets on behalf
                               //!< of other nodes.
    ...
    /**
     * \brief Clears the routing table and frees the memory assigned to each one
     * of its entries.
     */
    void Clear();
     ...
     *
     * Imagine a routing table like this: [A,B] [B,C] [C,C]; being each pair of
     * the form [dest addr, next-hop addr]. In this case, if this function is
     * invoked with [A,B] then pair [C,C] is returned because C is the next hop
     * that must be used to forward a data packet destined to A. That is, C is a
     * neighbor of this node, but B isn't. This function finds the appropriate
     * neighbor for forwarding a packet.
     *
     * \param[in] entry The routing table entry which indicates the destination
     * node we are interested in.
     *
     * \param[out] outEntry The appropriate routing table entry which indicates
     * the next hop which must be used for forwarding a data packet, or NULL if
  ...
  private:
    /**
     * \brief Tests whether or not the specified route uses a non-OLSR outgoing
     * interface. \param route The route to be tested. \returns True if the
     * outgoing interface of the specified route is a non-OLSR interface, false
     * otherwise.
     */
    ...
    /**
     * \brief Sends a TC message (if there exists any MPR selector) and
     * reschedules the TC timer.
     */
    void TcTimerExpire();

    Timer m_midTimer; //!< Timer for the MID message.
    /**
     * \brief \brief Sends a MID message (if the node has more than one interface)
     * and resets the MID timer.
     */
    void MidTimerExpire();

    Timer m_hnaTimer; //!< Timer for the HNA message.
    /**
     * \brief Sends an HNA message (if the node has associated hosts/networks) and
     * reschedules the HNA timer.
     */
    void HnaTimerExpire();

    /**
     * \brief Removes tuple if expired. Else timer is rescheduled to expire at
    ...
    bool m_linkTupleTimerFirstTime; //!< Flag to indicate if it is the first time
                                    //!< the LinkTupleTimer fires.
    /**
    ...
    /**
     * \brief Removes 2_hop neighbor tuple_ if expired. Else the timer is
     * rescheduled to expire at tuple_->time().
     *
     * The task of actually removing the tuple is left to the OLSR agent.
     *
    ...
    /**
     * \brief Removes MPR selector tuple_ if expired. Else the timer is
     * rescheduled to expire at tuple_->time().
     *
     * The task of actually removing the tuple is left to the OLSR agent.
     *
    ...
    /**
     * \brief Removes topology tuple_ if expired. Else the timer is rescheduled to
    ...
    /**
     * \brief Removes interface association tuple_ if expired. Else the timer is
     * rescheduled to expire at tuple_->time().
     *
    ...
    /**
     * \brief Removes association tuple_ if expired. Else timer is rescheduled to
     * expire at tuple_->time().
     *
     ...
     * forwarding, or a duplicate tuple in other case. \param localIface The
     * address of the interface where the message was received from. \param
    ...
    /**
     * \brief Enqueues an %OLSR message which will be sent with a delay of (0,
     * delay].
     *
     * This buffering system is used in order to piggyback several %OLSR messages
     * in a same %OLSR packet.
     *
    ...
    /**
     * \brief Creates as many %OLSR packets as needed in order to send all
     * buffered %OLSR messages.
     *
    ...
    /**
     * \brief Creates a new %OLSR HELLO message which is buffered for being sent
     * later on.
     */
    void SendHello();

    /**
     * \brief Creates a new %OLSR TC message which is buffered for being sent
     * later on.
     */
    void SendTc();

    /**
     * \brief Creates a new %OLSR MID message which is buffered for being sent
     * later on.
     */
    void SendMid();

    /**
     * \brief Creates a new %OLSR HNA message which is buffered for being sent
     * later on.
     */
    void SendHna();
     ...
     *
     * \param tuple link tuple with the information of the link to the neighbor
     * which has been lost.
     */
    ...
    /**
     * \brief Adds an interface association tuple to the Interface Association
     * Set.
     *
    ...
    /**
     * \brief Removed an interface association tuple to the Interface Association
     * Set.
     *
     ...
     * \param receiverIface the address of the interface where the message was
     * received from. \param senderIface the address of the interface where the
     ...
     * \param senderIface The address of the interface where the message was sent
     * from.
     *
     */
     ...
     *
     * The Host Network Association Set is updated (if needed) with the
     * information of the received HNA message.
     *
     ...
     * (following \RFC{3626} specification). Neighbor Set is also updated if
     * needed. \param msg The received message. \param hello The received HELLO
     * sub-message. \param receiverIface The interface that received the message.
    ...
    /**
     * \brief Updates the 2-hop Neighbor Set according to the information
     * contained in a new received HELLO message (following \RFC{3626}). \param
     * msg The received message. \param hello The received HELLO sub-message.
     */
    void PopulateTwoHopNeighborSet(const olsr::MessageHeader& msg,
                                   const olsr::MessageHeader::Hello& hello);

    /**
     * \brief Updates the MPR Selector Set according to the information contained
     * in a new received HELLO message (following \RFC{3626}). \param msg The
     * received message. \param hello The received HELLO sub-message.
     */
    ...
    std::map<Ptr<Socket>, Ipv4InterfaceAddress> m_sendSockets; //!< Container of sockets and the
                                                               //!< interfaces they are opened onto.
