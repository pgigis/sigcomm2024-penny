...
class OlsrHelper : public Ipv4RoutingHelper
{
  public:
     ...
     *
     * This method allows the user to specify an interface on which OLSR is not to
     * be installed on
     */
     ...
     * \param c NodeContainer of the set of nodes for which the
     * OlsrRoutingProtocol should be modified to use a fixed stream \return the
     * number of stream indices assigned by this helper
     */
  ...
  private:
    ...
    std::map<Ptr<Node>, std::set<uint32_t>>
        m_interfaceExclusions; //!< container of interfaces excluded from OLSR
                               //!< operations
};
