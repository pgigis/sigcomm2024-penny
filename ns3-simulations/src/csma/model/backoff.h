...
class Backoff
{
  public:
    /**
     * Minimum number of backoff slots (when multiplied by m_slotTime, determines
     * minimum backoff time)
     */
    uint32_t m_minSlots;

    /**
     * Maximum number of backoff slots (when multiplied by m_slotTime, determines
    ...
    /**
     * Length of one slot. A slot time, it usually the packet transmission time,
     * if the packet size is fixed.
     */
    Time m_slotTime;
  ...
  private:
    /**
     * Number of times that the transmitter has tried to unsuccessfuly transmit
     * the current packet.
     */
