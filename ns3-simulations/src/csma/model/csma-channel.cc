...
CsmaChannel::TransmitStart(Ptr<const Packet> p, uint32_t srcId)
{
    ...
    if (!IsActive(srcId))
    {
        NS_LOG_ERROR("CsmaChannel::TransmitStart(): Seclected source is not "
                     "currently attached to network");
        return false;
    }
...
CsmaChannel::TransmitEnd()
{
    ...
    {
        NS_LOG_ERROR("CsmaChannel::TransmitEnd(): Seclected source was detached "
                     "before the end of "
                     "the transmission");
        retVal = false;
    }
