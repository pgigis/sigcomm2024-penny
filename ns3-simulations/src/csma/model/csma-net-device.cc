...
CsmaNetDevice::TransmitAbort()
{
    ...
    else
    {
        ...
        NS_ASSERT_MSG(packet,
                      "CsmaNetDevice::TransmitAbort(): IsEmpty false but "
                      "no Packet on queue?");
        m_currentPkt = packet;
...
CsmaNetDevice::TransmitReadyEvent()
{
    ...
    // gap has passed.  If there are pending transmissions, we use this
    // opportunity to start the next transmit.
    //
    ...
    else
    {
        Ptr<Packet> packet = m_queue->Dequeue();
        NS_ASSERT_MSG(packet,
                      "CsmaNetDevice::TransmitReadyEvent(): IsEmpty false "
