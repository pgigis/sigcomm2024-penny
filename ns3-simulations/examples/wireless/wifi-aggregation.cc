...
//
// The user can select the distance between the stations and the APs and can
// enable/disable the RTS/CTS mechanism. Example: ./ns3 run "wifi-aggregation
// --distance=10 --enableRts=0
// --simulationTime=20"
//
// The output prints the throughput measured for the 4 cases/networks described
// above. When default aggregation parameters are enabled, the maximum A-MPDU
// size is 65 kB and the throughput is maximal. When aggregation is disabled,
// the throughput is about the half of the physical bitrate. When only A-MSDU is
// enabled, the throughput is increased but is not maximal, since the maximum
// A-MSDU size is limited to 7935 bytes (whereas the maximum A-MPDU size is
// limited to 65535 bytes). When A-MSDU and A-MPDU are both enabled (= two-level
// aggregation), the throughput is slightly smaller than the first scenario
// since we set a smaller maximum A-MPDU size.
//
