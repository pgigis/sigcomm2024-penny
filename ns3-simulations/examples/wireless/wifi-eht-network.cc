...
// This is a simple example in order to show how to configure an IEEE 802.11be
// Wi-Fi network.
//
// It outputs the UDP or TCP goodput for every EHT MCS value, which depends on
// the MCS value (0 to 13), the channel width (20, 40, 80 or 160 MHz) and the
// guard interval (800ns, 1600ns or 3200ns). The PHY bitrate is constant over
// all the simulation run. The user can also specify the distance between the
// access point and the station: the larger the distance the smaller the
// goodput.
//
// The simulation assumes a configurable number of stations in an infrastructure
// network:
//
//  STA     AP
//    *     *
...
// By selecting an acknowledgment sequence for DL MU PPDUs, it is possible to
// aggregate a Round Robin scheduler to the AP, so that DL MU PPDUs are sent by
// the AP via DL OFDMA.
 ...
 * \param rxBytes a vector of the amount of bytes received by each server
 * application \param udp true if UDP is used, false if TCP is used \param
 * serverApp a container of server applications \param payloadSize the size in
 * bytes of the packets \param tputInterval the duration of an intermediate time
 * interval \param simulationTime the simulation time in seconds
 */
void
...
main(int argc, char* argv[])
{
    ...
    double frequency2{0};      // whether the second link operates in the 2.4, 5 or 6
                               // GHz (0 means no second link exists)
    double frequency3{0};      // whether the third link operates in the 2.4, 5 or 6
                               // GHz (0 means no third link exists)
    std::size_t nStations{1};
    std::string dlAckSeqType{"NO-OFDMA"};
    bool enableUlOfdma{false};
    bool enableBsrp{false};
    int mcs{-1};                // -1 indicates an unset value
    uint32_t payloadSize = 700; // must fit in the max TX duration when
                                // transmitting at MCS 0 over an RU of 26 tones
    ...
    for (int mcs = minMcs; mcs <= maxMcs; mcs++)
    {
        ...
        for (int channelWidth = 20; channelWidth <= maxChannelWidth;) // MHz
        {
            for (int gi = 3200; gi >= 800;) // Nanoseconds
            {
                 ...
                 * and one with 312.5 kHz bands for, e.g., non-HT PPDUs (for more
                 * details, see issue #408 (CLOSED))
                 */
                ...
                // requests collide and hence the throughput may be lower than expected.
                // Therefore, we relax the check that the throughput cannot decrease by
                // introducing a scaling factor (or tolerance)
