 ...
 * by NSF grant CNS-1050226 (Multilayer Network Resilience Analysis and
 * Experimentation on GENI)
 *
 * This program reads an upper triangular adjacency matrix (e.g.
 * adjacency_matrix.txt) and node coordinates file (e.g. node_coordinates.txt).
 * The program also set-ups a wired network topology with P2P links according to
 * the adjacency matrix with nx(n-1) CBR traffic flows, in which n is the number
 * of nodes in the adjacency matrix.
 */
...
main(int argc, char* argv[])
{
    ...
    //  DropTailQueue::MaxPackets affects the # of dropped packets, default
    //  value:100 Config::SetDefault ("ns3::DropTailQueue::MaxPackets",
    //  UintegerValue (1000));
    ...
    for (int i = 0; i < n_nodes; i++)
    {
        for (int j = 0; j < n_nodes; j++)
        {
            if (i != j)
            {
                ...
                // start at different time, however they will still send at the same
                // rate.
                ...
                OnOffHelper onoff("ns3::UdpSocketFactory",
                                  InetSocketAddress(ip_addr,
                                                    port)); // traffic flows from node[i] to node[j]
