 ...
 * The channel condition is determined using the model specified in [1],
 * Table 6.2-1. The pathloss is determined using the model specified in [1],
 * Table 6.2.1-1. The model for the fast fading is the one described in 3GPP
 * TR 38.901 v15.0.0, the model parameters are those specified in [1],
 * Table 6.2.3-1.
 *
 * This example generates the output file 'example-output.txt'. Each row of the
 * file is organized as follows:
 * Time[s] TxPosX[m] TxPosY[m] RxPosX[m] RxPosY[m] ChannelState SNR[dB]
 * Pathloss[dB] We also provide a bash script which reads the output file and
 * generates two figures: (i) map.gif, a GIF representing the simulation
 * scenario and vehicle mobility; (ii) snr.png, which represents the behavior of
 * the SNR.
 *
 ...
 * \param params A structure that holds a bunch of parameters needed by
 * ComputSnr function to calculate the average SNR
 */
static void
