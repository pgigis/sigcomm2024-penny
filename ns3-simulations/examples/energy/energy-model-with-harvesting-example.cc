 ...
 * every 1 second. Each node is powered by a BasicEnergySource, which is
 * recharged by a BasicEnergyHarvester, and the WiFi radio consumes energy for
 * the transmission/ reception of the packets.
 *
 * For the receiver node, the example prints the energy consumption of the WiFi
 * radio, the power harvested by the energy harvester and the residual energy in
 * the energy source.
 *
 * The nodes initial energy is set to 1.0 J, the transmission and reception
 * entail a current consumption of 0.0174 A and 0.0197 A, respectively (default
 * values in WifiRadioEnergyModel). The energy harvester provides an amount of
 * power that varies according to a random variable uniformly distributed in [0
 * 0.1] W, and is updated every 1 s. The energy source voltage is 3 V (default
 * value in BasicEnergySource) and the residual energy level is updated every 1
 * second (default value).
 *
 * The simulation start at time 0 and it is hard stopped at time 10 seconds.
 * Given the packet size and the distance between the nodes, each transmission
 * lasts 0.0023s. As a result, the destination node receives 10 messages.
 *
 */
